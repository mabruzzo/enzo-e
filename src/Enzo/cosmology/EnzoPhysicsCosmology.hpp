...
public:  // interface
  ...
  /// Constructor
  EnzoPhysicsCosmology(enzo_float hubble_constant_now,
                       enzo_float omega_matter_now, enzo_float omega_baryon_now,
                       enzo_float omega_cdm_now, enzo_float omega_lambda_now,
                       enzo_float comoving_box_size,
                       enzo_float max_expansion_rate,
                       enzo_float initial_redshift, enzo_float final_redshift)
      : Physics(),
    ...
    ASSERT3("EnzoPhysicsCosmology::EnzoPhysicsCosmology()",
            ...
            std::abs(omega_matter_now_ - (omega_cdm_now_ + omega_baryon_now_)) <
                1e-7);
  }
  ...
  void set_omega_matter_now(enzo_float value) { omega_matter_now_ = value; }
  void set_omega_baryon_now(enzo_float value) { omega_baryon_now_ = value; }
  void set_omega_cdm_now(enzo_float value) { omega_cdm_now_ = value; }
  void set_omega_lambda_now(enzo_float value) { omega_lambda_now_ = value; }
  void set_comoving_box_size(enzo_float value) { comoving_box_size_ = value; }
  void set_max_expansion_rate(enzo_float value) { max_expansion_rate_ = value; }
  void set_initial_redshift(enzo_float value) { initial_redshift_ = value; }
  void set_final_redshift(enzo_float value) { final_redshift_ = value; }
  ...
  void compute_expansion_timestep(enzo_float* dt_expansion,
                                  enzo_float time) const;

  void compute_expansion_factor(enzo_float* cosmo_a, enzo_float* cosmo_dadt,
                                enzo_float time) const;

  /// Return density unit at current time / redshift in terms of `g / cm^3`
  /// (requires set_current_time()).
  double density_units() const {
    /* Density unit is defined so that the mean physical matter density of the
     universe is 1. This can be written as:

     `rho_bar_m_0 * (1 + z)^3 / rho_unit = 1`

     Where `rho_bar_m_0` is the mean physical matter density at redshift 0, `z`
     is redshift, and `rho_unit` is the density unit in `g / cm^3`.

     We can then write this as:

     `rho_unit = `Omega_m_0 * rho_crit_0 * (1 + z)^3`,

     where `Omega_m_0` is a cosmological parameter which can be set in the input
     parameter file, and `rho_crit_0` is defined as `3 * H_0^2 / (8 * pi * G)`,
     where `H_0` is the expansion rate of the universe at redshift 0 (with
     dimensions of inverse time), and `G` is the gravitational constant. `H_0`
     can be written as `H_0 / h * h`, where `h` is given the name
     `hubble_constant_now_` in the code below. Putting this all together gives
     ...
     The reason for this inconsistency is unclear. It is possibly due to
     inconsistent values for `G`.
   ...
   */
    ...
    return (3.0 / (8.0 * cello::pi)) * enzo_constants::H0_over_h *
           enzo_constants::H0_over_h / enzo_constants::standard_grav_constant *
           omega_matter_now_ * hubble_constant_now_ * hubble_constant_now_ *
  ...
  double time_units() const {
    /* Time unit is defined so that the following is true (which has the effect
      of simplifying Poisson's equation):

      `4 * pi * G * rho_bar_m_com * time_unit^2 / a_unit^3 = 1`,

      Where `G` is the gravitational constant.`rho_bar_m_com` is the mean
      comoving matter density of the universe, `time_unit` is the time unit in
      seconds, and `a_unit` is the "unit cosmological scale factor". To
      understand where the factor of `a_unit^3` come from, see Equations 8, 17,
      18 of Greg L. Bryan et al 2014 ApJS 211 19, and note that Laplacian(`phi`)
      has dimensions of `time^(-2) * a^(2).

      In Enzo-E, `a_unit` is defined so that `a` is 1 at the initial redshift
      (`z_i`), so that:
      `(1+z_i)^(-1) / a_unit = 1`, which means that `a_unit = 1 / (1 + z_i)`.

      `rho_bar_m_0` can be written as `Omega_m_0 * rho_crit_0`, where
      `Omega_m_0` is a cosmological parameter which can be set in the input
      parameter file, and `rho_crit_0` is defined as `3 * H_0^2 / (8 * pi * G)`,
      where `H_0` is the expansion rate of the universe at redshift 0 (with
      dimensions of inverse time). Plugging this all in gives:
   ...
   */
    ...
    return sqrt(2.0 / 3.0) /
           (enzo_constants::H0_over_h * hubble_constant_now_ *
            sqrt(omega_matter_now_ * (1 + initial_redshift_) *
                 (1 + initial_redshift_) * (1 + initial_redshift_)));
  }

  /// Return velocity unit in cm/s
  double velocity_units() const {
    /* Equation 17 of Greg L. Bryan et al 2014 ApJS 211 19, shows that the
       dimensions of velocity are `a` times `comoving length` over `time`. As
       such, this expression is equivalent to:

       `velocity_unit` = `a_unit` * `length_unit / ((1 + z) * `time_unit`)

       where `z` is the current redshift, and `a_unit` is (1 + `z_i`), with
       `z_i` being the initial redshift.

       The `1.0e7` term is the result of multiplying `Mpc_cm` by `H0_over_h`.
    */
    return 1.0e7 * sqrt(3.0 / 2.0) * comoving_box_size_ *
           sqrt(omega_matter_now_) * sqrt(1 + initial_redshift_);
  }
