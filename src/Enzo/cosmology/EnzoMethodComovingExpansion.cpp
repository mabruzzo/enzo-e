...
EnzoMethodComovingExpansion::EnzoMethodComovingExpansion(
    ...
    : Method(), comoving_coordinates_(comoving_coordinates) {
...
void EnzoMethodComovingExpansion::compute(Block* block) throw() {
  ...
  */
  ...
  if ((block->is_leaf() && comoving_coordinates_ && field.field_count() > 0)) {
    ...
    int has_history =
        ((field.num_history() > 0) && (field.history_time(1) > 0.));
    enzo_float compute_time;
    if (has_history) {
      compute_time = 0.5 * (enzo_block->time() + field.history_time(1));
    } else {
      compute_time = enzo_block->time();
    }

    //      printf ("DEBUG_VELOCITY time old new = %g
    ...
    */
    ...
    (&rank, &m, &idual, &Coefficient, (int*)&HydroMethod, &gamma, pressure,
     density_new, total_energy_new, internal_energy_new, velocity_x_new,
     velocity_y_new, velocity_z_new, density_old, total_energy_old,
     internal_energy_old, velocity_x_old, velocity_y_old, velocity_z_old,
...
double EnzoMethodComovingExpansion::timestep(Block* block) throw() {
  ...
  if (!comoving_coordinates_) return (double)dtExpansion;
