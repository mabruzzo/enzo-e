...
enzo_float EnzoPhysicsCosmology::time_from_redshift(enzo_float redshift) const {
  ...
  if (omega_matter_now_ < 1 && omega_lambda_now_ == 0) {
    eta = acosh(1 + 2 * (1 - omega_matter_now_) / omega_matter_now_ /
                        (1 + redshift));
    time_hubble_0 = omega_matter_now_ /
                    (2 * pow(1.0 - omega_matter_now_, 1.5)) * (sinh(eta) - eta);
  }

  /* 3) For omega_matter_now_ > 1 && omega_lambda_now_ == 0, use sin/cos. */

  if (omega_matter_now_ > 1 && omega_lambda_now_ == 0) {
    eta = acos(1.0 - 2.0 * (1.0 - omega_matter_now_) / omega_matter_now_ /
                         (1.0 + redshift));
    time_hubble_0 = omega_matter_now_ /
                    (2.0 * pow(1.0 - omega_matter_now_, 1.5)) *
                    (eta - sin(eta));
  }
  ...
  return time_hubble_0 / (hubble_constant_now_ * 3.24e-18) / time_units;
}

//======================================================================

void EnzoPhysicsCosmology::compute_expansion_factor(enzo_float* cosmo_a,
                                                    enzo_float* cosmo_dadt,
  ...
  *cosmo_a = -1.0;
  ...
  enzo_float time_hubble_0 = time * this->time_units() * hubble_constant_now_ *
                             enzo_constants::H0_over_h;
  ...
  if (omega_matter_now_ < 1 && omega_lambda_now_ < OMEGA_TOLERANCE) {
    ...
    x = 2 * time_hubble_0 * pow(1.0 - omega_matter_now_, 1.5) /
        omega_matter_now_;
    ...
    *cosmo_a =
        omega_matter_now_ / (2 * (1 - omega_matter_now_)) * (cosh(eta) - 1);
    *cosmo_a *=
        (1 + initial_redshift_);  // to convert to code units, divide by [a]
  }
  ...
  if (fabs(omega_curvature_now_) < OMEGA_TOLERANCE &&
      omega_lambda_now_ > OMEGA_TOLERANCE) {
    *cosmo_a = pow(enzo_float(omega_matter_now_ / (1 - omega_matter_now_)),
                   enzo_float(1.0 / 3.0)) *
               pow(enzo_float(sinh(1.5 * sqrt(1.0 - omega_matter_now_) *
                                   time_hubble_0)),
                   enzo_float(2.0 / 3.0));
    *cosmo_a *=
        (1 + initial_redshift_);  // to convert to code units, divide by [a]
...
void EnzoPhysicsCosmology::compute_expansion_timestep(enzo_float* dt_expansion,
                                                      enzo_float time) const {
  /* Error check. */

  ASSERT("EnzoPhysicsCosmology::compute_expansion_timestep",
         "Initial time in code units is 0", initial_time_in_code_units() != 0);
