...
EnzoMethodInference::EnzoMethodInference(int level_base, int level_array,
                                         int level_infer,
  ...
  is_sync_child_ = scalar_descr_sync->new_value("method_inference_sync_child");

  //    parent-to-child sync counter
  is_sync_parent_ =
      scalar_descr_sync->new_value("method_inference_sync_parent");

  //    mask defining overlapping inference arrays to create
  is_mask_ = scalar_descr_void->new_value("method_inference_mask");

  //    count of inference arrays overlapping block
  is_count_ = scalar_descr_int->new_value("method_inference_count");
...
void EnzoMethodInference::compute_overdensity_(Block* block, char* mask, int nx,
                                               int ny, int nz) {
...
void EnzoMethodInference::merge_masks(Block* block, int n_in, char* mask_in,
                                      int ic3[3]) {
  const int level = block->level();
...
void EnzoMethodInference::merge_masks_2to1_(char* mask, char* mask_in,
                                            int level, int ic3[3]) {
  // get my mask size
  int nx, ny, nz;
  std::tie(nx, ny, nz) = mask_dims_(level);
...
void EnzoMethodInference::merge_masks_1to1_(char* mask, char* mask_in,
                                            int level) {
  // get my mask size
  int nx, ny, nz;
  std::tie(nx, ny, nz) = mask_dims_(level);
...
Index EnzoLevelArray::get_block_index_() {
  ...
  for (int level = 1; level <= level_base_; level++) {
    ...
    int ic3[3] = {(thisIndex[0] >> tree_shift) & 1,
...
void EnzoMethodInference::request_data(Block* block, int ia3[3]) {
  ...
  if (block->is_leaf()) {
    ...
    ASSERT1("EnzoMethodInference::request_data()",
            "Field group %s is empty or does not exist", field_group_.c_str(),
            (num_fields_ > 0));
    ...
    if (level > level_infer_) {
...
#endif
          coarsen_(a_c, ncx, ncy, ncz, ncx, ncy, ncz, ex, ey, ez, a_f, mfx, mfy,
                   mfz, nfx, nfy, nfz, ex, ey, ez);
      ...
      ASSERT2("EnzoMethodInference::request_data",
              "Mismatch betwen expected %d and actual %d buffer size", i_b, nb,
              (i_b == nb));
...
#endif
    proxy_level_array[index3].p_transfer_data(index_block, nb,
                                              buffer_values.data());
...
void EnzoLevelArray::p_transfer_data(Index index, int nb,
                                     enzo_float* buffer_values) {
#ifdef TRACE_INFER
  CkPrintf("TRACE_INFER p_transfer_data RECV %d %d %d\n", thisIndex[0],
           thisIndex[1], thisIndex[2]);
#endif
  // Loop through fields
  int i_b = 0;
  ...
  for (int i_f = 0; i_f < num_fields_; i_f++) {
    ...
    if (level < level_infer_) {
...
#endif
        interpolate_(a_f, mfx, mfy, mfz, nfx, nfy, nfz, efx, efy, efz, a_c, ncx,
                     ncy, ncz, ncx, ncy, ncz, ecx, ecy, ecz);
      ...
      CkPrintf("DEBUG_INFER copy a_d %d %d %d  %d %d %d  %d %d %d\n", mfx, mfy,
               mfz, nbx - 2, nby - 2, nbz - 2, 0, 0, 0);
      CkPrintf("DEBUG_INFER copy a_s %d %d %d  %d %d %d  %d %d %d\n", ncx, ncy,
               ncz, ncx - 2, ncy - 2, ncz - 2, 0, 0, 0);
#endif
      copy_(a_f, mfx, mfy, mfz, nbx - 2, nby - 2, nbz - 2, a_c, ncx, ncy, ncz,
            ncx - 2, ncy - 2, ncz - 2);
    }
  ...
  ASSERT1("EnzoLevelArray::p_transfer_data()",
          "volume_ratio_ %g not between 0.0 and 1.0 as expected", volume_ratio_,
...
void EnzoLevelArray::apply_inference() {
    ...
    for (int iz = 0; iz < niz_; iz++) {
      for (int iy = 0; iy < niy_; iy++) {
        for (int ix = 0; ix < nix_; ix++) {
          ...
          wsum +=
              (ix + 1 + 2 * (iy + 1) - 3 * (iz + 1)) * field_values_[i_f][i];
        }
      }
    }
    CkPrintf("DEBUG_INFER %s SUM %d %d %d %g\n", field_name.c_str(),
             thisIndex[0], thisIndex[1], thisIndex[2], sum);
    CkPrintf("DEBUG_INFER %s MIN %d %d %d %g\n", field_name.c_str(),
             thisIndex[0], thisIndex[1], thisIndex[2], min);
    CkPrintf("DEBUG_INFER %s MAX %d %d %d %g\n", field_name.c_str(),
             thisIndex[0], thisIndex[1], thisIndex[2], max);
    CkPrintf("DEBUG_INFER %s WSUM %d %d %d %g\n", field_name.c_str(),
             thisIndex[0], thisIndex[1], thisIndex[2], wsum);
    fflush(stdout);
  }
#endif
  //==================================================
  ...
  double center[3] = {0.5 * (lower[0] + upper[0]), 0.5 * (lower[1] + upper[1]),
                      0.5 * (lower[2] + upper[2])};
  ...
  CkPrintf("TRACE_INFER rectangle %d %g %g %g %g %g %g\n",
           cello::simulation()->cycle(), lower[0], lower[1], lower[2], upper[0],
           upper[1], upper[2]);
#endif
}
...
void EnzoMethodInference::update(Block* block, int n, char* buffer,
                                 int il3[3]) {
...
EnzoMethodInference::get_block_portion_(Index index, int index_field,
                                        int ia3[3]) {
  const int rank = cello::rank();
...
void EnzoMethodInference::coarsen_(enzo_float* a_c, int mcx, int mcy, int mcz,
                                   int ncx, int ncy, int ncz, int ecx, int ecy,
                                   int ecz, const enzo_float* a_f, int mfx,
                                   int mfy, int mfz, int nfx, int nfy, int nfz,
  ...
  if (rank == 1) {
    for (int icx = 0; icx < mcx; icx++) {
      int i_f = icx * 2;
      int i_c = icx;
      a_c[i_c] = 0.5 * (a_f[i_f + d[0]] + a_f[i_f + d[1]]);
    }
  } else if (rank == 2) {
    for (int icy = 0; icy < mcy; icy++) {
      for (int icx = 0; icx < mcx; icx++) {
        ...
        int i_c = icx + mcx * (icy);
        a_c[i_c] = 0.25 * (a_f[i_f + d[0]] + a_f[i_f + d[1]] + a_f[i_f + d[2]] +
                           a_f[i_f + d[3]]);
      }
    }
  } else if (rank == 3) {
    for (int icz = 0; icz < mcz; icz++) {
      for (int icy = 0; icy < mcy; icy++) {
        for (int icx = 0; icx < mcx; icx++) {
          ...
          a_c[i_c] =
              0.125 * (a_f[i_f + d[0]] + a_f[i_f + d[1]] + a_f[i_f + d[2]] +
                       a_f[i_f + d[3]] + a_f[i_f + d[4]] + a_f[i_f + d[5]] +
                       a_f[i_f + d[6]] + a_f[i_f + d[7]]);
        }
      }
    }
  }
...
void EnzoLevelArray::interpolate_(enzo_float* a_f, int mfx, int mfy, int mfz,
                                  int nfx, int nfy, int nfz, int efx, int efy,
                                  int efz, const enzo_float* a_c, int mcx,
                                  int mcy, int mcz, int ncx, int ncy, int ncz,
  ...
  if (cello::rank() == 1) {
    for (auto ifx = 0; ifx < nfx; ifx++) {
      ...
      auto i_f = ifx;
      auto i_c = icx;
      a_f[i_f] = (wx0 * a_c[i_c + d[0]] + wx1 * a_c[i_c + d[1]]);
    }
    ...
    for (auto ify = 0; ify < nfy; ify++) {
      ...
      for (auto ifx = 0; ifx < nfx; ifx++) {
        ...
        a_f[i_f] = (wy0 * wx0 * a_c[i_c + d[0]] + wy0 * wx1 * a_c[i_c + d[1]] +
                    wy1 * wx0 * a_c[i_c + d[2]] + wy1 * wx1 * a_c[i_c + d[3]]);
      }
    }
...
void EnzoLevelArray::copy_(enzo_float* a_dst, int mdx, int mdy, int mdz,
                           int ndx, int ndy, int ndz, const enzo_float* a_src,
                           int msx, int msy, int msz, int nsx, int nsy,
                           int nsz) {
...
void EnzoBlock::p_method_infer_exit() { compute_done(); }
...
void EnzoMethodInference::create_level_arrays_(Block* block) {
  ...
  ASSERT2("EnzoMethodInference::create_level_arrays_",
          "Block %s not in expected refinement level %d", block->name().c_str(),
          level, level == level_base_);
    ...
    for (int kz = 0; kz < mz; kz++) {
      for (int ky = 0; ky < my; ky++) {
        for (int kx = 0; kx < mx; kx++) {
          ...
          if (mask[k]) {
            Index3 index(i3[0] * mx + kx, i3[1] * my + ky, i3[2] * mz + kz);
            proxy_level_array[index].insert(field_group_, level_base_,
                                            level_array_, level_infer_, nax,
                                            nay, naz);
          }
        }
      }
