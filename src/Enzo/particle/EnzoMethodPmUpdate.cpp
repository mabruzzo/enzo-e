...
#ifdef DEBUG_UPDATE
#define TRACE_PM(MESSAGE) CkPrintf("%s:%d %s\n", __FILE__, __LINE__, MESSAGE);
#else
...
EnzoMethodPmUpdate::EnzoMethodPmUpdate(ParameterGroup p)
    : Method(),
...
void EnzoMethodPmUpdate::compute(Block* block) throw() {
  ...
  if (block->is_leaf() && num_is_grav > 0) {
    ...
    CkPrintf("DEBUG_UPDATE asum %g %g %g vsum %g %g %g\n", a3sum[0], a3sum[1],
             a3sum[2], v3sum[0], v3sum[1], v3sum[2]);
    CkPrintf("DEBUG_UPDATE asum2 %g %g %g vsum2 %g %g %g\n", a3sum2[0],
             a3sum2[1], a3sum2[2], v3sum2[0], v3sum2[1], v3sum2[2]);
#endif
  }
