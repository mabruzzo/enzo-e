...
public:
  ...
  EnzoMethodFeedbackSTARSS(CkMigrateMessage* m) : Method(m), ir_feedback_(-1) {}
  ...
  int determineSN(double age_Myr, int* nSNII, int* nSNIA, double mass_Msun,
                  double tunit, float dt);

  int determineWinds(double age_Myr, double* eWinds, double* mWinds,
                     double* zWinds, double mass_Msun, double metallicity_Zsun,
                     double tunit, double dt);

  // this can raise errors -- remove const throw() ???
  void deposit_feedback(Block* block, double ejectaEnergy, double ejectaMass,
                        double ejectaMetals, const enzo_float up,
                        const enzo_float vp, const enzo_float wp,
                        const enzo_float xp, const enzo_float yp,
                        const enzo_float zp, const int ix, const int iy,
                        const int iz, const int winds, const int nSNII,
                        const int nSNIa, const double starZ) const throw();

  void transformComovingWithStar(enzo_float* density, enzo_float* velocity_x,
                                 enzo_float* velocity_y, enzo_float* velocity_z,
                                 const enzo_float up, const enzo_float vp,
                                 const enzo_float wp, const int mx,
                                 const int my, const int mz,
                                 int direction) const throw();

  void add_accumulate_fields(EnzoBlock* enzo_block) throw();

  // window function for calculating CiC fractions. May make more sense to put
  // this in Cello somewhere
