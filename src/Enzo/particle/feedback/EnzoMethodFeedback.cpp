...
EnzoMethodFeedback::EnzoMethodFeedback(ParameterGroup p) : Method() {
  ...
  if (particle.num_particles(it) > 0) {
    ...
    for (int ib = 0; ib < nb; ib++) {
      ...
      for (int ip = 0; ip < np; ip++) {
        ...
        pmass[ipdm] =
            pmass[ipdm] - ejecta_mass_;  // subtract mass from particle
        ...
        if (metal) metal[i] += ejecta_metal_fraction_ * ejecta_mass_ * inv_vol;
