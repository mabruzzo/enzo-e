...
EnzoMethodDistributedFeedback::EnzoMethodDistributedFeedback(ParameterGroup p)
    : Method() {
  ...
  use_ionization_feedback_ = p.value_logical("use_ionization_feedback", false);
    ...
    for (int ib = 0; ib < nb; ib++) {
      ...
      for (int ip = 0; ip < np; ip++) {
...
#ifdef DEBUG_FEEDBACK
        CkPrintf(
            "DistributedFeedback (%s) -- Particle %i of %i particles in batch "
            "%i local at pos (%g, %g, %g) - local = %i\n",
        ...
        */
        ...
        const double time_last_sn =
            37.7 * enzo_constants::Myr_s / enzo_units->time();
        ...
        if ((plifetime[ipdl] <= 0.0) ||
          ...
          const float lambda =
              expected_sn_s99 * 1.0E-6 * star_maker_method->minimum_star_mass();
...
#ifdef DEBUG_FEEDBACK
          CkPrintf(
              "DistributedFeedback (%s) - This particle with m %f and id %d "
              "has %d supernova \n",
              enzo_block->name().c_str(), pmass[ipdm], id[ipid], number_of_sn);
#endif
          if (number_of_sn == 0) {
            explosion_flag = 0;
            continue;
          ...
          for (int kk = 0; kk < number_of_sn; kk++) {
            ...
            delay_time *=
                enzo_constants::yr_s / enzo_units->time();  // code units
            ...
            if ((delay_time > star_age) &&
...
#ifdef DEBUG_FEEDBACK
              CkPrintf(
                  "DistributedFeedback (%18s): %d of %d - delay: %f  age+dt: "
                  "%g  pm: %g \n",
                  enzo_block->name().c_str(), kk + 1, number_of_sn, delay_time,
                  star_age, progenitor_mass);
#endif
              ...
              if (explosion_flag == -1) {
                ...
                explosion_flag += 1;  // add to the counters - this is not
            ...
            //            if ( star_age < delay_time + 0.1 *
            //            enzo_constants::Myr_s / enzo_units->time()){
        ...
        double tsoon7 = soonest_explosion * enzo_units->time() /
                        (1.0E7 * enzo_constants::yr_s);
        double wind_energy = 0.0;
        if (time_first_sn_ > 0) {
          // --- for testing ---
          wind_energy =
              1.0E48 / (8.0 * enzo_constants::mass_solar);  // non zero erg
        } else {
          wind_energy = s99_wind_energy(
              td7, tsoon7);  // in cm^2/s^2 (i.e. per unit mass in cgs)
        }
        wind_energy = wind_energy * wind_mass *
                      enzo_constants::mass_solar;  // now total E in erg

        star_age = current_time - pcreation[ipdc];  // code units
        td7 = star_age * enzo_units->time() / enzo_constants::yr_s /
...
#ifdef DEBUG_FEEDBACK
    CkPrintf(
        "DistributedFeedback (%s) ------------1------------ Number of local / "
        "total particles : %i %i\n",
...
#ifdef DEBUG_FEEDBACK
    CkPrintf(
        "DistributedFeedback (%s) ------------2------------ Number of local / "
        "total particles : %i %i\n",
        enzo_block->name().c_str(), particle.num_local_particles(it),
        particle.num_particles(it));
#endif
...
void EnzoMethodDistributedFeedback::add_ionization_feedback(
  ...
  double ionized = enzo_constants::kboltz * 1.0E4 / std::min(0.6, mol_weight) /
                   enzo_constants::mass_hydrogen /
...
#ifdef DEBUG_FEEDBACK
  CkPrintf(
      "DistributedFeedback (%s): This cell should be getting ionized with "
      "radius = %g (pc). volume ratio = %g. will_explode = %i ge = %g ionized "
      "= %g\n",
      enzo_block->name().c_str(), stromgren_radius / enzo_constants::pc_cm,
...
void EnzoMethodDistributedFeedback::inject_feedback(
  ...
  */
  ...
  if (ke_f < 0) {
    ...
    avg_n *= inv_ncell * enzo_units->density() /
             enzo_constants::mass_hydrogen;      // in cgs
  ...
  if (ke_f > 0) {
    ...
    this->add_feedback_to_grid(
        ...
        stencil_rad_,  // local grid cell center  - should be 1 for 3x3x3
                       // stencil (0,1,2) - 2 for 5x5x5 stencil (0,1, 2, 3,4)
