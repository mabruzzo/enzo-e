...
EnzoMethodStarMakerSTARSS::EnzoMethodStarMakerSTARSS(ParameterGroup p)
...
void EnzoMethodStarMakerSTARSS::compute(Block* block) throw() {
  ...
  enzo_float* density_particle_accumulate =
      field.is_field("density_particle_accumulate")
  ...
  for (int iz = gz; iz < nz + gz; iz++) {
    for (int iy = gy; iy < ny + gy; iy++) {
      for (int ix = gx; ix < nx + gx; ix++) {
        ...
        double mu;
        // compute MMW -- TODO: Make EnzoComputeMeanMolecularWeight class and
        // reference mu_field here?
        if (primordial_chemistry > 0) {
          ...
          double ndens_times_mH = d_el[i] + dHI[i] + dHII[i] +
                                  0.25 * (dHeI[i] + dHeII[i] + dHeIII[i]);
        ...
        double metallicity =
            (metal) ? metal[i] / density[i] / enzo_constants::metallicity_solar
                    : 0.0;
        ...
        CkPrintf(
            "MethodFeedbackSTARSS -- overdensity threshold passed! rho=%f\n",
            dmean);
#endif

        // check velocity divergence < 0
        if (!this->check_velocity_divergence(velocity_x, velocity_y, velocity_z,
        ...
        // check that alpha < 1
        if (use_altAlpha_) {
          if (!this->check_self_gravitating_alt(total_energy[i], potential[i]))
            continue;
        }

        else {
          if (!this->check_self_gravitating(
                  mean_particle_mass, density[i], temperature[i], velocity_x,
                  velocity_y, velocity_z, lunit, vunit, rhounit, i, idx, idy,
                  idz, dx, dy, dz))
            continue;
        }
        ...
        // In order to check cooling time, must have
        // use_temperature_threshold=true;
        ...
        if (cooling_time) {  // if we are evolving a "cooling_time" field
          if (!this->check_cooling_time(cooling_time[i], total_density, tunit,
                                        rhounit))
            continue;
        }

        // check that M > Mjeans
        if (!check_jeans_mass(temperature[i], mean_particle_mass, density[i],
                              cell_mass, munit, rhounit))
          continue;
        ...
        double f_shield = this->h2_self_shielding_factor(
            density, metallicity, rhounit, lunit, i, idx, idy, idz, dx, dy, dz);
        ...
        double tff =
            sqrt(3 * cello::pi /
                 (32 * enzo::grav_constant_cgs() * density[i] * rhounit)) /
            tunit;
        /* Determine Mass of new particle
                 WARNING: this removes the mass of the formed particle from the
                          host cell.  If your simulation has very small (>15
           Msun) baryon mass per cell, it will break your sims! - AIW
        */
        ...
        if (maximum_star_mass < 0) {
          maximum_star_mass =
              this->maximum_star_fraction_ * cell_mass * munit_solar;  // Msun
        }

        double bulk_SFR = f_shield * this->maximum_star_fraction_ * cell_mass *
                          munit_solar / divisor;

        // Probability has the last word
        // FIRE-2 uses p = 1 - exp (-MassShouldForm*dt / M_gas_particle) to
        // convert a whole particle to star particle
        //  We convert a fixed portion of the baryon mass (or the calculated
        //  amount)

        double p_form =
            1.0 -
            std::exp(-bulk_SFR * dt * (tunit / enzo_constants::Myr_s) /
        ...
        /* New star is mass_should_form up to f_shield*maximum_star_fraction_ *
           baryon mass of the cell, but at least 15 msun */
        double new_mass =
            std::min(f_shield * maximum_star_fraction_ * cell_mass,
                     maximum_star_mass / munit_solar);
        ...
        if ((new_mass * munit_solar < minimum_star_mass)  // too small
            || (random > p_form)                          // too unlikely
            || (new_mass > cell_mass)  // too big compared to cell
        ) {
#ifdef DEBUG_SF_CRITERIA
          CkPrintf(
              "MethodStarMakerSTARSS -- star mass is either too big, too "
              "small, or failed the dice roll\n");
#endif
          continue;
        }
        ...
        double max_massPerStar = 5e3;  // TODO: either make this a new parameter
                                       // or replace 'maximum_star_mass'
        ...
        if (new_mass * munit_solar > max_massPerStar) {  //
          ...
          // spread over three dynamical times. This is to prevent huge
          // particles suddenly dumping a HUGE amount of ionizing radiation at
          // once.
          // TODO: Enforce that if dt < 3 dynamical times, new star particles
          // form in the next timestep
          //       Can do this by creating the particle now so we can still
          //       access it at the next timestep, but not assigning it any
          //       attributes until it's actually supposed to form

          n_newStars = std::floor(new_mass * munit_solar / mass_split);
          massPerStar = new_mass / n_newStars;
#ifdef DEBUG_SF_CRITERIA
          CkPrintf(
              "MethodStarMakerSTARSS -- Predicted cluster mass %1.3e Msun > "
              "%1.3e Msun;\n"
              "                         splitting into %d particles with mass "
              "%1.3e Msun\n",
              new_mass * munit_solar, max_massPerStar, n_newStars,
              massPerStar * munit_solar);
#endif
        }
        for (int n = 0; n < n_newStars; n++) {
...
#ifdef DEBUG_SF_CRITERIA
          CkPrintf(
              "MethodStarMakerSTARSS -- Forming star in gas with number "
              "density %f cm^-3\n",
              ndens);
#endif
          ...
          double vy = 0.0;
          double vz = 0.0;
          for (int ix_ = std::max(0, ix - 3); ix_ <= std::min(ix + 3, mx);
               ix_++) {
            for (int iy_ = std::max(0, iy - 3); iy_ <= std::min(iy + 3, my);
                 iy_++) {
              for (int iz_ = std::max(0, iz - 3); iz_ <= std::min(iz + 3, mz);
                   iz_++) {
          ...
          if (std::abs(vx) > max_velocity)
            vx = vx / std::abs(vx) * max_velocity;
          if (std::abs(vy) > max_velocity)
            vy = vy / std::abs(vy) * max_velocity;
          if (std::abs(vz) > max_velocity)
            vz = vz / std::abs(vz) * max_velocity;
          ...
          // TODO: Need to have some way of calculating lifetime based on
          // particle mass
          plifetime[io] =
              25.0 * enzo_constants::Myr_s /
              enzo_units->time();  // lifetime (not accessed for STARSS FB)
          ...
          // TODO: If particle position is updated to CM instead of being
          // cell-centered, will have to
          //       remove mass using CiC, which could complicate things because
          //       that CiC cloud could leak into the ghost zones. Would have to
          //       use same refresh+accumulate machinery as MethodFeedbackSTARSS
          //       to account for this.
...
#ifdef DEBUG_STORE_INITIAL_PROPERTIES
          enzo_float* pmass0 =
              (enzo_float*)particle.attribute_array(it, ia_m_0, ib);
          enzo_float* px0 =
              (enzo_float*)particle.attribute_array(it, ia_x_0, ib);
          enzo_float* py0 =
              (enzo_float*)particle.attribute_array(it, ia_y_0, ib);
          enzo_float* pz0 =
              (enzo_float*)particle.attribute_array(it, ia_z_0, ib);
          enzo_float* pvx0 =
              (enzo_float*)particle.attribute_array(it, ia_vx_0, ib);
          enzo_float* pvy0 =
              (enzo_float*)particle.attribute_array(it, ia_vy_0, ib);
          enzo_float* pvz0 =
              (enzo_float*)particle.attribute_array(it, ia_vz_0, ib);
