...
EnzoSinkParticle::EnzoSinkParticle(Block* block, int ib, int ip,
  ...
  paccrate =
      (enzo_float*)particle.attribute_array(it, ia_accrate, batch_index_);
  pmetalfrac =
      metals ? (enzo_float*)particle.attribute_array(it, ia_mf, batch_index_)
             : nullptr;
...
bool EnzoSinkParticle::cell_in_accretion_zone(int i, int j, int k) throw() {
  ...
  // Get the components of the displacement vector of the center of the cell
  // from the particle
...
void EnzoSinkParticle::update(enzo_float density_change, int index) throw() {
  ...
  if (cello::particle_descr()->has_attribute(it, "metal_fraction"))
    total_pmetal_mass_change_ =
        (density_change / density[index]) * metal_density[index];

  // Set density_sink equal to minus the density change
  density_source[index] = -density_change;

  // These variables store the change of the particle's momentum due to
...
void EnzoSinkParticle::write_particle_data() throw() {
  ...
  if (pmetalfrac)
    pmetalfrac[particle_index_ * dmf] =
        (old_metal_mass + total_pmetal_mass_change_) /
        pmass[particle_index_ * dm];
