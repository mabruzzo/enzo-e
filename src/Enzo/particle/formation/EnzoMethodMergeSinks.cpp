...
EnzoMethodMergeSinks::EnzoMethodMergeSinks(ParameterGroup p)
  ...
  ASSERT("EnzoMethodMergeSinks::EnzoMethodMergeSinks()",
         "EnzoMethodMergeSinks requires that we run a 3D problem (Domain:rank "
         "= 3)",
         cello::rank());
  ...
  ASSERT(
      "EnzoMethodMergeSinks::EnzoMethodMergeSinks()",
  ...
  ASSERT("EnzoMethodMergeSinks::EnzoMethodMergeSinks()",
         "EnzoMethodMergeSinks requires that number of root blocks across all "
         "axes "
         "is at least 2, i.e. that Mesh:root_blocks = [a,b,c], with a, b and c "
         "all "
         "greater than 2",
  ...
  if (num_particles > 1) {
    ...
    for (int i = 0; i < ngroups; i++) {
      ...
      if (group_size[i] > 1) {
        ...
        pmetal = metals ? (enzo_float*)particle.attribute_array(it, ia_mf, ib1)
                        : nullptr;
        ...
        for (int j = 1; j < group_size[i]; j++) {
          ...
          pmetal = metals
                       ? (enzo_float*)particle.attribute_array(it, ia_mf, ib2)
                       : nullptr;
          ...
          CkPrintf(
              ...
              "Particle 2: Mass = %g, "
              "Position = (%g,%g,%g), ID = %ld \n",
              i + 1, pmass1, px1, py1, pz1, pid1, pmass2, px2, py2, pz2, pid2);
...
#ifdef DEBUG_MERGESINKS
          CkPrintf(
              "Particle number %d in group %d out of %d on Block %s "
