  ...
  for (int iz = gz; iz < nz + gz; iz++) {
    for (int iy = gy; iy < ny + gy; iy++) {
      for (int ix = gx; ix < nx + gx; ix++) {
        ...
        double mean_particle_mass =
            nominal_mol_weight * enzo_constants::mass_hydrogen;
        double ndens = rho_cgs / mean_particle_mass;

        double mass = density[i] * dx * dy * dz * enzo_units->mass() /
        ...
        if (!this->check_self_gravitating(
                mean_particle_mass, rho_cgs, temperature[i], velocity_x,
                velocity_y, velocity_z, enzo_units->length(),
                enzo_units->velocity(), enzo_units->density(), i, 1, mx,
                mx * my, dx, dy, dz))
          continue;
        ...
        const double f_h2 = this->h2_self_shielding_factor(
            density, metallicity, enzo_units->density(), enzo_units->length(),
            i, 1, mx, mx * my, dx, dy, dz);
        mass *= f_h2;  // apply correction (f_h2 = 1 if not used)

        if (!this->check_velocity_divergence(velocity_x, velocity_y, velocity_z,
                                             i, 1, mx, mx * my, dx, dy, dz))
          continue;
        // Check whether mass in [min_mass, max_range] range and if specified,
        // Jeans unstable
        ...
        double star_fraction =
            this->use_dynamical_time_
                ? std::min(this->efficiency_ * enzo_block->dt *
                               enzo_units->time() / tdyn,
                           1.0)
                : this->efficiency_;
        ...
        if (star_fraction * mass < this->star_particle_min_mass_) {
          ...
          double probability =
              this->efficiency_ * mass / this->star_particle_min_mass_;
          if (rnum > probability) {
          ...
          // maximum particle mass OR the maximum gas->stars conversion
          // fraction. AJE: Note, this forces there to be at most one particle
          // formed per
          //      cell per timestep. In principle, this could be bad if
          //      the computed gas->stars mass (above) is >> than maximum
          //      particle mass b/c it would artificially extend the lifetime of
          //      the SF region and presumably increase the amount of SF and
          //      burstiness of the SF and feedback cycle. Check this!!!!

          if (star_fraction * mass > this->star_particle_max_mass_) {
#ifdef DEBUG_SF
            CkPrintf(
                "DEBUG_SF: StochasticSF - SF mass = %g ; max particle mass = "
                "%g\n",
        ...
        plifetime[io] = tdyn;            // 10.0 * enzo_constants::Myr_s /
                                         // enzo_units->time() ; // lifetime
        ...
        if (density[i] < 0) {
          CkPrintf(
              "StochasticSF: density index star_fraction mass: %g %i %g %g\n",
