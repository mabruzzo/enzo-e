...
EnzoBondiHoyleSinkParticle::EnzoBondiHoyleSinkParticle(Block* block, int ib,
                                                       int ip,
                                                       double accretion_radius,
                                                       double const_G)
    : EnzoSinkParticle(block, ib, ip, accretion_radius), const_G_(const_G) {
  init_();
}
...
void EnzoBondiHoyleSinkParticle::set_c_s_inf_2_() throw() {
  ...
  // Copy host_cell_1d_index_ into a new const int called `i` for make code less
  // verbose
...
double EnzoBondiHoyleSinkParticle::get_cell_weight_(double r2) throw() {
  ...
  const double kernel_radius =
      (r_BH_ < 0.25 * min_cell_width)
          ? 0.25 * min_cell_width
          : ((r_BH_ <= 0.5 * accretion_radius_) ? r_BH_
                                                : 0.5 * accretion_radius_);
...
double EnzoBondiHoyleSinkParticle::get_cell_bound_fraction_(int ix, int iy,
                                                            int iz) throw() {
  // Get cell widths
  double hx, hy, hz;
  block_->cell_width(&hx, &hy, &hz);
  ...
  for (int i_mc = 0; i_mc < 8; i_mc++) {
    for (int j_mc = 0; j_mc < 8; j_mc++) {
      for (int k_mc = 0; k_mc < 8; k_mc++) {
        ...
        // Boolean which stores whether the mini-cell is bound to the sink
        // particle
        bool mini_cell_is_bound;

        // Compute square of distance of center of mini-cell from sink particle
        const double r2 = (x - px_) * (x - px_) + (y - py_) * (y - py_) +
                          (z - pz_) * (z - pz_);

        // If r is less than 10^(-6) times the minimum cell width, assume
        // mini-cell is bounded. This avoids having to compute gravitational
        // potential energy for small r. In practice, this should never happen,
        // since this function is not called for the sink particle's host cell,
        // but let's just put this in for safety.
        if (r2 < 1.0e-12 * min_cell_width * min_cell_width)
          mini_cell_is_bound = true;

        else {
          // Treat the mini-cell as a test particle on a Keplerian orbit around
          // the sink particle. If its specific orbital energy is non-negative,
          // it is not bound to the particle. If negative, we compute the
          // periapsis of the orbit, and if it is less than a quarter of the
          // mininum cell width, then the mini-cell is bound (Equation 15). Note
          // that if the current position is less than a quarter of the minimum
          // cell width from the sink particle, and the specific orbital energy
          // is negative, then we do not need to compute periapsis since we
          // already know it must be less than or equal the current distance to
          // the sink particle.
          ...
          if (specific_orbital_energy >= 0.0)
            mini_cell_is_bound = false;
          else if (r <= 0.25 * min_cell_width)
            mini_cell_is_bound = true;
          else {
            ...
            const double j_mag_2 = j_x * j_x + j_y * j_y + j_z * j_z;
...
void EnzoBondiHoyleSinkParticle::set_accretion_rate_() throw() {
  ...
  for (decltype(cell_weights_.size()) i = 0; i < cell_weights_.size(); ++i)
    weighted_mean_density +=
        cell_weights_[i] * density[acc_zone_1d_indices_[i]];
  ...
  const double rho_inf =
      weighted_mean_density / alpha_(1.2 * min_cell_width / r_BH_);
...
void EnzoBondiHoyleSinkParticle::compute_relative_velocity_(
    const double x, const double y, const double z, double* vx_rel,
    double* vy_rel, double* vz_rel) throw() {
...
#define NTABLE 51
  ...
  double alphatable[NTABLE] = {
      820.254, 701.882, 600.752, 514.341, 440.497, 377.381, 323.427, 277.295,
      237.845, 204.1,   175.23,  150.524, 129.377, 111.27,  95.7613, 82.4745,
      71.0869, 61.3237, 52.9498, 45.7644, 39.5963, 34.2989, 29.7471, 25.8338,
      22.4676, 19.5705, 17.0755, 14.9254, 13.0714, 11.4717, 10.0903, 8.89675,
      7.86467, 6.97159, 6.19825, 5.52812, 4.94699, 4.44279, 4.00497, 3.6246,
      3.29395, 3.00637, 2.75612, 2.53827, 2.34854, 2.18322, 2.03912, 1.91344,
      1.80378, 1.70804, 1.62439};
  ...
  if (x < XMIN) {
    ...
    return alphatable[idx] *
           pow(alphatable[idx + 1] / alphatable[idx], alpha_exp);
  }
...
void EnzoBondiHoyleSinkParticle::compute(double density_threshold,
  ...
  for (int i = 0; i < n_cells; i++) {
    ...
    double density_change = bound_fractions_[i] * cell_weights_[i] *
                            accretion_rate_ * block_->dt() * inv_cell_volume;

    // Restrict density change via density_threshold and max_mass_fraction
    density_change =
        std::min(std::min(density_change, density[index] - density_threshold),
