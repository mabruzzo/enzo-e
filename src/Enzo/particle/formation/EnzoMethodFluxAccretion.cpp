...
EnzoMethodFluxAccretion::EnzoMethodFluxAccretion(ParameterGroup p)
    : EnzoMethodAccretion(p) {
  // The number of cell widths within the accretion radius cannot be larger than
  // any of the ghost depths minus 1. This is because this method requires
  // computing derivatives, so cells at the edge of the accretion zone must have
  // another cell "outside" it.
  ...
  ASSERT("EnzoMethodFluxAccretion::EnzoMethodFluxAccretion() ",
         "The accretion radius must be no greater than than the ghost depth "
         "minus one."
  ...
  if (num_particles > 0) {
    ...
    for (int ib = 0; ib < nb; ib++) {
      ...
      for (int ip = 0; ip < np; ip++) {
        ...
        EnzoFluxSinkParticle sp = EnzoFluxSinkParticle(
            block, ib, ip, accretion_radius, density_threshold);
