...
public:
  ...
  ///            `density_threshold`         - density threshold set by the
  ///            accretion method
  EnzoFluxSinkParticle(Block* block, int ib, int ip, double accretion_radius,
  ...
  /// the accretion rate over cell volume multiplied by the timestep multiplied
  /// by the fraction of mass in the whole accretion zone which is in the cell
  /// (i.e., the mass in the cell divided by the mass in the accretion zone).
...
private:
  ...
  /// - Computes the total mass flux into the accretion zone divided by the cell
  /// volume
  ///   (Equation 35)
  /// - Computes the accretion rate divided by cell volume (Equation 36)
  void init_() throw();

  /// Computes and returns the mass flux divided by the cell volume through a
  /// given cell, specified by `ix`,`iy`,`iz`. See Equation 35.
  double get_mass_flux_over_cell_volume_(int ix, int iy, int iz) throw();

  /// Computes and sets accretion rate divided by cell volume according to
  /// Equation 36.
  ...
  /// Value of the accretion rate. Computed by
  /// set_accretion_rate_over_cell_volume(). This does not necessarily determine
  /// the total mass increase of the sink particle, since the actual density
  /// change of each cell will be limited by max_mass_fraction_ and
  /// density_threshold_
