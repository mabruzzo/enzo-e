...
EnzoMethodSinkMaker::EnzoMethodSinkMaker(ParameterGroup p) noexcept
  ...
  ASSERT("EnzoMethodSinkMaker::EnzoMethodSinkMaker()",
         "Cannot use sink_maker method with rank < 3", cello::rank() == 3);
...

{
  ...
  // This vector will store the indices of cells in which sinks are formed, and
  // so will be used later to set new values for density fields.
  ...
  block->index_global(&ix_block, &iy_block, &iz_block, &nx_block, &ny_block,
                      &nz_block);

  const int n_blocks = nx_block * ny_block * nz_block;
  const int global_block_index =
      INDEX(ix_block, iy_block, iz_block, nx_block, ny_block);

  // Now get the total number of cells per block
  const int n_cells_per_block = mx * my * mz;

  // We want a unique ID for each cell for each cycle. This will be used to set
  // sink particle IDs and will be used for the seeds for the random number
  // generator which generates the initial particle positions.
  const uint64_t global_cell_index_start =
      (enzo::simulation()->cycle() * n_blocks + global_block_index) *
      n_cells_per_block;
  ...
  for (int iz = gz; iz < mz - gz; iz++) {
    for (int iy = gy; iy < my - gy; iy++) {
      for (int ix = gx; ix < mx - gx; ix++) {
        ...
        // Check if the mass of the potential sink particle is greater than the
        // minimum sink mass
        ...
        if (!jeans_length_not_resolved_(block, block_cell_index, const_G))
          continue;

        // Check whether flow is converging
        if (!flow_is_converging_(block, ix, iy, iz)) continue;

        // If check_density_maximum_ is true, check if this cell's density is a
        // local maximum
        if (check_density_maximum_ &&
            !density_is_local_maximum_(block, ix, iy, iz))
          continue;

        // If we are here, this cell satisfies all the conditions for forming a
        // sink particle.

        // Compute global cell index
        const uint64_t global_cell_index =
            global_cell_index_start + block_cell_index;
        ...
        // Get 3 seeds for the random number generator using the global cell
        // index and offset_seed_shift_
        ...
        // x_offset is in range
        // [-hx*max_offset_cell_fraction_,hx*max_offset_cell_fraction_]
        generator.seed(x_seed);
        const double x_offset = hx * max_offset_cell_fraction_ *
                                (2.0 * distribution(generator) - 1.0);
        // y_offset is in range
        // [-hy*max_offset_cell_fraction_,hy*max_offset_cell_fraction_]
        generator.seed(y_seed);
        const double y_offset = hy * max_offset_cell_fraction_ *
                                (2.0 * distribution(generator) - 1.0);
        // z_offset is in range
        // [-hz*max_offset_cell_fraction_,hz*max_offset_cell_fraction_]
        generator.seed(z_seed);
        const double z_offset = hz * max_offset_cell_fraction_ *
                                (2.0 * distribution(generator) - 1.0);
        ...
        if (metal_density) {
          pmetal_fraction = (enzo_float*)particle.attribute_array(
              it, ia_metal_fraction, ibatch);
  ...
  // Loop over the cells which formed sink particles and set new values for
  // density fields.
  for (decltype(cells_forming_sinks.size()) i = 0;
       i < cells_forming_sinks.size(); i++) {
    int ind = cells_forming_sinks[i];
    EnzoMethodStarMaker::rescale_densities(enzo::block(block), ind,
...
bool EnzoMethodSinkMaker::jeans_length_not_resolved_(Block* block, int i,
  ...
  // Do this by getting the specific internal energy in this cell, and then
  // computing the sound speed squared from that.
  ...
  enzo_float* bx = field.is_field("bfield_x")
                       ? (enzo_float*)field.values("bfield_x")
                       : nullptr;
  enzo_float* by = field.is_field("bfield_y")
                       ? (enzo_float*)field.values("bfield_y")
                       : nullptr;
  enzo_float* bz = field.is_field("bfield_z")
                       ? (enzo_float*)field.values("bfield_z")
                       : nullptr;

  // Get the specific internal energy
  enzo_float specific_ie_cell;
  if (dual_energy)
    specific_ie_cell = specific_ie_field[i];
  else if (bx)
    specific_ie_cell =
        specific_te[i] - 0.5 * (vx[i] * vx[i] + vy[i] * vy[i] + vz[i] * vz[i]) -
  ...
  const enzo_float jeans_length =
      sqrt(cello::pi * c_s_2 / (const_G * density[i]));
...
bool EnzoMethodSinkMaker::flow_is_converging_(Block* block, int ix, int iy,
                                              int iz) throw() {
  // Get field data
  Field field = block->data()->field();
  int mx, my, mz;
  ...
  // Since trace is also equal to the sum of the eigenvalues, if the trace is
  // non-negative, at least one of the eigenvalues is non-negative.
  const double a_11 =
      (vx[INDEX(ix + 1, iy, iz, mx, my)] - vx[INDEX(ix - 1, iy, iz, mx, my)]) /
      (2.0 * hx);
  const double a_22 =
      (vy[INDEX(ix, iy + 1, iz, mx, my)] - vy[INDEX(ix, iy - 1, iz, mx, my)]) /
      (2.0 * hy);
  const double a_33 =
      (vz[INDEX(ix, iy, iz + 1, mx, my)] - vz[INDEX(ix, iy, iz - 1, mx, my)]) /
      (2.0 * hz);
  ...
  const double a_12 =
      0.5 *
      ((vx[INDEX(ix, iy + 1, iz, mx, my)] - vx[INDEX(ix, iy - 1, iz, mx, my)]) /
           (2.0 * hy) +
       (vy[INDEX(ix + 1, iy, iz, mx, my)] - vy[INDEX(ix - 1, iy, iz, mx, my)]) /
           (2.0 * hx));

  const double a_13 =
      0.5 *
      ((vx[INDEX(ix, iy, iz + 1, mx, my)] - vx[INDEX(ix, iy, iz - 1, mx, my)]) /
           (2.0 * hz) +
       (vz[INDEX(ix + 1, iy, iz, mx, my)] - vz[INDEX(ix - 1, iy, iz, mx, my)]) /
           (2.0 * hx));

  const double a_23 =
      0.5 *
      ((vy[INDEX(ix, iy, iz + 1, mx, my)] - vy[INDEX(ix, iy, iz - 1, mx, my)]) /
           (2.0 * hz) +
       (vz[INDEX(ix, iy + 1, iz, mx, my)] - vz[INDEX(ix, iy - 1, iz, mx, my)]) /
           (2.0 * hy));
  ...
  const double B = -1.0 * (a_11 * a_22 + a_11 * a_33 + a_22 * a_33 +
                           a_12 * a_12 + a_23 * a_23 + a_13 * a_13);
...
bool EnzoMethodSinkMaker::density_is_local_maximum_(Block* block, int ix,
                                                    int iy, int iz) throw() {
...
void EnzoMethodSinkMaker::do_checks_(Block* block) throw() {
  ...
  ASSERT("EnzoMethodSinkMaker", "sink_maker must precede pm_update",
         enzo::problem()->method_precedes("sink_maker", "pm_update"));

  // Check if either PPM or VL+CT method is being used.
  ASSERT("EnzoMethodSinkMaker", "accretion requires ppm or vlct methods.",
  ...
  // a fixed physical density quantity will increase with time. So if the
  // density threshold is above the density floor at the start of the
  // simulation, it is guaranteed to be above the density floor at all times.
  ASSERT2("EnzoMethodSinkMaker",
          ...
          "(code density units).",
          density_threshold, density_floor, density_threshold >= density_floor);
