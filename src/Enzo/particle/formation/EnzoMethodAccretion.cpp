...
EnzoMethodAccretion::EnzoMethodAccretion(ParameterGroup p)
    : Method(),
      accretion_radius_cells_(p.value_float("accretion_radius_cells", 4.0)),
      physical_density_threshold_cgs_(
          p.value_float("physical_density_threshold_cgs", 1.e-24)),
  ...
  ASSERT("EnzoMethodAccretion::EnzoMethodAccretion()",
         "EnzoMethodAccretion requires that we run a 3D problem (Domain: rank "
         "= 3)",
         cello::rank());
  ...
  const int min_ghost_depth =
      std::min(ghost_depth[0], std::min(ghost_depth[1], ghost_depth[2]));
  ...
  // Second refresh: add source fields values in ghost zones to
  // source_accumulate fields values in active cells in neighbouring blocks
  ...
  refresh_accretion->add_field_src_dst("density_source",
                                       "density_source_accumulate");
  refresh_accretion->add_field_src_dst("mom_dens_x_source",
                                       "mom_dens_x_source_accumulate");
  refresh_accretion->add_field_src_dst("mom_dens_y_source",
                                       "mom_dens_y_source_accumulate");
  refresh_accretion->add_field_src_dst("mom_dens_z_source",
                                       "mom_dens_z_source_accumulate");
  refresh_accretion->add_field_src_dst("te_dens_source",
                                       "te_dens_source_accumulate");
...
void EnzoBlock::p_method_accretion_end() {
  EnzoMethodAccretion* method =
      static_cast<EnzoMethodAccretion*>(this->method());
...
void EnzoMethodAccretion::update_fields(EnzoBlock* enzo_block) throw() {
  ...
  enzo_float* mom_dens_x_source =
      (enzo_float*)field.values("mom_dens_x_source");
  enzo_float* mom_dens_x_source_accumulate =
      (enzo_float*)field.values("mom_dens_x_source_accumulate");
  enzo_float* mom_dens_y_source =
      (enzo_float*)field.values("mom_dens_y_source");
  enzo_float* mom_dens_y_source_accumulate =
      (enzo_float*)field.values("mom_dens_y_source_accumulate");
  enzo_float* mom_dens_z_source =
      (enzo_float*)field.values("mom_dens_z_source");
  ...
  for (int iz = gz; iz < mz - gz; iz++) {
    for (int iy = gy; iy < my - gy; iy++) {
      for (int ix = gx; ix < mx - gx; ix++) {
        ...
        // Update color fields
        EnzoMethodStarMaker::rescale_densities(enzo_block, i,
                                               density[i] / old_dens);

        // Update velocity
        const enzo_float old_vx = vx[i];
        const enzo_float new_mom_dens_x = old_dens * old_vx +
                                          mom_dens_x_source[i] +
                                          mom_dens_x_source_accumulate[i];
        vx[i] = new_mom_dens_x / density[i];
        const enzo_float old_vy = vy[i];
        const enzo_float new_mom_dens_y = old_dens * old_vy +
                                          mom_dens_y_source[i] +
                                          mom_dens_y_source_accumulate[i];
        vy[i] = new_mom_dens_y / density[i];
        const enzo_float old_vz = vz[i];
        const enzo_float new_mom_dens_z = old_dens * old_vz +
                                          mom_dens_z_source[i] +
                                          mom_dens_z_source_accumulate[i];
        vz[i] = new_mom_dens_z / density[i];

        // Change in specific total energy is just due to change in specific
        // kinetic energy
...
void EnzoMethodAccretion::do_checks_(const Block* block) throw() {
  ...
  ASSERT("EnzoMethodAccretion", "merge_sinks must precede accretion_compute",
         enzo::problem()->method_precedes("merge_sinks", "accretion"));
  ...
  ASSERT("EnzoMethodAccretion", "accretion requires ppm or vlct methods.",
  ...
  // simulation, it is guaranteed to be abolve the density floor at all times.
  const double density_threshold =
      physical_density_threshold_cgs_ / enzo::units()->density();

  CkPrintf(
      "physical_density_threshold_cgs_ = %.15e\n"
      "density_units = %.15e\n"
      "density_floor = %.15e\n",
      physical_density_threshold_cgs_, enzo::units()->density(), density_floor);
