...
EnzoFluxSinkParticle::EnzoFluxSinkParticle(Block* block, int ib, int ip,
...
void EnzoFluxSinkParticle::init_() throw() {
  ...
  // Variable to store the total mass flux into the accretion zone, divided by
  // the cell volume.
      ...
      for (int ix = min_ind_x_; ix < max_ind_x_; ix++) {
        if (cell_in_accretion_zone(ix, iy, iz)) {
          ...
          total_mass_flux_over_cell_volume +=
              get_mass_flux_over_cell_volume_(ix, iy, iz);
  ...
  set_accretion_rate_over_cell_volume_(total_mass_flux_over_cell_volume,
                                       mean_density);
...
double EnzoFluxSinkParticle::get_mass_flux_over_cell_volume_(int ix, int iy,
                                                             int iz) throw() {
  // Get cell widths
  double hx, hy, hz;
  ...
  // Compute divergence of "density * (gas velocity - particle velocity)"
  // (Equation 36)
  const double d_rho_vx_dx =
      (density[INDEX(ix + 1, iy, iz, mx, my)] *
           (vx_gas[INDEX(ix + 1, iy, iz, mx, my)] - pvx_) -
       density[INDEX(ix - 1, iy, iz, mx, my)] *
           (vx_gas[INDEX(ix - 1, iy, iz, mx, my)] - pvx_)) /
      (2.0 * hx);
  const double d_rho_vy_dy =
      (density[INDEX(ix, iy + 1, iz, mx, my)] *
           (vy_gas[INDEX(ix, iy + 1, iz, mx, my)] - pvy_) -
       density[INDEX(ix, iy - 1, iz, mx, my)] *
           (vy_gas[INDEX(ix, iy - 1, iz, mx, my)] - pvy_)) /
      (2.0 * hy);
  const double d_rho_vz_dz =
      (density[INDEX(ix, iy, iz + 1, mx, my)] *
           (vz_gas[INDEX(ix, iy, iz + 1, mx, my)] - pvz_) -
       density[INDEX(ix, iy, iz - 1, mx, my)] *
           (vz_gas[INDEX(ix, iy, iz - 1, mx, my)] - pvz_)) /
      (2.0 * hz);
...
void EnzoFluxSinkParticle::compute(double max_mass_fraction) throw() {
  ...
  for (int i = 0; i < n_cells; i++) {
    ...
    // Equation 37
    // Note: we have a density change on the LHS rather than a mass change,
    // which is why we have "accretion rate over cell volume" on the RHS rather
    // than "accretion rate", and instead of dividing by "n_cells times rho bar"
    // we multiply by "inverse sum of densities".
    double density_change = accretion_rate_over_cell_volume_ * density[index] *
                            inv_sum_of_densities * block_->dt();

    // Restrict density change via density_threshold and max_mass_fraction
    density_change =
        std::min(std::min(density_change, density[index] - density_threshold_),
