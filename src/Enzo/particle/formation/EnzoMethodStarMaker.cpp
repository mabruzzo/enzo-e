...
EnzoMethodStarMaker::EnzoMethodStarMaker(ParameterGroup p) : Method() {
  ...
  use_overdensity_threshold_ =
      p.value_logical("use_overdensity_threshold", false);
  overdensity_threshold_ = p.value_float("overdensity_threshold", 0.0);
  use_critical_metallicity_ =
      p.value_logical("use_critical_metallicity", false);
  critical_metallicity_ = p.value_float("critical_metallicity", 0.0);
  use_cooling_time_ = p.value_logical("use_cooling_time", false);
  use_temperature_threshold_ =
      p.value_logical("use_temperature_threshold", false);
...
int EnzoMethodStarMaker::check_self_gravitating_alt(const double total_energy,
                                                    const double potential) {
  if (!this->use_self_gravitating_) return 1;
...
int EnzoMethodStarMaker::check_self_gravitating(
    ...
    enzo_float* vz, const double lunit, const double vunit,
    const double rhounit, const int& index, const int& dix, const int& diy,
  ...
  alpha = (div_v_norm2 + cs2 / dx2) /
          (8 * cello::pi * enzo::grav_constant_cgs() * density * rhounit);
#ifdef DEBUG_SF
  CkPrintf("MethodStarMaker -- alpha = %f\n", alpha);
#endif
...
double EnzoMethodStarMaker::h2_self_shielding_factor(
    ...
    const int& diz, const double dx, const double dy, const double dz) {
  if (!this->use_h2_self_shielding_) return 1;
...
int EnzoMethodStarMaker::check_jeans_mass(const double temperature,
                                          ...
                                          const double mass, const double munit,
                                          const double rhounit) {
  if (!use_jeans_mass_) return 1;

  const double gamma = 5.0 / 3.0;
  const double minimum_jeans_mass = 1000 * enzo_constants::mass_solar;
  double cs2 =
      (gamma * enzo_constants::kboltz * temperature) / mean_particle_mass;

  double m_jeans =
      (cello::pi / 6) * pow(cs2, 1.5) /
...
int EnzoMethodStarMaker::check_velocity_divergence(
  ...
  double div = 0.0;
  if (vx)
    div += 0.5 * (vx[index + dix] - vx[index - dix]) / dx;  // in units of dx
  if (vy)
    div += 0.5 * (vy[index + diy] - vy[index - diy]) / dy;  // in units of dy
  if (vz)
    div += 0.5 * (vz[index + diz] - vz[index - diz]) / dz;  // in units of dz
...
int EnzoMethodStarMaker::check_cooling_time(const double& cooling_time,
  ...
  double dynamical_time =
      pow(3.0 * cello::pi / 32.0 / enzo::grav_constant_cgs() /
              (total_density * rhounit),
          0.5);  // s
#ifdef DEBUG_SF
...
#endif
  return !(this->use_critical_metallicity_) + (Z >= critical_metallicity_);
}
...
#endif
  return !(this->use_temperature_threshold_) + (T < temperature_threshold_);
}
