...
EnzoMethodBondiHoyleAccretion::EnzoMethodBondiHoyleAccretion(ParameterGroup p)
...
void EnzoMethodBondiHoyleAccretion::compute_(Block* block) {
  ...
  if (num_particles > 0) {
    ...
    const double const_G = enzo::grav_constant_cgs() *
                           enzo::units()->density() * enzo::units()->time() *
                           enzo::units()->time();
    ...
    for (int ib = 0; ib < nb; ib++) {
      ...
      for (int ip = 0; ip < np; ip++) {
        ...
        EnzoBondiHoyleSinkParticle sp = EnzoBondiHoyleSinkParticle(
            block, ib, ip, accretion_radius, const_G);
