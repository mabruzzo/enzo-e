...
public:
  ...
  EnzoMethodStarMaker(CkMigrateMessage* m) : Method(m) {}
  ...
  static void rescale_densities(EnzoBlock* enzo_block, const int index,
                                const double density_ratio) throw();

  double minimum_star_mass() const noexcept { return star_particle_min_mass_; }

  /// return minimum AMR level where star formation can occur
  ///
  /// @note
  /// This can optionally be overwritten by subclasses
  virtual int sf_minimum_level() const noexcept { return 0; }
...
protected:  // methods
  ...
  int check_velocity_divergence(enzo_float* vx, enzo_float* vy, enzo_float* vz,
                                const int& index, const int& dix,
                                const int& diy, const int& diz, const double dx,
                                const double dy, const double dz);

  int check_mass(const double& m);

  int check_self_gravitating(const double mean_particle_mass,
                             ...
                             const double rhounit, const int& index,
                             const int& dix, const int& diy, const int& diz,
                             const double dx, const double dy, const double dz);

  int check_self_gravitating_alt(const double total_energy,
                                 const double potential);
  double h2_self_shielding_factor(enzo_float* rho, const double metallicity,
                                  const double dunit, const double lunit,
                                  const int& index, const int& dix,
                                  const int& diy, const int& diz,
                                  const double dx, const double dy,
                                  const double dz);
  int check_jeans_mass(const double temperature,
                       const double mean_particle_mass, const double density,
                       const double mass, const double munit,
                       const double rhounit);
  int check_cooling_time(const double& cooling_time,
                         const double& total_density, const double rhounit,
                         const double tunit);
