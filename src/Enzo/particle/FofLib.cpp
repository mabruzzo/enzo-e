...
static void QuickSort(unsigned long n, enzo_float arr[]) {
  ...
  for (;;) {
    if (ir - l < M) {
      ...
      arr[j] = a;
      jstack += 2;
      if (jstack > NSTACK) ErrorHandler("stack size too small in QuickSort");
      if (ir - i + 1 >= j - l) {
        istack[jstack] = ir;
        istack[jstack - 1] = i;
        ir = j - 1;
...
static struct treenode* BuildTree(enzo_float* x, int nidx, int* idxlist,
  ...
  while (1) {
    ...
    if (tree[curnode].nidx <= leafsize) { /* We are a leaf */
      ...
      tree[RIGHT(curnode)].idxlist =
          tree[curnode].idxlist + (tree[curnode].nidx + 1) / 2;
...
static struct treenode* BuildVarTree(enzo_float* x, enzo_float* link, int nidx,
                                     int* idxlist, int leafsize) {
  /* Routine to build a binary tree of particle positions. */
  int i, n, nodes, leaves, curnode;
  int splitptr = 0, splitidx;
  enzo_float width;
  ...
  for (n = 1; n < nidx; n++) {
    for (i = 0; i < 3; i++) {
      tree[curnode].xmin[i] =
          FMIN(tree[curnode].xmin[i], x[3 * n + i] - link[n]);
      tree[curnode].xmax[i] =
          FMAX(tree[curnode].xmax[i], x[3 * n + i] + link[n]);
    }
  }
  ...
  while (1) {
    ...
    if (tree[curnode].nidx <= leafsize) { /* We are a leaf */
      ...
      for (i = 1; i < (tree[curnode].nidx - 1) / 2; i++)
        leftsplit =
            FMAX(leftsplit,
                 x[3 * tree[curnode].idxlist[0] + tree[curnode].splitdim] +
                     link[tree[curnode].idxlist[0]]);
...
static void AddToGroup(struct treenode* tree, int rootnode, int* fifo,
                       int* fifotail, int* group, int groupnum,
                       int* groupsize) {
  ...
  while (1) {
    ...
    if (tree[curnode].splitdim == -1) { /* This is a leaf */
      ...
      SETNEXT(curnode);
      if ((curnode <= rootnode) || (curnode == rootnode + 1))
        return;
      else
        continue;
...
static int CheckContain(struct treenode node, enzo_float* pos,
                        enzo_float link) {
...
static void SearchTree(enzo_float link, enzo_float* x, enzo_float* pos,
                       struct treenode* tree, int* fifo, int* fifotail,
                       int* group, int groupnum, int* groupsize) {
  ...
  while (1) {
    ...
    if (CheckContain(tree[curnode], pos, link)) {
      ...
      AddToGroup(tree, curnode, fifo, fifotail, group, groupnum, groupsize);
      SETNEXT(curnode);
      if (curnode == ROOT)
        return;
      else
...
static void SearchVarTree(enzo_float* link, enzo_float* x, enzo_float* pos,
                          enzo_float thislink, struct treenode* tree, int* fifo,
                          int* fifotail, int* group, int groupnum,
                          int* groupsize) {
...
enzo_float MinBoxDistSqr(enzo_float* x, enzo_float* boxmin,
                         enzo_float* boxmax) {
...
static void NeighborSearchTree(int nneighbor, enzo_float* x, int thisidx,
  ...
  while (nload < nneighbor) {
    ...
    for (n = nload, nload0 = nload;
         n < IMIN(nload0 + tree[curnode].nidx, nneighbor); n++) {
  ...
  while (1) {
    ...
    if (sqrdistances[nneighbor - 1] < MinBoxDistSqr(x + 3 * thisidx,
                                                    tree[curnode].xmin,
                                                    tree[curnode].xmax)) {
      SETNEXT(curnode);
      if (curnode == ROOT) break;
...
static void AddToNeighborList(struct treenode* tree, int thisidx, int rootnode,
                              int* neighborlist, int* nneighbor) {
...
static int FindNeighborTree(enzo_float* x, int thisidx, enzo_float rad,
                            struct treenode* tree, int* neighborlist) {
...
#define LEAFSIZE 8
int FofVar(int npart, enzo_float* x, enzo_float* link, int* group,
           int** groupsize) {
  ...
  for (n = 0; n < npart; n++) {
    if (group[n] != -1)
      ...
      group[n] = groupnum;      /* Assign groupnum to particle */
    ...
    for (fifohead = fifotail = 0; fifohead <= fifotail; fifohead++)
      ...
      SearchVarTree(link, x, x + 3 * fifo[fifohead], link[fifo[fifohead]], root,
                    fifo, &fifotail, group, groupnum, *groupsize + groupnum);
...
int FofVarList(int npart, enzo_float* x, enzo_float* link, int* group,
               int** groupsize, int*** grouplist) {
  ...
  for (n = 0; n < npart; n++) {
    ...
    (*grouplist)[groupnum] =
        (int*)realloc((*grouplist)[groupnum], fifotail * sizeof(int));
    groupnum++; /* Increment group number */
  }
...
int Fof(int npart, enzo_float* x, enzo_float link, int* group,
        int** groupsize) {
  int *fifo, fifohead, fifotail, *idxlist;
  int groupnum = 0;
  int n, m;
  ...
  for (n = 0; n < npart; n++) {
    ...
    for (fifohead = fifotail = 0; fifohead <= fifotail; fifohead++)
      /* Search for friends and add to fifo */
      SearchTree(link, x, x + 3 * fifo[fifohead], root, fifo, &fifotail, group,
...
int FofList(int npart, enzo_float* x, enzo_float link, int* group,
            int** groupsize, int*** grouplist) {
  int fifohead, fifotail, *idxlist;
  int groupnum = 0;
  int n, m;
  ...
  for (n = 0; n < npart; n++) {
    ...
    for (fifohead = fifotail = 0; fifohead <= fifotail; fifohead++)
      ...
      SearchTree(link, x, x + 3 * (*grouplist)[groupnum][fifohead], root,
                 ...
                 *groupsize + groupnum);
    /* Free unused parts of group list */
    (*grouplist)[groupnum] = (int*)realloc(
        (*grouplist)[groupnum], (*groupsize)[groupnum] * sizeof(int));
    groupnum++; /* Increment group number */
  }
...
int FofPrune(int npart, int ngroup, int* group, int** groupsize, int minsize) {
  ...
  for (n = newgroups = 0; n < ngroup; n++)
    if ((*groupsize)[n] < minsize)
      newgroup[n] = -1;
    else
      newgroup[n] = newgroups++;
...
void NearNeighbor(int npart, enzo_float* x, int nneighbor, int* neighborlist) {
  ...
  for (n = 0; n < npart; n++)
    NeighborSearchTree(nneighbor, x, n, root, neighborlist + nneighbor * n);
...
void NearNeighborPartial(int npart, enzo_float* x, int nneighbor, int nsearch,
                         int* searchlist, int* neighborlist) {
...
void FindNeighbor(int npart, enzo_float* x, enzo_float rad, int*** neighborlist,
  ...
  for (n = 0; n < npart; n++) {
    ...
    (*neighborlist)[n] =
        (int*)realloc((*neighborlist)[n], nneighbor[n] * sizeof(int));
  }
...
void FindNeighborPartial(int npart, enzo_float* x, int nsearch, int* searchlist,
                         enzo_float* searchrad, int*** neighborlist,
                         int* nneighbor) {
  ...
  for (n = 0; n < nsearch; n++) {
    ...
    if (!((*neighborlist)[n] = (int*)calloc(npart, sizeof(int))))
      ErrorHandler("unable to allocate output array in FindNeighbor");
    /* Find neighbors and record how many */
    nneighbor[n] = FindNeighborTree(x, searchlist[n], searchrad[n], root,
                                    (*neighborlist)[n]);
    /* Free unused parts of neighbor list */
    (*neighborlist)[n] =
        (int*)realloc((*neighborlist)[n], nneighbor[n] * sizeof(int));
