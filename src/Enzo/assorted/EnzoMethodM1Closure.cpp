...
EnzoMethodM1Closure::EnzoMethodM1Closure(ParameterGroup p)
    : Method(),
      // the following attributes get initialized straight from the parameter
      // file
      ...
      recombination_radiation_(
          p.value_logical("recombination_radiation", false)),
      H2_photodissociation_(p.value_logical("H2_photodissociation", false)),
      lyman_werner_background_(
          p.value_logical("lyman_werner_background", false)),
      LWB_J21_(p.value_float("LWB_J21", -1.0)),
      cross_section_calculator_(
          p.value_string("cross_section_calculator", "vernier")),
  ...
  // conflicts in variable names)
  {
    ...
    for (int i = 0; i < N_groups_; i++) {
      ...
      energy_lower_[i] =
          p.list_value_float(i, "energy_lower", 1.0 + bin_width * i);
      energy_upper_[i] =
          p.list_value_float(i, "energy_upper", 1.0 + bin_width * (i + 1));
  ...
  refresh_injection->set_callback(
      CkIndex_EnzoBlock::p_method_m1_closure_solve_transport_eqn());
}
...
void EnzoMethodM1Closure::compute(Block* block) throw() {
  ...
  // they will still participate in the global sum at the end of
  // inject_photons(), but they will also execute the callback function
  // following the contribute() call. This means the non-leaves would end up
  // calling compute_done() twice which results in skipped cycles

  Field field = block->data()->field();
  int mx, my, mz;
  field.dimensions(0, &mx, &my, &mz);  // field dimensions, including ghost
                                       // zones
  int gx, gy, gz;
  ...
  for (int i = 0; i < N_groups; i++) {
    std::string istring = std::to_string(i);
    enzo_float* N_i_d =
        (enzo_float*)field.values("photon_density_" + istring + "_deposit");
    for (int j = 0; j < m; j++) {
      N_i_d[j] = 0.0;
    }
  }
  ...
  for (int i = 0; i < N_groups; i++) {
    for (int j = 0; j < N_species; j++) {
      CkPrintf(
          "[i,j] = [%d,%d]; sigN = %1.2e cm^2; sigE = %1.2e cm^2; eps = %1.2e "
          "eV\n",
          i, j, *(scalar.value(scalar.index(sigN_string(i, j)))),
          *(scalar.value(scalar.index(sigE_string(i, j)))),
          *(scalar.value(scalar.index(eps_string(i)))) /
              enzo_constants::erg_eV);
    }
  }
#endif
...
double EnzoMethodM1Closure::timestep(Block* block) throw() {
  ...
  return courant * h_min /
         (3.0 * clight_frac * enzo_constants::clight / enzo_units->velocity());
}

//-----------------------------------------------------------------------

double EnzoMethodM1Closure::integrate_simpson(
    double a, double b,
    int n,  // Number of intervals
    std::function<double(double, double, double, int)> f, double v1, double v2,
    int v3) throw() {
  ...
  return (f(a, v1, v2, v3) + f(b, v1, v2, v3) + 2 * sum_evens + 4 * sum_odds) *
         h / 3;
}

double EnzoMethodM1Closure::planck_function(double nu, double T, double clight,
                                            int dependent_variable) throw() {
  ...
  switch (dependent_variable) {
    ...
    case 2:  // energy density
      prefactor = 8 * cello::pi * enzo_constants::hplanck * nu * nu * nu /
                  (clight * clight * clight);
      break;
  }

  return prefactor / (std::exp(enzo_constants::hplanck * nu /
                               (enzo_constants::kboltz * T)) -
                      1);
}
...
double EnzoMethodM1Closure::get_star_temperature(double M) throw() {
  ...
  if (M / enzo_constants::mass_solar < 0.43)
    L = 0.23 * pow(M / enzo_constants::mass_solar, 2.3);
  else if (M / enzo_constants::mass_solar < 2)
    L = pow(M / enzo_constants::mass_solar, 4.0);
  else if (M / enzo_constants::mass_solar < 55)
    L = 1.4 * pow(M / enzo_constants::mass_solar, 3.5);
  else
    L = 32000 * (M / enzo_constants::mass_solar);

  // mass-radius relations (not valid for particles representing stellar
  // populations)
  if (M / enzo_constants::mass_solar < 1)
    R = pow(M / enzo_constants::mass_solar, 0.8);
  else
    R = pow(M / enzo_constants::mass_solar, 0.57);
...
double EnzoMethodM1Closure::get_radiation_custom(EnzoBlock* enzo_block,
                                                 double energy, double pmass,
                                                 double plum, double dt,
                                                 double inv_vol,
                                                 int igroup) throw() {
  Scalar<double> scalar = enzo_block->data()->scalar_double();

  // if either particle_luminosity parameter is set, give all particles the same
  // luminosity, otherwise just use whatever value was passed into this function
  ...
  double plum_i = plum * this->SED_[igroup] /
                  (energy * enzo_constants::erg_eV);  // converted to photons/s
  ...
  for (int j = 0; j < N_species; j++) {
    ...
    CkPrintf(
        "MethodM1Closure::get_radiation_custom -- j = %d; energy = %f eV; "
        "sigma_j = %1.2e cm^2; mL = %1.2e \n",
        j, energy, sigma_j, mL);
#endif
  ...
  *(scalar.value(scalar.index(eps_string(igroup) + mL_string(igroup)))) +=
      energy * enzo_constants::erg_eV * mL;
...
double EnzoMethodM1Closure::get_radiation_blackbody(
    EnzoBlock* enzo_block, double pmass, double freq_lower, double freq_upper,
    double clight, double dt, double cell_volume, int igroup) throw() {
  // Does all calculations in CGS

  int n = 10;  // number of partitions for simpson's method

  // need to use lambda expression to pass in planck_function() function as a
  // parameter because member function in c++ are automatically attached to the
  // `this` pointer, so you need to capture `this` in order for the compiler to
  // know which function to point to

  if (freq_lower == 0.0)
    freq_lower = 1.0;  // planck function undefined at zero
  ...
  double N_integrated = integrate_simpson(
      freq_lower, freq_upper, n,
      [this](double a, double b, double c, int d) {
        return planck_function(a, b, c, d);
      },
      T, clight, planck_case_N);
  double E_integrated = integrate_simpson(
      freq_lower, freq_upper, n,
      [this](double a, double b, double c, int d) {
        return planck_function(a, b, c, d);
      },
    ...
    *(scalar.value(scalar.index(sigN_string(igroup, j) + mL_string(igroup)))) +=
        integrate_simpson(
            ...
            [this, j](double nu, double b, double c, int d) {
              return sigma_vernier(
                         enzo_constants::hplanck * nu / enzo_constants::erg_eV,
                         j) *
...
#ifdef DEBUG_INJECTION
  CkPrintf(
      "MethodM1Closure::get_radiation_blackbody -- [freq_lower, freq_upper] = "
      "[%1.2e, %1.2e], N_integrated = %1.2e cm^-3, T = %1.2e K, Ndot = %1.2e "
      "photons/s \n",
...
void EnzoMethodM1Closure::inject_photons(EnzoBlock* enzo_block,
                                         int igroup) throw() {
  ...
  enzo_float* N =
      (enzo_float*)field.values("photon_density_" + std::to_string(igroup));
  ...
  double freq_lower =
      E_lower * enzo_constants::erg_eV / enzo_constants::hplanck;
  double freq_upper =
      E_upper * enzo_constants::erg_eV / enzo_constants::hplanck;
  ...
  for (int ib = 0; ib < nb; ib++) {
    ...
    for (int ip = 0; ip < np; ip++) {
      ...
      if (radiation_spectrum == "blackbody") {
        ...
        // and integrate over the SED to get the total injection rate. The
        // current implementation in RAMSES assumes that SED of gas stays
        // constant.
        dN = get_radiation_blackbody(enzo_block, pmass_cgs, freq_lower,
                                     freq_upper, clight, dt, cell_volume,
                                     igroup);
      } else if (radiation_spectrum == "custom") {
        // This function samples a user-defined SED to get the injection rate
        // into each group. Uses the `luminosity` particle attribute to
        // calculate the photon injection rate. If `particle_luminosity` is set
        // to something > 0, all particles will be given the same luminosity
        // specified by that parameter
        double plum_cgs =
            plum[ipdL] * munit * lunit * lunit / (tunit * tunit * tunit);
        dN = get_radiation_custom(enzo_block, E_mean, pmass_cgs, plum_cgs, dt,
                                  1 / cell_volume, igroup);
      }
...
void M1Tables::read_hll_eigenvalues(std::string hll_file) throw() {
  ...
  while (inFile >> this->hll_table_f_[i] >> this->hll_table_theta_[i] >>
         this->hll_table_lambda_min_[i] >> this->hll_table_col3_[i] >>
         this->hll_table_col4_[i] >> this->hll_table_lambda_max_[i])
    i++;

  inFile.close();
}

void EnzoMethodM1Closure::compute_hll_eigenvalues(double f, double theta,
...
double EnzoMethodM1Closure::flux_function(double U_l, double U_lplus1,
  ...
  if (type == "GLF") {
    ...
    return (lmax * Q_l - lmin * Q_lplus1 +
            lmax * lmin * clight * (U_lplus1 - U_l)) /
           (lmax - lmin);
  }
...
double EnzoMethodM1Closure::deltaQ_faces(double U_l, double U_lplus1,
                                         double U_lminus1, double Q_l,
                                         double Q_lplus1, double Q_lminus1,
                                         double clight, double lmin,
                                         double lmax,
                                         std::string flux_type) throw() {
  // calls flux_function(), and calculates Q_{i-1/2} - Q_{i+1/2}

  return flux_function(U_lminus1, U_l, Q_lminus1, Q_l, clight, lmin, lmax,
                       flux_type) -
         flux_function(U_l, U_lplus1, Q_l, Q_lplus1, clight, lmin, lmax,
                       flux_type);
}

//--------------------------------------------------------------------------

void EnzoMethodM1Closure::get_reduced_variables(double* chi, double (*n)[3],
                                                int i, double clight,
                                                enzo_float* N, enzo_float* Fx,
                                                enzo_float* Fy,
                                                enzo_float* Fz) throw() {
  double Fnorm = sqrt(Fx[i] * Fx[i] + Fy[i] * Fy[i] + Fz[i] * Fz[i]);
  double f = N[i] > 0 ? std::min(Fnorm / (clight * N[i]), 1.0)
                      : 0.0;  // reduced flux ( 0 < f < 1)
  *chi = (3 + 4 * f * f) /
         (5 + 2 * sqrt(4 - 3 * f * f));  // isotropy measure (1/3 < chi < 1)
  if (Fnorm > 0.0) {
    (*n)[0] = Fx[i] / Fnorm;
    (*n)[1] = Fy[i] / Fnorm;
    (*n)[2] = Fz[i] / Fnorm;
  ...
  CkPrintf(
      "i = %d; N = %1.2e; Fx = %1.2e; Fy = %1.2e; Fz = %1.2e; f = %1.2e; chi = "
      "%1.2e; n=[%1.2e,%1.2e,%1.2e]\n",
...
void EnzoMethodM1Closure::get_U_update(
    EnzoBlock* enzo_block, double* N_update, double* Fx_update,
    double* Fy_update, double* Fz_update, enzo_float* N, enzo_float* Fx,
    enzo_float* Fy, enzo_float* Fz, double hx, double hy, double hz, double dt,
    double clight, int i, int idx, int idy, int idz) throw() {
  Field field = enzo_block->data()->field();
  // if rank >= 1
  enzo_float* P00 = (enzo_float*)field.values("P00");
  // if rank >= 2
  ...
  // if rank >= 1
  *N_update += dt / hx *
               deltaQ_faces(N[i], N[i + idx], N[i - idx], Fx[i], Fx[i + idx],
                            Fx[i - idx], clight, lmin_x, lmax_x, flux_type);

  *Fx_update +=
      dt / hx *
      deltaQ_faces(Fx[i], Fx[i + idx], Fx[i - idx], P00[i], P00[i + idx],
                   P00[i - idx], clight, lmin_x, lmax_x, flux_type);

  // if rank >= 2
  *N_update += dt / hy *
               deltaQ_faces(N[i], N[i + idy], N[i - idy], Fy[i], Fy[i + idy],
                            Fy[i - idy], clight, lmin_y, lmax_y, flux_type);

  *Fx_update +=
      dt / hy *
      deltaQ_faces(Fx[i], Fx[i + idy], Fx[i - idy], P10[i], P10[i + idy],
                   P10[i - idy], clight, lmin_y, lmax_y, flux_type);

  *Fy_update +=
      dt / hx *
      deltaQ_faces(Fy[i], Fy[i + idx], Fy[i - idx], P01[i], P01[i + idx],
                   P01[i - idx], clight, lmin_x, lmax_x, flux_type);

  *Fy_update +=
      dt / hy *
      deltaQ_faces(Fy[i], Fy[i + idy], Fy[i - idy], P11[i], P11[i + idy],
                   P11[i - idy], clight, lmin_y, lmax_y, flux_type);

  // if rank >= 3
  *N_update += dt / hz *
               deltaQ_faces(N[i], N[i + idz], N[i - idz], Fz[i], Fz[i + idz],
                            Fz[i - idz], clight, lmin_z, lmax_z, flux_type);

  *Fx_update +=
      dt / hz *
      deltaQ_faces(Fx[i], Fx[i + idz], Fx[i - idz], P20[i], P20[i + idz],
                   P20[i - idz], clight, lmin_z, lmax_z, flux_type);

  *Fy_update +=
      dt / hz *
      deltaQ_faces(Fy[i], Fy[i + idz], Fy[i - idz], P21[i], P21[i + idz],
                   P21[i - idz], clight, lmin_z, lmax_z, flux_type);

  *Fz_update +=
      dt / hx *
      deltaQ_faces(Fz[i], Fz[i + idx], Fz[i - idx], P02[i], P02[i + idx],
                   P02[i - idx], clight, lmin_x, lmax_x, flux_type);

  *Fz_update +=
      dt / hy *
      deltaQ_faces(Fz[i], Fz[i + idy], Fz[i - idy], P12[i], P12[i + idy],
                   P12[i - idy], clight, lmin_y, lmax_y, flux_type);

  *Fz_update +=
      dt / hz *
      deltaQ_faces(Fz[i], Fz[i + idz], Fz[i - idz], P22[i], P22[i + idz],
...
double EnzoMethodM1Closure::sigma_vernier(double energy, int type) throw() {
  ...
  // Uses fits from Vernier et al. (1996) to calculate photoionization
  // cross-section between photons of energy E and gas of a given species. Then
  // need to average this value over
...
void EnzoMethodM1Closure::get_photoionization_and_heating_rates(
    EnzoBlock* enzo_block, double clight) throw() {
  // Calculates photoionization and heating rates in each cell according to
  // RAMSES-RT prescription See pg. 14 of
  // https://grackle.readthedocs.io/_/downloads/en/latest/pdf/ for relavent
  // Grackle parameters. ionization -- first term of eq. A21 --
  // sum_i(sigmaN*clight*Ni), where i iterates over frequency groups ionization
  // rates should be in code_time^-1 heating rates should be in erg s^-1 cm^-3 /
  // nHI NOTE: Rates are zero if mean energy in a bin is less than the
  ...
  enzo_float* RT_HI_ionization_rate =
      (enzo_float*)field.values("RT_HI_ionization_rate");
  enzo_float* RT_HeI_ionization_rate =
      (enzo_float*)field.values("RT_HeI_ionization_rate");
  enzo_float* RT_HeII_ionization_rate =
      (enzo_float*)field.values("RT_HeII_ionization_rate");

  enzo_float* RT_heating_rate = (enzo_float*)field.values("RT_heating_rate");

  std::vector<enzo_float*> chemistry_fields = {HI_density, HeI_density,
                                               HeII_density};

  std::vector<enzo_float*> ionization_rate_fields = {
      RT_HI_ionization_rate, RT_HeI_ionization_rate, RT_HeII_ionization_rate};

  double tunit = enzo_units->time();
  double rhounit = enzo_units->density();

  std::vector<double> Eion = {13.6 * enzo_constants::erg_eV,
                              24.59 * enzo_constants::erg_eV,
                              54.42 * enzo_constants::erg_eV};
  ...
  for (int i = 0; i < mx * my * mz; i++) {
    ...
    for (int j = 0; j < N_species; j++) {  // loop over species
      ...
      for (int igroup = 0; igroup < this->N_groups_;
           igroup++) {  // loop over groups
        double sigmaN =
            *(scalar.value(scalar.index(sigN_string(igroup, j))));  // cm^2
        double sigmaE =
            *(scalar.value(scalar.index(sigE_string(igroup, j))));       // cm^2
        double eps = *(scalar.value(scalar.index(eps_string(igroup))));  // erg

        double N_i = (photon_densities[igroup])[i] * Nunit;  // cm^-3
        double n_j = (chemistry_fields[j])[i] * rhounit /
                     masses[j];  // number density of species j

        double ediff = eps * sigmaE - Eion[j] * sigmaN;

        ionization_rate += sigmaN * clight * N_i;
        heating_rate +=
            std::max(n_j * clight * N_i * ediff, 0.0);  // Equation A16
      }
      (ionization_rate_fields[j])[i] =
          ionization_rate * tunit;  // update fields with new value, put
                                    // ionization rates in 1/time_units
    }
  ...
  if (this->H2_photodissociation_) {
    enzo_float* RT_H2_photodissociation_rate =
        (enzo_float*)field.values("RT_H2_dissociation_rate");
    for (int i = 0; i < mx * my * mz; i++) {
      double N =
          (photon_densities[0])[i] * Nunit;  // LW-group assumed to be group 0
...
double EnzoMethodM1Closure::get_alpha(double T, int species,
                                      char rec_case) throw() {
...
int EnzoMethodM1Closure::get_b_boolean(double E_lower, double E_upper,
                                       int species) throw() {
  // boolean 1 or 0 which specifies whether or not photon from given
  // recombination lies within given energy range

  switch (species) {  // species after recombination
    case 0:           // HI
      if ((E_lower <= 13.6) && (13.6 < E_upper))
        return 1;
      else
        break;
    case 1:  // HeI
      if ((E_lower <= 24.59) && (24.59 < E_upper))
        return 1;
      else
        break;
    case 2:  // HeII
      if ((E_lower <= 54.42) && (54.42 < E_upper))
        return 1;
      else
        break;
  }
...
double EnzoMethodM1Closure::C_add_recombination(EnzoBlock* enzo_block,
                                                enzo_float* T, int i,
                                                int igroup, double E_lower,
                                                double E_upper) throw() {
  ...
  std::vector<std::string> chemistry_fields = {"HI_density", "HeI_density",
                                               "HeII_density"};
  ...
  for (std::size_t j = 0; j < chemistry_fields.size(); j++) {
    ...
    double n_e = e_density[i] * rhounit /
                 mH;  // electrons have same mass as protons in code units

    C += b * (alpha_A - alpha_B) * n_j * n_e / Cunit;

#ifdef DEBUG_RECOMBINATION
    CkPrintf(
        "MethodM1Closure::C_add_recombination -- j=%d; alpha_A = %1.3e; "
        "alpha_B = %1.3e; n_j = %1.3e; n_e = %1.3e; b_boolean = %d\n",
...
#ifdef DEBUG_RECOMBINATION
  CkPrintf(
      "MethodM1Closure::C_add_recombination -- [E_lower, E_upper] = [%.2f, "
      "%.2f]; dN_dt[i] = %1.3e; dt = %1.3e\n",
...
double EnzoMethodM1Closure::D_add_attenuation(EnzoBlock* enzo_block,
  ...
  for (std::size_t j = 0; j < chemistry_fields.size(); j++) {
    ...
    CkPrintf("[i,j]=[%d,%d]; sigN_ij=%1.2e; n_j=%1.2e; clight=%1.2e\n", igroup,
             j, sigN_ij, n_j, clight);
#endif
  }

#ifdef DEBUG_ATTENUATION
  CkPrintf("i=%d; D=%e s^-1; dt = %e\n", i, D / tunit,
           enzo_block->dt * enzo_units->time());
#endif
  return D;
}
...
void EnzoMethodM1Closure::solve_transport_eqn(EnzoBlock* enzo_block,
                                              int igroup) throw() {
    ...
    for (int iy = gy; iy < my - gy; iy++) {
      for (int ix = gx; ix < mx - gx; ix++) {
        ...
        get_U_update(enzo_block, &N_update, &Fx_update, &Fy_update, &Fz_update,
                     N, Fx, Fy, Fz, hx, hy, hz, dt, clight_code, i, idx, idy,
                     idz);
        ...
        CkPrintf(
            "i = %d; N_update = %f; Fx_update = %f; Nnew[i] = %f; hx = %f; dt "
            "= %f \n",
  ...
  for (int iz = gz; iz < mz - gz; iz++) {
    for (int iy = gy; iy < my - gy; iy++) {
      for (int ix = gx; ix < mx - gx; ix++) {
        ...
        if (isnan(N[i])) {
          ERROR("EnzoMethodM1Closure::solve_transport_eqn()", "N[i] is NaN!\n");
        }
      }
    }
  }
...
void EnzoMethodM1Closure::add_LWB(EnzoBlock* enzo_block, double J21) {
  // Adds cosmological Lyman-Werner background. If J21 > 0, LWB intensity will
  // be constant with a value specified by J21. Otherwise, J21 will be
  // calculated using the redshift-dependent polynomial fit from Wise et al.
  // (2012) The LW radiation field is assumed to be group 0
  ...
  if (J21 >= 0.0) {
    ...
    ASSERT("EnzoMethodM1Closure::add_LWB",
           "Must be running a cosmology simulation to use a time-dependent "
    ...
    if (z >
        30) {  // function valid for z < 30 (not many Pop III stars at z > 30)
      return;
    }
  ...
  double dnu = (this->energy_upper_[0] - this->energy_lower_[0]) *
               enzo_constants::erg_eV /
               enzo_constants::hplanck;  // frequency in Hz

  double Nbackground =
      4 * cello::pi * JLW / (energy_cgs * enzo_constants::clight) * dnu / Nunit;

  Scalar<double> scalar = enzo_block->data()->scalar_double();

  double Nbackground_previous = cello::is_initial_cycle(InitCycleKind::fresh)
                                    ? 0.0
                                    : *(scalar.value(scalar.index("N_LWB")));
...
void EnzoMethodM1Closure::call_inject_photons(EnzoBlock* enzo_block) throw() {
  ...
  //
  // "vernier_average" -- calculates cross section from sigma_vernier()
  // function,
  //    then averages that value over all star particles in the simulation,
  //    weighted by mass * luminosity
  //  "vernier" -- just sets cross sections equal to values from sigma_vernier()
  //  "custom" -- sets cross sections to user-specified values in the parameter
  //  file
  Scalar<double> scalar = enzo_block->data()->scalar_double();
  ...
  if (this->cross_section_calculator_ == "vernier_average") {
    ...
    for (int i = 0; i < N_groups; i++) {
      for (int j = 0; j < N_species; j++) {
        ...
        temp[index] =
            *(scalar.value(scalar.index(sigN_string(i, j) + mL_string(i))));
      }
    }
    for (int i = 0; i < N_groups; i++) {
      for (int j = 0; j < N_species; j++) {
        ...
        temp[index] =
            *(scalar.value(scalar.index(sigE_string(i, j) + mL_string(i))));
      }
    }

    CkCallback callback(
        CkIndex_EnzoBlock::p_method_m1_closure_set_global_averages(NULL),
        enzo_block->proxy_array());

    enzo_block->contribute(temp, CkReduction::sum_double, callback);
  } else {  // just set sigmaN = sigmaE = either sigma_vernier or custom value,
            // and eps = mean(energy)
    ...
    for (int i = 0; i < N_groups; i++) {
      ...
      *(scalar.value(scalar.index(eps_string(i)))) =
          energy * enzo_constants::erg_eV;  // erg
...
void EnzoBlock::p_method_m1_closure_set_global_averages(CkReductionMsg* msg) {
  EnzoMethodM1Closure* method =
      static_cast<EnzoMethodM1Closure*>(this->method());
  method->set_global_averages(this, msg);
}

//----

void EnzoMethodM1Closure::set_global_averages(EnzoBlock* enzo_block,
                                              CkReductionMsg* msg) throw() {
  // contribute does global reduction over ALL blocks by default (not just
  // leaves) call compute_done here for non leaves so that we don't waste time
  ...
  double* temp =
      (double*)msg->getData();  // pointer to vector containing
                                // numerator/denominators of eqs. (B6)-(B8)
                                // temp[0:N_groups] hold the denominators ->
                                // sum(m*L_i) temp[N_groups:] hold the
                                // numerators -> sum(<eps/sigN/sigE>_ij * m*L_i)
...
void EnzoMethodM1Closure::call_solve_transport_eqn(
    EnzoBlock* enzo_block) throw() {
  EnzoUnits* enzo_units = enzo::units();
...
void EnzoMethodM1Closure::sum_group_fields(EnzoBlock* enzo_block) throw() {
  // this function does two things sums group fields together and stores them in
  // integrated fields
  Field field = enzo_block->data()->field();
  int mx, my, mz;
  field.dimensions(0, &mx, &my, &mz);  // field dimensions, including ghost
