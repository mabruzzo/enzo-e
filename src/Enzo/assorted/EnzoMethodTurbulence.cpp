...
#define TRACE_TURBULENCE                                                       \
  CkPrintf("%s:%d TRACE DEBUG_TURBULENCE\n", __FILE__, __LINE__);
#else
#define TRACE_TURBULENCE /*   */
#endif
...
EnzoMethodTurbulence::EnzoMethodTurbulence(double edot, double density_initial,
...
void EnzoMethodTurbulence::compute(Block* block) throw() {
  ...
  enzo_float* velocity[3] = {(enzo_float*)field.values("velocity_x"),
                             (enzo_float*)field.values("velocity_y"),
                             (enzo_float*)field.values("velocity_z")};
  enzo_float* driving[3] = {(enzo_float*)field.values("driving_x"),
  ...
  enzo_block->contribute(n * sizeof(double), g, r_method_turbulence_type,
                         callback);
}
...
void EnzoMethodTurbulence::compute_resume(Block* block,
  ...
  if (edot_ != 0.0) {
    ...
    //
    //   float gv0 = GlobVal[0];
    //   if (gv0 < 1e-30 && gv0 > -1e-30 && MetaData->TopGridRank == 3)
    //   {ERROR_MESSAGE}
    ...
    //   if (gv0 < 1e-30 && gv0 > -1e-30 && MetaData->TopGridRank == 2) *norm =
    //   0.0001;
    //     else    *norm = 1.25*dt*RandomForcingEdot*numberOfGridZones/gv0;
  ...
  if (block->index().is_root()) {
    ...
    monitor->print("Method",
                   "sum v*a*d    "
                   "%.17g",
                   g[index_turbulence_vad]);
    monitor->print("Method",
                   "sum a*a*d    "
                   "%.17g",
                   g[index_turbulence_aad]);
    monitor->print("Method",
                   "sum v*v*d/t  "
                   "%.17g",
                   g[index_turbulence_vvdot]);
    monitor->print("Method",
                   "sum v*v/t    "
                   "%.17g",
                   g[index_turbulence_vvot]);
    monitor->print("Method",
                   "sum v*v*d    "
                   "%.17g",
                   g[index_turbulence_vvd]);
    monitor->print("Method",
                   "sum v*v      "
                   "%.17g",
                   g[index_turbulence_vv]);
    monitor->print("Method",
                   "sum d*d      "
                   "%.17g",
                   g[index_turbulence_dd]);

    monitor->print("Method",
                   "sum d*ax     "
                   "%.17g",
                   g[index_turbulence_dax]);
    monitor->print("Method",
                   "sum d*ay     "
                   "%.17g",
                   g[index_turbulence_day]);
    monitor->print("Method",
                   "sum d*az     "
                   "%.17g",
                   g[index_turbulence_daz]);

    monitor->print("Method",
                   "sum d*vx     "
                   "%.17g",
                   g[index_turbulence_dvx]);
    monitor->print("Method",
                   "sum d*vy     "
                   "%.17g",
                   g[index_turbulence_dvy]);
    monitor->print("Method",
                   "sum d*vz     "
                   "%.17g",
                   g[index_turbulence_dvz]);

    monitor->print("Method",
                   "sum d*ln(d)  "
                   "%.17g",
                   g[index_turbulence_dlnd]);
    monitor->print("Method",
                   "sum zones    "
                   "%.17g",
                   g[index_turbulence_zones]);
    ...
    monitor->print("Method",
                   "min/max Density         "
                   "%.17g",
                   g[index_turbulence_mind] / g[index_turbulence_maxd]);
  }
...
void EnzoMethodTurbulence::compute_resume_(Block* block,
                                           CkReductionMsg* msg) throw() {
  TRACE_TURBULENCE;
  ...
  double norm =
      (edot_ != 0.0)
          ? (sqrt(g[0] * g[0] + 2.0 * n * g[1] * dt * edot_) - g[0]) / g[1]
          : 0.0;
  ...
  const enzo_float bm[3] = {enzo_float(g[index_turbulence_dax] / n),
  ...
  for (int iz = gz; iz < gz + nz; iz++) {
    for (int iy = gy; iy < gy + ny; iy++) {
      for (int ix = gx; ix < gx + nx; ix++) {
        ...
        for (int id = 0; id < rank; id++) {
          //	  	  te[i] += v3[id][i]*a3[id][i]*norm +
          //0.5*a3[id][i]*norm*a3[id][i]*norm; 	  	  v3[id][i] += a3[id][i]*norm;
