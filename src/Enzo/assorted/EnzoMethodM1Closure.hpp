...
public:
  ...
  int hll_table_f(int i, int j) const throw() {
    return hll_table_f_[100 * i + j];
  }
  int hll_table_theta(int i, int j) const throw() {
    return hll_table_theta_[100 * i + j];
  }

  double hll_table_lambda_min(int i, int j) const throw() {
    return hll_table_lambda_min_[100 * i + j];
  }
  double hll_table_lambda_max(int i, int j) const throw() {
    return hll_table_lambda_max_[100 * i + j];
  }
  double hll_table_col3(int i, int j) const throw() {
    return hll_table_col3_[100 * i + j];
  }
  double hll_table_col4(int i, int j) const throw() {
    return hll_table_col4_[100 * i + j];
  }
...
public:  // interface
  ...
  virtual std::string name() throw() { return "m1_closure"; }
...
protected:  // methods
  ...
  double integrate_simpson(double a, double b, int n,
                           ...
                           double v1, double v2, int v3) throw();
  ...
  double planck_function(double nu, double T, double clight,
  ...
  double get_radiation_custom(EnzoBlock* enzo_block, double energy,
                              double pmass, double plum, double dt,
                              double inv_vol, int igroup) throw();
  ...
  double get_radiation_blackbody(EnzoBlock* enzo_block, double pmass,
                                 double freq_lower, double freq_upper,
                                 double clight, double dt, double cell_volume,
                                 int igroup) throw();
  ...
  double flux_function(double U_l, double U_lplus1, double Q_l, double Q_lplus1,
                       double clight, double lmin, double lmax,
                       std::string type) throw();

  void compute_hll_eigenvalues(double f, double theta, double* lmin,
                               double* lmax, double clight) throw();

  double deltaQ_faces(double U_l, double U_lplus1, double U_lminus1, double Q_l,
                      double Q_lplus1, double Q_lminus1, double clight,
                      double lmin, double lmax, std::string flux_type) throw();

  void get_reduced_variables(double* chi_idx, double (*n_idx)[3], int i,
                             double clight, enzo_float* N, enzo_float* Fx,
                             enzo_float* Fy, enzo_float* Fz) throw();

  void get_pressure_tensor(EnzoBlock* enzo_block, enzo_float* N, enzo_float* Fx,
                           enzo_float* Fy, enzo_float* Fz,
                           double clight) throw();

  void get_U_update(EnzoBlock* enzo_block, double* N_update, double* Fx_update,
                    double* Fy_update, double* Fz_update, enzo_float* N,
                    enzo_float* Fx, enzo_float* Fy, enzo_float* Fz, double hx,
                    double hy, double hz, double dt, double clight, int i,
                    int idx, int idy, int idz) throw();
  ...
  // Interaction with matter is completely local, so don't need a refresh before
  // this step

  /// computes the photon-loss term from attenuation by local gas
  double D_add_attenuation(EnzoBlock* enzo_block, double clight, int i,
                           int igroup) throw();
  ...
  double C_add_recombination(EnzoBlock* enzo_block, enzo_float* T, int i,
                             int igroup, double E_lower, double E_uppe) throw();
  ...
  void get_photoionization_and_heating_rates(EnzoBlock* enzo_block,
                                             double clight) throw();
