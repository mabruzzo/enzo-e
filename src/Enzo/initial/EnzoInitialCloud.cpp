...
double uniform_dist_transform_(std::minstd_rand& generator, bool include_zero,
                               bool include_one) {
  ...
  WavePerturbation(std::size_t Nwaves, uint32_t seed, double amplitude,
    ...
    if (Nwaves_ > 0 && amplitude_ > 0.) {
      ...
      for (std::size_t i = 0; i < Nwaves; i++) {
        ...
        double lambda =
            min_lambda + (max_lambda - min_lambda) *
                             uniform_dist_transform_(generator, true, true);
        ...
        phi_.push_back(cello::pi *
                       uniform_dist_transform_(generator, true, false));
      }
    }
  }
  ...
  void print_summary() const noexcept {
    if (Nwaves_ == 0) {
      ...
      for (std::size_t i = 0; i < Nwaves_; i++) {
        cello::monitor()->print(
            "EnzoInitialCloud",
            "Perturb %2u/%d: kx=%+1.8e ky=%+1.8e kz=%+1.8e phi=%1.8e", i,
            Nwaves_, kx_[i], ky_[i], kz_[i], phi_[i]);
      }
    }
  }

  double operator()(double xc, double yc, double zc, double hx, double hy,
    ...
    for (std::size_t i = 0; i < Nwaves_; i++) {
      double ci = (std::sin(kx_[i] * hx * 0.5) * std::sin(ky_[i] * hy * 0.5) *
                   std::sin(kz_[i] * hz * 0.5)) /
                  (kx_[i] * ky_[i] * kz_[i]);
...
class SphereRegion {
public:
  SphereRegion(double center_x, double center_y, double center_z,
               double radius) {
    center_[0] = center_x;
    center_[1] = center_y;
    center_[2] = center_z;
...
public:
  CloudInitHelper(Block* block, int subsample_n, SphereRegion region,
    ...
    subcell_widths_ = {hx / n_subcells_axis, hy / n_subcells_axis,
                       hz / n_subcells_axis};
    ...
    if (block->index().is_root()) {
      perturb_generator_.print_summary();
    }
  }
  ...
  void query_cell(int iz, int iy, int ix, double& frac_enclosed,
    ...
    left[0] = cell_xf_[ix];
    right[0] = cell_xf_[ix + 1];
    left[1] = cell_yf_[iy];
    right[1] = cell_yf_[iy + 1];
    left[2] = cell_zf_[iz];
    right[2] = cell_zf_[iz + 1];
          ...
          for (int sub_iy = 0; sub_iy < num_subcells_axis_; sub_iy++) {
            ...
            for (int sub_ix = 0; sub_ix < num_subcells_axis_; sub_ix++) {
              ...
              if (region_.check_point(sub_xc, sub_yc, sub_zc)) {
                ...
                perturb_sum += perturb_generator_(
                    sub_xc, sub_yc, sub_zc, subcell_widths_[0],
                    subcell_widths_[1], subcell_widths_[2]);
              }
            }
          }
        }
...
// the active_zone
void check_uniform_bfield_(EFlt3DArray bfield, const std::string field_name,
                           const int gx, const int gy, const int gz) {
  EFlt3DArray temp =
      bfield.subarray(CSlice(gz, -gz), CSlice(gy, -gy), CSlice(gx, -gx));
...
public:
    ...
    if (initialize_uniform_bfield_) {
      ASSERT("MHDHandler::MHDHandler",
             ("to initialize uniform bfields, the \"bfield_x\", \"bfield_y\", "
              "and \"bfield_z\" fields must be defined"),
             has_bfield_);
      ...
      magnetic_edens_wind_ =
          0.5 * (bfield_x(gz, gy, gx) * bfield_x(gz, gy, gx) +
  ...
  static MHDHandler construct_MHDHandler(Block* block,
    ...
    Field field = block->data()->field();
    bool has_bfield =
        (field.is_field("bfield_x") || field.is_field("bfield_y") ||
         field.is_field("bfield_z"));
    bool has_interface_bfield =
...
void EnzoInitialCloud::enforce_block(Block* block,
  ...
  CloudInitHelper init_helper(
      block, subsample_n_,
      SphereRegion(cloud_center_x_, cloud_center_y_, cloud_center_z_,
                   cloud_radius_),
      WavePerturbation(perturb_Nwaves_, perturb_seed_, perturb_amplitude_,
                       perturb_min_wavelength_, perturb_max_wavelength_));
  ...
  if (dual_energy) {
    ...
    ASSERT("EnzoInitialCloud::enforce_block",
           ...
           eint_wind_ == 0);
    eint_density =
        ((etot_wind_ - 0.5 * velocity_wind_ * velocity_wind_) * density_wind_ -
         mhd_handler.magnetic_edens_wind());
  }
  ...
  for (int iz = 0; iz < density.shape(0); iz++) {
    for (int iy = 0; iy < density.shape(1); iy++) {
      for (int ix = 0; ix < density.shape(2); ix++) {
        ...
        if (use_cloud_dye) {
          cloud_dye_density(iz, iy, ix) =
              (frac_enclosed * density_cloud_ * perturbation);
        }
        if (set_metal_density) {
