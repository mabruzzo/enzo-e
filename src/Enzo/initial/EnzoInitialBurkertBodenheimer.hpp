...
class EnzoInitialBurkertBodenheimer : public Initial {
  ...
  /// @ingroup  Enzo
  /// @brief    [\ref Enzo] initial conditions for 3D array of collapsing
  /// spheres

public:  // interface
  /// CHARM++ constructor
  EnzoInitialBurkertBodenheimer(int cycle, double time, int rank,
                                const int array[3], double radius_relative,
                                double particle_ratio, double mass,
                                double temperature, int densityprofile) throw()
      : Initial(cycle, time),
        rank_(rank),
