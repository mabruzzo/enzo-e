...
EnzoInitialTurbulence::EnzoInitialTurbulence(int init_cycle, double init_time,
...
void EnzoInitialTurbulence::enforce_block(Block* block,
...

{
  ...
  ASSERT("EnzoInitialTurbulence", "Block does not exist", block != NULL);
  ...
  ASSERT("EnzoInitializeTurbulence::enforce_block()", "Missing Field 'density'",
         d);
  ...
  ASSERT3("EnzoInitialTurbulence::enforce_block()",
          "Root grid mesh dimensions %d %d %d must be equal or 1", Nx, Ny, Nz,
          (Ny == 1) || ((Nz == 1) && (Nx == Ny)) || (Nx == Ny && Ny == Nz));
  ...
  if (rank == 2) {
    ...
    (&rank, &Nx, (enzo_float*)v3[0], (enzo_float*)v3[1], &ndx, &ndy, &o3[0],
     &o3[1]);

  } else if (rank == 3) {
    FORTRAN_NAME(turboinit)
    (&rank, &Nx, (enzo_float*)v3[0], (enzo_float*)v3[1], (enzo_float*)v3[2],
     &ndx, &ndy, &ndz, &o3[0], &o3[1], &o3[2]);
  }

  if (rank >= 1)
    for (int i = 0; i < ndx * ndy * ndz; i++) a3[0][i] = v3[0][i];
  if (rank >= 2)
    for (int i = 0; i < ndx * ndy * ndz; i++) a3[1][i] = v3[1][i];
  if (rank >= 3)
    for (int i = 0; i < ndx * ndy * ndz; i++) a3[2][i] = v3[2][i];
