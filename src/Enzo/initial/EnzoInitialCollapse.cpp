...
//----------------------------------------------------------------------
void EnzoInitialCollapse::enforce_block(Block* block,
...

{
  ...
  ASSERT("EnzoInitialCollapse", "Block does not exist", block != NULL);
  ...
  const double energy = 1e-3 * (enzo_constants::kboltz)*temperature_ /
                        ((gamma - 1.0) * (1.0 * enzo_constants::mass_hydrogen));
