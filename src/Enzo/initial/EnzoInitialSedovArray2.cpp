...
EnzoInitialSedovArray2::EnzoInitialSedovArray2(const EnzoConfig* config) throw()
...
//----------------------------------------------------------------------
void EnzoInitialSedovArray2::enforce_block(Block* block,
...

{
  ...
  ASSERT("EnzoInitialSedovArray2", "Block does not exist", block != NULL);

  Field field = block->data()->field();

  ASSERT("EnzoInitialSedovArray2", "Insufficient number of fields",
         field.field_count() >= 4);
  ...
  double hx, hy;
  field.cell_width(xbm, xbp, &hx, ybm, ybp, &hy);
