...
EnzoInitialSedovRandom::EnzoInitialSedovRandom(const EnzoConfig* config) throw()
...
void EnzoInitialSedovRandom::enforce_block(Block* block,
                                           const Hierarchy* hierarchy) throw()

{
  Field field = block->data()->field();

  ASSERT("EnzoInitialSedovRandom", "Insufficient number of fields",
         field.field_count() >= 4);

  enzo_float* d = (enzo_float*)field.values(field.field_id("density"));

  enzo_float* te = (enzo_float*)field.values(field.field_id("total_energy"));
...
#define INDEX_RN(ix, iy, iz, ib)                                               \
  ((ix) + kxp * ((iy) + kyp * ((iz) + kzp * (ib))))
    ...
    for (int iy = 0; iy < kyp; iy++) {
      for (int ix = 0; ix < kxp; ix++) {
        ...
        for (int rnbeg = 1; rnbeg <= rnum; rnbeg++) {
          rn_array[INDEX_RN(ix, iy, iz, rnbeg)] =
              rand_te_mult(rd);  // adding te multiplier
        }
  ...
  // r - r=0 is number of blasts, rest are total energy multipliers for each
  // blast
    ...
    for (int ky = kym; ky < kyp; ky++) {
      ...
      for (int kx = kxm; kx < kxp; kx++) {
        ...
        for (int blast_num = 1; blast_num <= blasts_current_box; blast_num++) {
          // std::cout << "Executing " << blast_num << " of " <<
          // blasts_current_box << std::endl;
          ...
          double snx =
              xc - 0.5 * hxa + sedov_radius + fabs(rx) * hxa;  // xc + rx * hxa;
          ...
          for (int iz = 0; iz < mz; iz++) {
            ...
            for (int iy = 0; iy < my; iy++) {
              ...
              for (int ix = 0; ix < mx; ix++) {
                ...
                double r2 = (snx - x) * (snx - x) + (sny - y) * (sny - y) +
                            (snz - z) * (snz - z);  // x*x + y*y + z*z;
