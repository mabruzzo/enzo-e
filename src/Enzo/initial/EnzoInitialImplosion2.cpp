...
EnzoInitialImplosion2::EnzoInitialImplosion2(int init_cycle,
                                             double init_time) throw()
...
void EnzoInitialImplosion2::enforce_block(Block* block,
                                          const Hierarchy* hierarchy) throw()

{
  ASSERT("EnzoInitialImplosion2", "Block does not exist", block != NULL);

  Field field = block->data()->field();

  ASSERT("EnzoInitialImplosion2::enforce_block",
         "Insufficient number of fields", field.field_count() >= 4);
