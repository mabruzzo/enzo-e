...
void EnzoInitialIsolatedGalaxy::InitializeExponentialGasDistribution(
  ...
  enzo_float* p =
      field.is_field("pressure") ? (enzo_float*)field.values("pressure") : NULL;
                                ...
                                enzo_units->length())  // we are in the disk
        {
          ...
          if (this->analytic_velocity_) {
            ...
            ASSERT(
                ...
                "InitializeExponentialGasDistribution",
            ...
            vcirc = std::sqrt(vcirc * enzo::grav_constant_cgs() * Mvir_cgs /
                              rvir_cgs);
...
void EnzoInitialIsolatedGalaxy::ReadParticlesFromFile(
  ...
  if (this->gas_fraction_ <= 0.2) {
    lu = enzo_constants::kpc_cm;  // HACK AT THE MOMENT - use old units for
                                  // MW-size galaxy
    mu = 1.0;
  }
...
double EnzoInitialIsolatedGalaxy::InterpolateVcircTable(double radius) {
  ...
  for (i = 0; i < VCIRC_TABLE_LENGTH; i++) {
    if (radius < this->vcirc_radius[i]) break;
  }
