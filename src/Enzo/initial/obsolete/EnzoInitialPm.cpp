...
void EnzoInitialPm::enforce_block(Block* block,
...
void EnzoInitialPm::uniform_placement_(Block* block, Field field,
                                       Particle particle) {
  ...
  if (particle.type_exists("dark")) {
    ...
    mask_->evaluate(bitmask, t, nx, nx, xv, ny, ny, yv, nz, nz, zv);
            ...
            for (int kz = 0; kz < rz; kz++) {
              for (int ky = 0; ky < ry; ky++) {
                for (int kx = 0; kx < rx; kx++) {
                  ...
                  if (rank >= 1)
                    xa[ipb * ps] = xv[ix] - 0.5 * hx + (kx + 0.5) * hx / rx;
                  if (rank >= 2)
                    ya[ipb * ps] = yv[iy] - 0.5 * hy + (ky + 0.5) * hy / ry;
                  if (rank >= 3)
                    za[ipb * ps] = zv[iz] - 0.5 * hz + (kz + 0.5) * hz / rz;
...
void EnzoInitialPm::density_placement_(Block* block, Field field,
                                       Particle particle) {
  int did;
  int mx, my, mz;
  int nx, ny, nz;
  ...
  ASSERT1("EnzoInitialPm()", "Initial:pm:mpp mass per particle %f must be > 0",
          mpp_, mpp_ > 0);
  ...
  for (int ip = 0; ip < np; ip++) {
    ...
    // }
    ims = imin;
    ASSERT6("EnzoInitialPm", "[%d %d %d] %f <= %f < %f", imin, ims, imax,
            ms[ims], r, ms[ims + 1], (ms[ims] <= r && r < ms[ims + 1]));
