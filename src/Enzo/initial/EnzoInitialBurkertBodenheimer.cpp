...

{
  ...
  ASSERT("EnzoInitialBurkertBodenheimer", "Block does not exist",
         block != NULL);
  ...
  ASSERT("EnzoInitialBurkertBodenheimer::enforce_block",
         "This problem must be run in 3D.", rank == 3);

  double hx, hy, hz;
  field.cell_width(bxm, bxp, &hx, bym, byp, &hy, bzm, bzp, &hz);
  ...
  // const double energy = (1e-3*(enzo_constants::kboltz)*temperature_ / ((gamma
  // - 1.0) * (1.0 * enzo_constants::mass_hydrogen)))/enzo_units->energy();
  const double mol_weight = (double)enzo::fluid_props()->mol_weight();
  const double energy =
      ((temperature_ / enzo_units->kelvin_per_energy_units()) /
  ...
  const double density =
      (mass_ * enzo_constants::mass_solar / enzo_units->mass()) /
      (4.0 / 3.0 * (cello::pi)*rx * ry * rz);

  // velocity at termination radius (if provided)
  const double outer_velocity =
      enzo_config->initial_burkertbodenheimer_outer_velocity * 1.0E5 /
  ...
  /*
    ...
    CkPrintf("%s: calculated mass (assuming uniform density) = %e\n",
           __FUNCTION__, (density*(4.0/3.0*(cello::pi)*rx*ry*rz))*
    (enzo_units->mass())/(enzo_constants::mass_solar));
  */
  // exit(-99);
  ...
  int kxp =
      MIN((int)ceil((bxp - dxm + rx) / (dxp - dxm) * array_[0]) + 1, array_[0]);
  int kyp =
      MIN((int)ceil((byp - dym + ry) / (dyp - dym) * array_[1]) + 1, array_[1]);
  int kzp =
      MIN((int)ceil((bzp - dzm + rz) / (dzp - dzm) * array_[2]) + 1, array_[2]);
  ...
  std::fill_n(vx, m, 0.0);
  if (metal)
    std::fill_n(metal, m, outer_metal_fraction * (density / density_ratio));
            ...
            for (int ix = 0; ix < mx; ix++) {
              ...
              if (in_sphere) {
                ...
                if (RotatingSphere == true) {
                  ...
                  // double RotVelocityx = -2*(cello::pi)*y /
                  // SphereRotationalPeriod; double RotVelocityy =
                  // 2*(cello::pi)*x / SphereRotationalPeriod; double
                  // RotVelocityz = 0.0;
                ...
                // if(i == 20) {
                // CkPrintf("Density = %e\n", d[i]);
                // CkPrintf("Angular Velocity = %e rad/s", sqrt(vx[i]*vx[i] +
                // vy[i]*vy[i] + vz[i]*vz[i])/radius / enzo_units->time());
                // CkExit(-99);
                //}
              }
            }
