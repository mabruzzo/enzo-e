...
EnzoInitialShuCollapse::EnzoInitialShuCollapse(int cycle, double time,
...
void EnzoInitialShuCollapse::enforce_block(Block* block,
...

{
  ...
  ASSERT("EnzoInitialShuCollapse::EnzoInitialShuCollapse()",
         "Shu Collapse must have periodic boundary conditions.",
         px && py && pz);
  ...
  ASSERT("EnzoInitialShuCollapse::EnzoInitialShuCollapse()",
         "Truncation radius must no more than a quarter of the domain width in "
         "all "
         "dimensions.",
  ...
  // Check that instability_parameter_ is greater than 2.0. This ensures that
  // the system actually collapses.
  ASSERT("EnzoInitialShuCollapse",
  ...
  ASSERT("EnzoInitialShuCollapse", "Block does not exist", block != NULL);
  ...
  const enzo_float specific_ke =
      0.5 * (drift_velocity_[0] * drift_velocity_[0] +
  ...
  // (i.e., pretend gas is isothermal). This ensures that it has the same
  // (1/r^2) profile as the density field. This means that the specific internal
  // energy is `nominal_sound_speed_` squared over (`gamma` - 1), where `gamma`
  // is slightly bigger than 1. Note that the true sound speed will be `gamma`
  // multiplied by `nominal_sound_speed_` everywhere initially, and will be
  // non-uniform as the system evolves.
