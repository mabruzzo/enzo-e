...
EnzoInitialBCenter::EnzoInitialBCenter(Parameters* parameters, int cycle,
                                       double time, bool update_etot) throw()
    : Initial(cycle, time),
...
void bfieldi_helper_(EFlt3DArray& bfield, CelloView<double, 3>& Aj,
                     CelloView<double, 3>& Ak, int dim, double dj, double dk) {
  ...
  if (dim == 0) {
    ...
    Aj_right =
        Aj.subarray(CSlice(1, fc_mz + 1), CSlice(0, fc_my), CSlice(0, fc_mx));
    Ak_right =
        Ak.subarray(CSlice(0, fc_mz), CSlice(1, fc_my + 1), CSlice(0, fc_mx));
  } else if (dim == 1) {
    // Aj_right = Az(iz,iy-1/2,ix+1/2), Ak_right = Ax(iz+1/2,iy-1/2,ix)
    Aj_right =
        Aj.subarray(CSlice(0, fc_mz), CSlice(0, fc_my), CSlice(1, fc_mx + 1));
    Ak_right =
        Ak.subarray(CSlice(1, fc_mz + 1), CSlice(0, fc_my), CSlice(0, fc_mx));
  } else {
    // Aj_right = Ax(iz-1/2,iy+1/2,ix), Ak_right = Ay(iz-1/2,iy,ix+1/2)
    Aj_right =
        Aj.subarray(CSlice(0, fc_mz), CSlice(1, fc_my + 1), CSlice(0, fc_mx));
    Ak_right =
        Ak.subarray(CSlice(0, fc_mz), CSlice(0, fc_my), CSlice(1, fc_mx + 1));
  }
...
void EnzoInitialBCenter::enforce_block(Block* block,
  ...
  if (values_[0] != nullptr || values_[1] != nullptr || values_[2] != nullptr) {
    ...
    if (values_[0] != nullptr) {
      values_[0]->evaluate(data_ptrs[0], t, mx, mx, xc, my + 1, my + 1, yf,
                           mz + 1, mz + 1, zf);
    }
    if (values_[1] != nullptr) {
      values_[1]->evaluate(data_ptrs[1], t, mx + 1, mx + 1, xf, my, my, yc,
                           mz + 1, mz + 1, zf);
    }
    if (values_[0] != nullptr) {
      values_[2]->evaluate(data_ptrs[2], t, mx + 1, mx + 1, xf, my + 1, my + 1,
                           yf, mz, mz, zc);
    }
