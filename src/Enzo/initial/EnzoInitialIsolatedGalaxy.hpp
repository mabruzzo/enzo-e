...
public:  // interface
  ...
  EnzoInitialIsolatedGalaxy(CkMigrateMessage* m) : Initial(m) {}
  ...
  void InitializeParticles(Block* block, Particle* particle);
  ...
  void ReadParticlesFromFile(const int& nl, enzo_float* position[],
                             enzo_float* velocity[], enzo_float* mass,
  ...
  double gauss_mass(const double rho_zero, const double xpos, const double ypos,
                    const double zpos, const double dx);
