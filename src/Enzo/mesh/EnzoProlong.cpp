...
EnzoProlong::EnzoProlong(std::string type, bool positive,
  ...
  if (type == "3A")
    ...
    ERROR1("EnzoProlong::EnzoProlong", "Unrecognized interpolation method %s",
           type.c_str());
  }
}
//----------------------------------------------------------------------
...
void EnzoProlong::apply(precision_type precision, void* values_f, int m3_f[3],
                        int o3_f[3], int n3_f[3], const void* values_c,
                        int m3_c[3], int o3_c[3], int n3_c[3],
                        bool accumulate) {
  if (!accumulate) {
    ...
    if (!use_linear_) {
      ...
      prolong_linear.apply(precision, values_f, m3_f, o3_f, n3_f, values_c,
                           m3_c, o3_c, n3_c, accumulate);
      for (int i = 0; i < cello::rank(); i++) {
        n3_c[i] += 2;
        o3_c[i] -= 1;
      }
    ...
    ProlongLinear prolong_linear;
    prolong_linear.apply(precision, values_f, m3_f, o3_f, n3_f, values_c, m3_c,
                         o3_c, n3_c, accumulate);
  }
}
//----------------------------------------------------------------------

void EnzoProlong::apply_(enzo_float* values_f, int m3_f[3], int o3_f[3],
                         int n3_f[3], const enzo_float* values_c, int m3_c[3],
                         int o3_c[3], int n3_c[3], bool accumulate) {
  int rank = cello::rank();
  ...
  (&rank, ((enzo_float*)(values_c)) + o_c, pdims, pstart, pend, r3,
   ((enzo_float*)(temp_f)) + o_f, gdims, gstart, work, &method_, &positive_,
   &error);
