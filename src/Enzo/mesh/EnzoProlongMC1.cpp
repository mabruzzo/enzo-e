...
EnzoProlongMC1::EnzoProlongMC1(std::string type) throw()
  ...
  if (type == "ThirdOrderA")
    method_ = 0;
  else if (type == "SecondOrderA")
    method_ = 1;
  else if (type == "SecondOrderB")
    method_ = 2;
  else if (type == "SecondOrderC")
    method_ = 3;
  else if (type == "FirstOrderA")
    method_ = 4;
  else {
    ERROR1("EnzoProlongMC1::EnzoProlongMC1",
           "Unrecognized interpolation method %s", type.c_str());
  }
}
//----------------------------------------------------------------------
...
void EnzoProlongMC1::apply(precision_type precision, void* values_f,
...
void EnzoProlongMC1::apply_(enzo_float* values_f, int nd3_f[3], int im3_f[3],
                            int n3_f[3], const enzo_float* values_c,
  ...
  if (n3_f[1] == 1) {
    ...
    for (int ix_c = 0; ix_c < n3_c[0]; ix_c++) {
      for (int iy_c = 0; iy_c < n3_c[1]; iy_c++) {
        ...
        work[i_c] = 0.25 * (values_c[i_c] + values_c[i_c + dx_c] +
                            values_c[i_c + dy_c] + values_c[i_c + dx_c + dy_c]);
      }
    }
    ...
    for (int ix_c = 0; ix_c < n3_c[0]; ix_c++) {
      ...
      for (int iy_c = 0; iy_c < n3_c[1]; iy_c++) {
        ...
        enzo_float df0 =
            std::min(fabs(fb - work[i00_c]), fabs(fb - work[i11_c]));
        enzo_float df1 =
            std::min(fabs(fb - work[i10_c]), fabs(fb - work[i01_c]));
    ...
    ERROR("EnzoProlongMC1", "3D not implemented yet");
