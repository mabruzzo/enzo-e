...
EnzoRefineParticleMass::EnzoRefineParticleMass(
    double min_refine, double max_coarsen, int max_level, bool include_ghosts,
    std::string output, double level_exponent) throw()
...
int EnzoRefineParticleMass::apply(Block* block) throw() {
  ...
  const double mass_min_refine =
      min_refine_ * pow(2.0, level * level_exponent_);
  const double mass_max_coarsen =
      max_coarsen_ * pow(2.0, level * level_exponent_);

  double ratio = 1.0 * count / (nx * ny * nz);

  int adapt_result =
      (ratio > mass_min_refine)
          ? adapt_refine
