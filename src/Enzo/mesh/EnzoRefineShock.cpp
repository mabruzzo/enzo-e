...
EnzoRefineShock::EnzoRefineShock(double pressure_min_refine,
                                 ...
                                 bool comoving_coordinates, int max_level,
                                 bool include_ghosts,
...
int EnzoRefineShock::apply(Block* block) throw() {
  ...
  ASSERT("EnzoRefineShock::apply", "velocity_x field must be defined",
         (id_velocity >= 0));

  void* v3[3] = {(rank >= 1) ? field.values("velocity_x") : NULL,
  ...
  evaluate_block_((const enzo_float**)v3, (const enzo_float*)te,
                  (const enzo_float*)de, (const enzo_float*)p,
                  (enzo_float*)output, nxd, nyd, nzd, nx, ny, nz, gx, gy, gz,
...
void EnzoRefineShock::evaluate_block_(const enzo_float* v3[],
                                      const enzo_float* te,
                                      const enzo_float* de, const enzo_float* p,
                                      enzo_float* output, int ndx, int ndy,
                                      int ndz, int nx, int ny, int nz, int gx,
                                      int gy, int gz, bool* any_refine,
    ...
    for (int iz = gz; iz < nz + gz; iz++) {
      for (int iy = gy; iy < ny + gy; iy++) {
        for (int ix = gx; ix < nx + gx; ix++) {
          ...
          enzo_float dp =
              fabs(p[i + id] - p[i - id]) / (std::min(p[i + id], p[i - id]));
          ...
          bool l_refine = (dv < 0.0) && (dp > pressure_min_refine_) &&
                          (er > energy_ratio_min_refine_);

          bool l_same = (dv < 0.0) && (dp > pressure_max_coarsen_) &&
...
#ifdef DEBUG_ENZO_REFINE_SHOCK
  CkPrintf("%s dp limits %lf %lf  dp min/max %lf %lf\n", pressure_max_coarsen_,
           pressure_min_refine_, dp_min, dp_max);
  CkPrintf("%s er limits %lf %lf  er min/max %lf %lf\n",
           energy_ratio_max_coarsen_, energy_ratio_min_refine_, er_min, er_max);
#endif
}
//======================================================================
