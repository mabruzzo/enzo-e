...
EnzoRefineMass::EnzoRefineMass(double min_refine, double max_coarsen,
...
//	  float(MetaData.TopGridDims[dim]);
{
  ...
  if (cosmology) {
    if (mass_type == "dark") {
      ...
      mass_ratio_ =
          cosmology->omega_baryon_now() / cosmology->omega_matter_now();
    } else {
      ERROR1("EnzoRefineMass::EnzoRefineMass()", "Unknown mass_type %s",
             mass_type.c_str());
    }
  } else {
    mass_ratio_ = 0.0;
...
int EnzoRefineMass::apply(Block* block) throw() {
  ...
  ASSERT1("EnzoRefineMass::apply()", "Undefined field name %s", name_.c_str(),
          id_field >= 0);
  ...
  switch (precision) {
        ...
        for (int iz = gz; iz < mz - gz; iz++) {
          for (int iy = gy; iy < my - gy; iy++) {
            for (int ix = gx; ix < mx - gx; ix++) {
              ...
              sum += mass;
#endif
              if (mass < mass_max_coarsen)
                out4[i] = -1;
              else if (mass < mass_min_refine)
                out4[i] = 0;
              else
                out4[i] = +1;
            }
    ...
    case precision_quadruple:
      ...
      break;
    default:
      ERROR2("EnzoRefineMass::apply", "Unknown precision %d for field %d",
             precision, 0);
      break;
  }
