...
EnzoProlongPoisson::EnzoProlongPoisson() throw() : Prolong() {
  TRACE("EnzoProlongPoisson::EnzoProlongPoisson");
}

//----------------------------------------------------------------------

void EnzoProlongPoisson::apply(precision_type precision, void* values_f,
                               int nd3_f[3], int im3_f[3], int n3_f[3],
                               const void* values_c, int nd3_c[3], int im3_c[3],
                               int n3_c[3], bool accumulate) {
  TRACE6("EnzoProlongPoisson fine   %d:%d %d:%d %d:%d", im3_f[0],
         n3_f[0] + im3_f[0], im3_f[1], n3_f[1] + im3_f[1], im3_f[2],
         n3_f[2] + im3_f[2]);

  TRACE6("EnzoProlongPoisson coarse %d:%d %d:%d %d:%d", im3_c[0],
         n3_c[0] + im3_c[0], im3_c[1], n3_c[1] + im3_c[1], im3_c[2],
         n3_c[2] + im3_c[2]);

  apply_((enzo_float*)values_f, nd3_f, im3_f, n3_f, (const enzo_float*)values_c,
         nd3_c, im3_c, n3_c, accumulate);
}

//----------------------------------------------------------------------

void EnzoProlongPoisson::apply_(enzo_float* values_f, int nd3_f[3],
                                int im3_f[3], int n3_f[3],
                                const enzo_float* values_c, int nd3_c[3],
                                int im3_c[3], int n3_c[3], bool accumulate) {
  int dx_c = 1;
  int dy_c = nd3_c[0];
  ...
  ASSERT("EnzoProlongPoisson::apply_", "accumulate=true is not implemented yet",
         !accumulate);

  for (int i = 0; i < rank; i++) {
    const char* xyz = "xyz";
    ASSERT3(
        "EnzoProlongPoisson::apply_",
        "fine array %c-axis %d must be twice the size of the coarse axis %d",
        xyz[i], n3_f[i], n3_c[i], n3_f[i] == n3_c[i] * 2);

    ASSERT2("EnzoProlongPoisson::apply_",
            "fine grid %c-axis %d must be divisible by 4", xyz[i], n3_f[i],
            n3_f[i] % 4 == 0);
  }
  ...
  if (n3_f[1] == 1) {
    for (int ix0 = 0; ix0 < n3_f[0]; ix0 += 4) {
      ...
      for (int ix = ix0; ix < ix0 + 4; ix++) {
        ...
        values_f[i_f] =
            (c1[icx] * values_c[i_c] + c2[icx] * values_c[i_c + dx_c]);
      }
    }
      ...
      for (int iy0 = 0; iy0 < n3_f[1]; iy0 += 4) {
        ...
        for (int ix = ix0; ix < ix0 + 4; ix++) {
          ...
          for (int iy = iy0; iy < iy0 + 4; iy++) {
            ...
            int i_c = (im3_c[0] + ic_x) + nd3_c[0] * ((im3_c[1] + ic_y));
            int i_f = (im3_f[0] + if_x) + nd3_f[0] * ((im3_f[1] + if_y));

            values_f[i_f] = (c1[icx] * c1[icy] * values_c[i_c] +
          ...
          for (int ix = ix0; ix < ix0 + 4; ix++) {
            ...
            for (int iy = iy0; iy < iy0 + 4; iy++) {
              ...
              for (int iz = iz0; iz < iz0 + 4; iz++) {
                ...
                int i_c = (im3_c[0] + ic_x) +
                          nd3_c[0] * ((im3_c[1] + ic_y) +
                                      nd3_c[1] * (im3_c[2] + ic_z));
                int i_f = (im3_f[0] + if_x) +
                          nd3_f[0] * ((im3_f[1] + if_y) +
                                      nd3_f[1] * (im3_f[2] + if_z));
                ...
                values_f[i_f] =
                    (c1[icx] * c1[icy] * c1[icz] * values_c[i_c] +
                     ...
                     c2[icx] * c2[icy] * c2[icz] *
                         values_c[i_c + dx_c + dy_c + dz_c]);
              }
            }
          }
        }
