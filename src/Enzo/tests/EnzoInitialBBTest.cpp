...
EnzoInitialBBTest::EnzoInitialBBTest(
    int cycle, double time, const double center[3],
    const double drift_velocity[3], double mean_density,
    double fluctuation_amplitude, double truncation_radius,
    double nominal_sound_speed, double angular_rotation_velocity,
...
void EnzoInitialBBTest::enforce_block(Block* block,
...

{
  ...
  ASSERT("EnzoInitialBBTest::EnzoInitialBBTest()",
         "Truncation radius must be no more than one quarter of the domain "
         "width in "
         "all dimensions.",
  ...
  ASSERT("EnzoInitialBBTest", "Block does not exist", block != NULL);
  ...
  // It's likely that not all these fields are required for the problem, and
  // that not all of the required fields need to be initialized, but let's just
  // do it for safety.
      ...
      for (int ix = 0; ix < mx; ix++) {
        ...
        if (r2 > truncation_radius_ * truncation_radius_) {
          ...
          if (std::abs(x_rel < tiny_number) && (y_rel == 0.0)) {
            // In this case, where x_rel is tiny and y_rel is zero, phi is
            // undefined, so just set density to be mean density, set velocity
            // to be drift velocity
            d[i] = mean_density_;
            vx[i] = drift_velocity_[0];
            vy[i] = drift_velocity_[1];
            vz[i] = drift_velocity_[2];
            ...
            if (x_rel > tiny_number)
              phi = atan(y / x);
            else if (x_rel < -tiny_number) {
              phi = (y_rel >= 0.0) ? (atan(y / x) + cello::pi)
                                   : (atan(y / x) - cello::pi);
            } else {
            ...
            d[i] =
                mean_density_ * (1.0 + fluctuation_amplitude_ * cos(2.0 * phi));
            ...
            // x-component
            vx[i] = -angular_rotation_velocity_ * r * sin(theta) * sin(phi) +
                    drift_velocity_[0];

            // y-component
            vy[i] = angular_rotation_velocity_ * r * sin(theta) * cos(phi) +
                    drift_velocity_[1];

          }  // Close else on line 253
        }  // Close else on line 241

        // Set specific total energy by computing specific kinetic and internal
        // energies
        const enzo_float specific_ke =
            0.5 * (vx[i] * vx[i] + vy[i] * vy[i] + vz[i] * vz[i]);

        // Set initial pressure to be density times square of
        // `nominal_sound_speed_` (i.e., pretend gas is isothermal). This
        // ensures that it has the same (1/r^2) profile as the density field.
        // This means that the specific internal energy is
        // `nominal_sound_speed_` squared over (`gamma` - 1), where `gamma` is
        // slightly bigger than 1. Note that the true sound speed will be
        // `gamma` multiplied by `nominal_sound_speed_` everywhere initially,
