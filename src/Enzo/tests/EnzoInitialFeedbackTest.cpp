...
EnzoInitialFeedbackTest::EnzoInitialFeedbackTest(int cycle, double time,
  ...
  const std::vector<std::string> other_species = {"HII_density", "HeI_density",
                                                  "HeII_density",
                                                  "HeIII_density", "e_density"};
...
void EnzoInitialFeedbackTest::enforce_block(
  ...
  int ia_L = particle.has_attribute(it, "luminosity")
                 ? particle.attribute_index(it, "luminosity")
                 : -1;
  ...
  for (int i = 0; i < this->num_particles; i++) {
    if (mask[i]) {
      ...
      CkPrintf(
          "EnzoInitialFeedbackTest (%s): Mass MyPe id_counter istatic id %f %d "
          "%d %d %d\n",
      ...
      id[ipp] = CkMyPe() + (ParticleData::id_counter[cello::index_static()]++) *
                               CkNumPes();
      pmass[ipp] =
          this->mass[i] * enzo_constants::mass_solar / enzo_units->mass();
      px[ipp] = this->position[0][i];
      py[ipp] = this->position[1][i];
      pz[ipp] = this->position[2][i];
      plum[ipp] = this->luminosity[i] *
                  enzo_units->time();  // luminosity in photons/time
      pvx[ipp] = 0.0;
      pvy[ipp] = 0.0;
      pvz[ipp] = 0.0;
      ...
      pform[ipp] = 1.0E-10 * enzo_constants::yr_s /
