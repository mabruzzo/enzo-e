...
EnzoInitialAccretionTest::EnzoInitialAccretionTest(
    int cycle, double time, const double sink_position[3],
    const double sink_velocity[3], double sink_mass, double gas_density,
    double gas_pressure, double gas_radial_velocity) throw()
    : Initial(cycle, time),
...
void EnzoInitialAccretionTest::enforce_block(Block* block,
...

{
  ...
  ASSERT("EnzoInitialAccretionTest", "Block does not exist", block != NULL);
  ...
  // Initialise all fields to zero except for density, velocity, and
  // total_energy
  std::fill_n(p, m, 0.0);
  std::fill_n(ds, m, 0.0);
  ...
  for (int iz = gz; iz < mz - gz; iz++) {
    for (int iy = gy; iy < my - gy; iy++) {
      for (int ix = gx; ix < mx - gx; ix++) {
        ...
        const double r = sqrt((cell_center_x - sink_position_[0]) *
        ...
        if (r < 1.0e-6 * min_cell_width) {
          ...
          vx[index] =
              -gas_radial_velocity_ * (cell_center_x - sink_position_[0]) / r;
          vy[index] =
              -gas_radial_velocity_ * (cell_center_y - sink_position_[1]) / r;
          vz[index] =
              -gas_radial_velocity_ * (cell_center_z - sink_position_[2]) / r;
        }
      }
    }
  }
  ...
  if (block->check_position_in_block(sink_position_[0], sink_position_[1],
                                     sink_position_[2])) {
