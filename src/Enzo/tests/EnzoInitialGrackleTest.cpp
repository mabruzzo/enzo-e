...
EnzoInitialGrackleTest::EnzoInitialGrackleTest(int cycle, double time,
                                               ParameterGroup p) noexcept
    : Initial(cycle, time),
      min_max_H_number_density_{
          p.value_float("minimum_H_number_density", 0.1),
...
void EnzoInitialGrackleTest::enforce_block(Block* block,
  ...
  double H_n_slope =
      log10(min_max_H_number_density_[1] / min_max_H_number_density_[0]) /
      double(nx);

  double temperature_slope =
      log10(min_max_temperature_[1] / min_max_temperature_[0]) / double(ny);

  double metallicity_slope =
      log10(min_max_metallicity_[1] / min_max_metallicity_[0]) / double(nz);
  ...
  CelloView<enzo_float, 3> metal_density =
      field.view<enzo_float>("metal_density");

  // now we load in species fields
  CelloView<enzo_float, 3> HI_density =
      (primordial_chemistry > 0) ? field.view<enzo_float>("HI_density")
                                 : CelloView<enzo_float, 3>();
  CelloView<enzo_float, 3> HII_density =
      (primordial_chemistry > 0) ? field.view<enzo_float>("HII_density")
                                 : CelloView<enzo_float, 3>();
  CelloView<enzo_float, 3> HeI_density =
      (primordial_chemistry > 0) ? field.view<enzo_float>("HeI_density")
                                 : CelloView<enzo_float, 3>();
  CelloView<enzo_float, 3> HeII_density =
      (primordial_chemistry > 0) ? field.view<enzo_float>("HeII_density")
                                 : CelloView<enzo_float, 3>();
  CelloView<enzo_float, 3> HeIII_density =
      (primordial_chemistry > 0) ? field.view<enzo_float>("HeIII_density")
                                 : CelloView<enzo_float, 3>();
  CelloView<enzo_float, 3> e_density = (primordial_chemistry > 0)
                                           ? field.view<enzo_float>("e_density")
                                           : CelloView<enzo_float, 3>();

  CelloView<enzo_float, 3> HM_density =
      (primordial_chemistry > 1) ? field.view<enzo_float>("HM_density")
                                 : CelloView<enzo_float, 3>();
  CelloView<enzo_float, 3> H2I_density =
      (primordial_chemistry > 1) ? field.view<enzo_float>("H2I_density")
                                 : CelloView<enzo_float, 3>();
  CelloView<enzo_float, 3> H2II_density =
      (primordial_chemistry > 1) ? field.view<enzo_float>("H2II_density")
                                 : CelloView<enzo_float, 3>();

  CelloView<enzo_float, 3> DI_density =
      (primordial_chemistry > 2) ? field.view<enzo_float>("DI_density")
                                 : CelloView<enzo_float, 3>();
  CelloView<enzo_float, 3> DII_density =
      (primordial_chemistry > 2) ? field.view<enzo_float>("DII_density")
                                 : CelloView<enzo_float, 3>();
  CelloView<enzo_float, 3> HDI_density =
      (primordial_chemistry > 2) ? field.view<enzo_float>("HDI_density")
                                 : CelloView<enzo_float, 3>();
      ...
      for (int ix = 0; ix < nx + gx; ix++) {  // H Number Density
        ...
        const enzo_float cur_density =
            (enzo_constants::mass_hydrogen *
             pow(10.0, ((H_n_slope * (ix - gx)) +
                        log10(min_max_H_number_density_[0]))) /
        ...
        metal_density(iz, iy, ix) =
            pow(10.0, ((metallicity_slope * (iz - gz)) +
        ...
        if (primordial_chemistry > 0) {
          ...
          HeI_density(iz, iy, ix) =
              cur_density * (1.0 - HydrogenFractionByMass);
    ...
    for (int iy = 0; iy < ny + gy; iy++) {    // Temperature
      for (int ix = 0; ix < nx + gx; ix++) {  // H Number Density
        ...
        if (primordial_chemistry == 0) {
          ...
          if (primordial_chemistry > 1) {
            tmp += HM_density(iz, iy, ix) +
                   0.5 * (H2I_density(iz, iy, ix) + H2II_density(iz, iy, ix));
          }
          if (primordial_chemistry > 2) {
