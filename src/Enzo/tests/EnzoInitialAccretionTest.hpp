...
///           a given initial position and velocity in a static medium of gas
///           with constant density and internal energy.
...
public:  // interface
  /// CHARM++ constructor
  EnzoInitialAccretionTest(int cycle, double time,
                           const double sink_position[3],
                           const double sink_velocity[3], double sink_mass,
                           double gas_density, double gas_pressure,
                           double gas_radial_velocity) throw();
...
private:  // attributes
  ...
  /// Initial radial velocity of the gas (with the origin being the sink
