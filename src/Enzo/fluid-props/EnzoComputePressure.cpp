...
EnzoComputePressure::EnzoComputePressure(double gamma,
    ...
    : Compute(), gamma_(gamma), comoving_coordinates_(comoving_coordinates) {}
...
void EnzoComputePressure::compute_pressure(const EnzoFieldAdaptor& fadaptor,
                                           const CelloView<enzo_float, 3>& p,
                                           bool mhd, bool dual_energy,
                                           double gamma,
                                           int stale_depth,    /* 0 */
  ...
  if ((grackle_method != nullptr) & !ignore_grackle) {
    ...
    ASSERT("EnzoComputePressure::compute_pressure", "Shapes must be the same",
    ...
    if (dual_energy) {
      ...
      const RdOnlyEFltArr vy =
          (rank >= 2) ? fadaptor.view("velocity_y") : RdOnlyEFltArr();
      const RdOnlyEFltArr vz =
          (rank >= 3) ? fadaptor.view("velocity_z") : RdOnlyEFltArr();

      // fetch bfield arrays
      const RdOnlyEFltArr bx =
          (mhd) ? fadaptor.view("bfield_x") : RdOnlyEFltArr();
      const RdOnlyEFltArr by =
          (mhd & (rank >= 2)) ? fadaptor.view("bfield_y") : RdOnlyEFltArr();
      const RdOnlyEFltArr bz =
          (mhd & (rank >= 3)) ? fadaptor.view("bfield_z") : RdOnlyEFltArr();

      if (rank == 1) {
        auto loop_body = [=](int iz, int iy, int ix) {
          enzo_float ke = 0.5 * (vx(iz, iy, ix) * vx(iz, iy, ix));
          enzo_float me_den =
              mhd ? 0.5 * (bx(iz, iy, ix) * bx(iz, iy, ix)) : 0.;
          p(iz, iy, ix) =
              gm1 * (d(iz, iy, ix) * (te(iz, iy, ix) - ke) - me_den);
        };
        enzo_utils::exec_loop(mz, my, mx, stale_depth, loop_body);
