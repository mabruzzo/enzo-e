...
private:
  void cleanup_() {
    switch (tag_) {
#define EOS_ENTRY_(IND, _)                                                     \
  case IND: {                                                                  \
    delete ptr_union_.alt_##IND;                                               \
    break;                                                                     \
  }
      EOS_ALTERNATIVE_LIST
#undef EOS_ENTRY_
      ...
      default:
        ERROR1("EnzoEOSVariant::cleanup", "unhandled tag: %d", tag_);
    }
  }
...
#define EOS_ENTRY_(IND, TYPE)                                                  \
  EnzoEOSVariant(TYPE eos) {                                                   \
    ptr_union_.alt_##IND = new TYPE{eos};                                      \
    tag_ = IND;                                                                \
  }
...
#undef EOS_ENTRY_
  ...
  EnzoEOSVariant(const EnzoEOSVariant& other) : EnzoEOSVariant() {
...
#undef EOS_ENTRY_
      ...
      default:
        ERROR1("EnzoEOSVariant::EnzoEOSVariant", "unhandled tag: %d", tag_);
    }
  ...
  /// move constructor
  EnzoEOSVariant(EnzoEOSVariant&& other) : EnzoEOSVariant() {
    this->swap(other);
  }
  ...
  typename detail::visit_returnT<Visitor, EOSTypes_>::type visit(
      Visitor&& vis) const noexcept {
...
#define EOS_ENTRY_(IND, T)                                                     \
  case IND: {                                                                  \
    const T& ref = *(ptr_union_.alt_##IND);                                    \
    return vis(ref);                                                           \
  }
      EOS_ALTERNATIVE_LIST
#undef EOS_ENTRY_
      ...
      default:
        ERROR1("EnzoEOSVariant::visit", "unhandled tag: %d", tag_);
    }
  }
...
#define EOS_ENTRY_(IND, TYPE)                                                  \
  ...
  struct EnzoEOSVariant::access_helper_<TYPE> {                                \
    static TYPE* get_ptr(const EnzoEOSVariant::PtrUnion_& ptr_union,           \
                         int tag) {                                            \
