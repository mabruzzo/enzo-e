...
struct IsBarotropicVisitor {
  template <typename T>
  bool operator()(T eos) const {
    return T::is_barotropic();
  }
};
}  // namespace
...
void EnzoPhysicsFluidProps::primitive_from_integration(
  ...
  for (const std::string& key : quantity_list) {
    ...
    ASSERT6("EnzoPhysicsFluidProps::primitive_from_integration",
            ...
            mz, my, mx, prim_array.shape(0), prim_array.shape(1),
            prim_array.shape(2),
            ((prim_array.shape(0) == mz) && (prim_array.shape(1) == my) &&
  ...
  for (const std::string& key : passive_list) {
    ...
    for (int iz = stale_depth; iz < mz - stale_depth; iz++) {
      for (int iy = stale_depth; iy < my - stale_depth; iy++) {
        for (int ix = stale_depth; ix < mx - stale_depth; ix++) {
          out_specific(iz, iy, ix) =
              cur_conserved(iz, iy, ix) / density(iz, iy, ix);
        }
      }
    }
  }
...
void EnzoPhysicsFluidProps::apply_floor_to_energy_and_sync(
  ...
  const RdOnlyEFlt3DArray bx =
      (mag) ? RdOnlyEFlt3DArray(integration_map.at("bfield_x"))
            : RdOnlyEFlt3DArray();
  const RdOnlyEFlt3DArray by =
      (mag) ? RdOnlyEFlt3DArray(integration_map.at("bfield_y"))
            : RdOnlyEFlt3DArray();
  const RdOnlyEFlt3DArray bz =
      (mag) ? RdOnlyEFlt3DArray(integration_map.at("bfield_z"))
            : RdOnlyEFlt3DArray();
  ...
  for (int iz = stale_depth; iz < (density.shape(0) - stale_depth); iz++) {
    for (int iy = stale_depth; iy < (density.shape(1) - stale_depth); iy++) {
      for (int ix = stale_depth; ix < (density.shape(2) - stale_depth); ix++) {
        ...
        enzo_float v2 =
            (vx(iz, iy, ix) * vx(iz, iy, ix) + vy(iz, iy, ix) * vy(iz, iy, ix) +
        ...
        if (idual) {
          ...
          etot(iz, iy, ix) =
              enzo_utils::apply_floor(etot(iz, iy, ix), etot_floor);
        }
      }
    }
  }
