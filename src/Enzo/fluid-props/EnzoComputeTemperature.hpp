...
public:  // interface
  ...
  EnzoComputeTemperature(double density_floor, double temperature_floor,
                         double mol_weight, bool comoving_coordinates);
  ...
  EnzoComputeTemperature(const EnzoPhysicsFluidProps* fluid_props,
    ...
    density_floor_ =
        floor_conf.has_density_floor() ? (double)floor_conf.density() : 0.0;
    temperature_floor_ = floor_conf.has_temperature_floor()
                             ? (double)floor_conf.temperature()
                             : 0.0;
  ...
  std::string name() throw() { return "temperature"; }

  void compute_(Block* block, enzo_float* t, bool recompute_presure = true,
