...
EnzoBoundary::EnzoBoundary(axis_enum axis, face_enum face,
                           std::shared_ptr<Mask> mask,
                           boundary_type type) throw()
    : Boundary(axis, face, mask), boundary_type_(type) {}
...
void EnzoBoundary::enforce(Block* block, face_enum face, axis_enum axis) const
    throw() {
  ...
  if (face == face_all) {
    ...
    switch (boundary_type_) {
      ...
      case boundary_type_outflow:
        ...
        break;
      default:
        ERROR("EnzoBoundary::enforce", "Undefined boundary type");
        break;
    }
  }
}
...
void EnzoBoundary::enforce_reflecting_(Field field, Block* block,
                                       face_enum face, axis_enum axis) const
    throw() {
  Data* data = block->data();
  ...
  for (int index = 0; index < field.field_count(); index++) {
    ...
    enforce_reflecting_precision_(face, axis, array, nx, ny, nz, gx, gy, gz, cx,
                                  cy, cz, vx, vy, vz, x, y, z, xm, ym, zm, xp,
                                  yp, zp, t);
  }
...
void EnzoBoundary::enforce_reflecting_precision_(
    face_enum face, axis_enum axis, enzo_float* array, int nx, int ny, int nz,
    int gx, int gy, int gz, int cx, int cy, int cz, bool vx, bool vy, bool vz,
    double* x, double* y, double* z, double xm, double ym, double zm, double xp,
    double yp, double zp, double t) const throw() {
  int mx = nx + 2 * gx + cx;
  int my = ny + 2 * gy + cy;
  int mz = nz + 2 * gz + cz;
...
void EnzoBoundary::enforce_outflow_(Field field, Block* block, face_enum face,
  ...
  for (int index = 0; index < field.field_count(); index++) {
    ...
    enforce_outflow_precision_(face, axis, array, nx, ny, nz, gx, gy, gz, cx,
                               cy, cz, x, y, z, xm, ym, zm, xp, yp, zp, t);
  }
  delete[] xc;
  delete[] yc;
  delete[] zc;
...
void EnzoBoundary::enforce_outflow_precision_(
    face_enum face, axis_enum axis, enzo_float* array, int nx, int ny, int nz,
    int gx, int gy, int gz, int cx, int cy, int cz, double* x, double* y,
    double* z, double xm, double ym, double zm, double xp, double yp, double zp,
