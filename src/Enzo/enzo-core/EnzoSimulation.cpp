...
/// @date     2011-03-17
/// @brief    Implementation of EnzoSimulation user-dependent class member
/// functions
...
EnzoSimulation::EnzoSimulation(const char parameter_file[], int n)
  ...
  CkCallback callback(CkIndex_EnzoSimulation::r_startup_begun(NULL), thisProxy);
...
void EnzoSimulation::set_msg_check(Index index, EnzoMsgCheck* msg) {
  if (msg_check_map_[index] != NULL) {
    ...
    ASSERT3("EnzoSimulation::set_msg_check",
            "index %08x %08x %08x is already in the msg_check mapping", v3[0],
            v3[1], v3[2], (msg == NULL));
  }
...
EnzoMsgCheck* EnzoSimulation::get_msg_check(Index index) {
  ...
  if (msg == NULL) {
    ...
    ASSERT3("EnzoSimulation::get_msg_check",
            "index %08x %08x %08x is not in the msg_check mapping", v3[0],
            v3[1], v3[2], (msg != NULL));
  }
...
MsgRefine* EnzoSimulation::get_msg_refine(Index index) {
  ...
  if (msg == NULL) {
    ...
    ASSERT3("EnzoSimulation::get_msg_refine",
            "index %08x %08x %08x is not in the msg_refine mapping", v3[0],
            v3[1], v3[2], (msg != NULL));
  }
