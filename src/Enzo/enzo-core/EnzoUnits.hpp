...
public:  // interface
  /// Constructor
  EnzoUnits() throw() : Units(), cosmology_(NULL) {}

  /// CHARM++ PUP::able declaration
  PUPable_decl(EnzoUnits);

  /// CHARM++ migration constructor for PUP::able
  EnzoUnits(CkMigrateMessage* m) : Units(m), cosmology_(NULL) {}
  ...
  EnzoPhysicsCosmology* cosmology() { return cosmology_; }
...
public:  // virtual methods
  ...
  double photon_number_density() const {
    // The ratio of ionizing photons emitted per hydrogen atom at reionization
    // is just 1.5-3 (Bolton and Haehnelt 2007), so it makes sense to define our
    // cosmological photon number density units as proportional to
    // the cosmological baryon density units to keep the numbers of order unity
    // in code units.
