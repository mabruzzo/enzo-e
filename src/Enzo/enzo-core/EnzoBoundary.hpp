...
public:  // interface
  ...
  EnzoBoundary(CkMigrateMessage* m)
      : Boundary(m), boundary_type_(boundary_type_undefined) {}
...
protected:  // functions
  ...
  void enforce_reflecting_(Field field, Block* block, face_enum face,
                           axis_enum axis) const throw();

  /// Template for reflecting boundary conditions on different precisions
  void enforce_reflecting_precision_(face_enum face, axis_enum axis,
                                     enzo_float* array, int nx, int ny, int nz,
                                     int gx, int gy, int gz, int cx, int cy,
                                     int cz, bool vx, bool vy, bool vz,
                                     double* x, double* y, double* z, double xm,
                                     double ym, double zm, double xp, double yp,
                                     double zp, double t) const throw();

  /// Checks if the field name matches one of the vector fields
  bool has_vector_name_(std::string field_name, std::string component) const
      throw();

  //--------------------------------------------------

  /// Enforce outflow boundary conditions on a boundary face
  void enforce_outflow_(Field field, Block* block, face_enum face,
                        axis_enum axis) const throw();

  /// Enforce outflow boundary conditions on a boundary face
  void enforce_outflow_precision_(face_enum face, axis_enum axis,
                                  enzo_float* array, int nx, int ny, int nz,
                                  int gx, int gy, int gz, int cx, int cy,
                                  int cz, double* x, double* y, double* z,
                                  double xm, double ym, double zm, double xp,
                                  double yp, double zp, double t) const throw();

  //--------------------------------------------------

  /// Enforce inflow boundary conditions on a boundary face
  void enforce_inflow_(Field field, Block* block, face_enum face,
                       axis_enum axis) const throw();
