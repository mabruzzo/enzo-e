...
void EnzoMsgCheck::set_data_msg(DataMsg* data_msg) {
  if (data_msg_) {
    WARNING("EnzoMsgCheck::set_data_msg()", "overwriting existing data_msg_");
    delete data_msg_;
  }
  data_msg_ = data_msg;
}
...
void* EnzoMsgCheck::pack(EnzoMsgCheck* msg) {
  ...
  ASSERT2("EnzoMsgCheck::pack()",
          "buffer size mismatch %ld allocated %d packed", (pc - (char*)buffer),
          size, (pc - (char*)buffer) == size);
...
EnzoMsgCheck* EnzoMsgCheck::unpack(void* buffer) {
  ...
  EnzoMsgCheck* msg =
      (EnzoMsgCheck*)CkAllocBuffer(buffer, sizeof(EnzoMsgCheck));
...
Index EnzoMsgCheck::index_send() { return index_send_; }

//----------------------------------------------------------------------

void EnzoMsgCheck::set_index_send(Index index) { index_send_ = index; }
...
void EnzoMsgCheck::print(const char* msg) {
  ...
  CkPrintf("%d ENZO_MSG_CHECK block_size_ %d %d %d\n", CkMyPe(), block_size_[0],
           block_size_[1], block_size_[2]);
