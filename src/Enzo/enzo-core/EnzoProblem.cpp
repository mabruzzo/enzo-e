...
EnzoProblem::EnzoProblem() throw() : Problem() {}
...
Boundary* EnzoProblem::create_boundary_(std::string type, int index,
                                        Config* config,
...
/// @param parameters Parameters object (for evaluating expressions)
{
  ...
  if (config->boundary_mask[index]) {
    std::string param_str =
        "Boundary:" + config->boundary_list[index] + ":mask";
...
Initial* EnzoProblem::create_initial_(std::string type, int index,
                                      Config* config,
                                      Parameters* parameters) throw() {
  //--------------------------------------------------
  ...
  if (type == "hdf5") {
    initial = new EnzoInitialHdf5(
        cycle, time, enzo_config->initial_hdf5_max_level,
        enzo_config->initial_hdf5_format, enzo_config->initial_hdf5_blocking,
    ...
    initial =
        new EnzoInitialCollapse(cycle, time, enzo_config->initial_collapse_rank,
    ...
    initial =
        new EnzoInitialCosmology(cycle, time, enzo::fluid_props()->gamma(),
    ...
    initial = new EnzoInitialTurbulence(
        cycle, time, enzo_config->initial_turbulence_density,
    ...
    initial = new EnzoInitialPm(
        parameters, param_str, cycle, time, enzo_config->initial_pm_field,
        enzo_config->initial_pm_mpp, enzo_config->initial_pm_level);
    ...
    initial = new EnzoInitialSoup(cycle, time, p_group);
  } else if (type == "burkertbodenheimer") {
    initial = new EnzoInitialBurkertBodenheimer(
        cycle, time, enzo_config->initial_burkertbodenheimer_rank,
    ...
    initial = new EnzoInitialAccretionTest(
        cycle, time, enzo_config->initial_accretion_test_sink_position,
    ...
    initial = new EnzoInitialBBTest(
        cycle, time, enzo_config->initial_bb_test_center,
    ...
    initial = Problem::create_initial_(type, index, config, parameters);
  }
...
Stopping* EnzoProblem::create_stopping_(std::string type,
                                        Config* config) throw()
/// @param type   Type of the stopping criterion to create (ignored)
/// @param config  Configuration parameter class
{
  const EnzoConfig* enzo_config = enzo::config();
  return new EnzoStopping(
      enzo_config->stopping_cycle, enzo_config->stopping_time,
      enzo_config->stopping_seconds, enzo_config->stopping_redshift);
}

//----------------------------------------------------------------------

Refine* EnzoProblem::create_refine_(std::string type, int index, Config* config,
                                    Parameters* parameters) throw() {
  const EnzoConfig* enzo_config = enzo::config();
  ...
  if (type == "shock") {
    return new EnzoRefineShock(
        ...
        config->adapt_min_refine2[index], config->adapt_max_coarsen2[index],
        enzo::fluid_props()->gamma(), enzo_config->physics_cosmology,
    ...
    return new EnzoRefineMass(
        config->adapt_min_refine[index], config->adapt_max_coarsen[index],
        config->adapt_max_level[index], config->adapt_include_ghosts[index],
        config->adapt_output[index], config->adapt_field_list[index][0],
...
Solver* EnzoProblem::create_solver_(std::string solver_type, int index_solver,
                                    Config* config) throw()
...
/// @param config Configuration parameters class
{
  ...
  if (solver_type == "cg") {
    solver = new EnzoSolverCg(enzo_config->solver_list[index_solver],
                              ...
                              solve_type, index_prolong, index_restrict,
    ...
    solver = new EnzoSolverDd(enzo_config->solver_list[index_solver],
                              enzo_config->solver_field_x[index_solver],
                              enzo_config->solver_field_b[index_solver],
                              enzo_config->solver_monitor_iter[index_solver],
                              enzo_config->solver_restart_cycle[index_solver],
    ...
    solver =
        new EnzoSolverBiCgStab(enzo_config->solver_list[index_solver],
                               enzo_config->solver_field_x[index_solver],
                               enzo_config->solver_field_b[index_solver],
                               enzo_config->solver_monitor_iter[index_solver],
                               enzo_config->solver_restart_cycle[index_solver],
    ...
    solver =
        new EnzoSolverDiagonal(enzo_config->solver_list[index_solver],
                               enzo_config->solver_field_x[index_solver],
                               enzo_config->solver_field_b[index_solver],
                               enzo_config->solver_monitor_iter[index_solver],
                               enzo_config->solver_restart_cycle[index_solver],
    ...
    solver = new EnzoSolverJacobi(
        ...
        enzo_config->solver_field_b[index_solver],
        enzo_config->solver_monitor_iter[index_solver],
        enzo_config->solver_restart_cycle[index_solver], solve_type,
        index_prolong, index_restrict, enzo_config->solver_weight[index_solver],
        enzo_config->solver_iter_max[index_solver]);

  } else if (solver_type == "mg0") {
    solver = new EnzoSolverMg0(enzo_config->solver_list[index_solver],
                               enzo_config->solver_field_x[index_solver],
                               enzo_config->solver_field_b[index_solver],
                               enzo_config->solver_monitor_iter[index_solver],
                               enzo_config->solver_restart_cycle[index_solver],
  ...
  ASSERT1("EnzoProblem::create_solver()", "Unknown solver %s",
          solver_type.c_str(), solver != NULL);
...
Compute* EnzoProblem::create_compute(std::string name, Config* config) throw()
/// @param name  Name of the compute to create
{
  ...
  if (name == "temperature") {
    ...
    ASSERT2("EnzoProblem::create_compute",
            ...
            compute->name().c_str(), name.c_str(), compute->name() == name);
  }
...
Method* EnzoProblem::create_method_(std::string name, int index_method,
                                    Config* config,
...
/// @param config Configuration parameters class
{
  ...
  if (name == "ppm") {
    ...
    method =
        new EnzoMethodTurbulence(enzo_config->method_turbulence_edot,
    ...
    ASSERT1("EnzoProblem::create_solver_()", "Cannot find solver \"%s\"",
            solver_name.c_str(),
            0 <= index_solver && index_solver < enzo_config->num_solvers);

    Prolong* prolong =
        create_prolong_(p_group.value_string("prolong", "linear"), config);
    ...
    method = new EnzoMethodCheck(
        enzo_config->method_check_num_files, enzo_config->method_check_ordering,
        enzo_config->method_check_dir, enzo_config->method_check_monitor_iter,
    ...
    if (flavor == "threshold") {
      ...
      ERROR1("EnzoProblem::create_method_",
             "\"accretion\" method has flavor \"%s\", which is not one of the "
             "possible options: "
  ...
  if (method) {
    ...
    ASSERT2("EnzoProblem::create_method",
            ...
            method->name().c_str(), name.c_str(), method->name() == name);
  }
...
Prolong* EnzoProblem::create_prolong_(std::string type,
                                      Config* config) throw() {
  Prolong* prolong = 0;
  ...
  if (type == "enzo") {
    prolong = new EnzoProlong(enzo_config->prolong_enzo_type,
...
Physics* EnzoProblem::create_physics_(std::string type, int index,
                                      Config* config,
                                      Parameters* parameters) throw() {
  // move creation of p_accessor up the call stack?
  ...
  if (type == "cosmology") {
    ...
    physics = Problem::create_physics_(type, index, config, parameters);
  }
...
void EnzoProblem::initialize_physics_coda_(Config* config,
  ...
  for (const std::string& name : required) {
    if (physics(name) != nullptr) {
      continue;
    }
    physics_list_.push_back(
        create_physics_(name, physics_list_.size(), config, parameters));
  }
...
Restrict* EnzoProblem::create_restrict_(std::string type,
                                        Config* config) throw() {
  Restrict* restrict = 0;
