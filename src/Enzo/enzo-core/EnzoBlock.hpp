...
public:  // interface
  ...
  EnzoBlock() : CBase_EnzoBlock(), dt(0.0), redshift(0.0) {
...
public:  /// entry methods
  ...
  void p_dot_recv_parent(int n, long double* dot_block,
                         std::vector<int> is_array, int i_function, int iter);
...
protected:  // methods
  ...
  int create_msg_check_(EnzoMsgCheck** msg_check, int num_files,
                        std::string ordering, std::string name_dir = "",
                        bool* is_first = nullptr);
