...
EnzoMethodPpml::EnzoMethodPpml(ParameterGroup p)
    : Method(), comoving_coordinates_(enzo::cosmology() != nullptr) {
...
double EnzoMethodPpml::timestep(Block* block) throw() {
  ...
  if (field.num_permanent() > 0) {  // TODO: revisit if-clause. This could be
    ...
    (enzo_block->GridDimension, enzo_block->GridDimension + 1,
     ...
     &enzo_block->CellWidth[1], &enzo_block->CellWidth[2], d, vx, vy, vz, bx,
     by, bz, &dtBaryons);
