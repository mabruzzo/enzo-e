...
EnzoMHDIntegratorStageCommands::EnzoMHDIntegratorStageCommands(
  ...
  str_vec_t primitive_field_list = riemann_solver_->primitive_quantity_keys();
...
EnzoMHDIntegratorStageCommands::parse_bfield_choice_(
  ...
  std::transform(choice.begin(), choice.end(), formatted.begin(), ::tolower);
...
void EnzoMHDIntegratorStageCommands::compute_update_stage(
    ...
    std::array<EnzoEFltArrayMap, 3> flux_maps_xyz, EnzoEFltArrayMap dUcons_map,
    const EnzoEFltArrayMap accel_map, EFlt3DArray interface_vel_arr,
    const str_vec_t& passive_list, EnzoBfieldMethod* bfield_method_,
    unsigned short stage_index, double cur_dt, int stale_depth,
  ...
  for (int dim = 0; dim < 3; dim++) {
    ...
    compute_flux_(dim, cur_dt, cell_widths_xyz[dim], primitive_map, pl_map,
                  pr_map, flux_maps_xyz[dim], dUcons_map, interface_vel_arr_ptr,
                  *reconstructor, bfield_method_, stale_depth, passive_list);
  }
  ...
  if (bfield_method_ != nullptr) {
    bfield_method_->update_all_bfield_components(
        cur_stage_integration_map, flux_maps_xyz[0], flux_maps_xyz[1],
        flux_maps_xyz[2], out_integration_map, cur_dt, stale_depth);
  }
  ...
  integration_quan_updater_->update_quantities(tstep_begin_integration_map,
                                               dUcons_map, out_integration_map,
...
void EnzoMHDIntegratorStageCommands::compute_flux_(
    ...
    EnzoEFltArrayMap& primitive_map, EnzoEFltArrayMap& priml_map,
    EnzoEFltArrayMap& primr_map, EnzoEFltArrayMap& flux_map,
  ...
  reconstructor.reconstruct_interface(primitive_map, priml_map, primr_map, dim,
                                      stale_depth, passive_list);
  ...
  if (bfield_method != nullptr) {
    bfield_method->correct_reconstructed_bfield(priml_map, primr_map, dim,
                                                cur_stale_depth);
  }

  // Next, compute the fluxes
  riemann_solver_->solve(priml_map, primr_map, flux_map, dim, cur_stale_depth,
                         passive_list, interface_velocity_arr_ptr);
  ...
  // [partial] timestep)
  integration_quan_updater_->accumulate_flux_component(
...
void EnzoMHDIntegratorStageCommands::compute_source_terms_(
    ...
    const EnzoEFltArrayMap& primitive_map, const EnzoEFltArrayMap& accel_map,
...
double EnzoMHDIntegratorStageCommands::timestep(
    EnzoEFltArrayMap integration_map, CelloView<const enzo_float, 3> pressure,
  ...
  if (this->is_pure_hydro()) {
    auto loop_body = [=, &dtBaryons](int iz, int iy, int ix) {
      double cs =
          (double)eos.sound_speed(density(iz, iy, ix), pressure(iz, iy, ix));
    ...
    auto loop_body = [=, &dtBaryons](int iz, int iy, int ix) {
      ...
      double cfast = (double)eos.fast_magnetosonic_speed<0>(
          density(iz, iy, ix), pressure(iz, iy, ix), bfieldc_x(iz, iy, ix),
          bfieldc_y(iz, iy, ix), bfieldc_z(iz, iy, ix));
