...
int enzo::SetMinimumSupport(EnzoBlock& block,
  ...
  if (field.num_permanent() > 0) {  // TODO: revisit if-clause. This could be
    ...
    ASSERT("EnzoBlock::SetMinimumSupport",
           ...
           enzo_utils::consistent_cube_cellwidths(
               block.CellWidth[0], block.CellWidth[1], block.CellWidth[2]));
    ...
    MinimumSupportEnergyCoefficient =
        ...
        CosmoFactor * minimum_pressure_support_parameter * block.CellWidth[0] *
        block.CellWidth[0];
    ...
    if (uses_dual_energy_formalism) {
      ...
      for (i = 0; i < size; i++)
        total_energy[i] =
            MAX((enzo_float)(internal_energy[i] +
