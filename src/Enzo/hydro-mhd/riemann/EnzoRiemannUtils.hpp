...
inline lutarray<LUT> active_fluxes(const lutarray<LUT> prim,
  ...
  if (LUT::total_energy >= 0) {
    fluxes[LUT::total_energy] =
        ((etot + ptot) * vi - (Bi * vi + (Bj * vj + Bk * vk)) * Bi);
  }

  // Fluxes for Bi,Bj,Bk
  if (LUT::bfield_i >= 0) {
    fluxes[LUT::bfield_i] = 0;
  }
  if (LUT::bfield_j >= 0) {
    fluxes[LUT::bfield_j] = Bj * vi - Bi * vj;
  }
  if (LUT::bfield_k >= 0) {
    fluxes[LUT::bfield_k] = Bk * vi - Bi * vk;
  }
...
static inline enzo_float passive_eint_flux(
    const enzo_float density_l, const enzo_float pressure_l,
    const enzo_float density_r, const enzo_float pressure_r,
    const EnzoEOSIdeal& eos, const enzo_float density_flux) noexcept {
...
static void solve_passive_advection(
    ...
    const CelloView<const enzo_float, 3>& density_flux, const int stale_depth,
...
static void prep_dual_energy_arrays_(
    ...
    ScratchArrays_* ptr, CelloView<enzo_float, 3>& internal_energy_flux,
