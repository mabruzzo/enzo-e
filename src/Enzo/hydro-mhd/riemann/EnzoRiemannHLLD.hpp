...
public:  // methods
  FORCE_INLINE void operator()(const int iz, const int iy,
    ...
    enzo_float pbl =
        0.5 * (bxsq + (SQR(wli[LUT::bfield_j]) + SQR(wli[LUT::bfield_k])));
    enzo_float pbr =
        0.5 * (bxsq + (SQR(wri[LUT::bfield_j]) + SQR(wri[LUT::bfield_k])));
    // kinetic energy:
    enzo_float kel = 0.5 * wli[LUT::density] *
                     (SQR(wli[LUT::velocity_i]) +
                      (SQR(wli[LUT::velocity_j]) + SQR(wli[LUT::velocity_k])));
    enzo_float ker = 0.5 * wri[LUT::density] *
                     (SQR(wri[LUT::velocity_i]) +
                      (SQR(wri[LUT::velocity_j]) + SQR(wri[LUT::velocity_k])));
    ...
    // symmetry
    ulst.e =
        (sdl * ul.e - ptl * wli[LUT::velocity_i] + ptst * spd[2] +
    ...
    // symmetry
    urst.e =
        (sdr * ur.e - ptr * wri[LUT::velocity_i] + ptst * spd[2] +
         bxi * (wri[LUT::velocity_i] * bxi +
                (wri[LUT::velocity_j] * ur.by + wri[LUT::velocity_k] * ur.bz) -
                vbstr)) *
        sdmr_inv;
