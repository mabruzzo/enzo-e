...
typedef enzo_float (*limiter_function_call_signature)(enzo_float, enzo_float,
                                                      enzo_float, enzo_float);
...
public:  // interface
  ...
  EnzoReconstructorPLM(std::vector<std::string> active_key_names,
                       enzo_float theta_limiter)
      : EnzoReconstructor(active_key_names), theta_limiter_(theta_limiter) {}

  void reconstruct_interface(const EnzoEFltArrayMap& prim_map,
                             EnzoEFltArrayMap& priml_map,
                             EnzoEFltArrayMap& primr_map, const int dim,
                             const int stale_depth,
                             const str_vec_t& passive_list);

  int total_staling_rate() { return 2; }

  int immediate_staling_rate() { return 1; }
...
void EnzoReconstructorPLM<Limiter>::reconstruct_interface(
  ...
  auto fn = [coord, limiter_func, theta_limiter, stale_depth, &prim_map,
             &priml_map,
             &primr_map](const std::string& key, const bool use_floor,
...
struct PLM_EnzoRKLimiter {
  ...
  enzo_float operator()(enzo_float vm1, enzo_float v, enzo_float vp1,
                        enzo_float theta_limiter) const {
