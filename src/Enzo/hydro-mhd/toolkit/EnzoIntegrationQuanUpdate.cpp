...
static void append_key_to_vec_(const str_vec_t& integration_quantity_keys,
                               FieldCat target_cat, const bool skip_bfield,
                               std::size_t* density_index, str_vec_t& key_vec) {
...
void EnzoIntegrationQuanUpdate::clear_dUcons_map(
  ...
  for (const std::string& key : integration_keys_) {
    init_arr(key);
  }
...
void EnzoIntegrationQuanUpdate::accumulate_flux_component(
  ...
  auto accumulate = [dtdx_i, stale_depth, coord, &flux_map,
                     &dUcons_map](const std::string& key) {
    ...
    const EFlt3DArray dU_center =
        coord.get_subarray(dU, full_ax, full_ax, CSlice(1, -1));
...
EnzoIntegrationQuanUpdate::load_integration_quan_(
    EnzoEFltArrayMap& map, int stale_depth) const noexcept {
...
void EnzoIntegrationQuanUpdate::update_quantities(
    ...
    const EnzoEFltArrayMap& dUcons_map, EnzoEFltArrayMap& out_integration_map,
  ...
  const std::vector<EFlt3DArray> cur_prim =
      load_integration_quan_(initial_integration_map, stale_depth);
  const std::vector<EFlt3DArray> out_prim =
      load_integration_quan_(out_integration_map, stale_depth);
  const std::vector<CelloView<const enzo_float, 3>> dU =
      load_integration_quan_(dUcons_map, stale_depth);
    ...
    for (int iy = 1; iy < (cur_prim[density_index_].shape(1) - 1); iy++) {
      for (int ix = 1; ix < (cur_prim[density_index_].shape(2) - 1); ix++) {
        ...
        for (std::size_t i = first_specific_index_; i < nfields; i++) {
          out_prim[i](iz, iy, ix) =
              (cur_prim[i](iz, iy, ix) * old_rho + dU[i](iz, iy, ix)) *
              inv_new_rho;
        }
      }
    }
...
void EnzoIntegrationQuanUpdate::update_passive_scalars_(
  ...
  for (const std::string& key : passive_list) {
    const EFlt3DArray cur_conserved =
        initial_integration_map.get(key, stale_depth);
      ...
      for (int iy = 1; iy < my - 1; iy++) {
        for (int ix = 1; ix < mx - 1; ix++) {
          out_conserved(iz, iy, ix) =
              cur_conserved(iz, iy, ix) + dU(iz, iy, ix);
        }
      }
    }
  }
