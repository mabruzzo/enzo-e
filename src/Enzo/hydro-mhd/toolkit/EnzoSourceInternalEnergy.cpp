...
void EnzoSourceInternalEnergy::calculate_source(
  ...
  const CelloView<const enzo_float, 3> pressure =
      prim_map.get("pressure", stale_depth);
  const CelloView<const enzo_float, 3> pressure_center =
      coord.get_subarray(pressure, full_ax, full_ax, CSlice(1, -1));

  const CelloView<enzo_float, 3> deint_dens =
      dUcons_map.get("internal_energy", stale_depth);
  const CelloView<enzo_float, 3> deint_dens_center =
      coord.get_subarray(deint_dens, full_ax, full_ax, CSlice(1, -1));

  // remove staled zone from interface_velocity
  CSlice stale_slice(stale_depth, -stale_depth);
  const CelloView<const enzo_float, 3> vel =
      interface_velocity.subarray(stale_slice, stale_slice, stale_slice);
  // load the face-centered values of the ith velocity component
  // define: vl(k,j,i)                 ->  vel_i(k,j,i+1/2)
  //         vr(k,j,i)                 ->  vel_i(k,j,i+3/2)
  const CelloView<const enzo_float, 3> vl =
      coord.get_subarray(vel, full_ax, full_ax, CSlice(0, -1));
  const CelloView<const enzo_float, 3> vr =
      coord.get_subarray(vel, full_ax, full_ax, CSlice(1, nullptr));
  ...
  for (int iz = 0; iz < pressure_center.shape(0); iz++) {
    for (int iy = 0; iy < pressure_center.shape(1); iy++) {
      for (int ix = 0; ix < pressure_center.shape(2); ix++) {
        ...
        deint_dens_center(iz, iy, ix) -=
            dtdx * p * (vr(iz, iy, ix) - vl(iz, iy, ix));
      }
    }
  }
}
