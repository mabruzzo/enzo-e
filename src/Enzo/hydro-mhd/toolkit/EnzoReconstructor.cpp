...
EnzoReconstructor* EnzoReconstructor::construct_reconstructor(
  ...
  std::transform(name.begin(), name.end(), formatted.begin(), ::tolower);
  ...
  if (formatted == std::string("nn")) {
    ...
    out =
        new EnzoReconstructorPLMEnzoRKLim(active_primitive_keys, theta_limiter);
  } else if (formatted == std::string("plm_athena")) {
    out =
        new EnzoReconstructorPLMAthenaLim(active_primitive_keys, theta_limiter);
  } else {
