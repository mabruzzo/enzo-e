...
int EnzoMethodPpml::SolveMHDEquations(EnzoBlock& block, enzo_float dt) {
  ...
  ASSERT("EnzoBlock::SolveMHDEquations",
         "This function requires the domain's rank to be 3", GridRank == 3);
  ...
  if (field.num_permanent() > 0) {  // TODO: revisit if-clause. This could be
    ...
    const std::array<enzo_float, 3> DomainLeftEdge = {
        enzo_float(lo_arr_[0]), enzo_float(lo_arr_[1]), enzo_float(lo_arr_[2])};
    ...
    size = 1;
    for (dim = 0; dim < GridRank; dim++) size *= block.GridDimension[dim];
    ...
    enzo_float* f1 = &temp[k * size];
    k++;
    enzo_float* f2 = &temp[k * size];
    k++;
    enzo_float* f3 = &temp[k * size];
    k++;
    enzo_float* f4 = &temp[k * size];
    k++;
    enzo_float* f5 = &temp[k * size];
    k++;
    enzo_float* f6 = &temp[k * size];
    k++;
    enzo_float* f7 = &temp[k * size];
    k++;
    enzo_float* g1 = &temp[k * size];
    k++;
    enzo_float* g2 = &temp[k * size];
    k++;
    enzo_float* g3 = &temp[k * size];
    k++;
    enzo_float* g4 = &temp[k * size];
    k++;
    enzo_float* g5 = &temp[k * size];
    k++;
    enzo_float* g6 = &temp[k * size];
    k++;
    enzo_float* g7 = &temp[k * size];
    k++;
    enzo_float* h1 = &temp[k * size];
    k++;
    enzo_float* h2 = &temp[k * size];
    k++;
    enzo_float* h3 = &temp[k * size];
    k++;
    enzo_float* h4 = &temp[k * size];
    k++;
    enzo_float* h5 = &temp[k * size];
    k++;
    enzo_float* h6 = &temp[k * size];
    k++;
    enzo_float* h7 = &temp[k * size];
    k++;
    enzo_float* ex = &temp[k * size];
    k++;
    enzo_float* ey = &temp[k * size];
    k++;
    enzo_float* ez = &temp[k * size];
    k++;
    enzo_float* qu1 = &temp[k * size];
    k++;
    enzo_float* qu2 = &temp[k * size];
    k++;
    enzo_float* qu3 = &temp[k * size];
    k++;
    enzo_float* qu4 = &temp[k * size];
    k++;
    enzo_float* qu5 = &temp[k * size];
    k++;
    enzo_float* qu6 = &temp[k * size];
    k++;
    enzo_float* qu7 = &temp[k * size];
    k++;

    ASSERT("EnzoBlock::SolveMHDEquations", "Insufficient temporary storage",
           k <= 31);
    ...
    // enzo_float *standard = NULL;
    //    if (NumberOfSubgrids > 0) standard =
    //    SubgridFluxes[0]->LeftFluxes[0][0];
    ...
    /* current PPML implementation only supports 3D and does not support color
     * fields */

    FORTRAN_NAME(ppml)
    (density, velox, veloy, veloz, bfieldx, bfieldy, bfieldz, dens_rx, velox_rx,
     veloy_rx, veloz_rx, bfieldx_rx, bfieldy_rx, bfieldz_rx, dens_ry, velox_ry,
     veloy_ry, veloz_ry, bfieldx_ry, bfieldy_ry, bfieldz_ry, dens_rz, velox_rz,
     veloy_rz, veloz_rz, bfieldx_rz, bfieldy_rz, bfieldz_rz, &dt,
     &CellWidthTemp[0], &CellWidthTemp[1], &CellWidthTemp[2],
     &block.GridDimension[0], &block.GridDimension[1], &block.GridDimension[2],
     block.GridStartIndex, block.GridEndIndex, &NumberOfSubgrids, leftface,
     rightface, istart, iend, jstart, jend, standard, dnindex, vxindex, vyindex,
     vzindex, bxindex, byindex, bzindex, f1, f2, f3, f4, f5, f6, f7, g1, g2, g3,
     g4, g5, g6, g7, h1, h2, h3, h4, h5, h6, h7, ex, ey, ez, qu1, qu2, qu3, qu4,
     qu5, qu6, qu7);
