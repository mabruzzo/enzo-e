...
void EnzoMatrixLaplace::matvec(int i_y, int i_x, Block* block, int g0) throw() {
...
void EnzoMatrixLaplace::matvec(precision_type precision, void* y, void* x,
                               int g0) throw() {
...
void EnzoMatrixLaplace::matvec_(enzo_float* Y, enzo_float* X, int g0) const
    throw() {
  const int idx = 1;
  const int idy = mx_;
  ...
  if (order_ == 2) {
      ...
      for (int iz = g0; iz < mz_ - g0; iz++) {
        for (int iy = g0; iy < my_ - g0; iy++) {
          for (int ix = g0; ix < mx_ - g0; ix++) {
            const int i = ix + mx_ * (iy + my_ * iz);
            enzo_float* xp = X + i;
            Y[i] = (c0x * (xp[0]) + c1x * (xp[-idx] + xp[idx]) +
                    c2x * (xp[-idx2] + xp[idx2])) +
                   (c0y * (xp[0]) + c1y * (xp[-idy] + xp[idy]) +
                    c2y * (xp[-idy2] + xp[idy2])) +
                   (c0z * (xp[0]) + c1z * (xp[-idz] + xp[idz]) +
                    c2z * (xp[-idz2] + xp[idz2]));
          }
        }
      }
        ...
        for (int iy = g0; iy < my_ - g0; iy++) {
          for (int ix = g0; ix < mx_ - g0; ix++) {
            ...
            Y[i] = (c0 * (X[i]) + c1 * (X[i - idx] + X[i + idx]) +
                   ...
                   (c0 * (X[i]) + c1 * (X[i - idy] + X[i + idy]) +
                    c2 * (X[i - idy2] + X[i + idy2]) +
                    c2 * (X[i - idy3] + X[i + idy3])) *
                       dy +
                   (c0 * (X[i]) + c1 * (X[i - idz] + X[i + idz]) +
                    c2 * (X[i - idz2] + X[i + idz2]) +
                    c3 * (X[i - idz3] + X[i + idz3])) *
                       dz;
          }
        }
      }
    ...
    ERROR1("EnzoMatrixLaplace::diagonal()",
           "Order %d operator is not supported", order_);
  }
}
