...
EnzoSolverDd::EnzoSolverDd(std::string name, std::string field_x,
                           std::string field_b, int monitor_iter,
                           int restart_cycle, int solve_type, int index_prolong,
                           int index_restrict, int min_level, int max_level,
                           int index_solve_coarse, int index_solve_domain,
                           int index_solve_smooth, int coarse_level)
...
void EnzoSolverDd::apply(std::shared_ptr<Matrix> A, Block* block) throw() {
  ...
  ASSERT2("EnzoSolverDd::apply()", "Coarse solver %s type %s != solve_level",
...
void EnzoSolverDd::restrict_recv(EnzoBlock* enzo_block, FieldMsg* msg) throw() {
...
void EnzoSolverDd::prolong_recv(EnzoBlock* enzo_block, FieldMsg* msg) throw() {
...
FieldMsg* EnzoSolverDd::pack_field_(EnzoBlock* enzo_block, int index_field,
                                    int refresh_type, int* ic3) {
  int if3[3] = {0, 0, 0};
  int g3[3];
  ...
  FieldFace* field_face =
      enzo_block->create_face(if3, ic3, g3, refresh_type, refresh);
...
void EnzoSolverDd::unpack_field_(EnzoBlock* enzo_block, FieldMsg* msg,
                                 int index_field, int refresh_type) {
  int if3[3] = {0, 0, 0};
  int g3[3];
  cello::field_descr()->ghost_depth(index_field, g3, g3 + 1, g3 + 2);
  if (refresh_type != refresh_fine)
