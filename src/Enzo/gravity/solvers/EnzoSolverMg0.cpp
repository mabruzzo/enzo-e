...
EnzoSolverMg0::EnzoSolverMg0(std::string name, std::string field_x,
                             std::string field_b, int monitor_iter,
                             int restart_cycle, int solve_type,
                             int index_prolong, int index_restrict,
                             int min_level, int max_level, int iter_max,
                             double res_tol, int index_smooth_pre,
                             int index_solve_coarse, int index_smooth_post,
                             int index_smooth_last, int coarse_level)
...
///        begin_cycle()
{
  *piter(enzo_block) = 0.0;
  ...
  if (A_->is_singular()) {
    ...
    SOLVER_CONTROL(enzo_block, "min", "max",
                   "2 calling begin_solve_2 (no shift)");
...
void EnzoSolverMg0::compute_shift_(EnzoBlock* enzo_block,
                                   long double* reduce) throw() {
  Field field = enzo_block->data()->field();
...
void EnzoSolverMg0::monitor_output_(EnzoBlock* enzo_block) {
  ...
  const bool l_output =
      ((enzo_block->index().is_root()) && ((iter == 0)));  // ||
...
void EnzoSolverMg0::restrict_recv(EnzoBlock* enzo_block, FieldMsg* msg) throw()
///
///      [ unpack B ]
///      if (sync.next())
///          begin_cycle()
{
  ...
  if (msg == nullptr)
    SOLVER_CONTROL(enzo_block, "coarse", "fine-1",
                   "16 restrict_recv_1 nullptr");
...
void EnzoSolverMg0::prolong_recv(EnzoBlock* enzo_block, FieldMsg* msg) throw()
///
///      [ unpack C ]
///      X = X + C
///      callback = p_post_smooth()
...
///         begin_cycle()
{
  ...
  const bool l_output = ((enzo_block->index().is_root()) &&
...
FieldMsg* EnzoSolverMg0::pack_residual_(EnzoBlock* enzo_block) throw() {
  ...
  FieldFace* field_face =
      enzo_block->create_face(if3, ic3, g3, refresh_coarse, refresh);
...
void EnzoSolverMg0::unpack_residual_(EnzoBlock* enzo_block,
                                     FieldMsg* msg) throw() {
  int if3[3] = {0, 0, 0};
  int g3[3] = {0, 0, 0};
  Refresh* refresh = new Refresh;
...
FieldMsg* EnzoSolverMg0::pack_correction_(EnzoBlock* enzo_block,
                                          int ic3[3]) throw() {
  ...
  FieldFace* field_face =
      enzo_block->create_face(if3, ic3, g3, refresh_fine, refresh);
...
void EnzoSolverMg0::unpack_correction_(EnzoBlock* enzo_block,
                                       FieldMsg* msg) throw() {
  int if3[3] = {0, 0, 0};
  int g3[3];
  ...
  FieldFace* field_face =
      enzo_block->create_face(if3, msg->ic3, g3, refresh_fine, refresh);
