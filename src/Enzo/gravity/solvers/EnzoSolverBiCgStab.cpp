...
///----------------------------------------------------------------------
void mutex_init_bcg_iter() { bcg_iter_node_lock = CmiCreateLock(); }
...
#define COPY_FIELD(BLOCK, MSG, ID, COPY)                                       \
  {                                                                            \
    ...
    if (X_bcg)                                                                 \
      for (int i = 0; i < m_; i++) X_bcg[i] = X[i];                            \
...
#define TRACE_SCALAR(BLOCK, NAME, SCALAR)                                      \
  if (CYCLE_START <= BLOCK->cycle() && BLOCK->cycle() < CYCLE_END) {           \
    CkPrintf("%s:%d %s TRACE_SCALAR %s = %Lg\n", __FILE__, __LINE__,           \
             BLOCK->name().c_str(), NAME, SCALAR);                             \
    fflush(stdout);                                                            \
  }
#else
#define TRACE_SCALAR(BLOCK, NAME, SCALAR) /* ... */
...
#define TRACE_FIELD(BLOCK, MSG, ID, NAME)                                      \
  ...
  if (ITER_START <= iter && iter < ITER_END) {                                 \
    if (CYCLE_START <= BLOCK->cycle() && BLOCK->cycle() < CYCLE_END) {         \
      ...
      CkPrintf(                                                                \
          "%d TRACE_FIELD %s %s %s iter %d values %d [%g %g %g] ghosts %d "    \
          "[%g %g %g]\n",                                                      \
          CkMyPe(), BLOCK->name().c_str(), MSG, NAME, iter, count_f, min_f,    \
          sum_f / count_f, max_f, count_g, min_g, sum_g / count_g, max_g);     \
      CkPrintf(                                                                \
          "%d TRACE_FIELD %s %s |%s| iter %d values %d [%g %g %g] ghosts %d "  \
          "[%g %g %g]\n",                                                      \
          CkMyPe(), BLOCK->name().c_str(), MSG, NAME, iter, count_f, min_af,   \
          sum_af / count_f, max_af, count_g, min_ag, sum_ag / count_g,         \
          max_ag);                                                             \
    }                                                                          \
  }
#else
...
#define TRACE_BCG(BLOCK, SOLVER, msg)                                          \
  if (CYCLE_START <= BLOCK->cycle() && BLOCK->cycle() <= CYCLE_END) {          \
    CkPrintf("%d %s %s:%d TRACE_BCG %s %s level %d\n", CkMyPe(),               \
             (BLOCK != NULL) ? BLOCK->name().c_str() : "no block", __FILE__,   \
             __LINE__,                                                         \
...
EnzoSolverBiCgStab::EnzoSolverBiCgStab(std::string name, std::string field_x,
                                       std::string field_b, int monitor_iter,
                                       int restart_cycle, int solve_type,
    ...
    : Solver(name, field_x, field_b, monitor_iter, restart_cycle, solve_type,
             index_prolong, index_restrict, min_level, max_level),
      A_(nullptr),
      res_tol_(res_tol),
...
void EnzoSolverBiCgStab::apply(std::shared_ptr<Matrix> A,
                               Block* block) throw() {
  TRACE_BCG(block, this, "apply");

#ifdef TRACE_CYCLE
  if (block->cycle() >= CYCLE_EXIT) CkExit();
#endif
...
void EnzoSolverBiCgStab::compute_(EnzoBlock* block) throw() {
  ...
  if (is_finest_(block)) {
    ...
    if (reuse_x) {
      ...
      enzo_float* R = (enzo_float*)field.values(ir_);
      if (R_copy)
        for (int i = 0; i < m_; i++) R_copy[i] = R[i];
#endif
    }
  }
...
void EnzoSolverBiCgStab::start_2(EnzoBlock* block,
  ...
  if (solve_type_ != solve_tree && msg != NULL) {
    ...
    ASSERT1("EnzoSolverBiCgStab::start_2", "Expecting (data[0] = %Lg) == 3",
            data[0], (data[0] == 3));
    S(c) = data[1];
    S(bs) = data[2];
    S(xs) = data[3];
...
void EnzoSolverBiCgStab::loop_0a(EnzoBlock* block,
  ...
  if (solve_type_ != solve_tree && msg != NULL) {
    long double* data = (long double*)msg->getData();
    ASSERT1("EnzoSolverBiCgStab::loop_0a", "Expecting (data[0] = %Lg) == 3",
            data[0], (data[0] == 3));
    S(beta_n) = data[1];
    S(bnorm) = data[2];
    S(r0s) = data[3];
  ...
  enzo_float* X = (enzo_float*)field.values(ix_);
  if (X0_copy)
    for (int i = 0; i < m_; i++) X0_copy[i] = X[i];
#endif
...
void EnzoSolverBiCgStab::loop_0(EnzoBlock* block) throw() {
  ...
  if (iter == 0) {
    ...
    S(err) = sqrt(S(beta_n)) / S(rho0);
    S(err0) = S(err);
    S(err_min) = S(err);
    S(err_max) = S(err);
  } else {
    S(err) = sqrt(S(rr)) / S(rho0);
    S(err_min) = std::min(S(err), S(err_min));
    S(err_max) = std::max(S(err), S(err_max));
  }
  ...
  const bool l_output = ((a3[0] == 0 && a3[1] == 0 && a3[2] == 0) &&
                         (block->level() == coarse_level_) &&
                         ((iter == 0) || (is_converged || is_diverged) ||
                          (monitor_iter_ && (iter % monitor_iter_) == 0)));

  if (l_output) {
    if (reuse_x) {
      monitor_output_(block, iter, sqrt(S(bnorm)), S(err_min) * sqrt(S(bnorm)),
                      S(err) * sqrt(S(bnorm)), S(err_max) * sqrt(S(bnorm)));
    } else {
      monitor_output_(block, iter, S(err0), S(err_min), S(err), S(err_max));
    }
  }
...
#ifdef DEBUG_CALLBACK
    CkPrintf("DEBUG_CALLBACK %s:%d %d\n", __FILE__, __LINE__,
             CkIndex_EnzoBlock::p_solver_bicgstab_loop_2());
    fflush(stdout);
#endif
...
void EnzoSolverBiCgStab::loop_25(EnzoBlock* block) throw() {
  ...
  block->refresh_start(ir_loop_3_,
...
void EnzoSolverBiCgStab::loop_4(EnzoBlock* block) throw() {
  ...
  CkCallback callback = CkCallback(
      CkIndex_EnzoBlock::r_solver_bicgstab_loop_5(NULL), block->proxy_array());
...
void EnzoSolverBiCgStab::loop_6(EnzoBlock* block, CkReductionMsg* msg) throw() {
  TRACE_BCG(block, this, "loop_6");

  if (solve_type_ != solve_tree && msg != NULL) {
    long double* data = (long double*)msg->getData();
    ASSERT1("EnzoSolverBiCgStab::loop_6", "Expecting (data[0] = %Lg) == 3",
            data[0], (data[0] == 3));
    S(vr0) = data[1];
    S(ys) = data[2];
    S(vs) = data[3];
...
#ifdef DEBUG_CALLBACK
    CkPrintf("DEBUG_CALLBACK %s:%d %d\n", __FILE__, __LINE__,
             CkIndex_EnzoBlock::p_solver_bicgstab_loop_8());
    fflush(stdout);
#endif
...
void EnzoSolverBiCgStab::loop_85(EnzoBlock* block) throw() {
  ...
  block->refresh_start(ir_loop_9_,
...
void EnzoSolverBiCgStab::loop_10(EnzoBlock* block) throw() {
  ...
  CkPrintf("DEBUG_REDUCE %s %s:%d %Lg %Lg %Lg %Lg %Lg\n", block->name().c_str(),
           __FILE__, __LINE__, reduce[0], reduce[1], reduce[2], reduce[3],
           reduce[4]);
  fflush(stdout);
#endif

  CkCallback callback = CkCallback(
      CkIndex_EnzoBlock::r_solver_bicgstab_loop_11(NULL), block->proxy_array());
...
void EnzoSolverBiCgStab::loop_12(EnzoBlock* block,
  ...
  if (solve_type_ != solve_tree && msg != NULL) {
    long double* data = (long double*)msg->getData();
    ASSERT1("EnzoSolverBiCgStab::loop_12", "Expecting (data[0] = %Lg) == 5",
            data[0], (data[0] == 5));
    S(omega_n) = data[1];
    S(omega_d) = data[2];
    S(ys) = data[3];
  ...
  if (S(omega_n) == 0.0) {
    WARNING1("EnzoSolverBiCgStab::loop12()", "Solver error: %s omega_n == 0",
             block->name().c_str());
    this->end(block, return_error);
  }
  if (S(omega_d) == 0.0) {
    WARNING1("EnzoSolverBiCgStab::loop12()", "Solver error: %s omega_d == 0",
             block->name().c_str());
    this->end(block, return_error);
  }
  if (S(omega) == 0.0) {
    WARNING1("EnzoSolverBiCgStab::loop12()", "Solver error: %s omega_ == 0",
             block->name().c_str());
    this->end(block, return_error);
  }
  ...
  CkCallback callback = CkCallback(
      CkIndex_EnzoBlock::r_solver_bicgstab_loop_13(NULL), block->proxy_array());
...
void EnzoSolverBiCgStab::loop_14(EnzoBlock* block,
  ...
  if (solve_type_ != solve_tree && msg != NULL) {
    long double* data = (long double*)msg->getData();
    ASSERT1("EnzoSolverBiCgStab::loop_14", "Expecting (data[0] = %Lg) == 2",
            data[0], (data[0] == 2));
    S(rr) = data[1];
    S(beta_n) = data[2];
  }
  ...
  if (S(beta_n) == 0.0) {
    WARNING1("EnzoSolverBiCgStab::loop14()", "Solver error: %s beta_n == 0",
             block->name().c_str());
    this->end(block, return_error);
  }
  ...
  CkCallback callback = CkCallback(
      CkIndex_EnzoBlock::r_solver_bicgstab_loop_15(NULL), block->proxy_array());
...
void EnzoSolverBiCgStab::inner_product_(EnzoBlock* block, int n,
                                        long double* reduce,
                                        const std::vector<int>& is_array,
                                        CkCallback callback, int i_function) {
  if (solve_type_ == solve_tree) {
...
void EnzoSolverBiCgStab::dot_compute_tree_(EnzoBlock* block, int n,
...
void EnzoSolverBiCgStab::dot_send_parent_(EnzoBlock* block, int n,
                                          long double* dot_block,
                                          const std::vector<int>& is_array,
                                          int i_function, int iter) {
  TRACE_DOT(block, "dot_send_parent", i_function);
  ASSERT2("EnzoSolverBiCgStab::dot_send_parent()",
          "level %d must be > coarse_level = %d", block->level(), coarse_level_,
          (block->level() > coarse_level_));

  Index index_parent = block->index().index_parent(min_level_);

  enzo::block_array()[index_parent].p_dot_recv_parent(n, dot_block, is_array,
                                                      i_function, iter);
}
...
void EnzoBlock::p_dot_recv_parent(int n, long double* dot_block,
                                  std::vector<int> is_array, int i_function,
                                  int iter) {
...
void EnzoSolverBiCgStab::dot_recv_parent(EnzoBlock* block, int n,
                                         long double* dot_block,
                                         const std::vector<int>& is_array,
                                         int i_function, int iter) {
...
void EnzoSolverBiCgStab::dot_send_children_(EnzoBlock* block, int n,
                                            long double* dot_local,
                                            const std::vector<int>& is_array,
                                            int i_function) {
  ...
  while (it_child.next(ic3)) {
    ...
    enzo::block_array()[index_child].p_dot_recv_children(n, dot_local, is_array,
                                                         i_function);
  }
}
...
void EnzoSolverBiCgStab::dot_recv_children(EnzoBlock* block, int n,
                                           long double* dot_local,
                                           const std::vector<int>& is_array,
                                           int i_function) {
...
void EnzoSolverBiCgStab::dot_save_(EnzoBlock* block, int n, long double* data,
                                   const std::vector<int>& is_array) {
  TRACE_DOT(block, "dot_save", -1);
  Scalar<long double> scalar = block->data()->scalar_long_double();
...
void EnzoSolverBiCgStab::dot_load_(EnzoBlock* block, int n, long double* data,
                                   const std::vector<int>& is_array) {
...
void EnzoSolverBiCgStab::dot_clear_(EnzoBlock* block, int n,
                                    const std::vector<int>& is_array) {
  Scalar<long double> scalar = block->data()->scalar_long_double();
...
void EnzoSolverBiCgStab::dot_increment_(EnzoBlock* block, int n,
                                        const std::vector<int>& is_array,
                                        long double* dot_block) {
...
void EnzoSolverBiCgStab::dot_done_(EnzoBlock* block, int i_function,
                                   const char* file, int line) {
  TRACE_DOT(block, "dot_done", i_function);
  switch (i_function) {
    case bcg_start_2:
      start_2(block, nullptr);
      break;
    case bcg_loop_0a:
      loop_0a(block, nullptr);
      break;
    case bcg_loop_6:
      loop_6(block, nullptr);
      break;
    case bcg_loop_12:
      loop_12(block, nullptr);
      break;
    case bcg_loop_14:
      loop_14(block, nullptr);
      break;
    default:
      ERROR1("EnzoSolverBiCgStab::dot_done()", "Unknown i_function %d",
             i_function);
  }
}
...
void EnzoSolverBiCgStab::new_register_refresh_() {
  ...
  if (solve_type_ == solve_tree) refresh_post->set_root_level(coarse_level_);
  ...
  if (solve_type_ == solve_tree) refresh_loop_3->set_root_level(coarse_level_);
  ...
  if (solve_type_ == solve_tree) refresh_loop_9->set_root_level(coarse_level_);
