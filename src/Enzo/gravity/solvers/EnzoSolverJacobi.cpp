...
#define TRACE_JACOBI(BLOCK, SOLVER, METHOD)                                    \
  if (BLOCK->cycle() >= DEBUG_TRACE_CYCLE) {                                   \
    CkPrintf("%s:%d %s %s TRACE_JACOBI active %d %s\n", __FILE__, __LINE__,    \
             BLOCK->name().c_str(),                                            \
             (SOLVER ? SOLVER->name().c_str() : "Unknown"),                    \
...
#define DEBUG_FIELD(BLOCK, IX, NAME)                                           \
  {                                                                            \
    ...
    CkPrintf("%-8s %-10s %d DEBUG_FIELD ||%s|| = [%g] (%g)\n",                 \
             BLOCK->name().c_str(), this->name().c_str(), __LINE__, NAME, xx,  \
             yy);                                                              \
  }
#else
#define DEBUG_FIELD(BLOCK, IX, NAME) /* ... */
...
EnzoSolverJacobi::EnzoSolverJacobi(std::string name, std::string field_x,
...
void EnzoSolverJacobi::apply(std::shared_ptr<Matrix> A, Block* block) throw() {
  TRACE_JACOBI(block, this, "apply()");

  begin_(block);

  if (solve_type_ == solve_level && !is_finest_(block)) Solver::end_(block);
...
void EnzoSolverJacobi::do_refresh_(Block* block) {
  ...
  block->refresh_start(ir_smooth_,
