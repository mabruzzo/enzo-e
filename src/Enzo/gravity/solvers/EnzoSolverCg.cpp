...
EnzoSolverCg::EnzoSolverCg(std::string name, std::string field_x,
                           ...
                           int iter_max, double res_tol, int index_precon)
...
/// ==> refresh P for AP = MATVEC (A,P)
{
  ...
  EnzoSolverCg* solver = static_cast<EnzoSolverCg*>(this->solver());
...
void EnzoSolverCg::loop_0a(EnzoBlock* enzo_block, CkReductionMsg* msg) throw() {
  ...
  enzo_block->refresh_start(ir_matvec_,
...
void EnzoSolverCg::loop_0b(EnzoBlock* enzo_block, CkReductionMsg* msg) throw() {
...
void EnzoSolverCg::shift_1(EnzoBlock* enzo_block) throw() {
  ...
  enzo_block->contribute(sizeof(long double), &reduce, sum_long_double_type,
                         callback);
}
...
void EnzoSolverCg::loop_2a(EnzoBlock* enzo_block) throw() {
  ...
  enzo_block->refresh_start(ir_loop_2_,
...
//  rz = rz2;
{
  ...
  enzo_block->contribute(sizeof(int), &iter, CkReduction::max_int, callback);
}
