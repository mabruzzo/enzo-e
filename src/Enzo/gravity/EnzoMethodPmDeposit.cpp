...
EnzoMethodPmDeposit::EnzoMethodPmDeposit(ParameterGroup p)
  ...
  for (int ipt = 0; ipt < num_is_grav; ipt++) {
    ...
    ASSERT1("EnzoMethodPmDeposit::EnzoMethodPmDeposit",
            ...
            particle_descr->type_name(it).c_str(), num_mass == 1);
  }
...
void deposit_particles_(const CelloView<enzo_float, 3>& density_particle_arr,
                        ...
                        int mx, int my, int mz, int gx, int gy, int gz) {
    ...
    for (int ib = 0; ib < particle.num_batches(it); ib++) {
      ...
      if (rank == 1) {
        ...
        for (int ip = 0; ip < np; ip++) {
          ...
          // If mass is a constant, then dm is 0, pmass[ip * dm] is pmass[0],
          // which just dereferences pmass.
...
/// @param[in]      gx,gy,gz Specifies the number of cells in the ghost zone
///     for each dimensions
void deposit_gas_(const CelloView<enzo_float, 3>& density_tot_arr, Field& field,
                  enzo_float dt_div_cosmoa, enzo_float hx_prop,
                  enzo_float hy_prop, enzo_float hz_prop, int mx, int my,
                  int mz, int gx, int gy, int gz) {
  int rank = cello::rank();
  ...
  (de, deposited_gas_density.data(), temp.data(), vxf, vyf, vzf, &dt_div_cosmoa,
   rfield.data(), &rank, &hx_prop, &hy_prop, &hz_prop, &mx, &my, &mz,
   &src_starti_x, &src_starti_y, &src_starti_z, &src_endi_x, &src_endi_y,
   &src_endi_z, &offset_val, &offset_val, &offset_val, &nx, &ny, &nz,
...
void EnzoMethodPmDeposit::compute(Block* block) throw() {
  ...
  if (block->is_leaf()) {
    ...
    // add mass from gas
    {
      ...
      deposit_gas_(density_tot_arr, field, gas_dt_div_cosmoa, hx * cosmo_a,
                   hy * cosmo_a, hz * cosmo_a, mx, my, mz, gx, gy, gz);
    }
  }
