...
struct BlockInfo {
  ...
  BlockInfo(Block* block) {
    ...
    field.ghost_depth(0, &(ghost_depth[0]), &(ghost_depth[1]),
                      &(ghost_depth[2]));
    block->data()->lower(&(lower_edge[0]), &(lower_edge[1]), &(lower_edge[2]));
    double xp, yp, zp;
    block->data()->upper(&xp, &yp, &zp);
    field.cell_width(lower_edge[0], xp, &(cell_width[0]), lower_edge[1], yp,
                     &(cell_width[1]), lower_edge[2], zp, &(cell_width[2]));
  }
...
public:
  GalaxyModel(const EnzoPotentialConfigGalaxy& pack_dfltU, const Units* units) {
    ...
    DM_density_ = (pack_codeU_.DM_mass /
                   (4.0 * cello::pi * std::pow(pack_codeU_.rcore, 3))) /
                  (std::log(1.0 + xtemp) - xtemp / (1.0 + xtemp));
  }

  /// compute the x,y,z components of the acceleration
  std::array<double, 3> accel(double G_code, double cosmo_a, double x, double y,
                              double z) const noexcept {
    double zheight =
        (pack_codeU_.amom_uvec[0] * x + pack_codeU_.amom_uvec[1] * y +
    ...
    double radius = sqrt(xplane * xplane + yplane * yplane + zplane * zplane +
                         zheight * zheight);
    ...
    double accel_R =
        ...
        sqrt(pow(pow(rcyl, 2) + pow(pack_codeU_.stellar_r +
                                        sqrt(pow(zheight, 2) +
    ...
    double accel_z =
        ...
        sqrt(pow(zheight, 2) + pow(pack_codeU_.stellar_z, 2)) * zheight /
        sqrt(pow(pow(rcyl, 2) + pow(pack_codeU_.stellar_r +
                                        sqrt(pow(zheight, 2) +
                                             pow(pack_codeU_.stellar_z, 2)),
                                    2),
    ...
    accel_sph =
        (radius == 0.0) ? 0.0 : std::fabs(accel_sph) / (radius * cosmo_a);
    accel_R = (rcyl == 0.0) ? 0.0 : std::fabs(accel_R) / (rcyl * cosmo_a);
    accel_z = (zheight == 0.0)
                  ? 0.0
                  : std::fabs(accel_z) * zheight / std::fabs(zheight) / cosmo_a;

    return {
        accel_sph * x + accel_R * xplane + accel_z * pack_codeU_.amom_uvec[0],
...
public:
  PointMassModel(const EnzoPotentialConfigPointMass& pack_dfltU,
    ...
    min_accel_ =
        pack_codeU_.mass / ((rcore_tmp * rcore_tmp * rcore_tmp) * cosmo_a);
  }
...
template <typename T>
void compute_accel_(const T functor, enzo_float* ax, enzo_float* ay,
                    enzo_float* az, double G_code, Particle* particle,
...
void EnzoMethodBackgroundAcceleration::compute_(Block* block) throw() {
  ...
  if (galaxy_pack_dfltU_ != nullptr) {
    double G_code = enzo::grav_constant_cgs() * units->density() *
    ...
    const PointMassModel functor(*point_mass_pack_dfltU_, units, cosmo_a,
                                 block_info.cell_width);
      ...
      for (int iy = gy; iy < my - gy; iy++) {
        for (int ix = gx; ix < mx - gx; ix++) {
          ...
          dt = std::min(enzo_float(dt),
                        enzo_float(sqrt(hx / (fabs(ax[i] + 1e-20)))));
        }
      }
    }
  }
    ...
    for (int iz = gz; iz < mz - gz; iz++) {
      for (int iy = gy; iy < my - gy; iy++) {
        for (int ix = gx; ix < mx - gx; ix++) {
          int i = ix + mx * (iy + iz * my);
          dt = std::min(enzo_float(dt),
                        enzo_float(sqrt(hy / (fabs(ay[i] + 1e-20)))));
        }
      }
    }
  }
    ...
    for (int iz = gz; iz < mz - gz; iz++) {
      for (int iy = gy; iy < my - gy; iy++) {
        for (int ix = gx; ix < mx - gx; ix++) {
          int i = ix + mx * (iy + iz * my);
          dt = std::min(enzo_float(dt),
                        enzo_float(sqrt(hz / (fabs(az[i] + 1e-20)))));
        }
      }
    }
  }
