...
EnzoComputeAcceleration::EnzoComputeAcceleration(int rank, int order)
    : Compute(), rank_(rank), order_(order) {
  ...
  if (order_ != 2 && order_ != 4) {
    ERROR1("EnzoComputeAcceleration", "Unknown order %d", order_);
  }
}
...
void EnzoComputeAcceleration::compute(Block* block) throw() { compute_(block); }
...
void EnzoComputeAcceleration::compute_(Block* block) {
    ...
    if (rank_ == 1) {
      ...
      for (int iy = 2; iy < my - 2; iy++) {
        for (int ix = 2; ix < mx - 2; ix++) {
          int i = ix + mx * iy;
          ax[i] =
              fx * (-p[i + dx2] + 8 * p[i + dx] - 8 * p[i - dx] + p[i - dx2]);
          ay[i] =
              fy * (-p[i + dy2] + 8 * p[i + dy] - 8 * p[i - dy] + p[i - dy2]);
        }
      }
        ...
        for (int iy = 2; iy < my - 2; iy++) {
          for (int ix = 2; ix < mx - 2; ix++) {
            ...
            ay[i] =
                fy * (-p[i + dy2] + 8 * p[i + dy] - 8 * p[i - dy] + p[i - dy2]);
            az[i] =
                fz * (-p[i + dz2] + 8 * p[i + dz] - 8 * p[i - dz] + p[i - dz2]);
          }
        }
      }
    }
  ...
  for (int ipt = 0; ipt < num_is_grav; ipt++) {
    ...
    if (particle.num_particles(it) > 0) {
      ...
      //  double dt_shift = 0.0;
      if (rank_ >= 1) {
        EnzoComputeCicInterp interp_x("acceleration_x", particle_type, "ax",
                                      dt_shift);
        interp_x.compute(block);
      }
      if (rank_ >= 2) {
        EnzoComputeCicInterp interp_y("acceleration_y", particle_type, "ay",
                                      dt_shift);
        interp_y.compute(block);
      }
      if (rank_ >= 3) {
        EnzoComputeCicInterp interp_z("acceleration_z", particle_type, "az",
                                      dt_shift);
