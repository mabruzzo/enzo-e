...
EnzoInitialHdf5::EnzoInitialHdf5(int cycle, double time, int max_level,
                                 std::string format, const int blocking[3],
                                 int monitor_iter_,
  ...
  if (format == "music") {
    ...
    for (int i = 0; i < particle_coords.size(); i++)
      particle_coords_[i] = "tzyx";

  } else {
    ERROR1("EnzoInitialHdf5::EnzoInitialHdf5()", "Unsupported format '%s'",
           format.c_str());
  }
...
void EnzoInitialHdf5::enforce_block(Block* block,
                                    const Hierarchy* hierarchy_unused) throw() {
  ...
  for (size_t index = 0; index < field_files_.size(); index++) {
    ...
    field_loader.open_file(field_files_[index], field_names_[index],
                           field_coords_[index], field_datasets_[index]);
    load_data(count_messages[level], block, level, min_level, field_loader);
  }
  ...
  for (size_t index = 0; index < particle_files_.size(); index++) {
    ...
    particle_loader.open_file(particle_files_[index], particle_types_[index],
                              particle_attributes_[index],
                              particle_coords_[index],
                              particle_datasets_[index]);
    load_data(count_messages[level], block, level, min_level, particle_loader);
  }

  // Update all blocks in range of this reader with the number of messages sent
  // to them.
  int lower[3], upper[3];
    ...
    for (int ax = lower[0]; ax < upper[0]; ax++) {
      for (int ay = lower[1]; ay < upper[1]; ay++) {
        for (int az = lower[2]; az < upper[2]; az++) {
          Index index_block =
              block->index_from_global(ax, ay, az, level, min_level);
...
void EnzoInitialHdf5::load_data(int& count_messages, Block* block, int level,
                                int min_level, DataLoader& loader) {
...
void EnzoInitialHdf5::get_reader_range(Index reader_index, int lower[3],
                                       int upper[3], int level) throw() {
  // Get the lower and upper values defining the region of parent blocks at
  // level-1 which refine to create the blocks on the given level.
...
void EnzoInitialHdf5::recv_data(Block* block, MsgInitial* msg_initial) {
  ...
  if (monitor_iter_ &&
    ...
    cello::monitor()->print("Initial", "hdf5 %d / %d", count_monitor_out,
                            blocking);
    count_monitor_out++;
  }
  count_monitor++;
...
int EnzoInitialHdf5::is_reader_(Index index) {
  ...
  return ((level == 0) && (a3[0] % blocking_[0] == 0) &&
          (a3[1] % blocking_[1] == 0) && (a3[2] % blocking_[2] == 0));
}

//----------------------------------------------------------------------

void EnzoInitialHdf5::root_block_range_(Index index, int array_lower[3],
                                        int array_upper[3]) {
  // Get array-of-octrees blocking
  int root_blocks[3];
  cello::hierarchy()->root_blocks(root_blocks, root_blocks + 1,
                                  root_blocks + 2);
  ...
  for (int it = 0; it < particle.num_types(); it++) {
    if (particle.has_constant(it, "root_level_mass")) {
      ...
      if (type_array == type_single) {
        ...
        initialize_particle_mass(array_float, particle, it, ia,
                                 ((float)*root_mass) / divisor);
      } else if (type_array == type_double) {
        double* root_mass = (double*)particle.constant_value(it, ic);
        initialize_particle_mass(array_double, particle, it, ia,
                                 ((double)*root_mass) / divisor);
      }
    }
  }
}
...
template <class T>
void EnzoInitialHdf5::initialize_particle_mass(T* array, Particle particle,
                                               int it, int ia, T mass) {
...
DataLoader::DataLoader(Block* block, std::string format) : block(block), m4() {
  ...
  // field.dimensions (0,&mx,&my,&mz); // (index_field,&mx,&my,&mz); int
  // index_field = field.field_id(name);
  field.size(&nx, &ny, &nz);
...
void DataLoader::open_file(std::string filename, std::string dataset,
                           std::string coordinates) {
  file = new FileHdf5("./", filename);
  file->file_open();

  if (CHECK_COSMO_PARAMS) check_cosmology_(file);

  type_data = type_unknown;
  file->data_open(dataset, &type_data, m4, m4 + 1, m4 + 2, m4 + 3);

  ASSERT1("EnzoInitialHdf5::enforce_block()", "Unsupported type_data %d",
          type_data,
          ((type_data == type_single) || (type_data == type_double)));
...
void DataLoader::load(int* block_index, Index index_block) {
  ...
  (index_block == block->index()) ? copy_data_local(data)
                                  : copy_data_remote(index_block, data);
  delete_array_(&data, type_data);
}

void DataLoader::read_dataset_(char** data, Index index_block,
                               int block_index[3]) {
  ...
  ASSERT3("EnzoInitialHdf5::enforce_block()", "bad field coordinates %d %d %d",
          IX, IY, IZ,
          ((IX < 4) && (IY < 4) && (IZ < 4)) &&
              ((IX != IY) || (IY == -1 && IZ == -1)) &&
              ((IX != IY && IY != IZ) || (IZ == -1)));
  ...
  // open the dataspace
  file->data_slice(m4[0], m4[1], m4[2], m4[3], n4[0], n4[1], n4[2], n4[3],
                   o4[0], o4[1], o4[2], o4[3]);
...
void DataLoader::delete_array_(char** array, int type_data) {
  if (type_data == type_single) {
    ...
    ERROR1("DataLoader::delete_array_()", "Unsupported data type %d",
           type_data);
  }
  (*array) = nullptr;
}
...
char* DataLoader::allocate_array_(int n, int type_data) {
  ...
  if (type_data == type_single) {
    ...
    data = nullptr;
    ERROR1("DataLoader::allocate_array_()", "Unsupported data type %d",
           type_data);
  }
  return data;
}
...
void FieldLoader::copy_data_local(char* data) { copy_dataset_to_field_(data); }
...
void ParticleLoader::read_msg(MsgInitial* msg_initial, char** data) {
  msg_initial->get_dataset(n4, h4, &nx, &ny, &nz, &IX, &IY, &IZ);
  int data_size;
  msg_initial->get_particle_data(&type, &attribute, data, &data_size,
                                 &type_data);
}
...
void ParticleLoader::copy_data_remote(Index index_block, char* data) {
  MsgInitial* msg_initial = new MsgInitial;
  msg_initial->set_dataset(n4, h4, nx, ny, nz, IX, IY, IZ);
  msg_initial->set_particle_data(type, attribute, data, nx * ny * nz,
                                 type_data);
  enzo::block_array()[index_block].p_initial_hdf5_recv(msg_initial);
}
...
void ParticleLoader::copy_dataset_to_particle_(char* data) {
  ...
  if (type_array == type_single) {
    if (type_data == type_single) {
      copy_particle_data_to_array_(array_float, (float*)data, particle, it, ia,
                                   np);
    } else if (type_data == type_double) {
      copy_particle_data_to_array_(array_float, (double*)data, particle, it, ia,
                                   np);
    }
  } else if (type_array == type_double) {
    if (type_data == type_single) {
      copy_particle_data_to_array_(array_double, (float*)data, particle, it, ia,
                                   np);
    } else if (type_data == type_double) {
      copy_particle_data_to_array_(array_double, (double*)data, particle, it,
                                   ia, np);
    }
  } else {
    ERROR3("EnzoInitialHdf5::enforce_block()",
           ...
           cello::precision_name[type_array], particle.type_name(it).c_str(),
  ...
  if (l_particle_displacements_) {
    for (int axis = 0; axis < 3; axis++) {
      ...
      if (ia == particle.attribute_position(it, axis)) {
        if (type_array == type_single) {
          update_particle_displacements_(array_float, particle, it, ia,
                                         lower_block[axis], h4[IX], axis);
        } else {
          update_particle_displacements_(array_double, particle, it, ia,
                                         lower_block[axis], h4[IX], axis);
        }
      }
    }
...
void ParticleLoader::copy_particle_data_to_array_(T* array, S* data,
                                                  Particle particle, int it,
                                                  int ia, int np) {
  for (int ip = 0; ip < np; ip++) {
    int ib, io;
    particle.index(ip, &ib, &io);
...
template <class T>
void ParticleLoader::update_particle_displacements_(T* array, Particle particle,
                                                    int it, int ia,
                                                    double lower, double h,
                                                    int axis) {
