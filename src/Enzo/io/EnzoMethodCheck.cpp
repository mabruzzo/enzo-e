...
EnzoMethodCheck::EnzoMethodCheck(int num_files, std::string ordering,
                                 ...
                                 int monitor_iter, bool include_ghosts)
    : Method(),
...
// [ Called on ip=0 only ]
{
  ...
  if (already_exists) {
    ...
    ASSERT1("IoEnzoWriter", "Cannot open hierarchy file %s for writing",
...
IoEnzoWriter::IoEnzoWriter(int num_files, std::string ordering,
                           int monitor_iter, bool include_ghosts) throw()
    : CBase_IoEnzoWriter(),
      num_files_(num_files),
      ordering_(ordering),
...
void EnzoBlock::p_check_write_first(int num_files, std::string ordering,
  ...
  bool is_first(false);
  const int index_file =
      create_msg_check_(&msg_check, num_files, ordering, name_dir, &is_first);
...
void EnzoBlock::p_check_write_next(int num_files, std::string ordering) {
  ...
  std::string name_dir{""};
  EnzoMsgCheck* msg_check;
  const int index_file = create_msg_check_(&msg_check, num_files, ordering);
...
void IoEnzoWriter::p_write(EnzoMsgCheck* msg_check) {
  ...
  msg_check->get_parameters(index_this, index_next, name_this, name_next,
  ...
  if (is_first) {
    ...
    // Create block list
    stream_block_list_ =
        create_block_list_(name_dir, stream_block_list.str() + ".block_list");
...
std::ofstream IoEnzoWriter::create_block_list_(std::string name_dir,
  ...
  ASSERT1("Simulation::create_block_list_",
          "Cannot open block_list file %s for writing", name_file.c_str(),
          stream_block_list);
...
void IoEnzoWriter::close_block_list_() { stream_block_list_.flush(); }
...
int EnzoBlock::create_msg_check_(EnzoMsgCheck** msg_check, int num_files,
                                 std::string ordering, std::string name_dir,
                                 bool* is_first) {
  ScalarData<long long>* scalar_data_long_long =
      data()->scalar_data_long_long();
  ...
  const long long count =
      *scalar_data_long_long->value(scalar_descr_long_long, is_count);
  ...
  *msg_check = new EnzoMsgCheck;
  ...
  (*msg_check)
      ->set_parameters(index_this, index_next, name_this, name_next,
...
std::string Simulation::file_create_dir_(
  ...
  ASSERT("Simulation::file_create_dir_", "Directory name must be non-empty",
         directory_format[0] != "");

  std::string name_dir =
      cello::expand_name(&directory_format, counter, cycle, time);

  return name_dir;
}

//----------------------------------------------------------------------
FileHdf5* IoEnzoWriter::file_open_(std::string path_name,
...
void IoEnzoWriter::file_write_block_(EnzoMsgCheck* msg_check) {
  ...
  msg_check->get_parameters(index_block, index_next, name_block, name_next,
                            block_order, is_first, is_last, name_dir);
  ...
  file_->group_write_meta(msg_check->adapt_buffer_, "adapt_buffer", type_int,
                          ADAPT_BUFFER_SIZE);
  ...
  data = new Data(size[0], size[1], size[2], num_field_data, lower[0], lower[1],
                  lower[2], upper[0], upper[1], upper[2], cello::field_descr(),
  ...
  if (nh > 0) {
    for (int i_h = 0; i_h < nh; i_h++) {
      ...
      for (int i_f = 0; i_f < nf; i_f++) {
        ...
        io_field_data->field_array(&buffer, &name, &type, &mx, &my, &mz, &nx,
                                   &ny, &nz);
  ...
  for (int it = 0; it < particle.num_types(); it++) {
    ...
    for (int ia = 0; ia < na; ia++) {
      ...
      const std::string name = "particle_" + particle.type_name(it) + "_" +
      ...
      ASSERT2("OutputData::write_particle_data()",
              "Particle count mismatch %d particles %d written", np, i0,
              np == i0);
...
DataMsg* EnzoBlock::create_data_msg_() {
  ...
  FieldFace* field_face = create_face(if3, ic3, g3, refresh_same, refresh);
...
void IoEnzoWriter::write_meta_(FileHdf5* file, Io* io, std::string type_meta) {
  for (size_t i = 0; i < io->meta_count(); i++) {
    ...
    if (type_meta == "group") {
      ...
      ERROR1("MethodOutput::write_meta_()", "Unknown type_meta \"%s\"",
             type_meta.c_str());
    }
  }
}
