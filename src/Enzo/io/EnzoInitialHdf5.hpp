...
public:  // interface
  ...
  virtual void open_file(std::string filename, std::string dataset,
...
public:  // interface
  ...
  void open_file(std::string filename, std::string field_name,
                 std::string field_coords, std::string field_dataset) {
...
public:
  ...
  ParticleLoader(Block* block, bool particle_displacements, std::string format)
  ...
  void open_file(std::string filename, std::string particle_type,
                 std::string particle_attribute, std::string particle_coords,
...
private:  // methods
  ...
  template <class T>
  void update_particle_displacements_(T* array, Particle particle, int it,
                                      int ia, double lower, double h, int axis);
...
public:  // interface
  /// Constructor
  EnzoInitialHdf5(int cycle, double time, int max_level, std::string format,
                  const int blocking[3], int monitor_iter,
  ...
  EnzoInitialHdf5(CkMigrateMessage* m)
      : Initial(m), max_level_(0), i_sync_msg_(-1) {}
  ...
  void get_reader_range(Index reader_index, int lower[3], int upper[3],
                        int level) throw();

  // Use the provided DataLoader to load data onto blocks at the specified
  // level.
  void load_data(int& count_messages, Block* block, int level, int min_level,
                 DataLoader& loader);
  ...
  template <class T>
  void initialize_particle_mass(T* array, Particle particle, int it, int ia,
                                T mass);
