...
#define TRACE_SYNC(SYNC, MSG)                                                  \
  CkPrintf("TRACE_SYNC %p %s %d/%d\n", (void*)(&SYNC),                         \
           std::string(MSG).c_str(), SYNC.value(), SYNC.stop());               \
  fflush(stdout);
#else
#define TRACE_SYNC(SYNC, MSG) /* ... */
#endif
...
#define TRACE_BLOCK(MSG, BLOCK)                                                \
  {                                                                            \
    ...
    CkPrintf("%d :%d TRACE_RESTART BLOCK %s %s  %lld %lld %lld  %lld\n",       \
             CkMyPe(), __LINE__, BLOCK->name().c_str(),                        \
             std::string(MSG).c_str(), memory->bytes(), memory->bytes_high(),  \
...
#define TRACE_READER(MSG, READER)                                              \
  {                                                                            \
    ...
    CkPrintf("%d :%d TRACE_RESTART READER %s %d  %lld %lld %lld  %lld\n",      \
             CkMyPe(), __LINE__, std::string(MSG).c_str(), READER->thisIndex,  \
...
#define TRACE_SIMULATION(MSG, SIMULATION)                                      \
  {                                                                            \
    ...
    CkPrintf("%d :%d TRACE_RESTART SIMULATION %s %d  %lld %lld %lld  %lld\n",  \
             CkMyPe(), __LINE__, std::string(MSG).c_str(),                     \
             SIMULATION->thisIndex, memory->bytes(), memory->bytes_high(),     \
             memory->bytes_highest(),                                          \
             EnzoMsgCheck::counter[cello::index_static()]);                    \
    fflush(stdout);                                                            \
  }
...
void Simulation::p_restart_enter(std::string name_dir) {
  ...
  // Create new empty IoEnzoReader chare array and distribute to other
  // processing elements
...
void Simulation::r_restart_start(CkReductionMsg* msg) {
  ...
  for (int i = 0; i < restart_num_files_; i++) {
    ...
    proxy_io_enzo_reader[i].p_init_root(restart_directory_, restart_file,
                                        max_level);
  }
}
...
void IoEnzoReader::p_init_root(std::string name_dir, std::string name_file,
                               int max_level) {
...
void IoEnzoReader::p_block_ready() { block_ready_(); }
...
void EnzoBlock::p_restart_refine(int ic3[3], int io_reader, int ip) {
  ...
  factory->create_block(data_msg, thisProxy, index_child, nx, ny, nz,
                        num_field_data, adapt_step_, cycle_, time_, dt_, narray,
                        array, refresh_fine, 27,
                        &child_face_level_curr_.data()[27 * IC3(ic3)], &adapt_,
                        cello::simulation(), io_reader, ip);
...
void IoEnzoReader::p_block_created() { block_created_(); }
...
std::ifstream Simulation::file_open_file_list_(std::string name_dir) {
  ...
  ASSERT1("Simulation::file_copen_file_list_",
          "Cannot open hierarchy file %s for reading", name_file.c_str(),
          stream_file_list);
...
std::ifstream IoEnzoReader::stream_open_blocks_(std::string name_dir,
  ...
  ASSERT1("Simulation::create_block_list_",
          "Cannot open block_list file %s for reading", name_file_full.c_str(),
          stream_block_list);
...
void IoEnzoReader::file_read_block_(EnzoMsgCheck* msg_check,
  ...
  // Read block Adapt
  int type, size;
  file_->group_read_meta(msg_check->adapt_buffer_, "adapt_buffer", &type,
                         &size);
...
void IoEnzoReader::file_read_block_fields_(DataMsg* data_msg, int nx, int ny,
                                           int nz) {
  ...
  for (int i_f = 0; i_f < num_fields; i_f++) {
    ...
    file_->data_open(dataset_name, &type_data, m4, m4 + 1, m4 + 2, m4 + 3);
    int mx, my, mz;
    int gx, gy, gz;
    ...
    file_read_dataset_(buffer, type_data, mx, my, mz, m4);
  ...
  for (int it = 0; it < num_types; it++) {
    ...
    for (int ia = 0; ia < na; ia++) {
      ...
      if (type_data == type_single) {
        copy_buffer_to_particle_attribute_(buffer_single, particle, it, ia, np);
      } else if (type_data == type_double) {
        copy_buffer_to_particle_attribute_(buffer_double, particle, it, ia, np);
      } else if (type_data == type_int8) {
        copy_buffer_to_particle_attribute_(buffer_int8, particle, it, ia, np);
      } else if (type_data == type_int16) {
        copy_buffer_to_particle_attribute_(buffer_int16, particle, it, ia, np);
      } else if (type_data == type_int32) {
        copy_buffer_to_particle_attribute_(buffer_int32, particle, it, ia, np);
      } else if (type_data == type_int64) {
        copy_buffer_to_particle_attribute_(buffer_int64, particle, it, ia, np);
      } else {
        ERROR1("IoEnzoReader::file_read_block_()",
               "Unsupported particle type_data %d", type_data);
      }
      delete[] buffer;
    }
  }
...
template <class T>
void IoEnzoReader::copy_buffer_to_particle_attribute_(T* buffer,
...
void IoEnzoReader::read_meta_(FileHdf5* file, Io* io, std::string type_meta) {
  for (size_t i = 0; i < io->meta_count(); i++) {
    ...
    if (type_meta == "group") {
      ...
      ERROR1("MethodOutput::read_meta_()", "Unknown type_meta \"%s\"",
             type_meta.c_str());
    }
  }
}
...
//----------------------------------------------------------------------
void IoEnzoReader::file_open_block_list_(std::string path_name,
...
void IoEnzoReader::file_read_dataset_(char* buffer, int type_data, int nx,
                                      int ny, int nz, int m4[4]) {
  ...
  file_->data_slice(m4[0], m4[1], m4[2], m4[3], n4[0], n4[1], n4[2], n4[3],
                    o4[0], o4[1], o4[2], o4[3]);
