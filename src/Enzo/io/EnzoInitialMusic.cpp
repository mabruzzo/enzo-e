...
//----------------------------------------------------------------------
void mutex_init() { throttle_node_lock = CmiCreateLock(); }

//----------------------------------------------------------------------

EnzoInitialMusic::EnzoInitialMusic(int cycle, double time,
    ...
    : Initial(cycle, time),
      ...
      throttle_seconds_stagger_(
          enzo_config->initial_music_throttle_seconds_stagger),
      throttle_seconds_delay_(
          enzo_config->initial_music_throttle_seconds_delay) {}
...
void EnzoInitialMusic::enforce_block(Block* block,
  ...
  for (size_t index = 0; index < field_files_.size(); index++) {
    ...
    if (throttle_node_files_) {
      if (FileHdf5::file_list[file_name] == nullptr) {
        ...
        CkPrintf("%d %g DEBUG_THROTTLE opening %s\n", CkMyPe(),
                 cello::simulation()->timer(), file_name.c_str());
        fflush(stdout);
#endif
    ...
    ASSERT3("EnzoInitialMusic::enforce_block()",
            "bad field coordinates %d %d %d", IX, IY, IZ,
            ((IX < 4) && (IY < 4) && (IZ < 4)) &&
                ((IX != IY) || (IY == -1 && IZ == -1)) &&
                ((IX != IY && IY != IZ) || (IZ == -1)));
    ...
    file->data_open(field_datasets_[index], &type_data, m4, m4 + 1, m4 + 2,
                    m4 + 3);
    // compute cell widths
    double h4[4] = {1, 1, 1, 1};
    ...
    // create memory space
    file->mem_create(n4[IX], n4[IY], n4[IZ], n4[IX], n4[IY], n4[IZ], 0, 0, 0);
    ...
    if (type_data == type_single) {
      ...
      ERROR3("EnzoInitialMusic::enforce_block()",
             "Unsupported data type %d in file %s field %s", type_data,
             file_name.c_str(), field_datasets_[index].c_str());
    }
    ...
    if (type_data == type_single) {
      copy_field_data_to_array_(array, data_float, mx, my, mz, nx, ny, nz, gx,
                                gy, gz, n4, IX, IY);

    } else if (type_data == type_double) {
      CkPrintf("DEBUG_INITIAL_HDF5 %g\n",
               data_double[gx + mx * (gy + my * gz)]);
      copy_field_data_to_array_(array, data_double, mx, my, mz, nx, ny, nz, gx,
                                gy, gz, n4, IX, IY);
    }
    ...
    const bool do_close =
        (throttle_node_files_ && can_close) || (!throttle_node_files_);
  ...
  for (size_t index = 0; index < particle_files_.size(); index++) {
    ...
    int m4[4] = {0, 0, 0, 0};
    int type_data = type_unknown;
    file->data_open(particle_datasets_[index], &type_data, m4, m4 + 1, m4 + 2,
                    m4 + 3);
    ...
    if (type_array == type_single) {
      if (type_data == type_single) {
        copy_particle_data_to_array_(array_float, data_float, particle, it, ia,
                                     np);
      } else if (type_data == type_double) {
        copy_particle_data_to_array_(array_float, data_double, particle, it, ia,
                                     np);
      }
    } else if (type_array == type_double) {
      if (type_data == type_single) {
        copy_particle_data_to_array_(array_double, data_float, particle, it, ia,
                                     np);
      } else if (type_data == type_double) {
        copy_particle_data_to_array_(array_double, data_double, particle, it,
                                     ia, np);
      }
    } else {
      ERROR3("EnzoInitialMusic::enforce_block()",
...
void EnzoInitialMusic::throttle_stagger_() {
  if (throttle_internode_) {
    ...
    if ((throttle_seconds_stagger_ > 0.0) && count_threads < node_size) {
      ++count_threads;
      int ms = 1000 * ((CkMyPe() / node_size) % throttle_group_size_) *
               throttle_seconds_stagger_;
#ifdef DEBUG_THROTTLE
      CkPrintf("%d %g DEBUG_THROTTLE %d ms stagger start\n", CkMyPe(),
               cello::simulation()->timer(), ms);
      fflush(stdout);
#endif
...
template <class T>
void EnzoInitialMusic::copy_field_data_to_array_(enzo_float* array, T* data,
                                                 int mx, int my, int mz, int nx,
                                                 int ny, int nz, int gx, int gy,
                                                 int gz, int n4[4], int IX,
                                                 int IY) const {
  for (int iz = 0; iz < nz; iz++) {
    int jz = iz + gz;
    for (int iy = 0; iy < ny; iy++) {
...
void EnzoInitialMusic::copy_particle_data_to_array_(T* array, S* data,
