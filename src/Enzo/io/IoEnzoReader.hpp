...
public:  // interface
  ...
  void p_init_root(std::string name_dir, std::string name_file, int max_level);
...
protected:  // functions
  ...
  std::ifstream stream_open_blocks_(std::string name_dir,
                                    std::string name_file);
  void file_read_hierarchy_();
  void read_meta_(FileHdf5* file, Io* io, std::string type_meta);
  void file_read_dataset_(char* buffer, int type_data, int nx, int ny, int nz,
                          int m4[4]);

  template <class T>
  void copy_buffer_to_particle_attribute_(T* buffer, Particle particle, int it,
                                          int ia, int np);
