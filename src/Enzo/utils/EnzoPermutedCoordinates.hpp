...
class EnzoPermutedCoordinates {
  /// @class    EnzoPermutedCoordinates
  /// @ingroup  Enzo
  /// @brief    [\ref Enzo] Encapsulates operations related to the cyclic
  /// permutation of the Cartesian Axes
...
public:  // interface
  ...
  EnzoPermutedCoordinates(int i_axis) : i_axis_(i_axis) {
    ASSERT1("EnzoPermutedCoordinates",
            "Constructor arg must be 0, 1, or 2. Not %d.", i_axis,
            i_axis > -1 && i_axis < 3);
  }
  ...
  inline CelloView<T, 3> left_edge_offset(const CelloView<T, 3>& array,
                                          int kstart, int jstart,
                                          int istart) const noexcept {
...
private:
  // helper_function
  inline void unit_vector_comp_(int v_axis, int& v_k, int& v_j,
                                int& v_i) const {
    v_i = (v_axis == 0) ? 1 : 0;
    v_j = (v_axis == 1) ? 1 : 0;
    v_k = (v_axis == 2) ? 1 : 0;
