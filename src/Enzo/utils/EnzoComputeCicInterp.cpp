...
EnzoComputeCicInterp::EnzoComputeCicInterp(std::string field_name,
                                           std::string particle_type,
                                           std::string particle_attribute,
                                           double dt)
    : it_p_(cello::particle_descr()->type_index(particle_type)),
      ia_p_(
          cello::particle_descr()->attribute_index(it_p_, particle_attribute)),
...
void EnzoComputeCicInterp::compute_(Block* block) {
  ...
  if (rank == 1) {
    for (int ib = 0; ib < nb; ib++) {
      ...
      enzo_float* vxa =
          lshift ? (enzo_float*)particle.attribute_array(it_p_, ia_vx, ib)
                 : nullptr;
    ...
    for (int ib = 0; ib < nb; ib++) {
      ...
      enzo_float* vxa =
          lshift ? (enzo_float*)particle.attribute_array(it_p_, ia_vx, ib)
                 : nullptr;
      enzo_float* vya =
          lshift ? (enzo_float*)particle.attribute_array(it_p_, ia_vy, ib)
                 : nullptr;
    ...
    for (int ib = 0; ib < nb; ib++) {
      ...
      enzo_float* vya =
          lshift ? (enzo_float*)particle.attribute_array(it_p_, ia_vy, ib)
                 : nullptr;
      enzo_float* vza =
          lshift ? (enzo_float*)particle.attribute_array(it_p_, ia_vz, ib)
                 : nullptr;
