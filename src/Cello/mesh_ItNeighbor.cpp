...
ItNeighbor::ItNeighbor(Block* block, int min_face_rank, int periodic[3],
                       int n3[3], Index index, int neighbor_type, int min_level,
                       int root_level)
    : block_(block),
      rank_(cello::rank()),
...
bool ItNeighbor::is_reset() const { return (of3_[0] == -2); }

//----------------------------------------------------------------------

bool ItNeighbor::is_reset_child_() const { return (ic3_[0] == -2); }
...
bool ItNeighbor::valid_() {
  ...
  const bool in_range = (min_face_rank_ <= face_rank && face_rank < rank_);
