...
MsgState::MsgState()
    : CMessage_MsgState(), time_(0.0), dt_(0.0), cycle_(0), stop_(0) {
  ++counter[cello::index_static()];
}

//----------------------------------------------------------------------

MsgState::~MsgState() { --counter[cello::index_static()]; }
...
void* MsgState::pack(MsgState* msg) {
  ...
  ASSERT2("MsgState::pack()", "buffer size mismatch %ld allocated %d packed",
...
MsgState* MsgState::unpack(void* buffer) {
  ...
  MsgState* msg = (MsgState*)CkAllocBuffer(buffer, sizeof(MsgState));
