...
public:  // interface
  ...
  ObjectSphere(double center[3], double radius) throw()
      : Object(), radius_(radius) {
    center_[0] = center[0];
    center_[1] = center[1];
    center_[2] = center[2];
  };

  ObjectSphere() throw() : Object(){};

  /// Charm++ PUP::able declarations
  PUPable_decl(ObjectSphere);

  ObjectSphere(CkMigrateMessage* m) : Object(m), center_(), radius_(0) {}
  ...
  char* save_data(char* buffer) const {
    ...
    ASSERT2("ObjectSphere::save_data()",
            "Expecting buffer size %d actual size %d", data_size(),
            (pc - buffer), (data_size() == (pc - buffer)));
