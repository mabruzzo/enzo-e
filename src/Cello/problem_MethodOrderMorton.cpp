...
void Block::r_method_order_morton_continue(CkReductionMsg* msg) {
  delete msg;
  static_cast<MethodOrderMorton*>(this->method())->compute_continue(this);
}
...
void MethodOrderMorton::send_weight(Block* block, int weight_child, bool self) {
  ...
  if ((!self || block->is_leaf()) && level > min_level_) {
    ...
    Index index_next =
        block->index().next(rank, na3, block->is_leaf(), min_level_);
...
void Block::p_method_order_morton_weight(int ic3[3], int weight,
                                         Index index_child) {
  static_cast<MethodOrderMorton*>(this->method())
      ->recv_weight(this, ic3, weight, false);
}

//----------------------------------------------------------------------

void MethodOrderMorton::recv_weight(Block* block, int ic3[3], int weight,
                                    bool self) {
...
void MethodOrderMorton::send_index(Block* block, int index_parent, int count,
                                   bool self) {
  *pcount_(block) = count;
  if (!block->is_leaf()) {
    ...
    for (int ic = 0; ic < cello::num_children(); ic++) {
      ...
      cello::block_array()[index_child].p_method_order_morton_index(index,
                                                                    count);
...
void MethodOrderMorton::recv_index(Block* block, int index, int count,
                                   bool self) {
  {
    char buffer[80];
...
void Block::r_method_order_morton_complete(CkReductionMsg* msg) {
  delete msg;
  static_cast<MethodOrderMorton*>(this->method())->compute_complete(this);
}
