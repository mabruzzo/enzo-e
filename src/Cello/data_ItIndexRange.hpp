...
public:  // interface
  ...
  ItIndexRange(CkMigrateMessage* m)
      : ItIndex(m), index_(0), first_(0), last_(0) {}
  ...
  virtual void first() throw() { index_ = first_; }
  ...
  virtual int value() const throw() { return int(index_); }

  /// Return whether iterating is complete
  virtual bool done() const throw() { return (index_ > last_); }
