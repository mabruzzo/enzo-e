...
#define TRACE_FACE_FLUXES(MSG)                                                 \
  CkPrintf("%d TRACE_FACE_FLUXES %s\n", CkMyPe(), MSG);
#else
#define TRACE_FACE_FLUXES(MSG) /* ... */
#endif
...
FaceFluxes::FaceFluxes(Face face, int index_field, int nx, int ny, int nz,
  ...
  ASSERT3("FaceFluxes::FaceFluxes",
          "Block dimensions %d %d %d must be even or 1", nx_, ny_, nz_,
          ((nx_ == 1 || ((nx_ & 1) == 0)) && (ny_ == 1 || ((ny_ & 1) == 0)) &&
           (nz_ == 1 || ((nz_ & 1) == 0))));
}
...
void FaceFluxes::set_flux_array(std::vector<cello_float> array, int dx, int dy,
                                int dz) {
...
void FaceFluxes::coarsen(int cx, int cy, int cz, int rank) {
  ...
  if (rank == 2) {
    ...
    ERROR1("FaceFluxes::coarsen()", "Unsupported rank = %d", rank);
  }
...
void FaceFluxes::print(Block* block, std::string message) {
  ...
  CkPrintf("FaceFluxes::print %s %p %s %d %d %d %g %g %g\n",
           block->name().c_str(), (void*)fluxes_, message.c_str(), mx, my, mz,
           min, avg, max);
}
