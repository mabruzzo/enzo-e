...
public:  // functions
  ...
  /// Set alignment
  void set_alignment(int alignment) throw() { alignment_ = alignment; }

  /// Set padding
  void set_padding(int padding) throw() { padding_ = padding; }
  ...
  int num_history() const throw() { return history_; }
  ...
  int alignment() const throw() { return alignment_; }

  /// padding in bytes between fields in memory
  int padding() const throw() { return padding_; }
  ...
  void centering(int id_field, int* cx, int* cy = 0, int* cz = 0) const throw();

  /// return whether the field variable is centered in the cell
  bool is_centered(int id_field) const {
    return (centering_[id_field][0] == 0 && centering_[id_field][1] == 0 &&
  ...
  bool conserved(int id_field) const throw() {
  ...
  bool is_permanent(int id_field) const throw() {
    return ((0 <= id_field) && (id_field < num_permanent_));
  }

  /// Return the number of permanent fields
  int num_permanent() const throw() { return num_permanent_; }
  ...
  int num_temporary() const throw() { return num_temporary_; }

private:  // functions
  void copy_(const FieldDescr& field_descr) throw();

  int insert_(const std::string& name_field, bool is_permanent = true) throw();
