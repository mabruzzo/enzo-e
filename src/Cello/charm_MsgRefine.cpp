...
MsgRefine::MsgRefine(Index index, int nx, int ny, int nz, int num_field_blocks,
                     int num_adapt_steps, int cycle, double time, double dt,
                     int refresh_type, int num_face_level, int* face_level,
                     Adapt* adapt_parent, int restart_io_reader)
    : CMessage_MsgRefine(),
      is_local_(true),
      data_msg_(nullptr),
      time_(time),
  ...
  CkPrintf("%d %s:%d DEBUG_MSG_REFINE creating %p %p\n", CkMyPe(), __FILE__,
           __LINE__, (void*)this, (void*)adapt_parent);
#endif
...
#endif
  if (data_msg_) {
    WARNING("MsgRefine::set_data_msg()", "overwriting existing data_msg_");
    delete data_msg_;
  }
  data_msg_ = data_msg;
}
...
void* MsgRefine::pack(MsgRefine* msg) {
  ...
  CkPrintf("%d DEBUG_MSG_REFINE %s:%d msg->data_msg_ = %p\n", CkMyPe(),
           __FILE__, __LINE__, msg->data_msg_);
  fflush(stdout);
#endif
...
#ifdef DEBUG_MSG_REFINE
  CkPrintf(
      "%d %s:%d DEBUG_MSG_REFINE ENTER MsgRefine::pack() msg %p --> buffer "
      "%p\n",
  ...
  ASSERT2("MsgRefine::pack()", "buffer size mismatch %ld allocated %d packed",
...
MsgRefine* MsgRefine::unpack(void* buffer) {
  ...
  MsgRefine* msg = (MsgRefine*)CkAllocBuffer(buffer, sizeof(MsgRefine));
...
#ifdef DEBUG_MSG_REFINE
  //  CkPrintf ("%s:%d DEBUG_MSG_REFINE CkFreeMsg
