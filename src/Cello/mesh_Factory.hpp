...
public:  // interface
  Factory() throw() : PUP::able() { TRACE("Factory::Factory()"); }
  ...
  virtual Hierarchy* create_hierarchy(int refinement, int min_level,
  ...
  virtual CProxy_Block new_block_proxy(DataMsg* data_msg, int nbx, int nby,
                                       int nbz) const throw();

  /// Create a new CHARM++ Block array
  virtual void create_block_array(DataMsg* data_msg, CProxy_Block block_array,
                                  int nbx, int nby, int nbz, int nx, int ny,
                                  int nz, int num_field_blocks) const throw();
  ...
  virtual void create_subblock_array(DataMsg* data_msg,
                                     CProxy_Block block_array, int min_level,
                                     int nbx, int nby, int nbz, int nx, int ny,
                                     int nz, int num_field_blocks) const
      throw();

  /// Create a new Block
  virtual void create_block(DataMsg* data_msg, CProxy_Block block_array,
                            ...
                            Simulation* simulation = 0, int io_reader = -1,
