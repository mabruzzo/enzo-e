...
public:  // interface
  /// Constructor
  Stopping(int stop_cycle, double stop_time, double stop_seconds) throw()
  ...
  Stopping(CkMigrateMessage* m)
      : PUP::able(m), stop_cycle_(0), stop_time_(0), stop_seconds_(0) {}
  ...
  double stop_cycle() const throw() { return stop_cycle_; };

  /// Return stopping time
  double stop_time() const throw() { return stop_time_; };

  /// Return stopping seconds
  double stop_seconds() const throw() { return stop_seconds_; };

public:  // virtual methods
  /// Return whether the simulation is done
  virtual bool complete(int curr_cycle, double curr_time) const throw();
