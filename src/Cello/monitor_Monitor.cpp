...
void Monitor::header() const {
  ...
  print("", "BEGIN CELLO: %s %02d %02d:%02d:%02d", month[t->tm_mon], t->tm_mday,
...
int Monitor::is_active(const char* component) const throw() {
  if (mode_ == monitor_mode_none) return false;

  if (mode_ == monitor_mode_root && CkMyPe() != 0) return false;
...
void Monitor::write(FILE* fp, const char* component, const char* format,
                    ...) const {
...
void Monitor::verbose(FILE* fp, const char* component, const char* format,
                      ...) const {
...
void Monitor::write_(FILE* fp, const char* component,
  ...
  if (fp == stdout) {
    PARALLEL_PRINTF("%s %s %s %s%c", process, time, component, message,
                    newline);
  } else {
    fprintf(fp, "%s %s %s %s%c", process, time, component, message, newline);
  }
}

//----------------------------------------------------------------------

void Monitor::write_verbatim(FILE* fp, const char* component,
                             const char* message) const {
  if (is_active(component)) {
    // Get parallel process text
    ...
    if (fp == stdout) {
      PARALLEL_PRINTF("%s %s %s %s\n", buffer_process, buffer_time, component,
                      message);
    } else {
      fprintf(fp, "%s %s %s %s\n", buffer_process, buffer_time, component,
              message);
