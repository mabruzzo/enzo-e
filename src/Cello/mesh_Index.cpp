...
bool Index::operator==(const Index& index) const {
  return (v_[0] == index.v_[0] && v_[1] == index.v_[1] && v_[2] == index.v_[2]);
}

//----------------------------------------------------------------------

bool Index::operator!=(const Index& index) const { return !(*this == index); }
...
Index Index::index_parent(int min_level) const {
  ...
  ASSERT2("Index::index_parent", "level of parent %d is less than min_level %d",
          level - 1, min_level, (level > min_level));
...
Index Index::index_ancestor(int level_ancestor, int min_level) const {
  ...
  ASSERT2("Index::index_ancestor_level",
          "level %d is greater than ancestor level %d", level_begin,
          level_ancestor, (level_begin >= level_ancestor));
  ASSERT2("Index::index_ancestor_level",
          "ancestor level %d is less than min_level %d", level_ancestor,
          min_level, (level_ancestor >= min_level));
...
bool Index::is_on_boundary(const int if3[3], const int n3[3]) const {
  return is_on_boundary(0, if3[0], n3[0]) || is_on_boundary(1, if3[1], n3[1]) ||
...
void Index::child(int level, int* icx, int* icy, int* icz,
                  int min_level) const {
  if (level > 0) {
...
bool Index::is_root() const { return (v_[0] == 0 && v_[1] == 0 && v_[2] == 0); }
...
#ifdef DEBUG_INDEX
  CkPrintf(
      "DEBUG_INDEX [%d:%d) [%d:%d) [%d:%d) - [%d:%d) [%d:%d) [%d:%d)  (%d %d "
      "%d) : %d\n",
      la3[0], ra3[0], la3[1], ra3[1], la3[2], ra3[2], lb3[0], rb3[0], lb3[1],
      rb3[1], lb3[2], rb3[2], ip3[0], ip3[1], ip3[2], (a ? -1 : ic));
#endif
  return (a ? -1 : ic);
}
...
void Index::set_level(int level) {
  ...
  ASSERT3("Index::set_level", "%s set_level() failed: level %d != level() %d",
...
void Index::set_array(int ix, int iy, int iz) {
  // right-bits = array
  ASSERT4("Index::set_array", "Array size (%d %d %d) out of range (maximum %d)",
          ix, iy, iz, (1 << INDEX_BITS_ARRAY) - 1,
          ((ix >> INDEX_BITS_ARRAY) == 0 && (iy >> INDEX_BITS_ARRAY) == 0 &&
...
void Index::print(std::string msg, int level) const {
  ...
  CkPrintf(
      "INDEX_PRINT %s L%d A %d %d %d tree %X %X %X  [%d %d %d] - [%d %d %d]\n",
      msg.c_str(), this->level(), ia3[0], ia3[1], ia3[2], it3[0], it3[1],
      it3[2], il3[0], il3[1], il3[2], il3[0] + r, il3[1] + r, il3[2] + r);
}

//----------------------------------------------------------------------

void Index::print(const char* msg, int max_level, int rank, const int nb3[3],
                  bool no_nl, void* simulation) const {
...
void Index::print_(FILE* fp, const char* msg, int max_level, int rank,
                   const int nb3[3], bool no_nl) const {
  if (fp != NULL) {
...
void Index::write(int ip, const char* msg, int max_level, int rank,
