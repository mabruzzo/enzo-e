...
ParticleData::ParticleData()
    : attribute_array_(), attribute_align_(), particle_count_() {
...
bool ParticleData::operator==(const ParticleData& particle_data) throw() {
  return (attribute_array_ == particle_data.attribute_array_) &&
...
ParticleData::~ParticleData() { --counter[cello::index_static()]; }
//----------------------------------------------------------------------

char* ParticleData::attribute_array(ParticleDescr* particle_descr, int it,
                                    int ia, int ib) {
...
int ParticleData::num_particles(ParticleDescr* particle_descr, int it,
                                int ib) const {
...
int ParticleData::num_local_particles(ParticleDescr* particle_descr) const {
  const int nt = particle_descr->num_types();
  int np = 0;
  for (int it = 0; it < nt; it++) {
...
int ParticleData::num_local_particles(ParticleDescr* particle_descr,
                                      int it) const {
  int nb = num_batches(it);
  int np = 0;
  for (int ib = 0; ib < nb; ib++) {
...
int ParticleData::num_local_particles(ParticleDescr* particle_descr, int it,
                                      int ib) const {
  if (!(0 <= it && it < particle_descr->num_types())) return 0;
  if (!(0 <= ib && ib < num_batches(it))) return 0;
  ...
  for (int it = 0; it < particle_descr->num_types(); it++) {
    int ia_c = -1;
    int cd = -1;
    if (particle_descr->has_attribute(it, "is_local"))
      particle_descr->attribute_index(it, "is_local");
...
int ParticleData::insert_particles(ParticleDescr* particle_descr, int it,
                                   int np,
  ...
  while (np_left > 0) {
    ...
    if (ip_start == 0) {
      ASSERT1("ParticleData::insert_particles",
              "Trying to insert negative particles: ib_this = %d", ib_this,
              ib_this >= 0);
...
int ParticleData::delete_particles(ParticleDescr* particle_descr, int it,
                                   int ib, const bool* mask) {
  check_arrays_(particle_descr, __FILE__, __LINE__);
  ...
  for (int ip = 0; ip < np; ip++) {
    ...
    if ((mask == NULL) || mask[ip]) {
      ...
      for (int ia = 0; ia < na; ia++) {
        if (!interleaved) mp = particle_descr->attribute_bytes(it, ia);
...
void ParticleData::scatter(ParticleDescr* particle_descr, int it, int ib,
                           int np, const bool* mask, const int* index, int n,
                           ParticleData* particle_array[], const bool copy) {
  ...
  int ia_copy = -1;
  int d_copy = -1;
  if (particle_descr->has_attribute(it, "is_copy"))
    ia_copy = particle_descr->attribute_index(it, "is_copy");
      ...
      if (copy) {
        for (int k = 0; k < n; k++) {
          ...
          if (!been_copied[pd]) {
            ...
            is_copy = (int64_t*)pd->attribute_array(particle_descr, it, ia_copy,
                                                    ib_dst);
            ...
            for (int ia = 0; ia < na; ia++) {
              ...
              char* a_dst = pd->attribute_array(particle_descr, it, ia, ib_dst);
              for (int iy = 0; iy < ny; iy++) {
...
int ParticleData::gather(ParticleDescr* particle_descr, int it, int n,
  ...
  std::copy(particle_array, particle_array + n, particle_array_sorted.begin());
  std::sort(particle_array_sorted.data(), particle_array_sorted.data() + n);

  // count number of particles to insert
  int np = 0;
  for (int k = 0; k < n; k++) {
    // ... skipping duplicate ParticleData objects
    if (k > 0 && (particle_array_sorted[k] == particle_array_sorted[k - 1]))
      continue;
    ...
    for (int ib = 0; ib < nb; ib++) {
      ...
      for (int ip = 0; ip < np; ip++) {
        for (int ia = 0; ia < na; ia++) {
          if (!interleaved) mp = particle_descr->attribute_bytes(it, ia);
          int ny = particle_descr->attribute_bytes(it, ia);
          char* a_src = pd->attribute_array(particle_descr, it, ia, ib);
...
bool ParticleData::position(ParticleDescr* particle_descr, int it, int ib,
...
bool ParticleData::velocity(ParticleDescr* particle_descr, int it, int ib,
...
void ParticleData::position_update(ParticleDescr* particle_descr, int it,
                                   int ib, long double dx, long double dy,
                                   long double dz) {
  const int ia_x = particle_descr->attribute_position(it, 0);
  const int ia_y = particle_descr->attribute_position(it, 1);
  const int ia_z = particle_descr->attribute_position(it, 2);
...
void ParticleData::copy_attribute_float_(ParticleDescr* particle_descr,
                                         int type, int it, int ib, int ia,
                                         double* coord) {
...
void ParticleData::update_attribute_float_(ParticleDescr* particle_descr,
                                           int type, int it, int ib, int ia,
                                           long double da) {
  const int dx = particle_descr->stride(it, ia);
...
void ParticleData::copy_position_int_(ParticleDescr* particle_descr, int type,
                                      int it, int ib, int ia, double* coord) {
  const int dx = particle_descr->stride(it, ia);
  const char* array = attribute_array(particle_descr, it, ia, ib);
  const int np = num_particles(particle_descr, it, ib);
  if (type == type_int8) {
    ...
    for (int ip = 0; ip < np; ip++)
      coord[ip] = double(1.0) * array_8[ip * dx] / PMAX_8;
  } else if (type == type_int16) {
    const int16_t* array_16 = (int16_t*)array;
    for (int ip = 0; ip < np; ip++)
      coord[ip] = double(1.0) * array_16[ip * dx] / PMAX_16;
  } else if (type == type_int32) {
    const int32_t* array_32 = (int32_t*)array;
    for (int ip = 0; ip < np; ip++)
      coord[ip] = double(1.0) * array_32[ip * dx] / PMAX_32;
  } else if (type == type_int64) {
    const int64_t* array_64 = (int64_t*)array;
    for (int ip = 0; ip < np; ip++)
      coord[ip] = double(1.0) * array_64[ip * dx] / PMAX_64;
  } else {
    ERROR1("ParticleData::copy_attribute_float_()",
           "Unknown particle attribute type %d", type);
  }
...
void ParticleData::update_position_int_(ParticleDescr* particle_descr, int type,
                                        int it, int ib, int ia, int64_t da) {
  const int dx = particle_descr->stride(it, ia);
  char* array = attribute_array(particle_descr, it, ia, ib);
  const int np = num_particles(particle_descr, it, ib);
  if (type == type_int8) {
...
char* ParticleData::save_data(ParticleDescr* particle_descr,
  ...
  ASSERT2("ParticleData::save_data()",
          "Buffer has size %ld but expecting size %d", (pc - buffer),
          data_size(particle_descr),
...
char* ParticleData::load_data(ParticleDescr* particle_descr, char* buffer) {
  // NOTE: integers stored first, then char's, to avoid alignment issues
  ...
  ASSERT1("ParticleData::load_data",
          "Trying to allocate negative particle types: nt = %d", nt, nt >= 0);
  ...
  for (int it = 0; it < nt; it++) {
    ...
    ASSERT1("ParticleData::load_data",
            "Trying to allocate negative particle batches: nb = %d", nb,
            nb >= 0);
    ...
    for (int ib = 0; ib < nb; ib++) {
      ...
      ASSERT1("ParticleData::load_data",
              "Trying to allocate negative particles: np = %d", np, np >= 0);
...
void ParticleData::debug(ParticleDescr* particle_descr) {
  ...
  for (int it = 0; it < nt; it++) {
    ...
    for (int ia = 0; ia < na; ia++)
      printf("particle %p:       %s\n", (void*)this,
             particle_descr->attribute_name(it, ia).c_str());
...
void ParticleData::write_ifrite(ParticleDescr* particle_descr, int it,
                                std::string file_name, double xm, double ym,
                                double zm, double xp, double yp, double zp) {
...
void ParticleData::resize_attribute_array_(ParticleDescr* particle_descr,
  ...
  if (attribute_array_[it][ib].size() != new_size) {
    ASSERT1("ParticleData::resize_attribute_array_",
            "Trying to allocate negative particles: new_size = %ld", new_size,
            new_size >= 0);
...
void ParticleData::check_arrays_(ParticleDescr* particle_descr,
  ...
  ASSERT4("ParticleData::check_arrays_",
          "%s:%d attribute_array_ is size %lu < %lu", file.c_str(), line,
          attribute_array_.size(), nt, attribute_array_.size() >= nt);
  ASSERT4("ParticleData::check_arrays_",
          "%s:%d particle_count_ is size %lu < %lu", file.c_str(), line,
          particle_count_.size(), nt, particle_count_.size() >= nt);
  ASSERT4("ParticleData::check_arrays_",
          "%s:%d attribute_align_ is size %lu < %lu", file.c_str(), line,
          attribute_align_.size(), nt, attribute_align_.size() >= nt);

  for (size_t it = 0; it < nt; it++) {
    size_t nb = num_batches(it);

    ASSERT5("ParticleData::check_arrays_",
            "%s:%d attribute_array_[%lu] is size %lu < %lu", file.c_str(), line,
            it, attribute_array_[it].size(), nb,
            attribute_array_[it].size() >= nb);
    ASSERT5("ParticleData::check_arrays_",
            "%s:%d particle_count_[%lu] is size %lu < %lu", file.c_str(), line,
            it, particle_count_[it].size(), nb,
            particle_count_[it].size() >= nb);
    ASSERT5("ParticleData::check_arrays_",
            "%s:%d attribute_align_[%lu] is size %lu < %lu", file.c_str(), line,
