...
void MsgRefresh::set_data_msg(DataMsg* data_msg) {
  if (data_msg_) {
    WARNING("MsgRefresh::set_data_msg()", "overwriting existing data_msg_");
    delete data_msg_;
  }
  data_msg_ = data_msg;
}
...
void* MsgRefresh::pack(MsgRefresh* msg) {
  ...
  ASSERT2("MsgRefresh::pack()", "buffer size mismatch %ld allocated %d packed",
...
MsgRefresh* MsgRefresh::unpack(void* buffer) {
  ...
  MsgRefresh* msg = (MsgRefresh*)CkAllocBuffer(buffer, sizeof(MsgRefresh));
