...
public:
  ...
  ViewCollec<T> operator()(
      const std::array<int, 3>& shape,
      const std::vector<std::pair<T, T>>& start_step_list) const noexcept {
...
void assert_allequal_array_elem(const ViewCollec<T>& a,
  ...
  ASSERT("assert_allequal_array_elem",
         "The size of the collections are not the same.", a.size() == b.size());
  for (std::size_t i = 0; i < a.size(); i++) {
    assert_allequal3D(a[i], b[i]);
  }
}
...
void common_checks_(const char* test_name, ViewCollec<T>& collec,
...

  {
    ...
    ASSERT1(test_name, "The items in the collection %s be contiguous.", phrase,
            collec.contiguous_items() == expect_contiguous);
  }
...
private:
  ...
  ViewCollec<double>* test_helper_(
      const char* f_name, ViewCollec<double>& collec,
      F& check_array_elem_values,
      const std::vector<CelloView<double, 3>>& alias_vec) const noexcept {
    std::size_t n_arrays = collec.size();
    std::array<int, 3> shape{collec.array_shape(0), collec.array_shape(1),
    ...
    auto all_arrays_are_aliases = [=, &alias_vec](ViewCollec<double>& collec) {
      ...
      for (std::size_t i = 0; i < n_arrays; i++) {
        if (!collec[i].is_alias(alias_vec[i])) {
          return false;
        }
      }
      return true;
    ...
    ASSERT(f_name,
           ("At least one array from the specified collection "
    ...
    // test the copy constructor
    {
      ...
      ASSERT(f_name,
             ("At least one array from the copy-constructed collection "
              "does not alias an array that it's supposed to"),
    ...
    common_checks_("InitializationTests::test_wrap_existing", *out_ptr, 2,
                   {5, 4, 3}, expect_contiguous);
    ASSERT(f_name,
           ("At least one array from the move-constructed collection "
            "does not alias an array that it's supposed to"),
  ...
  void test_wrap_existing() {
...

    {
      ...
      common_checks_("InitializationTests::test_wrap_existing", temp_collec, 2,
                     {5, 4, 3}, false);
    ...
    common_checks_("InitializationTests::test_wrap_existing", *collec_ptr, 2,
                   {5, 4, 3}, false);
  ...
  void test_new_alloc() {
...

    {
      ...
      // test properties of temp_collec
      common_checks_("InitializationTests::test_new_alloc", temp_collec, 2,
                     {5, 4, 3}, true);
    ...
    common_checks_("InitializationTests::test_new_alloc", *collec_ptr, 2,
                   {5, 4, 3}, true);
    check_array_elem_values(*collec_ptr);
  ...
  void test_with_factories() {
    for (bool use_single_arr : {false, true}) {
      ...
      ViewCollec<double> collec =
          factory({3, 4, 5}, {{1., 0.5}, {100., 2.}, {1000., 4.}});

      common_checks_("test_with_factories", collec, 3, {3, 4, 5},
                     use_single_arr);
    }
  }
...
class VariableAssignmentTests {
  ...
  void test_assignment_helper_(
    ...
    if (test_copy_assignment) {
      ...
      // move assignment
      const ViewCollec<double> src(
          main_factory({3, 4, 5}, {{1., 0.5}, {4., 2.}, {16., 4.}}));
      dest_collec = src;
    } else {
      test_name = "VariableAssignmentTests::test_assignment_helper_<false>";
      ViewCollec<double> src(
          main_factory({3, 4, 5}, {{1., 0.5}, {4., 2.}, {16., 4.}}));
  ...
  void test_assignment_(const ViewCollecFactory<double>& main_factory) {
    ...
    test_assignment_helper_<true>(main_factory, contig_factory, {1, 2, 3},
                                  {{-1., -0.5}, {-4., -2.}});
    test_assignment_helper_<true>(main_factory, wrapped_arr_factory, {3, 2, 1},
                                  {{-1., -0.5}, {-4., -2.}});

    // test move assignment
    test_assignment_helper_<false>(main_factory, contig_factory, {1, 2, 3},
                                   {{-1., -0.5}, {-4., -2.}});
    test_assignment_helper_<false>(main_factory, wrapped_arr_factory, {3, 2, 1},
                                   {{-1., -0.5}, {-4., -2.}});
  }
