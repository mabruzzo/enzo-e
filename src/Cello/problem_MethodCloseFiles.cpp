...
void MethodCloseFiles::compute(Block* block) throw() {
  ...
  const bool is_first_cycle =
      (block->cycle() == cello::config()->initial_cycle);
  if (is_first_cycle) {
    throttle_stagger_();
    CmiLock(MethodCloseFiles::node_lock);
    for (auto it = FileHdf5::file_list.begin(); it != FileHdf5::file_list.end();
         ++it) {
#ifdef DEBUG_THROTTLE
      CkPrintf("%d %g DEBUG_THROTTLE closed %s\n", CkMyPe(),
               cello::simulation()->timer(), it->first.c_str());
      fflush(stdout);
#endif
  ...
  ERROR("MethodCloseFiles::compute()",
        "\"close_files\" method can hang if CONFIG_SMP_MODE is not defined "
        "(smp=0)");
#endif
}
...
void MethodCloseFiles::throttle_stagger_() {
  if (seconds_stagger_ > 0.0) {
    ...
    if ((seconds_stagger_ > 0.0) && count_threads < node_size) {
      ++count_threads;
