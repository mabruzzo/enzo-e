...
void Adapt::initialize_self(Index index, int level_min, int level_now) {
...
void Adapt::update_neighbor(Index index, int level_min, int level_max,
                            bool can_coarsen) {
...
bool Adapt::update_bounds() {
  ...
  const bool changed = (self_.level_min_ != level_min) ||
...
bool Adapt::neighbors_converged() const {
  ...
  for (int i = 0; i < n; i++) {
    ...
    converged = converged && (neighbor.level_min_ == neighbor.level_max_);
  }
  return converged;
}
...
void Adapt::get_level_bounds(int* level_min, int* level_max,
...
void Adapt::get_neighbor_level_bounds(Index index, int* level_min,
                                      int* level_max, bool* can_coarsen) const {
...
void Adapt::print(std::string message, const Block* block, FILE* fp) const {
  if (fp != nullptr) {
    ...
    fprintf(fp, "\n");
    fprintf(fp, "%s periodicity %d %d %d\n", prefix.c_str(), periodicity_[0],
            periodicity_[1], periodicity_[2]);
    ...
    fprintf(fp, "\n");
    fprintf(fp, "%s (%d <= %d <= %d) C%d\n", prefix.c_str(), self_.level_min_,
            self_.level_now_, self_.level_max_, self_.can_coarsen_ ? 1 : 0);
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (block) {
        ...
        snprintf(neighbor_block, sizeof(neighbor_block), "%X:%X %X:%X,%X:%X",
                 ia3[0], it3[0], ia3[1], it3[1], ia3[2], it3[2]);
      }
      fprintf(fp, "%s   %d %s [%d %d] S%d C%d\n", prefix.c_str(), i,
              neighbor_block, info.level_min_, info.level_max_,
              info.is_sibling_ ? 1 : 0, info.can_coarsen_ ? 1 : 0);
    }
  }
}
