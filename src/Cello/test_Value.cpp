...
double expected2(double t, double x, double y, double z) noexcept {
  return (MASK2_VAL1 ? (EXPR2_VAL1)
                     : ((MASK2_VAL2 ? (EXPR2_VAL2) : (EXPR2_VAL3))));
}
...
void test_value_obj_(const Value& value, int num, std::string prefix = "") {
  ...
  double xv[nx], yv[ny], zv[nz];
  {
    ...
    Data data(nx, ny, nz, 1, xm, xp, ym, yp, zm, zp, &field_descr,
              &particle_descr);
  ...
  switch (num) {
    case 1:
      ...
      break;
    case 2:
      descr = "[expr,mask,expr,mask,expr]";
      expect_fn = &expected2;
      break;
    case 3:
      descr = "[expr,mask(png),expr]";
      expect_fn = &expected3;
      break;
    case 4:
      descr = "[float]";
      ...
      break;
    case 5:
      descr = "[float,mask,expr]";
      expect_fn = &expected5;
      break;
    default:
      ERROR("test_value_obj_", "the num argument must be 1-5");
  }
  ...
  for (int ix = 0; ix < nx; ix++) {
    for (int iy = 0; iy < ny; iy++) {
      for (int iz = 0; iz < nz; iz++) {
        ...
        l_equal =
            l_equal && (dvalues[i] == expect_fn(t, xv[ix], yv[iy], zv[iz]));
      }
    }
  }
  unit_assert(l_equal);
