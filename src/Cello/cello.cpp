...
const char* precision_name[8] = {"unknown",    "default",    "single",
                                 "double",     "extended64", "extended80",
                                 "extended96", "quadruple"};
...
const char* type_name[NUM_TYPES] = {
    ...
    "single",    "double", "extended64", "extended80", "extended96",
    "quadruple", "int8",   "int16",      "int32",      "int64"};
...
bool type_is_int(int type) {
  return (type == type_int8 || type == type_int16 || type == type_int32 ||
          type == type_int64);
}

bool type_is_float(int type) {
  return (type == type_single || type == type_double ||
          type == type_quadruple || type == type_extended64 ||
          type == type_extended80 || type == type_extended96);
}
bool type_is_valid(int type) {
  return (0 <= type && type < NUM_TYPES) && (type_bytes[type] > 0);
}
...
Simulation* simulation() { return proxy_simulation.ckLocalBranch(); }

//----------------------------------------------------------------------

const Factory* factory() { return simulation()->factory(); }
...
ScalarDescr* scalar_descr_double() {
  ...
  if (scalar_descr_double_[in] == nullptr) {
    scalar_descr_double_[in] =
        simulation() ? simulation()->scalar_descr_double() : new ScalarDescr;
  }
...
ScalarDescr* scalar_descr_long_double() {
  ...
  if (scalar_descr_long_double_[in] == nullptr) {
    scalar_descr_long_double_[in] =
        simulation() ? simulation()->scalar_descr_long_double()
                     : new ScalarDescr;
  }
...
ScalarDescr* scalar_descr_int() {
  ...
  if (scalar_descr_int_[in] == nullptr) {
    scalar_descr_int_[in] =
        simulation() ? simulation()->scalar_descr_int() : new ScalarDescr;
  }
...
ScalarDescr* scalar_descr_long_long() {
  ...
  if (scalar_descr_long_long_[in] == nullptr) {
    scalar_descr_long_long_[in] =
        simulation() ? simulation()->scalar_descr_long_long() : new ScalarDescr;
  }
...
ScalarDescr* scalar_descr_sync() {
  ...
  if (scalar_descr_sync_[in] == nullptr) {
    scalar_descr_sync_[in] =
        simulation() ? simulation()->scalar_descr_sync() : new ScalarDescr;
  }
...
ScalarDescr* scalar_descr_void() {
  ...
  if (scalar_descr_void_[in] == nullptr) {
    scalar_descr_void_[in] =
        simulation() ? simulation()->scalar_descr_void() : new ScalarDescr;
  }
...
ScalarDescr* scalar_descr_index() {
  ...
  if (scalar_descr_index_[in] == nullptr) {
    scalar_descr_index_[in] =
        simulation() ? simulation()->scalar_descr_index() : new ScalarDescr;
  }
...
CProxy_Block block_array() { return cello::hierarchy()->block_array(); }
...
Problem* problem() { return simulation() ? simulation()->problem() : nullptr; }
...
FieldDescr* field_descr() {
  ...
  if (field_descr_[in] == nullptr) {
    field_descr_[in] =
        simulation() ? simulation()->field_descr() : new FieldDescr;
  }
...
Grouping* field_groups() { return field_descr()->groups(); }
...
int define_field_in_group(std::string field_name, std::string group_name,
...
ParticleDescr* particle_descr() {
  ...
  if (particle_descr_[in] == nullptr) {
    particle_descr_[in] =
        simulation() ? simulation()->particle_descr() : new ParticleDescr;
  }
...
Grouping* particle_groups() { return particle_descr()->groups(); }
...
Units* units() { return problem() ? problem()->units() : nullptr; }
...
int rank() { return simulation() ? simulation()->rank() : 1; }

//----------------------------------------------------------------------

int num_children() { return 1 << rank(); }

//----------------------------------------------------------------------

int num_children(int r) { return (1 << r); }
...
std::string expand_name(const std::vector<std::string>* file_format,
  ...
  ASSERT1("cello::expand_name", "File name %s cannot contain '\\%%'",
          name.c_str(), name.find("\\%") == std::string::npos);
  ...
  for (size_t i = 0; i < file_format->size() - 1; i++) {
    ...
    if (arg == "cycle") {
      ...
      ERROR3("cello::expand_name",
             "Unknown file variable #%d '%s' for file '%s'", int(i),
...
bool ensure_directory_exists(const std::string& dir_name) {
  ...
  if (std::filesystem::is_directory(directory)) {
    ...
    ASSERT1("cello::ensure_directory_exists()", "Error creating directory %s",
...
std::string create_directory(const std::vector<std::string>* path_format,
                             int counter, int cycle, double time,
                             bool& already_exists) {
