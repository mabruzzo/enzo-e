...
void Block::stopping_enter_() { stopping_begin_(); }
...
void Block::stopping_begin_() {
  ...
  bool stopping_reduce =
      stopping_interval ? ((cycle_ % stopping_interval) == 0) : false;
  ...
  if (stopping_reduce || dt_ == 0.0) {
    ...
    fflush(stdout);
#endif
    contribute(2 * sizeof(double), min_reduce, CkReduction::min_double,
               callback);
...
void Block::r_stopping_compute_timestep(CkReductionMsg* msg) {
  ...
  bool turn_on =
      schedule_on ? schedule_on->write_this_cycle(cycle_, time_) : false;
  bool turn_off =
      schedule_off ? schedule_off->write_this_cycle(cycle_, time_) : false;

  static bool active = false;
  if (!active && turn_on) {
    active = true;
    simulation->monitor()->print("Performance",
    ...
    simulation->monitor()->print("Performance",
                                 "turning projections logging OFF\n");
...
void Block::stopping_balance_() {
  ...
  bool do_balance = (schedule && schedule->write_this_cycle(cycle_, time_));
