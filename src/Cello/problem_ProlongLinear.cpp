...
ProlongLinear::ProlongLinear() throw() : Prolong() {
  //  CkPrintf ("TRACE_PROLONG ProlongLinear()\n");
}

//----------------------------------------------------------------------

void ProlongLinear::apply(precision_type precision, void* values_f, int mf3[3],
                          int of3[3], int nf3[3], const void* values_c,
                          int mc3[3], int oc3[3], int nc3[3], bool accumulate) {
  TRACE6("ProlongLinear fine   %d:%d %d:%d %d:%d", of3[0], nf3[0] + of3[0],
         of3[1], nf3[1] + of3[1], of3[2], nf3[2] + of3[2]);

  TRACE6("ProlongLinear coarse %d:%d %d:%d %d:%d", oc3[0], nc3[0] + oc3[0],
         oc3[1], nc3[1] + oc3[1], oc3[2], nc3[2] + oc3[2]);
  ...
  switch (precision) {
    case precision_single:
      ...
      apply_((float*)values_f, mf3, of3, nf3, (const float*)values_c, mc3, oc3,
             nc3, accumulate);
    ...
    case precision_double:
      ...
      apply_((double*)values_f, mf3, of3, nf3, (const double*)values_c, mc3,
             oc3, nc3, accumulate);
    ...
    default:
      ...
      ERROR1("ProlongLinear::apply()", "Unknown precision %d", precision);
  }
}
...
template <class T>
void ProlongLinear::apply_(T* values_f, int mf3[3], int of3[3], int nf3[3],
                           const T* values_c, int mc3[3], int oc3[3],
                           int nc3[3], bool accumulate) {
  const int dcx = 1;
  const int dcy = mc3[0];
  ...
  for (int i = 0; i < rank; i++) {
    ...
    ASSERT3("ProlongLinear::apply_",
            "fine array %c-axis %d must be 2 times coarse axis %d", xyz[i],
            nf3[i], nc3[i], nf3[i] == 2 * nc3[i] || nf3[i] == 2 * (nc3[i] - 2));
  ...
  if (rank == 1) {
    ...
    for (int ifx = 0; ifx < nfx; ifx++) {
      ...
      temp_f[i_f] = wx0 * values_c[i_c] + wx1 * values_c[i_c + dcx];
    }
  ...
  if (accumulate == 1) {
    CkPrintf("TRACE_SUMS accum %d nc %d %d %d mn/avg/max %g %g %g\n",
             accumulate, nc3[0], nc3[1], nc3[2],
             //            oc3[0],oc3[1],oc3[2],
             //            mc3[0],mc3[1],mc3[2],
             cmin, cavg, cmax);
    CkPrintf("TRACE_SUMS accum %d nf %d %d %d mn/avg/max %g %g %g\n",
             accumulate, nf3[0], nf3[1], nf3[2],
             //            of3[0],of3[1],of3[2],
             //            mf3[0],mf3[1],mf3[2],
             fmin, favg, fmax);
  }
