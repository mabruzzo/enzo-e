...
void FluxData::allocate(int nx, int ny, int nz, std::vector<int> field_list,
                        bool single_array, std::vector<int>* cx_list,
                        std::vector<int>* cy_list, std::vector<int>* cz_list) {
  ...
  for (unsigned i_f = 0; i_f < nf; i_f++) {
    ...
    for (int axis = 0; axis < 3; axis++) {
      for (int face = 0; face < 2; face++) {
        ...
        block_fluxes_[i] = new FaceFluxes(Face(ix, iy, iz, axis, face),
                                          index_field, nx, ny, nz, cx, cy, cz);
...
char* FluxData::save_data(char* buffer) const {
  ...
  ASSERT2("FluxData::save_data()", "Buffer has size %ld but expecting size %d",
...
char* FluxData::load_data(char* buffer) {
  ...
  ASSERT2("FluxData::load_data()", "Buffer has size %ld but expecting size %d",
          (pc - buffer), data_size(), ((pc - buffer) == data_size()));
