...
void FileHdf5::file_open() throw() {
  ...
  ASSERT2("FileHdf5::file_open", "Return value %ld opening file %s", file_id_,
          file_name.c_str(), file_id_ >= 0);
...
void FileHdf5::file_create() throw() {
  ...
  ASSERT2("FileHdf5::file_create", "Return value %ld opening file %s", file_id_,
          file_name.c_str(), file_id_ >= 0);
...
void FileHdf5::file_close() throw() {
  ...
  ASSERT2("FileHdf5::file_close", "Return value %d closing file %s", retval,
          file_name.c_str(), (retval >= 0));
...
void FileHdf5::data_open(std::string name, int* type, int* m1, int* m2, int* m3,
                         int* m4) throw() {
...
void FileHdf5::data_slice(int m1, int m2, int m3, int m4, int n1, int n2,
                          int n3, int n4, int o1, int o2, int o3,
                          int o4) throw() {
  data_space_id_ = space_slice_(data_space_id_, m1, m2, m3, m4, n1, n2, n3, n4,
                                o1, o2, o3, o4);
}

//----------------------------------------------------------------------

void FileHdf5::mem_create(int mx, int my, int mz, int nx, int ny, int nz,
...
void FileHdf5::data_create(std::string name, int type, int m1, int m2, int m3,
                           int m4, int n1, int n2, int n3, int n4, int o1,
                           int o2, int o3, int o4) throw() {
  if (n1 == 0) n1 = m1;
  if (n2 == 0) n2 = m2;
  if (n3 == 0) n3 = m3;
  if (n4 == 0) n4 = m4;
  ...
  data_space_id_ =
      space_create_(m1, m2, m3, m4, n1, n2, n3, n4, o1, o2, o3, o4);

  // Create the new dataset

  data_id_ = H5Dcreate(group, name.c_str(), scalar_to_hdf5_(type),
                       data_space_id_, H5P_DEFAULT, data_prop_, H5P_DEFAULT);
#ifdef TRACE_DISK
...
void FileHdf5::data_read(void* buffer) throw() {
  ...
  fflush(stdout);
#endif
  int retval = H5Dread(data_id_, scalar_to_hdf5_(data_type_), mem_space_id_,
                       data_space_id_, H5P_DEFAULT, buffer);
...
void FileHdf5::data_write(const void* buffer) throw() {
  ...
  fflush(stdout);
#endif
  int retval = H5Dwrite(data_id_, scalar_to_hdf5_(data_type_), mem_space_id_,
                        data_space_id_, H5P_DEFAULT, buffer);

  // error check H5Dread

  ASSERT1("FileHdf5::data_write", "H5Dwrite() returned %d", retval,
          (retval >= 0));
}
...
void FileHdf5::file_read_scalar(void* buffer, std::string name,
                                int* type) throw() {
  std::string file_name = path_ + "/" + name_;
  ...
  ASSERT3("FileHdf5::file_read_meta",
          "H5Aopen_name() returned %ld opening %s in file %s", meta_id,
          name.c_str(), file_name.c_str(), (meta_id >= 0));
  ...
  fflush(stdout);
#endif
  int retval = H5Aread(meta_id, scalar_to_hdf5_(scalar_type), buffer);

  // error check H5Aread

  ASSERT1("FileHdf5::file_read_meta_", "H5Aread() returned %d", retval,
          (retval >= 0));
}

//----------------------------------------------------------------------

void FileHdf5::file_read_meta(void* buffer, std::string name, int* type,
...
void FileHdf5::data_read_meta(void* buffer, std::string name, int* type,
                              int* n1, int* n2, int* n3, int* n4) throw() {
  // error check file open
  std::string file_name = path_ + "/" + name_;
  ...
  ASSERT3("FileHdf5::data_read_meta",
          ...
          meta_id, name.c_str(), file_name.c_str(), (meta_id >= 0));
  ...
  ASSERT1("FileHdf5::data_read_meta", "H5Aread returned %d", retval,
          (retval >= 0));
}
...
void FileHdf5::group_chdir(std::string group_path) throw() {
  ...
  ASSERT1("FileHdf5::group_chdir", "Group name '%s' must begin with '/'",
          group_path.c_str(), group_path[0] == '/');
}
...
void FileHdf5::group_create() throw() {
  ...
  while (!done) {
    ...
    for (size_t i = 0; i < group_info.nlinks; i++) {
      ...
      H5Lget_name_by_idx(file_id_, group_full.c_str(), H5_INDEX_NAME,
                         H5_ITER_NATIVE, i, group_name, 80, H5P_DEFAULT);
      if (group == group_name) {
        group_exists = true;
        break;
      }
...
void FileHdf5::group_close() throw() {
  if (is_group_open_) {
    ...
    ASSERT2("FileHdf5::group_close", "Return value %d closing group %s", retval,
            group_name_.c_str(), (retval >= 0));
  }
  is_group_open_ = false;
}
...
void FileHdf5::group_read_meta(void* buffer, std::string name, int* type,
                               int* n1, int* n2, int* n3, int* n4) throw() {
  // error check file open
  std::string file_name = path_ + "/" + name_;
  ...
  ASSERT1("FileHdf5::group_read_meta", "H5Aread returned %d", retval,
          (retval >= 0));
}
...
void FileHdf5::write_meta_(hdf5_id type_id, const void* buffer,
                           std::string name, int type, int n1, int n2, int n3,
                           int n4) throw() {
  // error check file open
  std::string file_name = path_ + "/" + name_;

  ASSERT1("FileHdf5::write_meta_",
          "Trying to write metadata to the unopened file %s", file_name.c_str(),
          is_file_open_);
  ...
  hid_t meta_space_id =
      space_create_(n1, n2, n3, n4, n1, n2, n3, n4, 0, 0, 0, 0);
  ...
  fflush(stdout);
#endif
  hid_t meta_id = H5Acreate(type_id, name.c_str(), scalar_to_hdf5_(type),
                            meta_space_id, H5P_DEFAULT, H5P_DEFAULT);

  // error check H5Acreate

  ASSERT2("FileHdf5::write_meta_", "H5Acreate(%s) returned %ld", name.c_str(),
          meta_id, (meta_id >= 0));
  ...
  ASSERT1("FileHdf5::write_meta_", "H5Aclose() returned %d", retval,
          (retval >= 0));
}
...
hdf5_id FileHdf5::scalar_to_hdf5_(int type) const throw() {
  ...
  switch (type) {
    ...
    case type_extended96:
      ERROR1("FileHdf5::scalar_to_hdf5_", "type_%s not implemented",
             cello::type_name[type]);
      hdf5_type = 0;
      break;
    case type_single:
      hdf5_type =
          native ? H5T_NATIVE_FLOAT : (be ? H5T_IEEE_F32BE : H5T_IEEE_F32LE);
      break;
    case type_double:
      hdf5_type =
          native ? H5T_NATIVE_DOUBLE : (be ? H5T_IEEE_F64BE : H5T_IEEE_F64LE);
      break;
    case type_quadruple:
      if (native) {
    ...
    case type_short:
      hdf5_type =
          native ? H5T_NATIVE_SHORT : (be ? H5T_STD_I16BE : H5T_STD_I16LE);
      break;
    case type_int:
      hdf5_type =
          native ? H5T_NATIVE_INT : (be ? H5T_STD_I32BE : H5T_STD_I32LE);
      break;
    case type_long_long:
      hdf5_type =
          native ? H5T_NATIVE_LLONG : (be ? H5T_STD_I64BE : H5T_STD_I64LE);
      break;
    default:
...
int FileHdf5::hdf5_to_scalar_(hdf5_id hdf5_type) const throw() {
  ...
  if (hdf5_class == H5T_INTEGER) {
    ...
    ERROR2("FileHdf5::hdf5_to_scalar_", "Unknown type of class %d and size %d",
...
std::string FileHdf5::relative_to_absolute_(std::string path_relative,
                                            std::string path_absolute) const
    throw() {
...
void FileHdf5::get_extents_(hdf5_id data_space_id, int* n1, int* n2, int* n3,
                            int* n4) throw() {
  ...
  ASSERT1("FileHdf5::get_extents_", "rank %d is out of range", rank,
...
hdf5_id FileHdf5::space_create_(int m1, int m2, int m3, int m4, int n1, int n2,
                                int n3, int n4, int o1, int o2, int o3,
                                int o4) throw() {
  hsize_t dims[4] = {1};
  hsize_t count[4] = {1};
  ...
  ASSERT1("FileHdf5::space_create_", "rank %llu is out of range", rank,
          (1 <= rank && rank <= 4));
  ...
  if (rank == 1) {
    ...
    need_hyper = ((n1 != m1) || (o1 != 0) || (n2 != m2) || (o2 != 0));
  ...
  ASSERT1("FileHdf5::space_create_", "h5Screate_simple returned %ld", space_id,
          (space_id >= 0));
...
hdf5_id FileHdf5::space_slice_(hdf5_id space_id, int m1, int m2, int m3, int m4,
                               int n1, int n2, int n3, int n4, int o1, int o2,
                               int o3, int o4) throw() {
  ...
  hsize_t start[4] = {hsize_t(o1), hsize_t(o2), hsize_t(o3), hsize_t(o4)};
  hsize_t count[4] = {hsize_t(n1), hsize_t(n2), hsize_t(n3), hsize_t(n4)};
...
hdf5_id FileHdf5::open_dataset_(hdf5_id group, std::string name) throw() {
  ...
  ASSERT3("FileHdf5::open_dataset_",
          "H5Dopen() returned %ld opening %s in file %s", dataset_id,
          name.c_str(), (path_ + "/" + name_).c_str(), dataset_id >= 0);
...
hdf5_id FileHdf5::get_data_space_(hdf5_id data_id, std::string name) throw() {
  ...
  ASSERT3("FileHdf5::get_data_space_",
          "Dataset %s in file %s has unsupported rank %d", name.c_str(),
          (path_ + "/" + name_).c_str(), rank,
          (1 <= rank && rank <= MAX_DATA_RANK));
...
hdf5_id FileHdf5::get_attr_space_(hdf5_id attr_id, std::string name) throw() {
  ...
  ASSERT3("FileHdf5::get_attr_space_",
          "Attribute %s in file %s has unsupported rank %d", name.c_str(),
          (path_ + "/" + name_).c_str(), rank,
          (1 <= rank && rank <= MAX_DATA_RANK));
