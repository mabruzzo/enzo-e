...
public:  // functions
  ...
  OutputImage(int index, const Factory* factory, int process_count,
              const int root_size[3], const int root_blocks[3], int min_level,
              int max_level, int leaf_only, std::string image_type,
              int image_size[2], std::string image_reduce_type,
              std::string image_mesh_color, std::string image_mesh_order,
              std::string image_color_particle_attribute, double image_lower[],
              double image_upper[], int face_rank, int axis, bool image_log,
              bool image_abs, bool include_ghost, bool use_min_max,
...
private:  // functions
  ...
  void reduce_point_(double* data, int ix, int iy, double value,
                     double alpha = 1.0) throw();

  void reduce_line_(double* data, int ixm, int ixp, int iym, int iyp,
                    double value, double alpha = 1.0);
  void reduce_line_x_(double* data, int ixm, int ixp, int iy, double value,
                      double alpha = 1.0);
  void reduce_line_y_(double* data, int ix, int iym, int iyp, double value,
                      double alpha = 1.0);
