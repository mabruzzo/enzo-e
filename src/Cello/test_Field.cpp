...
bool check_view_props_(CelloView<T, 3> view, FieldT& field, int id, bool coarse,
                       bool include_ghosts, int history = 0) {
  int nx, ny, nz;
  T* ptr;
  ...
  bool matching_shape =
      ((view.shape(0) == nz) & (view.shape(1) == ny) & (view.shape(2) == nx));
...
void test_view_(Field& field, int id, std::string name = "",
  ...
  if (coarse) {
    ASSERT("test_view", "require history==0 and name==\"\" for coarse fields",
           (history == 0) && (name == ""));
    ...
    auto run_test = [&](ghost_choice gchoice, bool includes_ghost) {
      ...
      // load from id
      {
        ...
        passed &=
            check_view_props_(view, field, id, false, includes_ghost, history);
      }
      ...
      {
        const Field& const_field = field;
        CelloView<const T, 3> view = const_field.view<T>(id, gchoice, history);
...
PARALLEL_MAIN_BEGIN {
  ...
  //----------------------------------------------------------------------
  {
    ...
    unit_assert(field.values(i1) == field.values("f1"));
    unit_assert(field.values(i2) == field.values("f2"));
    unit_assert(field.values(i3) == field.values("f3"));
    unit_assert(field.values(i4) == field.values("f4"));
    unit_assert(field.values(i5) == field.values("f5"));
    ...
    size_t ng5 =
        sizeof(long double) * (g5[0] + m5[0] * (g5[1] + m5[1] * g5[2]));
    ...
    for (int iz = 0; iz < m1[2]; iz++) {
      for (int iy = 0; iy < m1[1]; iy++) {
        for (int ix = 0; ix < m1[0]; ix++) {
          ...
          v1[i] = (g1[0] <= ix && ix < m1[0] - g1[0] && g1[1] <= iy &&
                   iy < m1[1] - g1[1] && g1[2] <= iz && iz < m1[2] - g1[2])
                      ? 10
                      : -10;
        }
      }
    ...
    for (int iz = 0; iz < m2[2]; iz++) {
      for (int iy = 0; iy < m2[1]; iy++) {
        for (int ix = 0; ix < m2[0]; ix++) {
          ...
          v2[i] = (g2[0] <= ix && ix < m2[0] - g2[0] && g2[1] <= iy &&
                   iy < m2[1] - g2[1] && g2[2] <= iz && iz < m2[2] - g2[2])
                      ? 20
                      : -20;
        }
      }
    ...
    for (int iz = 0; iz < m3[2]; iz++) {
      for (int iy = 0; iy < m3[1]; iy++) {
        for (int ix = 0; ix < m3[0]; ix++) {
          ...
          v3[i] = (g3[0] <= ix && ix < m3[0] - g3[0] && g3[1] <= iy &&
                   iy < m3[1] - g3[1] && g3[2] <= iz && iz < m3[2] - g3[2])
                      ? 30
                      : -30;
        }
      }
    ...
    for (int iz = 0; iz < m4[2]; iz++) {
      for (int iy = 0; iy < m4[1]; iy++) {
        for (int ix = 0; ix < m4[0]; ix++) {
          ...
          v4[i] = (g4[0] <= ix && ix < m4[0] - g4[0] && g4[1] <= iy &&
                   iy < m4[1] - g4[1] && g4[2] <= iz && iz < m4[2] - g4[2])
                      ? 40
                      : -40;
        }
      }
    ...
    for (int iz = 0; iz < m5[2]; iz++) {
      for (int iy = 0; iy < m5[1]; iy++) {
        for (int ix = 0; ix < m5[0]; ix++) {
          ...
          v5[i] = (g5[0] <= ix && ix < m5[0] - g5[0] && g5[1] <= iy &&
                   iy < m5[1] - g5[1] && g5[2] <= iz && iz < m5[2] - g5[2])
                      ? 50
                      : -50;
        }
      }
...
#define HIST_INIT(ip, ih, i) (17.0 * (ip + 3) + 7.0 * (ih + 2) - 13.0 * (i + 7))
    ...
    for (int i = 0; i < M1; i++) {
      v1[i] = HIST_INIT(1, ih, i);
    }
    for (int i = 0; i < M2; i++) {
      v2[i] = HIST_INIT(2, ih, i);
    }
    for (int i = 0; i < M3; i++) {
      v3[i] = HIST_INIT(3, ih, i);
    }
    for (int i = 0; i < M4; i++) {
      v4[i] = HIST_INIT(4, ih, i);
    }
    for (int i = 0; i < M5; i++) {
      v5[i] = HIST_INIT(5, ih, i);
    }
    ...
    passed = true;
    for (int i = 0; i < M1; i++) {
      passed &= (v1[i] == HIST_INIT(1, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M2; i++) {
      passed &= (v2[i] == HIST_INIT(2, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M3; i++) {
      passed &= (v3[i] == HIST_INIT(3, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M4; i++) {
      passed &= (v4[i] == HIST_INIT(4, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M5; i++) {
      passed &= (v5[i] == HIST_INIT(5, ih, i));
    }
    unit_assert(passed);
    ...
    passed = true;
    for (int i = 0; i < M1; i++) {
      passed &= (v1h1[i] == HIST_INIT(1, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M2; i++) {
      passed &= (v2h1[i] == HIST_INIT(2, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M3; i++) {
      passed &= (v3h1[i] == HIST_INIT(3, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M4; i++) {
      passed &= (v4h1[i] == HIST_INIT(4, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M5; i++) {
      passed &= (v5h1[i] == HIST_INIT(5, ih, i));
    }
    unit_assert(passed);
    ...
    passed = true;
    for (int i = 0; i < M1; i++) {
      passed &= (v1h2[i] == HIST_INIT(1, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M2; i++) {
      passed &= (v2h2[i] == HIST_INIT(2, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M3; i++) {
      passed &= (v3h2[i] == HIST_INIT(3, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M4; i++) {
      passed &= (v4h2[i] == HIST_INIT(4, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M5; i++) {
      passed &= (v5h2[i] == HIST_INIT(5, ih, i));
    }
    unit_assert(passed);
    ...
    passed = true;
    for (int i = 0; i < M1; i++) {
      passed &= (v1h3[i] == HIST_INIT(1, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M2; i++) {
      passed &= (v2h3[i] == HIST_INIT(2, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M3; i++) {
      passed &= (v3h3[i] == HIST_INIT(3, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M4; i++) {
      passed &= (v4h3[i] == HIST_INIT(4, ih, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M5; i++) {
      passed &= (v5h3[i] == HIST_INIT(5, ih, i));
    }
    unit_assert(passed);
    ...
    for (int i = 0; i < M1; i++) {
      v1[i] = HIST_INIT(1, 0, i);
    }
    for (int i = 0; i < M2; i++) {
      v2[i] = HIST_INIT(2, 0, i);
    }
    for (int i = 0; i < M3; i++) {
      v3[i] = HIST_INIT(3, 0, i);
    }
    for (int i = 0; i < M4; i++) {
      v4[i] = HIST_INIT(4, 0, i);
    }
    for (int i = 0; i < M5; i++) {
      v5[i] = HIST_INIT(5, 0, i);
    }
    ...
    passed = true;
    for (int i = 0; i < M1; i++) {
      passed &= (v1[i] == s1 * HIST_INIT(1, 0, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M2; i++) {
      passed &= (v2[i] == s2 * HIST_INIT(2, 0, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M3; i++) {
      passed &= (v3[i] == s3 * HIST_INIT(3, 0, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M4; i++) {
      passed &= (v4[i] == s4 * HIST_INIT(4, 0, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M5; i++) {
      passed &= (v5[i] == s5 * HIST_INIT(5, 0, i));
    }
    unit_assert(passed);
    ...
    passed = true;
    for (int i = 0; i < M1; i++) {
      passed &= (v1[i] == HIST_INIT(1, 0, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M2; i++) {
      passed &= (v2[i] == HIST_INIT(2, 0, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M3; i++) {
      passed &= (v3[i] == HIST_INIT(3, 0, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M4; i++) {
      passed &= (v4[i] == HIST_INIT(4, 0, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M5; i++) {
      passed &= (v5[i] == HIST_INIT(5, 0, i));
    }
    unit_assert(passed);
    ...
    passed = true;
    for (int i = 0; i < M1; i++) {
      passed &= (v1[i] == r1 * HIST_INIT(1, 0, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M2; i++) {
      passed &= (v2[i] == r2 * HIST_INIT(2, 0, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M3; i++) {
      passed &= (v3[i] == r3 * HIST_INIT(3, 0, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M4; i++) {
      passed &= (v4[i] == r4 * HIST_INIT(4, 0, i));
    }
    unit_assert(passed);
    passed = true;
    for (int i = 0; i < M5; i++) {
      passed &= (v5[i] == r5 * HIST_INIT(5, 0, i));
    }
    unit_assert(passed);
