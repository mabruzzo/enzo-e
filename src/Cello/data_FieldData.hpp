...
public:  // interface
  ...
  FieldData(const FieldDescr* = NULL, int nx = 0, int ny = 1,
            int nz = 1) throw();
  ...
  void dimensions(const FieldDescr*, int id_field, int* mx, int* my = 0,
                  int* mz = 0) const throw();
  ...
  char* values(const FieldDescr*, int id_field, int history = 0) throw();
  ...
  /// Return array for the corresponding field, which may or may not
  /// contain ghosts depending on if they're allocated
  const char* values(const FieldDescr*, int id_field, int history = 0) const
      throw();
  const char* values(const FieldDescr* field_descr, std::string name,
  ...
  CelloView<const T, 3> view(const FieldDescr* field_descr, int id_field,
                             ghost_choice choice = ghost_choice::include,
                             int history = 0) const throw() {
    return const_cast<FieldData*>(this)->view<T>(field_descr, id_field, choice,
                                                 history);
  }
  ...
  char* coarse_values(const FieldDescr*, int id_field, int history = 0) throw();
  char* coarse_values(const FieldDescr* field_descr, std::string name,
                      int history = 0) throw() {
    return coarse_values(field_descr, field_descr->field_id(name), history);
  }

  /// Return array for the corresponding coarse field
  const char* coarse_values(const FieldDescr*, int id_field,
                            int history = 0) const throw();
  const char* coarse_values(const FieldDescr* field_descr, std::string name,
                            int history = 0) const throw() {
    return coarse_values(field_descr, field_descr->field_id(name), history);
  }
  ...
  template <class T>
  CelloView<T, 3> coarse_view(const FieldDescr* field_descr, int id_field,
                              int history = 0) throw() {
    using noconst_T = typename std::remove_cv<T>::type;
  ...
  template <class T>
  CelloView<T, 3> coarse_view(const FieldDescr* field_descr, std::string name,
                              int history = 0) throw() {
    return coarse_view<T>(field_descr, field_descr->field_id(name), history);
  }

  template <class T>
  CelloView<const T, 3> coarse_view(const FieldDescr* field_descr, int id_field,
                                    int history = 0) const throw() {
  ...
  CelloView<const T, 3> coarse_view(const FieldDescr* field_descr,
                                    std::string name, int history = 0) const
      throw() {
    return coarse_view<T>(field_descr, field_descr->field_id(name), history);
  }
  ...
  char* unknowns(const FieldDescr*, int id_field, int history = 0) throw();
  char* unknowns(const FieldDescr* field_descr, std::string name,
                 int history = 0) throw() {
    return unknowns(field_descr, field_descr->field_id(name), history);
  }

  const char* unknowns(const FieldDescr*, int id_field, int history = 0) const
      throw();
  const char* unknowns(const FieldDescr* field_descr, std::string name,
                       int history = 0) const throw() {
    return unknowns(field_descr, field_descr->field_id(name), history);
  }
  ...
  void cell_width(double xm, double xp, double* hx, double ym = 0,
                  double yp = 0, double* hy = 0, double zm = 0, double zp = 0,
                  double* hz = 0) const throw();

  /// Clear specified array(s) to specified value
  void clear(const FieldDescr*, float value = 0.0, int id_field_first = -1,
  ...
  /// Return whether array is allocated or not
  size_t permanent_size() const throw() { return array_permanent_.size(); }
  ...
  void deallocate_temporary(const FieldDescr*, int id) throw();
  ...
  bool ghosts_allocated() const throw() { return ghosts_allocated_; }

  /// Return the number of elements (nx,ny,nz) along each axis
  /// (including ghosts), and total number of bytes n
  int field_size(const FieldDescr*, int id_field, int* nx = 0, int* ny = 0,
                 int* nz = 0) const throw();

  /// Return the number of elements (nx,ny,nz) along each axis of the coarse
  /// field
  void coarse_dimensions(const FieldDescr*, int id_field, int* nx = 0,
                         int* ny = 0, int* nz = 0) const throw();

  /// Print basic field characteristics for debugging
  void print(const FieldDescr*, const char* message,
  ...
  void scale(const FieldDescr*, int iy, long double a, int ix,
...
private:  // functions
  ...
  template <class T>
  long double dot_(const T* X, const T* Y, int mx, int my, int mz, int nx,
                   int ny, int nz, int gx, int gy, int gz) const throw();

  template <class T>
  void scale_(T* Y, long double a, T* X, bool ghosts, int mx, int my, int mz,
              int nx, int ny, int nz, int gx, int gy, int gz) const throw();
  ...
  /// offsets_
  void restore_permanent_(const FieldDescr*, const char* array_from,
  ...
  template <class T>
  void print_(const T* field, const char* field_name, const char* message,
              FILE* fp, int ixm, int iym, int izm, int ixp, int iyp, int izp,
              int nx, int ny, int nz, int gx, int gy, int gz, int nxd,
              int nyd) const;

  template <class T>
  CelloView<T, 3> make_view_(const FieldDescr* field_descr, int id_field,
                             ghost_choice choice, int index_history,
                             bool coarse) throw();
