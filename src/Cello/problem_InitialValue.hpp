...
public:  // interface
  ...
  /// Constructor
  InitialValue(Parameters* parameters, int cycle, double time) throw();
  ...
  InitialValue(CkMigrateMessage* m)
      : Initial(m), num_fields_(0), initialized_values_(false), values_(NULL) {}
...
private:  // functions
  void copy_values_(FieldData* field_data, double* value, int index_field,
  ...
  template <class T>
  void copy_precision_(T* field, int offset, double* value, int nx, int ny,
                       int nz);
