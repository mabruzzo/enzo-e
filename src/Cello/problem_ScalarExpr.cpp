...
ScalarExpr::ScalarExpr(Param* param) throw() : param_(param), value_(0) {
...
template <class T>
void ScalarExpr::evaluate(T* value, double t, int ndx, int nx, double* xv,
                          int ndy, int ny, double* yv, int ndz, int nz,
                          double* zv, std::shared_ptr<Mask> mask,
                          T* deflt) const {
  ASSERT6("ScalarExpr::evaluate",
          "value dimension (%d %d %d) needs to be at least (%d %d %d)", ndx,
          ndy, ndz, nx, ny, nz, (ndx >= nx) && (ndy >= ny) && (ndz >= nz));
...
template void ScalarExpr::evaluate(float* value, double t, int ndx, int nx,
                                   double* x, int ndy, int ny, double* y,
                                   int ndz, int nz, double* z,
                                   std::shared_ptr<Mask> mask,
                                   float* deflt) const;
template void ScalarExpr::evaluate(double* value, double t, int ndx, int nx,
                                   double* x, int ndy, int ny, double* y,
                                   int ndz, int nz, double* z,
                                   std::shared_ptr<Mask> mask,
                                   double* deflt) const;
template void ScalarExpr::evaluate(long double* value, double t, int ndx,
                                   int nx, double* x, int ndy, int ny,
                                   double* y, int ndz, int nz, double* z,
                                   std::shared_ptr<Mask> mask,
                                   long double* deflt) const;
