...
PARALLEL_MAIN_BEGIN {
  ...
  }
  {
    ...
    bool lpad;
    box000_l0->get_start_size(if3_l0, nf3_l0, BlockType::none,
                              BlockType::receive, lpad = false);
    box000_l0->get_start_size(ic3_l0, nc3_l0, BlockType::none, BlockType::send,
                              lpad = true);
    box000_l1->get_start_size(if3_l1, nf3_l1, BlockType::none,
                              BlockType::receive, lpad = false);
    box000_l1->get_start_size(ic3_l1, nc3_l1, BlockType::none, BlockType::send,
                              lpad = true);
    box000_l4->get_start_size(if3_l4, nf3_l4, BlockType::none,
                              BlockType::receive, lpad = false);
    box000_l4->get_start_size(ic3_l4, nc3_l4, BlockType::none, BlockType::send,
                              lpad = true);
    box000_e0->get_start_size(if3_e0, nf3_e0, BlockType::none,
                              BlockType::receive, lpad = false);
    box000_e0->get_start_size(ic3_e0, nc3_e0, BlockType::none, BlockType::send,
                              lpad = true);
    box000_e1->get_start_size(if3_e1, nf3_e1, BlockType::none,
                              BlockType::receive, lpad = false);
    box000_e1->get_start_size(ic3_e1, nc3_e1, BlockType::none, BlockType::send,
                              lpad = true);
    box000_e4->get_start_size(if3_e4, nf3_e4, BlockType::none,
                              BlockType::receive, lpad = false);
    box000_e4->get_start_size(ic3_e4, nc3_e4, BlockType::none, BlockType::send,
                              lpad = true);

    CkPrintf("Linear      ghost = 0  face %d %d %d:", 0, 0, 0);
    CkPrintf("   ic %2d %2d nc %2d %2d if %2d %2d nf %2d %2d\n", ic3_l0[0],
             ic3_l0[1], nc3_l0[0], nc3_l0[1], if3_l0[0], if3_l0[1], nf3_l0[0],
             nf3_l0[1]);
    CkPrintf("Linear      ghost = 1  face %d %d %d:", 0, 0, 0);
    CkPrintf("   ic %2d %2d nc %2d %2d if %2d %2d nf %2d %2d\n", ic3_l1[0],
             ic3_l1[1], nc3_l1[0], nc3_l1[1], if3_l1[0], if3_l1[1], nf3_l1[0],
             nf3_l1[1]);
    CkPrintf("Linear      ghost = 4  face %d %d %d:", 0, 0, 0);
    CkPrintf("   ic %2d %2d nc %2d %2d if %2d %2d nf %2d %2d\n", ic3_l4[0],
             ic3_l4[1], nc3_l4[0], nc3_l4[1], if3_l4[0], if3_l4[1], nf3_l4[0],
             nf3_l4[1]);
    CkPrintf("EnzoProlong ghost = 0  face %d %d %d:", 0, 0, 0);
    CkPrintf("   ic %2d %2d nc %2d %2d if %2d %2d nf %2d %2d\n", ic3_e0[0],
             ic3_e0[1], nc3_e0[0], nc3_e0[1], if3_e0[0], if3_e0[1], nf3_e0[0],
             nf3_e0[1]);
    CkPrintf("EnzoProlong ghost = 1  face %d %d %d:", 0, 0, 0);
    CkPrintf("   ic %2d %2d nc %2d %2d if %2d %2d nf %2d %2d\n", ic3_e1[0],
             ic3_e1[1], nc3_e1[0], nc3_e1[1], if3_e1[0], if3_e1[1], nf3_e1[0],
             nf3_e1[1]);
    CkPrintf("EnzoProlong ghost = 4  face %d %d %d:", 0, 0, 0);
    CkPrintf("   ic %2d %2d nc %2d %2d if %2d %2d nf %2d %2d\n", ic3_e4[0],
             ic3_e4[1], nc3_e4[0], nc3_e4[1], if3_e4[0], if3_e4[1], nf3_e4[0],
             nf3_e4[1]);
  ...
  }
  {
    ...
    box100_l0->get_start_size(if3_l0, nf3_l0, BlockType::receive,
                              BlockType::receive, lpad = false);
    box100_l0->get_start_size(ic3_l0, nc3_l0, BlockType::send, BlockType::send,
                              lpad = true);

    box100_l1->get_start_size(if3_l1, nf3_l1, BlockType::receive,
                              BlockType::receive, lpad = false);
    box100_l1->get_start_size(ic3_l1, nc3_l1, BlockType::send, BlockType::send,
                              lpad = true);

    box100_l4->get_start_size(if3_l4, nf3_l4, BlockType::receive,
                              BlockType::receive, lpad = false);
    box100_l4->get_start_size(ic3_l4, nc3_l4, BlockType::send, BlockType::send,
                              lpad = true);

    box100_e0->get_start_size(if3_e0, nf3_e0, BlockType::receive,
                              BlockType::receive, lpad = false);
    box100_e0->get_start_size(ic3_e0, nc3_e0, BlockType::send, BlockType::send,
                              lpad = true);

    box100_e1->get_start_size(if3_e1, nf3_e1, BlockType::receive,
                              BlockType::receive, lpad = false);
    box100_e1->get_start_size(ic3_e1, nc3_e1, BlockType::send, BlockType::send,
                              lpad = true);

    box100_e4->get_start_size(if3_e4, nf3_e4, BlockType::receive,
                              BlockType::receive, lpad = false);
    box100_e4->get_start_size(ic3_e4, nc3_e4, BlockType::send, BlockType::send,
                              lpad = true);
    box100_e4->print("box100_e4");
