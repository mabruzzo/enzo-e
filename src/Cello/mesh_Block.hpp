...
public:  // interface
  ...
  inline Data* data() throw() { return data_; };

  inline const Data* data() const throw() { return data_; };

  /// Return the child Data associated with this Block
  inline Data* child_data() throw() { return child_data_; };
  inline const Data* child_data() const throw() { return child_data_; };
  ...
  int age() const throw() { return age_; };
  ...
  void cell_width(double* dx, double* dy = 0, double* dz = 0) const throw();
  ...
  void set_child_face_level_curr(const int ic3[3], const int if3[3],
                                 int level) {
    child_face_level_curr_[ICF3(ic3, if3)] = level;
  }

  void set_child_face_level_next(const int ic3[3], const int if3[3],
                                 int level) {
  ...
  void index_global(Index index, int* ix, int* iy, int* iz, int* nx, int* ny,
                    int* nz) const;

  void index_global(int* ix, int* iy, int* iz, int* nx, int* ny,
                    int* nz) const {
    index_global(index_, ix, iy, iz, nx, ny, nz);
  };

  /// Return the index for the block corresponding to the given global
  /// coordinates.
  ...
  void init_refine_(Index index, int nx, int ny, int nz, int num_field_blocks,
                    int num_adapt_steps, int cycle, double time, double dt,
                    int narray, char* array, int refresh_type,
                    int num_face_level, int* face_level, Adapt* adapt);
  ...
  ItFace it_face(int min_face_rank, Index index, const int* ic3 = 0,
                 const int* if3 = 0) throw();

  /// Return an iterator over neighbors

  ItNeighbor it_neighbor(Index index, int min_face_rank = -1,
  ...
  virtual const CProxy_Block proxy_array() const { return thisProxy; }
  ...
  void initial_exit_();
  void p_initial_exit() { initial_exit_(); }

  void r_initial_new_continue(CkReductionMsg* msg) {
    delete msg;
    initial_new_continue_();
  }
  ...
  int index_initial() const throw() { return index_initial_; }
  ...
  void p_compute_enter() { compute_enter_(); }

  void p_compute_continue() { compute_continue_(); }
  void r_compute_continue(CkReductionMsg* msg) {
    delete msg;
    compute_continue_();
  }

  void p_compute_exit() { compute_exit_(); }
  ...
  int index_method() const throw() { return index_method_; }
  ...
  int index_solver() const throw() {
    ASSERT1("Block::index_solver()", "%s index_solver_[] stack is empty",
            name().c_str(), (index_solver_.size() > 0));
...
public:
  void p_output_enter() { output_enter_(); }
  ...
  void p_output_exit() { output_exit_(); }
  ...
  void p_adapt_update() { adapt_update_(); }
  ...
  void adapt_recv_level(int adapt_step, Index index_send, int ic3[3],
                        std::vector<int> if3[3], int level_now, int level_new,
  ...
  void adapt_recv(Index index_send, const int of3[3], const int ic3[3],
                  int level_face_new, int level_relative, int level_max,
                  bool can_coarsen);
...
public:
  ...
  void control_sync_neighbor(int entry_point, int id, int neighbor_type,
                             int min_face_rank, int root_level);
...
public:
  ...
  /// Deletes all 'out-of-bounds' particles on the block, and for all the
  /// in-bounds particles, sets the 'is_copy' attribute to false
  int delete_non_local_particles_(int it);

  /// Send flux data to neighbors
  int refresh_load_flux_faces_(Refresh& refresh);

  void refresh_load_field_face_(Refresh& refresh, int refresh_type, Index index,
                                int if3[3], int ic3[3]);
  ...
  void refresh_load_flux_face_(Refresh& refresh, int refresh_type, Index index,
                               int if3[3], int ic3[3]);
...
protected:
  ...
  int refresh_load_coarse_face_(Refresh refresh, int refresh_type,
  ...
  void refresh_coarse_send_(Index index, Field field, Refresh& refresh,
                            int iam3[3], int iap3[3], int ifms3[3],
                            int ifps3[3], int ifmr3[3], int ifpr3[3],
                            std::string debug);
  ...
  void refresh_load_particle_face_(int refresh_type, Index index, int if3[3],
                                   int ic3[3]);
  ...
  int particle_create_array_neighbors_(Refresh* refresh,
  ...
  void particle_determine_periodic_update_(int* index_lower, int* index_upper,
                                           double* dpx, double* dpy,
                                           double* dpz);
  void particle_apply_periodic_update_(int nl, ParticleData* particle_list[],
                                       Refresh* refresh);

  /// Scatter particles of given types in type_list, to appropriate
  /// particle_array ParticleData elements
  void particle_scatter_neighbors_(int npa, ParticleData* particle_array[],
  ...
  int particle_load_faces_(int npa, ParticleData* particle_list[],
                           ParticleData* particle_array[], Index index_list[],
                           Refresh* refresh, const bool copy = false);
...
public:
  ...
  void p_stopping_load_balance() { stopping_load_balance_(); }
...
protected:
  ...
  void performance_start_(int index_region, std::string file = "",
                          int line = 0);
  void performance_stop_(int index_region, std::string file = "", int line = 0);
...
public:  // virtual functions
  ...
  /// Set Block's cycle
  void set_cycle(int cycle) throw() { cycle_ = cycle; }

  /// Set Block's time
  virtual void set_time(double time) throw() { time_ = time; }

  /// Set Block's timestep
  virtual void set_dt(double dt) throw() { dt_ = dt; }

  /// Set Block's stopping criteria
  void set_stop(double stop) throw() { stop_ = stop; }
  ...
  FieldFace* create_face(int if3[3], int ic3[3], int g3[3], int refresh_type,
                         Refresh* refresh, bool new_refresh = true) const;
...
protected:  // functions
  ...
  void check_child_(const int ic3[3], const char* msg, const char* file,
                    int line) const {
    ASSERT5(msg, "child %d %d %d out of range in file %s line %d", ic3[0],
            ic3[1], ic3[2], file, line,
            0 <= ic3[0] && ic3[0] <= 1 && 0 <= ic3[1] && ic3[1] <= 1 &&
                0 <= ic3[2] && ic3[2] <= 1);
  }

  void check_face_(const int if3[3], const char* msg, const char* file,
                   int line) const {
    ASSERT5(msg, "face %d %d %d out of range in file %s line %d", if3[0],
            if3[1], if3[2], file, line,
            -1 <= if3[0] && if3[0] <= 1 && -1 <= if3[1] && if3[1] <= 1 &&
                -1 <= if3[2] && if3[2] <= 1);
  }
  ...
  std::string id_() const throw() {
    ...
    int v3[3];
    index_.values(v3);
    snprintf(buffer, sizeof(buffer), "%08X-%08X-%08X", v3[0], v3[1], v3[2]);
    return buffer;
  }
  ...
  void determine_boundary_(bool is_boundary[3][2], bool* axm, bool* axp,
                           bool* aym, bool* ayp, bool* azm, bool* azp);
  ...
  Refresh* refresh() throw() { return refresh_.back(); }
...
protected:  // attributes
  ...
  /// Index and total count used for ordering blocks, e.g. for dynamic load
  /// balancing
