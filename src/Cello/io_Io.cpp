...
Io::Io() throw() : meta_name_() {}

//----------------------------------------------------------------------

void Io::meta_value(int index, void** buffer, std::string* name, int* type,
                    int* nxd, int* nyd, int* nzd) throw() {
  ASSERT1("Io::meta_value()", "index %d out of range", index,
...
char* Io::save_data(char* buffer) const {
  ...
  ASSERT2("Io::save_data()", "Expecting buffer size %d actual size %d",
          Io::data_size(), (pc - buffer), (Io::data_size() == (pc - buffer)));
