...
public:  // interface
  ...
  MethodNull(ParameterGroup p)
      : Method(), dt_(p.value_float("dt", std::numeric_limits<double>::max())) {
    init_refresh_();
  }

  MethodNull() : Method(), dt_(std::numeric_limits<double>::max()) {
    init_refresh_();
  }
  ...
  MethodNull(CkMigrateMessage* m) : Method(m), dt_(0.0) {}
...
public:  // virtual methods
  ...
  virtual std::string name() throw() { return "null"; }

  /// Compute maximum timestep for this method
  virtual double timestep(Block* block) throw() { return dt_; }
