...
void Input::read_simulation(Simulation* simulation) throw() {
  WARNING("Input::read_simulation()", "This function should not be called");
}
...
Block* Input::read_block(Block* block, std::string block_name) throw() {
  // Read fields
...
std::string Input::expand_name_(const std::string* name_p,
  ...
  ASSERT1("Input::expand_name_", "File name %s cannot contain '\\%%'",
