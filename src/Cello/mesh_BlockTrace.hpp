...
public:  // interface
  ...
  BlockTrace(int rank, int index_min[3], int index_max[3]) throw()
    ...
    ASSERT1("BlockTrace::BlockTrace()", "rank must satisfy 1 <= rank = %d <= 3",
            rank, (1 <= rank && rank <= 3));
  }
  ...
  bool next(bool is_leaf) {
    if (is_leaf) {
      ...
      if (child_stack_.size() > 0) {
        ...
        index_root_.set_array(index_curr_[0], index_curr_[1], index_curr_[2]);
      }
    } else {
    ...
    return ((child_stack_.size() == 0) && (index_curr_[0] == index_min_[0]) &&
            (index_curr_[1] == index_min_[1]) &&
            (index_curr_[2] == index_min_[2]));
  }

  /// Return the home Index of the BlockTrace
  Index home() { return index_home_; }
  ...
  void print(const char* message) {
    ...
    for (size_t i = 0; i < index_stack_.size(); i++) {
      ...
      CkPrintf("            %lu level %d a(%d %d %d) c(%d %d %d) [%d]\n", i,
               level, a3[0], a3[1], a3[2], c3[0], c3[1], c3[2], child);
    }
  }
