...
#define TRACE_PROLONG(MSG, PROLONG, mf3, if3, nf3, mc3, ic3, nc3)              \
  CkPrintf("TRACE_PROLONG %s:%d %s %s mf %d %d %d nf %d %d %d if %d %d %d\n",  \
           __FILE__, __LINE__, MSG, PROLONG->name().c_str(), mf3[0], mf3[1],   \
           mf3[2], nf3[0], nf3[1], nf3[2], if3[0], if3[1], if3[2]);            \
  CkPrintf("TRACE_PROLONG %s:%d %s %s mc %d %d %d nc %d %d %d ic %d %d %d\n",  \
           __FILE__, __LINE__, MSG, PROLONG->name().c_str(), mc3[0], mc3[1],   \
           mc3[2], nc3[0], nc3[1], nc3[2], ic3[0], ic3[1], ic3[2]);
...
void Block::refresh_start(int id_refresh, int callback) {
  ...
  if (refresh->is_active()) {
    ASSERT1("Block::refresh_start()", "refresh[%d] state is not inactive",
            id_refresh, (sync->state() == RefreshState::INACTIVE));
    ...
    ASSERT4("Block::refresh_start()",
            "refresh[%d] sync object %p is active (%d/%d)", id_refresh, sync,
...
void Block::refresh_wait(int id_refresh, int callback) {
  ...
  ASSERT1("Block::refresh_wait()", "Wait called with inactive Refresh[%d]",
          id_refresh, (refresh->is_active()));
  ...
  ASSERT1("Block::refresh_wait()", "Refresh[%d] not in 'active' state",
          id_refresh, (sync->state() == RefreshState::ACTIVE));
  ...
  for (size_t id_msg = 0; id_msg < refresh_msg_list_[id_refresh].size();
       id_msg++) {
...
void Block::refresh_check_done(int id_refresh) {
  ...
  ASSERT1("Block::refresh_check_done()",
          "Refresh[%d] must not be in inactive state", id_refresh,
  ...
  if ((sync->stop() == 0) ||
    ...
    ASSERT2("Block::refresh_wait()",
            "Refresh %d message list has size %lu instead of 0", id_refresh,
            refresh_msg_list_[id_refresh].size(),
...
void Block::refresh_exit(Refresh& refresh) {
  CHECK_ID(refresh.id());
  update_boundary_();
  control_sync(refresh.callback(), refresh.sync_type(), refresh.sync_exit(),
               refresh.min_face_rank(), refresh.neighbor_type(),
               refresh.root_level());
}
...
int Block::refresh_load_field_faces_(Refresh& refresh) {
  ...
  if (neighbor_type == neighbor_leaf || neighbor_type == neighbor_tree) {
    ...
    ItNeighbor it_neighbor = this->it_neighbor(
        index_, min_face_rank, neighbor_type, min_level, refresh.root_level());
    ...
    while (it_neighbor.next(if3)) {
      ...
      const int refresh_type = (level_face == level - 1)   ? refresh_coarse
                               : (level_face == level)     ? refresh_same
                               : (level_face == level + 1) ? refresh_fine
                                                           : refresh_unknown;
      ...
      if (pad == 0) {
        refresh_load_field_face_(refresh, refresh_type, index_neighbor, if3,
                                 ic3);
        ++count;
      } else {
        if (level_face == level) {
          ...
          count += refresh_load_coarse_face_(refresh, refresh_type,
                                             index_neighbor, if3, ic3);
        }
        if (level_face < level) {
          refresh_load_field_face_(refresh, refresh_type, index_neighbor, if3,
                                   ic3);
...
void Block::refresh_load_field_face_(Refresh& refresh, int refresh_type,
  ...
  FieldFace* field_face =
      create_face(if3, ic3, g3, refresh_type, &refresh, false);
...
int Block::refresh_load_coarse_face_(Refresh refresh, int refresh_type,
                                     Index index_neighbor, int if3[3],
                                     int ic3[3]) {
  ...
  if ((pad > 0) && (level != level_face)) {
    ...
    int g3[3] = {(rank >= 1) ? g : 0, (rank >= 2) ? g : 0, (rank >= 3) ? g : 0};
    ...
    ItNeighbor it_extra = this->it_neighbor(
        index_, refresh.min_face_rank(), refresh.neighbor_type(),
        cello::config()->mesh_min_level, refresh.root_level());
    ...
    int jf3[3] = {l_send ? if3[0] : -if3[0], l_send ? if3[1] : -if3[1],
                  l_send ? if3[2] : -if3[2]};
    ...
    if (l_send) {
      ...
      box_sr.get_start_stop(iam3, iap3, BlockType::extra,
                            BlockType::receive_coarse, lpad = true);
      box_sr.get_start_stop(ifms3, ifps3, BlockType::extra, BlockType::send,
                            lpad = true);
      box_sr.get_start_stop(ifmr3, ifpr3, BlockType::extra, BlockType::receive,
                            lpad = false);

      refresh_coarse_send_(index_neighbor, data()->field(), refresh, iam3, iap3,
                           ifms3, ifps3, ifmr3, ifpr3, "SR");
    ...
    if (l_send) {
      ...
      while (it_extra.next(ef3)) {
        ...
        if (index_extra != index_neighbor && l_valid_level) {
          ...
          bool overlap = box_sr.get_start_stop(tm3, tp3, BlockType::extra,
                                               BlockType::extra, lpad = true);
          if (overlap) {
            if (level_extra == level) {
              ...
              int if3_er[3] = {if3[0] - ef3[0], if3[1] - ef3[1],
                               if3[2] - ef3[2]};
              ...
              box_er.get_start_stop(iam3, iap3, BlockType::extra,
                                    BlockType::receive_coarse, lpad = true);
              box_er.get_start_stop(ifms3, ifps3, BlockType::extra,
                                    BlockType::extra, lpad = true);
              box_er.get_start_stop(ifmr3, ifpr3, BlockType::extra,
                                    BlockType::receive, lpad = false);

              refresh_coarse_send_(index_neighbor, data()->field(), refresh,
                                   iam3, iap3, ifms3, ifps3, ifmr3, ifpr3,
                                   "ER");

              ASSERT3("Block::refresh_load_coarse_face_",
                      "Face if3_er %d %d %d out of bounds", if3_er[0],
                      if3_er[1], if3_er[2],
      ...
      while (it_extra.next(ef3)) {
        ...
        if (index_extra != index_neighbor && l_valid_level) {
          ...
          // adjust for fine blocks pointing to neighboring fine block in same
          // parent
          for (int i = 0; i < 3; i++) {
            if (if3[i] == 0) {
          ...
          box_sr.set_block(BoxType_extra, (level_extra - level_send), if3_se,
                           ec3);
          ...
          if (overlap) {
            ...
            if (level_extra == level) {
              ...
              box_se.get_start_stop(iam3, iap3, BlockType::extra,
                                    BlockType::receive_coarse, lpad = true);
              box_se.get_start_stop(ifms3, ifps3, BlockType::extra,
                                    BlockType::extra, lpad = true);
              box_se.get_start_stop(ifmr3, ifpr3, BlockType::extra,
                                    BlockType::receive, lpad = false);

              int ma3[3];
              box_se.get_region_size(ma3);

              refresh_coarse_send_(index_extra, data()->field(), refresh, iam3,
                                   iap3, ifms3, ifps3, ifmr3, ifpr3, "SE");

              ASSERT3("Block::refresh_load_coarse_face_",
                      "Face if3_se %d %d %d out of bounds", if3_se[0],
                      if3_se[1], if3_se[2],
...
int Block::delete_non_local_particles_(int it) {
  Particle particle(cello::particle_descr(), data()->particle_data());
  ...
  ASSERT1("Block::clean_up_particles_",
          ...
          type_name.c_str(), particle.has_attribute(it, "is_copy"));
...
void Block::refresh_coarse_send_(Index index_neighbor, Field field,
                                 Refresh& refresh, int iam3[3], int iap3[3],
                                 int ifms3[3], int ifps3[3], int ifmr3[3],
                                 int ifpr3[3], std::string debug) {
  MsgRefresh* msg_refresh = new MsgRefresh;
  ...
  CHECK_ID(id_refresh);
  data_msg->set_coarse_array(field, iam3, iap3, ifms3, ifps3, ifmr3, ifpr3,
                             refresh.field_list_src(), refresh.field_list_dst(),
                             name(index_neighbor));
    ...
    if (neighbor_type == neighbor_leaf || neighbor_type == neighbor_tree) {
      ...
      while (it_neighbor.next(if3)) {
        ...
        if (level == level_face + 1) {
          ...
          const int g = refresh->ghost_depth();
          const int rank = cello::rank();
          int g3[3] = {(rank >= 1) ? g : 0, (rank >= 2) ? g : 0,
                       (rank >= 3) ? g : 0};
          ...
          for (int i_f = 0; i_f < nf; i_f++) {
            ...
            box_r.get_start_size(i3_f, n3_f, BlockType::receive,
                                 BlockType::receive, lpad = false);
            box_r.get_start_size(i3_c, n3_c, BlockType::receive,
                                 BlockType::receive_coarse, lpad = true);
            ...
            box_p.get_start_size(ip3_c, np3_c, BlockType::none,
                                 BlockType::receive_coarse, lpad = true);
            box_p.get_start_size(ip3_f, np3_f, BlockType::none,
                                 BlockType::receive, lpad = false);

            prolong->array_sizes_valid(n3_f, n3_c);
            const bool accumulate = refresh->accumulate(i_f);
            TRACE_PROLONG("coarse_apply", prolong, m3_f, ip3_f, np3_f, m3_c,
                          ip3_c, np3_c);
            prolong->apply(default_precision, field_values_dst, m3_f, ip3_f,
                           np3_f, coarse_field_src, m3_c, ip3_c, np3_c,
                           accumulate);

            if (accumulate) {
              DEBUG_PRINT_ARRAY0("refresh_coarse_apply coarse_field",
                                 coarse_field, m3_c, np3_c, ip3_c);
              DEBUG_PRINT_ARRAY0("refresh_field_apply field_values",
                                 field_values_dst, m3_f, np3_f, ip3_f);
            }
          }
        }
      }
...
int Block::refresh_load_particle_faces_(Refresh& refresh, const bool copy) {
  ...
  int nl = particle_load_faces_(npa, particle_list, particle_array, index_list,
                                &refresh, copy);
...
void Block::particle_send_(Refresh& refresh, int nl, Index index_list[],
  ...
  for (int il = 0; il < nl; il++) {
    ...
    ASSERT1("Block::particle_send_()",
            "id_refresh %d of refresh object is out of range", id_refresh,
            (0 <= id_refresh));
...
int Block::particle_load_faces_(int npa, ParticleData* particle_list[],
                                ParticleData* particle_array[],
                                Index index_list[], Refresh* refresh,
                                const bool copy) {
  ...
  int nl = particle_create_array_neighbors_(refresh, particle_array,
...
int Block::particle_create_array_neighbors_(Refresh* refresh,
                                            ParticleData* particle_array[],
                                            ParticleData* particle_list[],
                                            Index index_list[]) {
  const int rank = cello::rank();
  ...
  for (il = 0; it_neighbor.next(if3); il++) {
    ...
    refresh->get_particle_bin_limits(rank, refresh_type, if3, ic3, index_lower,
                                     index_upper);
...
void Block::particle_determine_periodic_update_(int* index_lower,
                                                int* index_upper, double* dpx,
...
void Block::particle_apply_periodic_update_(int nl,
                                            ParticleData* particle_list[],
                                            Refresh* refresh) {
  const int rank = cello::rank();
  const int level = this->level();
  ...
  while (it_neighbor.next(if3)) {
    ...
    particle_determine_periodic_update_(index_lower, index_upper, &dpx[il],
                                        &dpy[il], &dpz[il]);
  ...
  for (int il = 0; il < nl; il++) {
    ...
    if (((rank >= 1) && dpx[il] != 0.0) || ((rank >= 2) && dpy[il] != 0.0) ||
        ((rank >= 3) && dpz[il] != 0.0)) {
...
void Block::particle_scatter_neighbors_(int npa, ParticleData* particle_array[],
                                        std::vector<int>& type_list,
                                        Particle particle, const bool copy) {
  if (copy) {
    // Loop over particle types
    for (auto it_type = type_list.begin(); it_type != type_list.end();
         it_type++) {
      int it = *it_type;
...
int Block::refresh_load_flux_faces_(Refresh& refresh) {
  ...
  while (it_neighbor.next(if3)) {
    ...
    const int refresh_type = (level_face < level)   ? refresh_coarse
                             : (level_face > level) ? refresh_fine
                                                    : refresh_same;

    refresh_load_flux_face_(refresh, refresh_type, index_neighbor, if3, ic3);
...
void Block::refresh_load_flux_face_(Refresh& refresh, int refresh_type,
                                    Index index_neighbor, int if3[3],
                                    int ic3[3]) {
  ...
  if (refresh_type == refresh_coarse) {
    ...
    for (int i = 0; i < nf; i++) {
      FaceFluxes* face_fluxes =
          new FaceFluxes(*flux_data->block_fluxes(axis, face, i));
