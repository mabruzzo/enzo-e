...
public:  // interface
  ...
  /// Constructor
  Adapt() : valid_(false), rank_(0), min_level_(0), max_level_(0) {
  ...
  void set_face_level(const int if3[3], LevelType level_type, int level_min,
  ...
  /// Set level for the block in the given face and (neighbor's) child
  void set_face_level_last(const int ic3[3], const int if3[3], int level_min,
                           int level_max, bool can_coarsen);
  ...
  int face_level(const int if3[3], LevelType level_type) const {
    ASSERT("Adapt::face_level", "face_level() called with LevelType::last",
  ...
  inline void set_max_level(int max_level) { max_level_ = max_level; }
  /// Set the minimum allowable refinement level
  inline void set_min_level(int min_level) { min_level_ = min_level; }
  ...
  void update_neighbor(Index index, int level_min, int level_max,
                       bool can_coarsen);
  ...
  /// the block itself.)
  void get_level_bounds(int* level_min, int* level_max,
                        bool* can_coarsen) const;

  /// Return the current level bounds for the specified neighbor
  void get_neighbor_level_bounds(Index index, int* level_min, int* level_max,
                                 bool* can_coarsen) const;
  ...
  void print(std::string message, const Block* block = nullptr,
             FILE* fp = nullptr) const;

  /// Write block's neighbors to file for debugging
  void write(std::string filename, const Block* block,
             int cycle_start = 0) const;
...
private:  // methods
  ...
  LevelInfo* neighbor_(Index index) {
    ...
    for (int i = 0; i < n; i++) {
      if (neighbor_list_[i].index_ == index) return &neighbor_list_[i];
    }
    return nullptr;
  }
