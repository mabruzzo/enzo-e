...
public:  // interface
  ...
  ScalarExpr() throw() : param_(nullptr), value_(0) {}
  ...
  inline void pup(PUP::er& p) {
    ...
    p | is_null;
    if (!is_null) {
      ERROR("ScalarExpr::pup()", "not implemented when param_ isn't a nullptr");
    }
  ...
  template <class T>
  void evaluate(T* value, double t, int ndx, int nx, double* x, int ndy, int ny,
  ...
  template <class T>
  void evaluate(T* value, double t, int ndx, int nx, double* x, int ndy, int ny,
                double* y, int ndz, int nz, double* z) const {
