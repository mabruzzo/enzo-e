...
int Performance::new_counter(int type, std::string counter_name) {
...
void Performance::assign_counter(int index, long long value) {
...
void Performance::increment_counter(int index, long long value) {
  if (counter_type(index) == counter_type_user) {
    counter_values_[index] += value;
...
void Performance::new_region(int region_index, std::string region_name,
                             bool in_charm) throw() {
#ifdef TRACE_PERFORMANCE
...
void Performance::start_region(int id_region, std::string file,
                               int line) throw() {
#ifdef TRACE_PERFORMANCE
  CkPrintf("%d TRACE_PERFORMANCE Performance::start_region (%d,%s) %s:%d\n",
           CkMyPe(), id_region, region_name_[id_region].c_str(), file.c_str(),
           line);
#endif
  ...
  if (!region_started_[index_region]) {
    ...
    if (file == "") {
      WARNING1("Performance::start_region", "Region %s already started",
               region_name_[id_region].c_str());
    } else {
      WARNING3("Performance::start_region", "Region %s already started %s %d",
               region_name_[id_region].c_str(), file.c_str(), line);
    }
    return;
  }
...
void Performance::stop_region(int id_region, std::string file,
                              int line) throw() {
#ifdef TRACE_PERFORMANCE
  ...
  if (region_started_[index_region]) {
    ...
    if (file == "") {
      WARNING1("Performance::stop_region", "Region %s already stopped",
               region_name_[id_region].c_str());
    } else {
      WARNING3("Performance::stop_region", "Region %s already stopped %s %d",
               region_name_[id_region].c_str(), file.c_str(), line);
    }
    return;
  }
...
void Performance::region_counters(int index_region,
                                  long long* counters) throw() {
  if (!region_started_[index_region]) {
    for (int i = 0; i < num_counters(); i++) {
