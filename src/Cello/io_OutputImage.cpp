...
#define TRACE_MEMORY(MSG, BYTES)                                               \
  CkPrintf("TRACE_MEMORY %d : %d %s : %lu\n", CkMyPe(), __LINE__, (MSG),       \
           (long unsigned)(BYTES));                                            \
  fflush(stdout);
#else
#define TRACE_MEMORY(MSG, BYTES) /* ... */
...
OutputImage::OutputImage(
    int index, const Factory* factory, int process_count,
    const int root_size_in[3], const int root_blocks_in[3], int min_level,
    int max_level, int leaf_only, std::string image_type, int image_size[2],
    std::string image_reduce_type, std::string image_mesh_color,
    std::string image_mesh_order, std::string color_particle_attribute,
    double image_lower[], double image_upper[], int face_rank, int axis,
    bool image_log, bool image_abs, bool ghost, bool use_min_max,
  ...
  int root_size[3] = {root_size_in[0], root_size_in[1], root_size_in[2]};
  int root_blocks[3] = {root_blocks_in[0], root_blocks_in[1],
                        root_blocks_in[2]};
  image_size_[0] = image_size[0];
  image_size_[1] = image_size[1];
  if (image_reduce_type == "min") {
    op_reduce_ = reduce_min;
  } else if (image_reduce_type == "max") {
    op_reduce_ = reduce_max;
  } else if (image_reduce_type == "avg") {
    op_reduce_ = reduce_avg;
  } else if (image_reduce_type == "sum") {
  ...
  if (image_mesh_color == "level")
    mesh_color_type_ = mesh_color_level;
  else if (image_mesh_color == "process")
    mesh_color_type_ = mesh_color_process;
  else if (image_mesh_color == "age")
    mesh_color_type_ = mesh_color_age;
  else if (image_mesh_color == "order") {
    mesh_color_type_ = mesh_color_order;
    mesh_color_order_ = image_mesh_order;
  } else {
    ERROR1("OutputImage::OutputImage()",
           "Unrecognized output_image_mesh_color %s", image_mesh_color.c_str());
  }
  ...
  if (image_size_[0] == 0)
    image_size_[0] = (type_is_mesh_()) ? 2 * root_blocks[0] + 1 : root_size[0];
  if (image_size_[1] == 0)
    image_size_[1] = (type_is_mesh_()) ? 2 * root_blocks[1] + 1 : root_size[1];
...
OutputImage::~OutputImage() throw() {
  TRACE_MEMORY("delete image_data_",
               image_size_[0] * image_size_[1] * sizeof(double));
  TRACE_MEMORY("delete image_mesh_",
               image_size_[0] * image_size_[1] * sizeof(double));
...
void OutputImage::init() throw() { image_create_(); }
...
void OutputImage::finalize() throw() { Output::finalize(); }
...
// @param block  Block to write
{
  ...
  ASSERT("OutputImage::write_block", "cannot output rank == 1 problems",
         !(rank == 1));
  ...
  int index_field =
      (it_field_index_->size() > 0) ? it_field_index_->value() : -1;
  ...
  if (type_is_data_()) {
    if (index_field >= 0) {
      ...
      const char* field_values = (include_ghost_) ? field.values(index_field)
            ...
            if (dm3[IY] <= y && y <= dp3[IY]) {
              for (int iz = 0; iz < m3[IZ]; iz++) {
                ...
                if (rank < 3 || (zlo <= z && z <= zhi)) {
                  ...
                  reduce_box_filled_(image_data_, jxm, jxp, jym, jyp,
                                     (value * factor));
                }
              }
            }
  ...
  if (type_is_mesh_()) {
    ...
    if (face_rank_ >= 1) {
      ...
      reduce_box_filled_(image_mesh_, ixm + 3, ixp - 3, iym + 3, iyp - 3,
                         value);
    }
    ...
    if (face_rank_ <= 1) {
      {
        ...
        reduce_box_filled_(image_mesh_, ixm + 1, ixm + 2, ym - 1, ym + 1,
                           face_color);
      }
      {
        int if3[3] = {1, 0, 0};
        int face_level = block->face_level(if3);
        double face_color = mesh_color_(block, face_level);
        reduce_box_filled_(image_mesh_, ixp - 2, ixp - 1, ym - 1, ym + 1,
                           face_color);
      }
      {
        int if3[3] = {0, -1, 0};
        int face_level = block->face_level(if3);
        double face_color = mesh_color_(block, face_level);
        reduce_box_filled_(image_mesh_, xm - 1, xm + 1, iym + 1, iym + 2,
                           face_color);
      }
      {
        int if3[3] = {0, 1, 0};
        int face_level = block->face_level(if3);
        double face_color = mesh_color_(block, face_level);
        reduce_box_filled_(image_mesh_, xm - 1, xm + 1, iyp - 2, iyp - 1,
                           face_color);
      }
    }
    if (face_rank_ <= 0) {
      {
        ...
        int face_level = block->face_level(if3);
        double face_color = mesh_color_(block, face_level);
        reduce_box_filled_(image_mesh_, ixm + 1, ixm + 2, iym + 1, iym + 2,
                           face_color);
      }
      {
        int if3[3] = {1, -1, 0};
        int face_level = block->face_level(if3);
        double face_color = mesh_color_(block, face_level);
        reduce_box_filled_(image_mesh_, ixp - 2, ixp - 1, iym + 1, iym + 2,
                           face_color);
      }
      {
        int if3[3] = {-1, 1, 0};
        int face_level = block->face_level(if3);
        double face_color = mesh_color_(block, face_level);
        reduce_box_filled_(image_mesh_, ixm + 1, ixm + 2, iyp - 2, iyp - 1,
                           face_color);
      }
      {
        int if3[3] = {1, 1, 0};
        int face_level = block->face_level(if3);
        double face_color = mesh_color_(block, face_level);
        reduce_box_filled_(image_mesh_, ixp - 2, ixp - 1, iyp - 2, iyp - 1,
                           face_color);
      }
    }
  }
  ...
  for (it_particle_index_->first(); !it_particle_index_->done();
       it_particle_index_->next()) {
    const int it = it_particle_index_->value();

    const int ia_color =
        (color_particle_attribute_ != "")
            ? particle.attribute_index(it, color_particle_attribute_)
            : -1;
    ...
    for (int ib = 0; ib < nb; ib++) {
      ...
      particle.position(it, ib, position[0].data(), position[1].data(),
                        position[2].data());
...
void OutputImage::write_field_data(const FieldData* field_data,
...
void OutputImage::write_particle_data(const ParticleData* particle_data,
...
void OutputImage::update_remote(int m, char* buffer) throw() {
  ...
  if (op_reduce_ == reduce_min) {
    for (int k = 0; k < n; k++)
      image_data_[k] = std::min(image_data_[k], *p.d++);
    for (int k = 0; k < n; k++)
      image_mesh_[k] = std::min(image_mesh_[k], *p.d++);
  } else if (op_reduce_ == reduce_max) {
    for (int k = 0; k < n; k++)
      image_data_[k] = std::max(image_data_[k], *p.d++);
    for (int k = 0; k < n; k++)
      image_mesh_[k] = std::max(image_mesh_[k], *p.d++);
...
double OutputImage::mesh_color_(const Block* block, int level) const {
  ...
  if (mesh_color_type_ == mesh_color_level) {
    ...
    ScalarData<long long>* scalar_data =
        ((Block*)block)->data()->scalar_data_long_long();
    long long index =
        *scalar_data->value(cello::scalar_descr_long_long(), is_i);
    long long count =
        *scalar_data->value(cello::scalar_descr_long_long(), is_n);
    value = (count) > 0 ? 1.0 * index / count : 0;
  } else {
    ERROR1("OutputImage::mesh_color_()", "Unknown mesh_color_type_ %d",
           mesh_color_type_);
  }
...
void OutputImage::image_create_() throw() {
  ASSERT("OutputImage::image_create_", "image_ already created",
         image_data_ == NULL || image_mesh_ == NULL);

  TRACE_MEMORY("new image_data_",
               image_size_[0] * image_size_[1] * sizeof(double));
  TRACE_MEMORY("new image_mesh_",
               image_size_[0] * image_size_[1] * sizeof(double));
  image_data_ = new double[image_size_[0] * image_size_[1]];
  image_mesh_ = new double[image_size_[0] * image_size_[1]];
  ...
  for (int i = 0; i < image_size_[0] * image_size_[1]; i++)
    image_data_[i] = value0;
  for (int i = 0; i < image_size_[0] * image_size_[1]; i++)
    image_mesh_[i] = value0;
}
...
void OutputImage::image_write_() throw() {
  ...
  if (type_is_mesh_() && type_is_data_()) {
    ...
    for (int i = 0; i < m; i++)
      buf[i] = (image_data_[i] + 0.2 * image_mesh_[i]) / 1.2;
    data = buf.data();
  ...
  if (chmod(dir_name.c_str(), 0755) == -1) {
    ERROR2("OutputImage::image_write()", "chmod() return errno %d: error '%s'",
...
void OutputImage::image_close_() throw() {
  ...
  TRACE_MEMORY("delete image_data",
               image_size_[0] * image_size_[1] * sizeof(double));
  delete[] image_data_;
  image_data_ = nullptr;

  TRACE_MEMORY("delete image_mesh",
               image_size_[0] * image_size_[1] * sizeof(double));
  delete[] image_mesh_;
  image_mesh_ = nullptr;
}
...
void OutputImage::reduce_point_(double* data, int ix, int iy, double value,
                                double alpha) throw() {
  ...
  if (!(0.0 <= alpha && alpha <= 1.0)) {
    WARNING1("OutputImage::reduce_point_()",
             "Alpha %g is not between 0.0 and 1.0", alpha);
  }
...
void OutputImage::reduce_line_x_(double* data, int ixm, int ixp, int iy,
                                 double value, double alpha) {
...
void OutputImage::reduce_line_y_(double* data, int ix, int iym, int iyp,
                                 double value, double alpha) {
...
void OutputImage::reduce_box_filled_(double* data, int ixm, int ixp, int iym,
                                     int iyp, double value, double alpha) {
  for (int ix = ixm; ix <= ixp; ++ix) {
    for (int iy = iym; iy <= iyp; ++iy) {
      reduce_point_(data, ix, iy, value, alpha);
    }
