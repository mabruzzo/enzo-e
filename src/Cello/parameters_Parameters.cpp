...
Parameters::Parameters(Monitor* monitor) throw()
    : current_group_(),
      parameter_map_(),
...
///
{
  ...
  for (auto it_param = parameter_map_.begin(); it_param != parameter_map_.end();
       ++it_param) {
...
void Parameters::pup(PUP::er& p) {
  ...
  if (!p.isUnpacking()) {
    ...
    for (auto it_param = parameter_map_.begin();
         it_param != parameter_map_.end(); ++it_param) {
      n++;
    }
  }
  p | n;
...
/// @param    file_name An opened input parameter file or stdin
{
  ...
  ASSERT1("Parameters::read", "Error opening parameter file '%s' for reading",
          file_name, (file_pointer != NULL));
...
/// @param  file_name   An opened output parameter file or stdout
{
  ...
  ASSERT1("Parameters::write", "Error opening parameter file '%s' for writing",
          file_name, (fp != NULL));
...
/// @param  file_name   An opened output parameter file or stdout
{
  ...
  for (auto it_param = parameter_map_.begin(); it_param != parameter_map_.end();
    ...
    int i_group;
    for (i_group = 0; i_group < n_prev && i_group < n_curr &&
    ...
    for (int i = i_group; i < n_prev; i++) {
      --group_depth;
      indent_string =
          indent_string.substr(0, indent_string.size() - indent_size);
    ...
    for (int i = i_group; i < n_curr; i++) {
      if (type != param_write_monitor) {
        ...
        fprintf(fp, format, indent_string.c_str(), group_curr[i].c_str());
      }
...
int Parameters::value_integer(std::string parameter, int deflt) throw()
...
/// @return  Return integer parameter value if it exists, deflt if not
{
  ...
  ASSERT1("Parameters::value_integer", "Parameter %s is not an integer",
          parameter.c_str(), (!param || param->is_type(parameter_integer)));
...
void Parameters::set_integer(std::string parameter, int value) throw()
/// @param   parameter Parameter name
/// @param   value     Value to set the parameter
{
  Param* param = this->param(parameter_name_(parameter));

  ASSERT1("Parameters::set_integer", "Parameter %s is not an integer",
          parameter.c_str(), (!param || param->is_type(parameter_integer)));
...
double Parameters::value_float(std::string parameter, double deflt) throw()
/// @param   parameter Parameter name
/// @param   deflt     Default parameter value
/// @return  Return floating point (double) parameter value if it exists, deflt
/// if not
{
  Param* param = this->param(parameter);

  ASSERT1("Parameters::value_float", "Parameter %s is not a float",
          parameter.c_str(), (!param || param->is_type(parameter_float)));
...
void Parameters::set_float(std::string parameter, double value) throw()
/// @param   parameter Parameter name
/// @param   value     Value to set the parameter
{
  Param* param = this->param(parameter);

  ASSERT1("Parameters::set_float", "Parameter %s is not a float",
          parameter.c_str(), (!param || param->is_type(parameter_float)));
...
bool Parameters::value_logical(std::string parameter, bool deflt) throw()
/// @param   parameter Parameter name
/// @param   deflt     Default parameter value
/// @return  Return logical parameter value if it exists, deflt if not
{
  ...
  ASSERT2("Parameters::value_logical", "Parameter %s is type %d not a logical",
          parameter.c_str(), param->type(),
          (!param || param->is_type(parameter_logical)));

  char deflt_string[MAX_PARAMETER_FILE_WIDTH];
  snprintf(deflt_string, MAX_PARAMETER_FILE_WIDTH, "%s",
           deflt ? "true" : "false");
  monitor_access_(parameter, deflt_string);
...
void Parameters::set_logical(std::string parameter, bool value) throw()
/// @param   parameter Parameter name
/// @param   value     Value to set the parameter
{
  Param* param = this->param(parameter);

  ASSERT1("Parameters::set_logical", "Parameter %s is not logical",
          parameter.c_str(), (!param || param->is_type(parameter_logical)));
...
std::string Parameters::value_string(std::string parameter,
...
/// @return  Return string parameter value if it exists, deflt if not
{
  ...
  ASSERT1("Parameters::value_string", "Parameter %s is not a string",
          parameter.c_str(), (!param || param->is_type(parameter_string)));
...
void Parameters::set_string(std::string parameter, const char* value) throw()
/// @param   parameter Parameter name
/// @param   value     Value to set the parameter
{
  Param* param = this->param(parameter);

  ASSERT1("Parameters::set_string_string", "Parameter %s is not a string",
          parameter.c_str(), (!param || param->is_type(parameter_string)));
...
void Parameters::evaluate_float(std::string parameter, int n, double* result,
                                double* deflt, double* x, double* y, double* z,
                                double t) throw()
/// @param   parameter Parameter name
/// @param   n         Length of variable arrays
/// @param   result    Output array of evaluated floating point parameters
/// values if it exists, or deflt if not
...
void Parameters::evaluate_logical(std::string parameter, int n, bool* result,
                                  bool* deflt, double* x, double* y, double* z,
                                  double t) throw()
/// @param   parameter Parameter name
/// @param   n         Length of variable arrays
/// @param   result    Output array of evaluated logical parameters values if it
...
/// @param   t         T value
{
  ...
  if (param != NULL) {
    ...
    WARNING("Parameters::evaluate_logical", "param is NULL but deflt not set");
    //   for (int i=0; i<n; i++) result[i] = deflt[i];
  }
}
...
{
  Param* param = this->param(parameter);
  ASSERT1("Parameters::list_length", "Parameter %s is not a list",
          full_name(parameter).c_str(),
...
int Parameters::list_value_integer(int index, std::string parameter,
                                   int deflt) throw()
/// @param   index     Index of the integer list parameter element
/// @param   parameter Parameter name
/// @param   deflt     Default parameter value
/// @return  Return integer list parameter element value if it exists, deflt if
/// not
{
  Param* param = this->param(parameter, index);
  ASSERT2("Parameters::list_value_integer",
          "Parameter %s[%d] is not an integer", parameter.c_str(), index,
...
double Parameters::list_value_float(int index, std::string parameter,
                                    double deflt) throw()
/// @param   index     Index of the floating point (double) list parameter
/// element
/// @param   parameter Parameter name
/// @param   deflt     Default parameter value
/// @return  Return floating point (double) list parameter element value if it
/// exists, deflt if not
{
  Param* param = this->param(parameter, index);
  ASSERT2("Parameters::list_value_float", "Parameter %s[%d] is not a float",
...
bool Parameters::list_value_logical(int index, std::string parameter,
                                    bool deflt) throw()
/// @param   index     Index of the logical list parameter element
/// @param   parameter Parameter name
/// @param   deflt     Default parameter value
/// @return  Return logical list parameter element value if it exists, deflt if
/// not
{
  Param* param = this->param(parameter, index);
  ASSERT2("Parameters::list_value_logical", "Parameter %s[%d] is not a logical",
          parameter.c_str(), index,
          (!param || param->is_type(parameter_logical)));
  char deflt_string[MAX_PARAMETER_FILE_WIDTH];
  snprintf(deflt_string, MAX_PARAMETER_FILE_WIDTH, "%s",
...
std::string Parameters::list_value_string(int index, std::string parameter,
                                          std::string deflt) throw()
/// @param   index     Index of the string list parameter element
/// @param   parameter Parameter name
/// @param   deflt     Default parameter value
/// @return  Return string list parameter element value if it exists, deflt if
/// not
{
  Param* param = this->param(parameter, index);
  ASSERT2("Parameters::list_value_string", "Parameter %s[%d] is not a string",
          parameter.c_str(), index,
...
void Parameters::set_list_length(std::string parameter, int length) {
  Param* param = this->param(parameter);

  ASSERT1("Parameters::set_list_length", "Parameter %s is not a list",
          parameter.c_str(), (!param || param->is_type(parameter_list)));
  ...
  for (int i = 0; i < length; i++) {
    if ((*(param->value_list_))[i] == 0) (*(param->value_list_))[i] = new Param;
  }
}

//----------------------------------------------------------------------

void Parameters::set_list_integer(int index, std::string parameter,
                                  int value) throw() {
  Param* param = this->param(parameter, index);
...
void Parameters::set_list_float(int index, std::string parameter,
                                double value) throw() {
  Param* param = this->param(parameter, index);
...
void Parameters::set_list_logical(int index, std::string parameter,
                                  bool value) throw() {
  Param* param = this->param(parameter, index);
...
void Parameters::set_list_string(int index, std::string parameter,
...
void Parameters::list_evaluate_float(int index, std::string parameter, int n,
                                     double* result, double* deflt, double* x,
                                     double* y, double* z, double t) throw()
/// @param   index     Index into the list
/// @param   parameter Parameter name
/// @param   n         Length of variable arrays
/// @param   result    Output array of evaluated floating point expression list
/// parameter element values if it exists, or deflt if not
/// @param   deflt     Array of default values
/// @param   x         Array of X values
/// @param   y         Array of Y values
/// @param   z         Array of Z values
...
void Parameters::list_evaluate_logical(int index, std::string parameter, int n,
                                       bool* result, bool* deflt, double* x,
                                       double* y, double* z, double t) throw()
/// @param   index     Index into the list
/// @param   parameter Parameter name
/// @param   n         Length of variable arrays
/// @param   result    Output array of evaluated logical expression list
/// parameter element values if it exists, or deflt if not
/// @param   deflt     Array of default values
/// @param   x         Array of X values
/// @param   y         Array of Y values
...
/// @param   t         Array of T values
{
  ...
  ASSERT2("Parameters::list_evaluate_logical",
          "Parameter %s[%d] is not a logical", parameter.c_str(), index,
          (!param || param->is_type(parameter_logical_expr)));
  if (param != NULL) {
    param->evaluate_logical(n, result, x, y, z, t);
  } else {
...
std::vector<std::string> Parameters::value_full_strlist(
  ...
  if (this->param(parameter) == nullptr) {
    ...
    for (int i = 0; i < len; i++) {
      out[i] = this->value(i, parameter, "none");
    }
    return out;
...
int Parameters::group_depth() const throw() { return current_group_.size(); }
...
void Parameters::group_pop(std::string group) throw() {
  ...
  ASSERT("Parameters::group_pop", "More calls to group_pop() than group_push()",
         (n > 0));

  if (group != "" && group != current_group_[n - 1]) {
    WARNING2("Parameters::group_pop",
             "group_pop(%s) does not match group_push(%s)", group.c_str(),
             current_group_[n - 1].c_str());
  }
...
void Parameters::group_clear() throw() { current_group_.clear(); }
...
std::vector<std::string> Parameters::leaf_parameter_names(
  ...
  validate_abs_group_name_("Parameters::leaf_parameter_names", full_group_name);
  ...
  for (auto it_param = parameter_map_.lower_bound(prefix);
    ...
    std::string suffix =
        (it_param->first).substr(prefix_size, std::string::npos);
...
parameter_type Parameters::type(std::string parameter) throw()
/// @param   parameter Parameter name
...
parameter_type Parameters::list_type(int index, std::string parameter) throw()
/// @param   index Index into the list
/// @param   parameter Parameter name
/// @return  Return type of the given parameter
{
...
void Parameters::monitor_access_(std::string parameter,
                                 std::string deflt_string, int index) throw() {
  if (!lmonitor_) return;
  ...
  std::string buffer = std::string("accessed ") + parameter_name_(parameter) +
                       index_string + " " + value;
...
void Parameters::monitor_write_(std::string parameter) throw() {
  ...
  snprintf(buffer, MONITOR_LENGTH, "Parameter write %s = %s",
           ...
           param ? param->value_to_string(param_write_monitor).c_str()
                 : "[undefined]");
...
int Parameters::readline_(FILE* fp, char* buffer, int buffer_length) throw()
...
/// @return  Whether the end of the file has been reached
{
  ...
  ASSERT("Parameters::readline_", "Input file line is too long",
         (i + 1 >= buffer_length - 1));
...
size_t Parameters::extract_groups_(const std::string parameter,
                                   std::string* group) {
  std::string p = parameter;
  size_t i_group = 0;  // group index for group[]
  size_t i_stop = p.find(":");
  while (i_stop != std::string::npos && i_group < MAX_GROUP_DEPTH) {
...
void Parameters::new_param_(std::string full_parameter, Param* param) throw() {
...
void Parameters::check() {
  for (auto it_param = parameter_map_.begin(); it_param != parameter_map_.end();
    ...
    if (!it_param->second->accessed()) {
      WARNING1("Parameters::check()", "Parmeter \"%s\" not accessed",
