...
public:  // interface
  ...
  FaceFluxes(Face face, int index_field, int nx, int ny, int nz, int cx = 0,
             int cy = 0, int cz = 0);
  ...
  FaceFluxes(const FaceFluxes& face_fluxes) { copy_(this, &face_fluxes); }

  ~FaceFluxes() { deallocate_storage(); }
