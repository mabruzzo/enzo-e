...
public:
  ...
  inline int operator[](std::size_t i) const { return v_[i]; }

  inline int& operator[](std::size_t i) { return v_[i]; }
  ...
  bool is_on_boundary(int axis, int face, int narray) const;
  ...
  bool is_sibling(Index index) const {
    ...
    return (level >= 1 && index.level() >= 1)
               ? (index_parent() == index.index_parent())
               : false;
  }
  ...
  bool is_nibling(Index index) const {
    ...
    return (level >= 1 && index.level() >= 2)
               ? (index_parent() == index.index_parent().index_parent())
               : false;
  }
  ...
  void child(int level, int* icx, int* icy, int* icz, int min_level = 0) const;
  ...
  inline void push_child(int icx, int icy = 0, int icz = 0, int min_level = 0) {
    const int level = this->level();
    set_level(level + 1);
  ...
  /// dimensionality, array size, and whether the corresponding block is a leaf
  /// or not
  Index next(int rank, const int na3[3], bool is_leaf, int min_level) const;

  void print(std::string msg, int level) const;

  void print(const char* msg, int max_level, int rank, const int nb3[3],
             bool no_nl, void* simulation = 0) const;

  void write(int ip, const char* msg, int max_level, int rank,
...
private:  // methods
  ...
  void print_(FILE* fp, const char* msg, int max_level, int rank,
...
#ifndef TEST
PUPbytes(Index) PUPbytes(NodeBits)
