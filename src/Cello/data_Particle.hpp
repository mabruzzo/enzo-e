...
public:  // interface
  /// Constructor
  Particle() : particle_descr_(NULL), particle_data_(NULL) {}

  /// Constructor
  Particle(ParticleDescr* particle_descr, ParticleData* particle_data) throw()
      : particle_descr_(particle_descr), particle_data_(particle_data) {
  ...
  int num_types() const { return particle_descr_->num_types(); }
  ...
  int num_constants(int it) const { return particle_descr_->num_constants(it); }
  ...
  char* constant_array(int it) { return particle_descr_->constant_array(it); }
  ...
  int stride(int it, int ia) const { return particle_descr_->stride(it, ia); }
  ...
  bool interleaved(int it) const { return particle_descr_->interleaved(it); }

  /// Return the current batch size.

  int batch_size() const { return particle_descr_->batch_size(); }
  ...
  Grouping* groups() { return particle_descr_->groups(); }
  ...
  char* attribute_array(int it, int ia, int ib) {
    return particle_data_->attribute_array(particle_descr_, it, ia, ib);
  }
  ...
  int num_batches(int it) const { return particle_data_->num_batches(it); }
  ...
  void scatter(int it, int ib, int np, const bool* mask, const int* index,
               int n, ParticleData** particle_array, const bool copy = false) {
    particle_data_->scatter(particle_descr_, it, ib, np, mask, index, n,
                            particle_array, copy);
  }
  ...
  void compress() { particle_data_->compress(particle_descr_); }
  void compress(int it) { particle_data_->compress(particle_descr_, it); }
  ...
  float efficiency() { return particle_data_->efficiency(particle_descr_); }
  ...
  float overhead() { return particle_data_->overhead(particle_descr_); }
  ...
  void position_update(int it, int ib, long double dx, long double dy,
                       long double dz) {
  ...
  int data_size() const { return particle_data_->data_size(particle_descr_); }
  ...
  void debug() const { particle_data_->debug(particle_descr_); }

  void write_ifrite(int it, std::string file_name, double xm, double ym,
                    double zm, double xp, double yp, double zp) {
    particle_data_->write_ifrite(particle_descr_, it, file_name, xm, ym, zm, xp,
                                 yp, zp);
  }
