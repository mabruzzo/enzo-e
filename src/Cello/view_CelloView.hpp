...
typedef std::conditional<sizeof(int) >= sizeof(std::ptrdiff_t), int,
                         std::ptrdiff_t>::type intp;
...
public:
  ...
  intp get_start() const {
    ASSERT("CSlice",
           ("Default constructed CSlices cannot be used without "
...
template <typename T>
bool check_bounds_(intp* shape, T first) {
  return *shape > first;
}
...
template <bool... vals>
using all_true =
    std::is_same<bool_pack<true, vals...>, bool_pack<vals..., true>>;
...
template <typename T>
intp calc_index_(const intp* stride, T first) {
  return first;
}
...
template <typename T>
intp calc_index_(const std::size_t D, const intp offset, const intp* stride,
                 const T* indices) {
...
static inline bool increment_outer_indices_(std::size_t D, intp* indices,
                                            const intp* shape) {
  std::size_t i = D - 1;
  ASSERT("increment_out_indices", "the dimension must be positive", D > 0);
  while (0 != (i--)) {
    indices[i] += 1;
...
public:  // interface
  ...
  CelloView() : shared_data_(), offset_(0), shape_(), stride_(){};
  ...
  int stride(unsigned int dim) const noexcept {
    ASSERT1("CelloView::stride", "%ui is greater than the number of dimensions",
            dim, dim < D);
...
void CelloView<T, D>::init_helper_(const std::shared_ptr<T>& shared_data,
  ...
  if (require_valid_data &
      ((shared_data.get() == nullptr) || (shared_data.use_count() == 0))) {
...
inline void prep_slices_(const CSlice* slices, const intp shape[],
  ...
  for (std::size_t i = 0; i < D; i++) {
    ...
    ASSERT3("prep_slices_",
            ...
            stop <= shape[i]);
    ASSERT4("prep_slices_",
            ("slice stop (%ld) doesn't exceed slice start "
...
bool CelloView<T, D>::is_alias(const CelloView<oT, oD>& other) const noexcept {
  ...
  if (!std::is_same<const_T, const_oT>::value) {
    return false;
  }
  ...
  for (std::size_t i = 0; i < D; i++) {
    ...
    if (different_extents || different_strides) {
      return false;
    }
  }
  return true;
