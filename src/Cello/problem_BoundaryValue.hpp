...
public:  // interface
  ...
  BoundaryValue(CkMigrateMessage* m) : Boundary(m), pairs_() {}
...
protected:  // functions
  template <class T>
  void copy_(T* field, double* value, int ndx, int ndy, int ndz, int nx, int ny,
             int nz, int ix0, int iy0, int iz0) const throw();
