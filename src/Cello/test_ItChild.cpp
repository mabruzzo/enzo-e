...
void clear(bool* c, int n) {
  for (int i = 0; i < n; i++) c[i] = 0;
}

#define IC(ix, iy, iz)                                                         \
  (((ix + 2) % 2) + 2 * (((iy + 2) % 2) + 2 * (((iz + 2) % 2))))
...
PARALLEL_MAIN_BEGIN {
  ...
  unit_assert(count == 4);
  unit_assert(ic[IC(0, 0, 0)] && ic[IC(1, 0, 0)] && ic[IC(0, 1, 0)] &&
              ic[IC(1, 1, 0)]);
  ...
  unit_assert(count == 8);
  unit_assert(ic[IC(0, 0, 0)] && ic[IC(1, 0, 0)] && ic[IC(0, 1, 0)] &&
              ic[IC(1, 1, 0)] && ic[IC(0, 0, 1)] && ic[IC(1, 0, 1)] &&
              ic[IC(1, 1, 0)] && ic[IC(1, 1, 1)]);
