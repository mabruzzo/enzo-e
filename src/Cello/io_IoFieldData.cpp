...
IoFieldData::IoFieldData() throw()
    : Io(), field_data_(0), field_index_(0), include_ghosts_(true) {
...
void IoFieldData::meta_value(int index, void** buffer, std::string* name,
                             int* type, int* mx, int* my, int* mz) throw() {}

//----------------------------------------------------------------------

void IoFieldData::field_array(void** buffer, std::string* name, int* type,
                              int* pmx, int* pmy, int* pmz, int* pnx, int* pny,
                              int* pnz) throw()
/// @param buffer [out] pointer to start of memory containing the requested data
/// item
...
/// @param pnx,pny,pnz [out] pointer to the array data item's actual size
{
  ...
  if (buffer) (*buffer) = (void*)field_data_->values(field_descr, field_index_);
  if (name)
    (*name) = std::string("field_") + field_descr->field_name(field_index_);
  int type_size = 0;
  if (type) {
    ...
    switch (precision) {
      ...
      case precision_quadruple:
        ...
        break;
      default:
        ERROR2("IoFieldData", "Unsupported precision type %d for field %s",
