...
CkReductionMsg* r_reduce_performance(int n, CkReductionMsg** msgs) {
  ...
  ASSERT1("r_reduce_performance",
          "Sanity check failed on expected accumulator array %d", length,
          (length < 500));
  accum.resize(length);
  accum.clear();
  ...
  for (int i = 0; i < n; i++) {
    ASSERT4(
        "r_reduce_performance()",
        "CkReductionMsg actual size %d is different from expected %lu num_sum "
        "%d num_max %d",
...
CkReductionMsg* r_reduce_method_debug(int n, CkReductionMsg** msgs) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    j = 1;
    for (int i_f = 0; i_f < num_fields; i_f++) {
      accum[j] = std::min(accum[j], values[j]);
      ++j;
      accum[j] = std::max(accum[j], values[j]);
      ++j;
      accum[j] += values[j];
...
CkReductionMsg* sum_long_double_n(int n, CkReductionMsg** msgs) {
  ...
  CkReductionMsg* msg =
      CkReductionMsg::buildNew((N + 1) * sizeof(long double), accum);
  delete[] accum;
  return msg;
}
