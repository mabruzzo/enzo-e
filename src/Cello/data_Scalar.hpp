...
public:  // interface
  /// Constructor
  Scalar(ScalarDescr* scalar_descr, ScalarData<T>* scalar_data) throw()
      : scalar_descr_(scalar_descr), scalar_data_(scalar_data) {}
  ...
  int index(std::string name) const { return scalar_descr_->index(name); }

  /// Return the name of the given scalar
  std::string name(int index) const { return scalar_descr_->name(index); }

  /// Return the length of the given scalar if array
  int length(int index) const { return scalar_descr_->length(index); }

  /// Return the number of variables currently stored (including scalar
  /// array elements)
  int size() const { return scalar_descr_->size(); }
  ...
  /// Allocate scalars
  void allocate() { return scalar_data_->allocate(scalar_descr_); }

  /// Return the specified scalar value
  T* value(int index) { return scalar_data_->value(scalar_descr_, index); }
