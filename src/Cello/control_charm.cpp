...
#define TRACE_SYNC(A)                                                          \
  if (TRACE_BLOCK) {                                                           \
    CkPrintf("%d %s:%d %s TRACE_SYNC %s entry %d id %d\n", CkMyPe(), __FILE__, \
             __LINE__, name_.c_str(), A, entry_point, id_sync);                \
    fflush(stdout);                                                            \
  }
#else
#define TRACE_CONTROL(A) ;
...
void Block::control_sync(int entry_point, int sync_type, int id_sync,
  ...
  if (sync_type == sync_quiescence) {
    ...
    control_sync_neighbor(entry_point, id_sync, min_face_rank, neighbor_type,
                          root_level);
    ...
    ERROR2("Block::control_sync()", "Unknown sync type %d neighbor type %d",
...
void Block::control_sync_quiescence(int entry_point) {
  if (index_.is_root()) CkStartQD(CkCallback(entry_point, proxy_main));
}
...
void Block::control_sync_neighbor(int entry_point, int id_sync,
                                  int min_face_rank, int neighbor_type,
                                  int root_level) {
  ...
  ASSERT1("control_sync()",
          "id %d must be specified for neighbor sync and >= 0", id_sync,
          (id_sync >= 0));
...
void Block::control_sync_count(int entry_point, int id_sync, int count) {
  ...
  CkPrintf("%s DEBUG_CONTROL control_sync_count %d %d %d/%d\n", name().c_str(),
           entry_point, id_sync, sync_count_[id_sync], sync_max_[id_sync]);
  fflush(stdout);
#endif
