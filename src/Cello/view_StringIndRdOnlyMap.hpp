...
class StringIndRdOnlyMap {
  ...
  ///
  /// We implement our own simple hash-table instead of using one of the
  /// standard library datastructures (i.e. ``std::map``) to avoid an extra
  /// level of indirection. This uses a simple open-addressed hash-table (with
  /// linear probing to address collisions). Since we don't allow the contents
  /// to be mutated, most of the negatives of an open-addressed hash-table do
  /// not apply.
  ///
...
public:
  /// Constructor
  StringIndRdOnlyMap() : data_(nullptr), num_keys_(0), capacity_(0), hash_() {}
  ...
  inline val_t operator[](const std::string& key) const noexcept {
    ...
    if (tmp.key == "") {
      ERROR1("StringIndRdOnlyMap::operator[]", "there is no key called %s",
             key.c_str());
    }
    return tmp.val;
  }
  ...
  inline const std::string& key(val_t i) const noexcept {
    if (i >= (num_keys_)) {
      ERROR2("StringIndRdOnlyMap::key",
             "Can't find key number %d. There only %d keys.", (int)i,
             (int)num_keys_);
    }
...
inline StringIndRdOnlyMap::StringIndRdOnlyMap(
  ...
  std::array<uint32_t, 12> allowed_caps = {7,  19, 31, 41,  53,  61,
                                           71, 83, 89, 101, 113, 127};
  ...
  for (std::size_t i = 0; i < keys.size(); i++) {
    ...
    if (pair.key == key) {
      ERROR1("StringIndRdOnlyMap::StringIndRdOnlyMap",
             "There can't be more than 1 key called %s.", key.c_str());
    }
    pair = {key, (val_t)i};
  }
}
