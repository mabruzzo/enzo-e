...
public:  // interface
  /// Create a new Solver
  Solver(std::string name, std::string field_x, std::string field_b,
         int monitor_iter, int restart_cycle, int solve_type, int index_prolong,
         int index_restrict, int min_level = 0,
  ...
  void set_sync_id(int sync_id) { id_sync_ = sync_id; }
...
protected:  // functions
  ...
  void monitor_output_(Block* block, int iter, double rr0 = 0.0,
                       double rr_min = 0.0, double rr = 0.0,
                       double rr_max = 0.0, bool final = false) throw();
  /// Add a new refresh object
  int add_refresh_();

  /// Perform vector copy X <- Y
  template <class T>
  void copy_(T* X, const T* Y, int mx, int my, int mz, bool active = true) const
      throw() {
    if (!active) return;
  ...
  int sync_id_() const throw() { return this->id_sync_; }
