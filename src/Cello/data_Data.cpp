...
Data::Data(int nx, int ny, int nz, int num_field_data, double xm, double xp,
           double ym, double yp, double zm, double zp, FieldDescr* field_descr,
           ParticleDescr* particle_descr) throw()
    : num_field_data_(num_field_data),
      field_data_(),
      particle_data_(),
      flux_data_() {
  if (field_descr == nullptr) field_descr = cello::field_descr();
  if (particle_descr == nullptr) particle_descr = cello::particle_descr();
...
void Data::field_cell_faces(double* x, double* y, double* z, int gx, int gy,
                            int gz, int cx, int cy, int cz) const {
...
void Data::field_cell_width(double* hx, double* hy, double* hz) const {
  double xm, ym, zm;
  this->lower(&xm, &ym, &zm);
  double xp, yp, zp;
  this->upper(&xp, &yp, &zp);
