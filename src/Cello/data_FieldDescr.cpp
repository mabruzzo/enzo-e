...
int FieldDescr::field_count() const throw() { return name_.size(); }
...
void FieldDescr::centering(int id_field, int* cx, int* cy, int* cz) const
    throw() {
  if (id_field >= 0) {
...
void FieldDescr::ghost_depth(int id_field, int* gx, int* gy, int* gz) const
    throw() {
  if (id_field >= 0) {
    int g3[3] = {ghost_depth_.at(id_field)[0], ghost_depth_.at(id_field)[1],
                 ghost_depth_.at(id_field)[2]};
    int gd[3] = {ghost_depth_default_[0], ghost_depth_default_[1],
