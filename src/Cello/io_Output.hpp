...
public:  // functions
  ...
  Schedule* schedule() throw() { return schedule_; }
  ...
  int stride_write() const throw() { return stride_write_; }

  int stride_wait() const throw() { return stride_wait_; }

  /// Return whether this process is a writer
  bool is_writer() const throw() { return (CkMyPe() == process_writer()); }
  ...
  void write_meta(Io* io) throw() { write_meta_(meta_type_file, io); }

  /// Write metadata to the current group in the file
  void write_meta_group(Io* io) throw() { write_meta_(meta_type_group, io); }
...
public:  // virtual functions
  ...
  /// Finalize output
  virtual void finalize() throw() { count_++; }
