...
Hierarchy* Factory::create_hierarchy(int refinement, int min_level,
...
IoBlock* Factory::create_io_block() const throw() { return new IoBlock; }
...
CProxy_Block Factory::new_block_proxy(DataMsg* data_msg, int nbx, int nby,
                                      int nbz) const throw() {
  TRACE3("Factory::new_block_array(na(%d %d %d)))", nbx, nby, nbz);
...
void Factory::create_block_array(DataMsg* data_msg, CProxy_Block proxy_block,
                                 int nbx, int nby, int nbz, int nx, int ny,
                                 int nz, int num_field_data) const throw() {
  TRACE7(
      "Factory::create_block_array(na(%d %d %d) n(%d %d %d) num_field_data %d)",
    ...
    for (int iy = 0; iy < nby; iy++) {
      for (int iz = 0; iz < nbz; iz++) {
        ...
        MsgRefine* msg = new MsgRefine(
            index, nx, ny, nz, num_field_data, count_adapt = 0, cycle, time, dt,
            refresh_same, num_face_level, face_level, nullptr);
...
void Factory::create_subblock_array(DataMsg* data_msg, CProxy_Block block_array,
                                    int min_level, int nbx, int nby, int nbz,
                                    int nx, int ny, int nz,
                                    int num_field_blocks) const throw() {
  TRACE8(
      "Factory::create_subblock_array(min_level %d na(%d %d %d) n(%d %d %d) "
      "num_field_blocks %d",
...
void Factory::create_block(DataMsg* data_msg, CProxy_Block block_array,
                           Index index, int nx, int ny, int nz,
                           int num_field_data, int count_adapt, int cycle,
                           double time, double dt, int narray, char* array,
                           int refresh_type, int num_face_level,
                           int* face_level, Adapt* adapt,
                           Simulation* simulation, int io_reader, int ip) const
    throw() {
  ...
  fflush(stdout);
#endif
  MsgRefine* msg = new MsgRefine(index, nx, ny, nz, num_field_data, count_adapt,
                                 cycle, time, dt, refresh_type, num_face_level,
                                 face_level, adapt, io_reader);
