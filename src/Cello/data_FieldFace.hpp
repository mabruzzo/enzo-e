...
public:  // interface
  ...
  void set_refresh_type(int refresh_type) { refresh_type_ = refresh_type; }

  Prolong* prolong() { return refresh_->prolong(); }

  Restrict* restrict() { return refresh_->restrict(); }
...
private:  // functions
  ...
  template <class T>
  size_t load_(T* array_face, const T* field_face, int nd3[3], int nf3[3],
               int im3[3], bool accumulate) throw();

  /// Precision-agnostic function for copying the field_face array into
  /// the field block ghosts; returns number of bytes copied
  template <class T>
  size_t store_(T* field_ghosts, const T* array_ghosts, int nd3[3], int nf3[3],
                int im3[3], bool accumulate) throw();

  /// Precision-agnostic function for copying a field block face
  /// into another block's ghost zones
  template <class T>
  void copy_(T* vd, int md3[3], int nd3[3], int id3[3], const T* vs, int ms3[3],
             int ns3[3], int is3[3], bool accumulate) throw();

  /// Multiply the given field by density to convert to conservative
  /// form if needed
  void mul_by_density_(Field field, int index_field, const int i3[3],
                       const int n3[3], const int m3[3]);

  /// Divide the given field by density to convert back to original
  /// form if needed
  void div_by_density_(Field field, int index_field, const int i3[3],
                       const int n3[3], const int m3[3]);
