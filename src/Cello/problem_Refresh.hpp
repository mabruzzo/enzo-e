...
public:  // interface
  ...
  Refresh(int ghost_depth, int min_face_rank, int neighbor_type, int sync_type,
          int sync_id, bool active = true) throw()
      : all_fields_(false),
        field_list_src_(),
        field_list_dst_(),
        all_particles_(false),
  ...
  void add_all_particles() { all_particles_ = true; }
  ...
  bool all_particles() const { return all_particles_; }

  /// Return whether, for all particle types participating
  /// in the refresh, all particles are copied to all
  /// neighbouring blocks
  bool particles_are_copied() const { return particles_are_copied_; }
  ...
  std::vector<int>& particle_list() { return particle_list_; }

  /// Add all data
  /// Add flux data
  void add_all_fluxes() { all_fluxes_ = true; }

  /// Return whether any (all) fluxes
  bool any_fluxes() const { return all_fluxes_; }
  ...
  void set_callback(int callback) { callback_ = callback; }
  ...
  void set_root_level(int root_level) { root_level_ = root_level; }

  /// Return the current minimum rank (dimension) of faces to refresh
  /// e.g. 0: everything, 1: omit corners, 2: omit corners and edges
  int min_face_rank() const { return min_face_rank_; }

  /// Set the ghost depth
  void set_ghost_depth(int ghost_depth) { ghost_depth_ = ghost_depth; }

  /// Set a new minimum rank (dimension) of faces to refresh
  /// e.g. 0: everything, 1: omit corners, 2: omit corners and edges
  void set_min_face_rank(int min_face_rank) { min_face_rank_ = min_face_rank; }

  /// Return the data field ghost depth
  int ghost_depth() const { return ghost_depth_; }
  ...
  /// non-leaf)
  int neighbor_type() const { return neighbor_type_; }
  ...
  int sync_type() const { return sync_type_; }

  // Return the id of the synchronization object (used for debugging
  // only)
  int sync_id() const { return sync_id_; }

  int sync_exit() const { return 3 * sync_id_ + 2; }
  ...
  void get_particle_bin_limits(int rank, int refresh_type, int if3[3],
                               int ic3[3], int lower[3], int upper[3]) {
      ...
      if (if3[axis] == -1) {
        ...
        if (refresh_type == refresh_same) {
          ...
          ERROR1("Refresh::get_particle_bin_limits()",
                 "unknown refresh_type %d", refresh_type);
        }
      }
    }
  }
  ...
  void set_id(int id_refresh) { id_refresh_ = id_refresh; }

  /// return the new refresh id in refresh_list_[]
  int id() const { return id_refresh_; }
  ...
  void set_prolong(int id_prolong) { id_prolong_ = id_prolong; }

  /// Return the prolongation operator for refresh
  Prolong* prolong();
  /// Return the prolongation id
  int index_prolong() const { return id_prolong_; }

  /// Set the restriction operator for refresh
  void set_restrict(int id_restrict) { id_restrict_ = id_restrict; }
