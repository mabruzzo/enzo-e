...
Output::~Output() throw() {
  ...
  delete io_block_;
  io_block_ = nullptr;
  delete io_field_data_;
  io_field_data_ = nullptr;
  delete io_particle_data_;
  io_particle_data_ = nullptr;
  delete it_field_index_;
  it_field_index_ = nullptr;
  delete it_particle_index_;
  it_particle_index_ = nullptr;
}
...
void Output::next() throw() { schedule_->next(); }

//======================================================================

std::string Output::expand_name_(const std::string* name_p,
                                 const std::vector<std::string>* args_p) const
    throw() {
  if (*name_p == "") return "";
  ...
  ASSERT1("Output::expand_name_", "File name %s cannot contain '\\%%'",
  ...
  ASSERT3("Output::expand_name_",
          ...
          count, name.c_str(), args.size(), args.size() == count);
...
void Output::write_simulation_(const Simulation* simulation) throw() {
#ifdef TRACE_OUTPUT
