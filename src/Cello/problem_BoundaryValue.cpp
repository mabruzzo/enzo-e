...
void BoundaryValue::enforce(Block* block, face_enum face, axis_enum axis) const
    throw() {
  ...
  if (!field.ghosts_allocated()) {
    ERROR("EnzoBoundary::enforce", "Function called with ghosts not allocated");
  }
    ...
    for (const std::string& field_name : field_list) {
      ...
      switch (precision) {
        case precision_single: {
          ...
          value.evaluate((float*)array + i0, t, ndx, nx, x + ix0, ndy, ny,
                         y + iy0, ndz, nz, z + iz0);
          if (mask_ != nullptr) {
            for (int i = 0; i < ndx * ndy * ndz; i++)
              ((float*)temp)[i] = ((float*)array)[i];
            delete[] ((float*)array);
            array = temp;
          }
        } break;
        case precision_double: {
          ...
          value.evaluate((double*)array + i0, t, ndx, nx, x + ix0, ndy, ny,
                         y + iy0, ndz, nz, z + iz0);
          if (mask_ != nullptr) {
            for (int i = 0; i < ndx * ndy * ndz; i++)
              ((double*)temp)[i] = ((double*)array)[i];
            delete[] ((double*)array);
            array = temp;
          }
        } break;
        ...
        case precision_quadruple: {
          ...
          value.evaluate((long double*)array + i0, t, ndx, nx, x + ix0, ndy, ny,
                         y + iy0, ndz, nz, z + iz0);
          if (mask_ != nullptr) {
            for (int i = 0; i < ndx * ndy * ndz; i++)
...
template <class T>
void BoundaryValue::copy_(T* field, double* value, int ndx, int ndy, int ndz,
                          int nx, int ny, int nz, int ix0, int iy0,
                          int iz0) const throw() {
