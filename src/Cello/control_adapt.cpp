...
#define TRACE_ADAPT(MSG, BLOCK)                                                \
  if (BLOCK->cycle() >= DEBUG_CYCLE_START) {                                   \
    CkPrintf(                                                                  \
        "TRACE_ADAPT %s %s step %d ready %d self conv %d neighb conv %d\n",    \
        BLOCK->name().c_str(), std::string(MSG).c_str(), BLOCK->adapt_step_,   \
        BLOCK->adapt_ready_ ? 1 : 0, BLOCK->adapt_.is_converged() ? 1 : 0,     \
...
void Block::adapt_begin_() {
  ...
  control_sync_neighbor(CkIndex_Block::p_adapt_called(), sync_id_adapt_begin,
                        min_face_rank, neighbor_leaf, 0);
}
...
void Block::adapt_barrier_() {
  if (!adapt_balanced_) {
    ...
    CkCallback callback =
        CkCallback(CkIndex_Block::r_adapt_next(nullptr), proxy_array());
    adapt_ready_ = true;
...
void Block::update_levels_() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    int level_max;
    bool can_coarsen;
    adapt_.get_neighbor_level_bounds(index_list[i], &level_min, &level_max,
                                     &can_coarsen);
...
void Block::adapt_refine_() {
  ...
  while (it_child.next(ic3)) {
    ...
    if (!is_child_(index_child)) {
      ...
      factory->create_block(data_msg, thisProxy, index_child, nx, ny, nz,
                            num_field_data, adapt_step_, cycle_, time_, dt_,
                            narray, array, refresh_fine, 27,
                            &child_face_level_curr_.data()[27 * IC3(ic3)],
                            &adapt_, cello::simulation());
...
void Block::adapt_send_level() {
  ...
  while (it_neighbor.next(of3)) {
    ...
    if (index_first[index_neighbor]) {
      msg_map[index_neighbor] =
          new MsgAdapt(adapt_step_, index_, ic3, of3, level, level_min,
                       level_max, can_coarsen);
    ...
    CkPrintf("DEBUG_ADAPT %s : %s send_level of3 %d %d %dic3 %d %d %d\n",
             name().c_str(), name(index_neighbor).c_str(), of3[0], of3[1],
             of3[2], ic3[0], ic3[1], ic3[2]);
#endif
...
void Block::p_adapt_recv_level(MsgAdapt* msg) {
  if (!adapt_ready_) {
    ...
    adapt_recv_level(msg->adapt_step_, msg->index_, msg->ic3_, msg->ofv_,
                     msg->level_now_, msg->level_min_, msg->level_max_,
                     msg->can_coarsen_);
    delete msg;
  }
}
...
void Block::adapt_recv_level(int adapt_step, Index index_send, int ic3[3],
                             std::vector<int> ofv[3], int level_face_curr,
                             int level_face_new, int level_face_max,
                             bool can_coarsen) {
  bool changed = false;
  int level_min;
  ...
  for (std::size_t i = 0; i < ofv[0].size(); i++) {
    ...
    adapt_.update_neighbor(index_send, level_face_new, level_face_max,
                           can_coarsen);
    ...
    adapt_.set_face_level_last(ic3, if3, level_face_new, level_face_max,
                               can_coarsen);
    ...
    if (!is_leaf()) {
      ERROR1("Block::p_adapt_recv_level()", "Block %s is not a leaf",
             name().c_str());
    }
    ...
    ASSERT2("adapt_recv_level()",
            ...
            level_face_curr, level, std::abs(level_face_curr - level) <= 1);

    const int level_diff = (level_face_curr - level);

    adapt_recv(index_send, of3, ic3, level_face_new, level_diff, level_face_max,
               can_coarsen);
...
void Block::adapt_recv(Index index_send, const int of3[3], const int ic3[3],
  ...
  if (level_relative == 0 || level_relative == +1) {
    ...
    while (it_face.next(jf3)) {
      ...
      while (it_child.next(jc3)) {
        Index index_child = index_.index_child(jc3);
        ItFace it_face_child =
            this->it_face(min_face_rank, index_child, jc3, jf3);
...
void Block::adapt_coarsen_() {
  ...
  ASSERT2("adapt_coarsen_()",
          ...
          is_leaf() ? "true" : "false", level, is_leaf() && level > 0);
...
void Block::p_adapt_delete() { ckDestroy(); }
...
void Block::initialize_child_face_levels_() {
  ...
  while (it_child.next(ic3)) {
    ...
    while (it_face.next(if3)) {
      ...
      const int level_face = adapt_.face_level(ip3, Adapt::LevelType::curr);
      int level_child_face = (inp == thisIndex) ? (level + 1) : level_face;
      if (refine_during_initialization(in)) level_child_face++;
...
bool Block::parent_face_(int ip3[3], const int if3[3], const int ic3[3]) const {
  bool retval = false;
  for (int i = 0; i < 3; i++) {
