...
public:  // interface
  /// Constructor
  Field(FieldDescr* field_descr, FieldData* field_data) throw()
      : field_descr_(field_descr), field_data_(field_data) {}
  ...
  void set_field_descr(FieldDescr* field_descr) { field_descr_ = field_descr; }
  ...
  /// Set padding
  void set_padding(int padding) throw() { field_descr_->set_padding(padding); }

  /// Set centering for a field
  void set_centering(int id, int cx, int cy = 0, int cz = 0) throw() {
    field_descr_->set_centering(id, cx, cy, cz);
  }

  /// Set ghost_depth for a field
  void set_ghost_depth(int id, int gx, int gy = 0, int gz = 0) throw() {
    field_descr_->set_ghost_depth(id, gx, gy, gz);
  }

  /// Set precision for a field
  void set_precision(int id, int precision) throw() {
  ...
  int field_count() const throw() { return field_descr_->field_count(); }
  ...
  Grouping* groups() { return field_descr_->groups(); }

  const Grouping* groups() const { return field_descr_->groups(); }

  /// alignment in bytes of fields in memory
  int alignment() const throw() { return field_descr_->alignment(); }

  /// padding in bytes between fields in memory
  int padding() const throw() { return field_descr_->padding(); }

  /// centering of given field
  void centering(int id, int* cx, int* cy = 0, int* cz = 0) const throw() {
    return field_descr_->centering(id, cx, cy, cz);
  }

  /// return whether the field variable is centered in the cell
  bool is_centered(int id) const { return field_descr_->is_centered(id); }

  /// depth of ghost zones of given field
  void ghost_depth(int id, int* gx, int* gy = 0, int* gz = 0) const throw() {
    return field_descr_->ghost_depth(id, gx, gy, gz);
  }

  /// Return precision of given field
  int precision(int id) const throw() { return field_descr_->precision(id); }

  /// Return the data type of a given field
  int data_type(int id) const throw() { return field_descr_->data_type(id); }
  ...
  int num_permanent() const throw() { return field_descr_->num_permanent(); }
  ...
  int num_temporary() const throw() { return field_descr_->num_temporary(); }
  ...
  int num_history() const { return field_descr_->num_history(); }
  ...
  void dimensions(int id_field, int* mx, int* my = 0, int* mz = 0) const
      throw() {
  ...
  const char* permanent() const throw() { return field_data_->permanent(); }
  ...
  void cell_width(double xm, double xp, double* hx, double ym = 0,
    ...
    field_data_->cell_width(xm, xp, hx, ym, yp, hy, zm, zp, hz);
  }

  /// Clear specified array(s) to specified value
  void clear(float value = 0.0, int id_first = -1, int id_last = -1) throw() {
  ...
  void allocate_coarse() throw() { field_data_->allocate_coarse(field_descr_); }
  void allocate_coarse(int id) throw() {
    field_data_->allocate_coarse(field_descr_, id);
  }

  /// Deallocate storage for the coarse fields
  void deallocate_coarse() throw() { field_data_->deallocate_coarse(); }
  /// Deallocate storage for the coarse fields
  void deallocate_coarse(int id) throw() { field_data_->deallocate_coarse(id); }
  ...
  void deallocate_permanent() throw() { field_data_->deallocate_permanent(); }
  ...
  /// a coarse field
  void coarse_dimensions(int id_field, int* nx = 0, int* ny = 0,
  ...
  int data_size() const { return field_data_->data_size(field_descr_); }
  ...
  void print(const char* message, bool use_file = false) const throw() {
