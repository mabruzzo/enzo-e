...
RefineDensity::RefineDensity(double min_refine, double max_coarsen,
...
int RefineDensity::apply(Block* block) throw() {
  ...
  if (precision == precision_single) {
    ...
    ERROR1("RefineDensity::apply()", "Unrecognized precision %d\n", precision);
...
//----------------------------------------------------------------------s
template <class T>
int RefineDensity::apply_(const T* array, int mx, int my, int mz, int gx,
                          int gy, int gz) const throw() {
  ...
  return any_refine ? adapt_refine : (all_coarsen ? adapt_coarsen : adapt_same);
}
