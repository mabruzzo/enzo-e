...
public:  // virtual functions
  ...
  virtual void file_read_scalar(void* buffer, std::string name,
                                int* s_type) throw();

  /// Read a metadata item associated with the file
  virtual void file_read_meta(void* buffer, std::string name, int* s_type,
                              int* n1 = 0, int* n2 = 0, int* n3 = 0,
                              int* n4 = 0) throw();

  /// Write a metadata item associated with the file
  virtual void file_write_meta(const void* buffer, std::string name, int type,
                               int n1 = 1, int n2 = 0, int n3 = 0,
                               int n4 = 0) throw() {
  ...
  virtual void data_create(std::string name, int type, int m1 = 1, int m2 = 0,
                           int m3 = 0, int m4 = 0, int n1 = 0, int n2 = 0,
                           int n3 = 0, int n4 = 0, int o1 = 0, int o2 = 0,
                           int o3 = 0, int o4 = 0) throw();

  /// Open an existing dataset for reading
  virtual void data_open(std::string name, int* type, int* m1 = 0, int* m2 = 0,
                         int* m3 = 0, int* m4 = 0) throw();

  /// Select a subset of the data
  virtual void data_slice(int m1, int m2, int m3, int m4, int n1, int n2,
  ...
  virtual void data_read_meta(void* buffer, std::string name, int* s_type,
                              int* n1 = 0, int* n2 = 0, int* n3 = 0,
                              int* n4 = 0) throw();

  /// Write a metadata item associated with the opened dataset
  virtual void data_write_meta(const void* buffer, std::string name, int type,
                               int n1 = 1, int n2 = 0, int n3 = 0,
                               int n4 = 0) throw() {
  ...
  /// Create memory space
  virtual void mem_create(int mx, int my, int mz, int nx, int ny, int nz,
                          int gx, int gy, int gz);

  virtual void mem_close() { space_close_(mem_space_id_); }
  ...
  void group_read_meta(void* buffer, std::string name, int* s_type, int* n1 = 0,
                       int* n2 = 0, int* n3 = 0, int* n4 = 0) throw();

  /// Write a metadata item associated with the opened group
  virtual void group_write_meta(const void* buffer, std::string name, int type,
                                int n1 = 1, int n2 = 0, int n3 = 0,
                                int n4 = 0) throw() {
...
public:  // functions
  ...
  char* allocate_buffer(int n, int type_data) {
    ...
    if (type_data == type_single) {
      ...
      data = nullptr;
      ERROR1("EnzoInitialHdf5::allocate_array_()", "Unsupported data type %d",
             type_data);
    }
    return data;
  }
...
protected:  // functions
  virtual void write_meta_(hdf5_id id, const void* buffer, std::string name,
                           int type, int n1 = 1, int n2 = 0, int n3 = 0,
                           int n4 = 0) throw();
...
private:  // functions
  ...
  std::string relative_to_absolute_(std::string path_relative,
                                    std::string path_current) const throw();

  /// Get output extents
  void get_extents_(hdf5_id space_id, int* n1, int* n2 = 0, int* n3 = 0,
                    int* n4 = 0) throw();

  /// create the space for the array on disk

  /// create data spaces for memory or disk data
  hdf5_id space_create_(int m1, int m2, int m3, int m4, int n1, int n2, int n3,
                        int n4, int o1, int o2, int o3, int o4) throw();

  /// given a space, select a slice
  hdf5_id space_slice_(hdf5_id space_id, int m1, int m2, int m3, int m4, int n1,
                       int n2, int n3, int n4, int o1, int o2, int o3,
                       int o4) throw();
