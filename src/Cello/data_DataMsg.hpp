...
public:  // interface
  ...
  DataMsg& operator=(const DataMsg& data_msg) throw() {
    ERROR("DataMsg::operator=()", "This method should not be called");
    return *this;
  }

  void pup(PUP::er& p) {
    WARNING("DataMsg::pup()", "DataMsg::pup() should never be called!");
  }
  ...
  char* field_array() { return field_array_u_; }

  /// Return the FieldData
  FieldData* field_data() { return field_data_u_; }
  ...
  FaceFluxes* face_fluxes(unsigned i) { return face_fluxes_list_[i]; }

  /// Return the number of FaceFluxes
  unsigned num_face_fluxes() const { return face_fluxes_list_.size(); }
  ...
  void set_coarse_array(Field field, int iam3[3], int iap3[3], int ifms3[3],
