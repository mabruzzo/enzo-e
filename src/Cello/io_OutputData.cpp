...
OutputData::OutputData(int index, const Factory* factory,
                       Config* config) throw()
    : Output(index, factory), text_block_count_(0) {
...
OutputData::~OutputData() throw() { close(); }
...
void OutputData::write_block(const Block* block) throw() {
  ...
  proxy_main.p_text_file_write(strlen(dir) + 1, dir, strlen(file) + 1, file,
                               strlen(line) + 1, line, count);
...
void OutputData::write_field_data(const FieldData* field_data,
  ...
  io_field_data()->field_array(&buffer, &name, &type, &nxd, &nyd, &nzd, &nx,
                               &ny, &nz);
...
void OutputData::write_particle_data(const ParticleData* particle_data,
                                     int it) throw() {
