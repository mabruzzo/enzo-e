...
FieldData::FieldData(const FieldDescr* field_descr, int nx, int ny,
                     int nz) throw()
    : array_permanent_(),
      temporary_size_(),
      array_temporary_(),
...
FieldData::~FieldData() throw() { deallocate_permanent(); }
...
void FieldData::dimensions(const FieldDescr* field_descr, int id_field, int* mx,
                           int* my, int* mz) const throw() {
  int nx, ny, nz;
  int gx, gy, gz;
  int cx, cy, cz;
  size(&nx, &ny, &nz);
...
const char* FieldData::values(const FieldDescr* field_descr, int id_field,
                              int index_history) const throw() {
  return (const char*)((FieldData*)this)
      ->values(field_descr, id_field, index_history);
}

//----------------------------------------------------------------------

char* FieldData::values(const FieldDescr* field_descr, int id_field,
                        int index_history) throw() {
  char* values = nullptr;
...
const char* FieldData::coarse_values(const FieldDescr* field_descr,
                                     int id_field, int index_history) const
    throw() {
  return (const char*)((FieldData*)this)
      ->coarse_values(field_descr, id_field, index_history);
}

//----------------------------------------------------------------------

char* FieldData::coarse_values(const FieldDescr* field_descr, int id_field,
                               int index_history) throw() {
  ...
  CkPrintf("DEBUG_COARSE_ARRAY %p returning %p[%d]\n", (void*)this,
           (void*)array_coarse_[id_field], id_field);
#endif
...
const char* FieldData::unknowns(const FieldDescr* field_descr, int id_field,
                                int index_history) const throw() {
  return (const char*)((FieldData*)this)->unknowns(field_descr, id_field);
}

//----------------------------------------------------------------------

char* FieldData::unknowns(const FieldDescr* field_descr, int id_field,
                          int index_history) throw() {
...
void FieldData::cell_width(double xm, double xp, double* hx, double ym,
                           double yp, double* hy, double zm, double zp,
                           double* hz) const throw() {
...
void FieldData::clear(const FieldDescr* field_descr, float value,
                      int id_field_first, int id_field_last) throw() {
  if (permanent_allocated()) {
    ...
    for (int id_field = id_field_first; id_field <= id_field_last; id_field++) {
      int nx, ny, nz;
      ...
      switch (precision) {
        ...
        case precision_quadruple:
          ...
          break;
        default:
          ERROR1("FieldData::clear",
                 "Clear called with unsupported precision %s",
                 cello::precision_name[precision]);
      }
    }
  } else {
    ERROR("FieldData::clear", "Called clear with unallocated arrays");
  }
}

//----------------------------------------------------------------------

void FieldData::allocate_permanent(const FieldDescr* field_descr,
...
void FieldData::allocate_coarse(const FieldDescr* field_descr,
                                int id_field) throw()
...
void FieldData::deallocate_coarse() throw() {
  for (size_t index_field = 0; index_field < array_coarse_.size();
       index_field++) {
...
void FieldData::reallocate_permanent(const FieldDescr* field_descr,
                                     bool ghosts_allocated) throw() {
  if (!permanent_allocated()) {
...
int FieldData::field_size(const FieldDescr* field_descr, int id_field, int* nx,
                          int* ny, int* nz) const throw() {
...
void FieldData::coarse_dimensions(const FieldDescr* field_descr, int id_field,
                                  int* mcx, int* mcy, int* mcz) const throw() {
...
void FieldData::print(const FieldDescr* field_descr, const char* message,
  ...
  ASSERT("FieldData::print", "FieldData not allocated", permanent_allocated());
  ...
  for (int index_field = 0; index_field < field_count; index_field++) {
    ...
    const char* field_name =
        strdup(field_descr->field_name(index_field).c_str());
    ...
    switch (field_descr->precision(index_field)) {
      case precision_single:
        print_((const float*)array_offset, field_name, message,  // lower,
               fp, ixm, iym, izm, ixp, iyp, izp, nx, ny, nz, gx, gy, gz, nxd,
               nyd);
        break;
      case precision_double:
        print_((const double*)array_offset, field_name, message,  // lower,
               fp, ixm, iym, izm, ixp, iyp, izp, nx, ny, nz, gx, gy, gz, nxd,
               nyd);
        break;
      case precision_quadruple:
        print_((const long double*)array_offset, field_name, message,  // lower,
               fp, ixm, iym, izm, ixp, iyp, izp, nx, ny, nz, gx, gy, gz, nxd,
               nyd);
        break;
      default:
...
double FieldData::dot(const FieldDescr* field_descr, int ix, int iy) throw() {
  ...
  switch (field_descr->precision(ix)) {
    ...
    case precision_double:
      ...
      break;
    case precision_quadruple:
      return dot_((long double*)x, (long double*)y, mx, my, mz, nx, ny, nz, gx,
                  gy, gz);
      break;
    default:
      ERROR2("FieldData::dot()", "Unknown precision %d for field id %d",
...
template <class T>
long double FieldData::dot_(const T* X, const T* Y, int mx, int my, int mz,
                            int nx, int ny, int nz, int gx, int gy,
                            int gz) const throw() {
...
void FieldData::scale(const FieldDescr* field_descr, int iy, long double a,
                      int ix, bool ghosts) throw() {
  ASSERT2("FieldData::scale()", "Calling scale on illegal fields: ix=%d iy=%d",
          ix, iy, (ix >= 0) && (iy >= 0));
  ...
  switch (field_descr->precision(ix)) {
    case precision_single:
      scale_((float*)y, a, (float*)x, ghosts, mx, my, mz, nx, ny, nz, gx, gy,
             gz);
      break;
    case precision_double:
      scale_((double*)y, a, (double*)x, ghosts, mx, my, mz, nx, ny, nz, gx, gy,
             gz);
      break;
    case precision_quadruple:
      scale_((long double*)y, a, (long double*)x, ghosts, mx, my, mz, nx, ny,
             nz, gx, gy, gz);
      break;
    default:
      ERROR2("FieldData::scale()", "Unknown precision %d for field id %d",
             field_descr->precision(ix), ix);
      break;
  }
}
...
template <class T>
void FieldData::scale_(T* Y, long double a, T* X, bool ghosts, int mx, int my,
...
void FieldData::units_scale_cgs(const FieldDescr* field_descr, int id,
                                double amount) {
  ...
  ASSERT1("FieldData::units_scale_cgs()", "Trying to scale field %d by 0.0", id,
          (amount != 0.0));
...
void FieldData::units_scale_code(const FieldDescr* field_descr, int id,
                                 double amount) {
  // Allocate units scaling for this field if not allocated
  units_allocate_(id);
  ...
  if (units_scaling_[id] != 1.0) {
    ...
    if (units_scaling_[id] != amount) {
      WARNING3("FieldData::units_scale_code()",
               "new scaling factor %g differs from old scaling factor %g for "
               "field %d\n",
...
char* FieldData::save_data(FieldDescr* field_descr, char* buffer) const {
  union {
    int* pi;
    char* pc;
  };
  ...
  ASSERT2("FieldData::save_data()", "Buffer has size %ld but expecting size %d",
...
char* FieldData::load_data(FieldDescr* field_descr, char* buffer) {
  union {
    int* pi;
    char* pc;
  };
  ...
  ASSERT2("FieldData::load_data()", "Buffer has size %ld but expecting size %d",
          (pc - buffer), data_size(field_descr),
          ((pc - buffer) == data_size(field_descr)));
...
int FieldData::adjust_padding_(int size, int padding) const throw() {
  return size + padding;
}

//----------------------------------------------------------------------

int FieldData::adjust_alignment_(int size, int alignment) const throw() {
...
int FieldData::align_padding_(int alignment) const throw() {
  long unsigned start_long =
      reinterpret_cast<long unsigned>(&array_permanent_[0]);
  return (alignment - (start_long % alignment)) % alignment;
}

template <class T>
void FieldData::print_(const T* field, const char* field_name,
                       const char* message,
                       // double lower [3],
                       FILE* fp, int ixm, int iym, int izm, int ixp, int iyp,
                       int izp, int nx, int ny, int nz, int gx, int gy, int gz,
  ...
  fprintf(fp, "%s [%s] %18.14Lg %18.14Lg %18.14Lg\n", message ? message : "",
          field_name, (long double)(min), (long double)(avg),
          (long double)(max));
}

//----------------------------------------------------------------------

void FieldData::restore_permanent_(const FieldDescr* field_descr,
  ...
  for (int id_field = 0; id_field < field_descr->field_count(); id_field++) {
    ...
    int offset1 =
        (nx1 - nx2) / 2 + nx1 * ((ny1 - ny2) / 2 + ny1 * (nz1 - nz2) / 2);
    offset1 = MAX(offset1, 0);

    int offset2 =
        (nx2 - nx1) / 2 + nx2 * ((ny2 - ny1) / 2 + ny2 * (nz2 - nz1) / 2);
...
template <class T>
CelloView<T, 3> FieldData::make_view_(const FieldDescr* field_descr,
...
template CelloView<float, 3> FieldData::make_view_(
    const FieldDescr* field_descr, int id_field, ghost_choice choice,
    int index_history, bool coarse) throw();
