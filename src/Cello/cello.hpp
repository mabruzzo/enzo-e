...
inline int INDEX2(int ix, int iy, int nx) { return ix + nx * iy; }
...
enum face_enum { face_lower = 0, face_upper = 1, face_all };

/// @enum     axis_enum
/// @brief    Axis [x|y|z]
enum axis_enum { axis_x = 0, axis_y = 1, axis_z = 2, axis_all };
...
/// @enum type_enum
/// @brief list of known scalar types, including ints as well as floats, used
/// for Field types and Particle attributes
enum type_enum {
...
#define DEBUG_PRINT_ARRAY_(NAME, ARRAY, m3, n3, ox, oy, oz)                    \
  {                                                                            \
    ...
    CkPrintf("m3 %d %d %d n3 %d %d %d o3 %d %d %d\n", m3[0], m3[1], m3[2],     \
             n3[0], n3[1], n3[2], ox, oy, oz);                                 \
    const int o = ox + m3[0] * (oy + m3[1] * oz);                              \
    for (int iz = 0; iz < n3[2]; iz++) {                                       \
      for (int iy = 0; iy < n3[1]; iy++) {                                     \
        CkPrintf("PADDED_ARRAY_VALUES %s %p %d %d %d: ", NAME, (void*)ARRAY,   \
                 0, iy, iz);                                                   \
...
#define SIZE_MAP_TYPE(COUNT, TYPE_1, TYPE_2, MAP)                              \
  {                                                                            \
    (COUNT) += sizeof(int);                                                    \
    (COUNT) += (MAP).size() * (sizeof(TYPE_1) + sizeof(TYPE_2));               \
  }
...
template <class T>
T sum(const T* array, int mx, int my, int mz, int ox, int oy, int oz, int nx,
      int ny, int nz) {
  T s = 0.0;
...
template <class T>
void copy(T* array_d, int mdx, int mdy, int mdz, int odx, int ody, int odz,
          const T* array_s, int msx, int msy, int msz, int osx, int osy,
          int osz, int nx, int ny, int nz) {
...
template <class T>
void check(T value, const char* message, const char* file, int line) {
...
inline int index_static() { return CkMyPe() % CONFIG_NODE_SIZE; }
...
std::string expand_name(const std::vector<std::string>* file_name, int counter,
...
std::string create_directory(const std::vector<std::string>* path_name,
