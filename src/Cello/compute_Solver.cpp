...
const char* solve_string[] = {
    ...
    "solve_tree",   // Solve in a root-level octree (e.g. for domain
                    // decomposition)
...
Solver::Solver(std::string name, std::string field_x, std::string field_b,
...
int Solver::add_refresh_() {
  ...
  Refresh refresh(ghost_depth, min_face_rank, neighbor_type_(), sync_type_(),
                  0);
...
void Solver::monitor_output_(Block* block, int iter, double rr0, double rr_min,
                             double rr, double rr_max, bool final) throw() {
  Monitor* monitor = cello::monitor();

  monitor->print(
      "Solver", "%s %s iter %04d  err %.16g [%g %g]", this->name().c_str(),
      final ? "final" : "", iter, (rr0 != 0) ? (rr / rr0) : 0.0,
      (rr0 != 0) ? (rr_min / rr0) : 0.0, (rr0 != 0) ? (rr_max / rr0) : 0.0);
}
...
bool Solver::reuse_solution_(int cycle) const throw() {
  ...
  return (solve_type_ == solve_tree ||
          ((cycle > 0) &&
           ((restart_cycle_ == 0) || (cycle % restart_cycle_) != 0)));
}
...
void Solver::end_(Block* block) {
  ...
  ASSERT2("Solver::end_()", "Solver mismatch was %d expected %d", index, index_,
          (index == index_));

  CkCallback(callback_, CkArrayIndexIndex(block->index()), block->proxy_array())
      .send();
}
...
bool Solver::is_finest_(Block* block) const {
  switch (solve_enum(solve_type_)) {
    ...
    case solve_block:
      ...
      break;
    default:
      ERROR2("Solver::is_finest_()", "Unexpected solve_type %d in Solver %s",
