...
void mutex_init_field_face() { field_face_node_lock = CmiCreateLock(); }
...
#define TRACE_FIELD_FACE(MSG)                                                  \
  CkPrintf("TRACE_FIELD_FACE %d %s:%d %p %d %s\n", CkMyPe(), __FILE__,         \
           __LINE__, this, FieldFace::counter[cello::index_static()], MSG);
#else
#define TRACE_FIELD_FACE(MSG) /* ... */
#endif
    ...
    if (n3[0] > 6 && (n3[1] > 6 || n3[1] == 1) && (n3[2] > 6 || n3[2] == 1)) { \
      ...
      for (int iz = 0; iz < n3[2]; iz++) {                                     \
        for (int iy = 0; iy < n3[1]; iy++) {                                   \
          if (DEBUG_PRINT)                                                     \
            CkPrintf("PADDED_ARRAY_VALUES %s %p %d %d %d: ", NAME,             \
                     (void*)ARRAY, 0, iy, iz);                                 \
...
extern "C" void FORTRAN_NAME(field_face_store_4)(float* field, float* array,
                                                 int* m3, int* n3,
                                                 int* accumulate);
extern "C" void FORTRAN_NAME(field_face_store_8)(double* field, double* array,
                                                 int* m3, int* n3,
                                                 int* accumulate);
extern "C" void FORTRAN_NAME(field_face_store_16)(long double* field,
                                                  long double* array, int* m3,
                                                  int* n3, int* accumulate);

enum enum_op_type { op_unknown, op_load, op_store };
...
void FieldFace::face_to_array(Field field, int* n, char** array) throw() {
  ASSERT("FieldFace::face_to_array()", "field_src.size() must be > 0",
         refresh_->any_fields());

  *n = num_bytes_array(field);
  *array = new char[*n];

  ASSERT("FieldFace::face_to_array()", "array size must be > 0", *n > 0);
...
void FieldFace::face_to_array(Field field, char* array) throw() {
  ...
  for (size_t i_f = 0; i_f < field_list_src.size(); i_f++) {
    ...
    if (refresh_type_ == refresh_coarse) {
      ...
      index_array += restrict()->apply(precision, array_face, nc3, i3_array,
                                       nc3, field_face, m3, i3, n3);
...
void FieldFace::array_to_face(char* array, Field field) throw() {
  ...
  for (size_t i_f = 0; i_f < field_list_dst.size(); i_f++) {
    ...
    if (refresh_type_ == refresh_fine) {
      ...
      ASSERT("FieldFace::array_to_face()", "No prolongation operator",
             (prolong() != nullptr));

      int ic3[3];
      int nc3[3];
      TRACE_ONCE;
      box.get_start_size(ic3, nc3, BlockType::send, BlockType::send,
                         lpad = true);
      ...
      prolong()->apply(precision, field_ghost, m3, i3, n3, array_ghost, mc3,
                       ic3, nc3, accumulate);

#ifdef DEBUG_ARRAY
      CkPrintf("field %lu\n", i_f);
#endif
      DEBUG_PRINT_ARRAY0("array_to_face array_ghost",
                         ((cello_float*)array_ghost), nc3, nc3, ic3);
      DEBUG_PRINT_ARRAY0("array_to_face field_ghost",
                         ((cello_float*)field_ghost), m3, n3, i3);

      index_array +=
          cello::sizeof_precision(precision) * nc3[0] * nc3[1] * nc3[2];
...
void FieldFace::face_to_face(Field field_src, Field field_dst) {
  ...
  for (size_t i_f = 0; i_f < field_list_src.size(); i_f++) {
    ...
    int id3[3], nd3[3];
    TRACE_ONCE;
    box.get_start_size(is3, ns3, BlockType::send, BlockType::send, lpad = true);
    ...
    if (refresh_type_ == refresh_fine) {
      ...
      prolong()->apply(precision, values_dst, m3, id3, nd3, values_src, m3, is3,
                       ns3, accumulate);

#ifdef DEBUG_ARRAY
      CkPrintf("field %lu\n", i_f);
#endif
      DEBUG_PRINT_ARRAY0("face_to_face values_src", ((cello_float*)values_src),
                         m3, ns3, is3);
      DEBUG_PRINT_ARRAY0("face_to_face values_dst", ((cello_float*)values_dst),
                         m3, nd3, id3);

    } else if (refresh_type_ == refresh_coarse) {
      // Restrict field

      restrict()->apply(precision, values_dst, m3, id3, nd3, values_src, m3,
                        is3, ns3, accumulate);
...
int FieldFace::num_bytes_array(Field field) throw() {
  ...
  ASSERT("FieldFace::num_bytes_array()",
         "array_size must be > 0, maybe field_list.size() is 0?", array_size);
...
char* FieldFace::save_data(char* buffer) const {
  ...
  ASSERT2("FieldFace::save_data()", "Buffer has size %ld but expecting size %d",
          (p - buffer), data_size(), ((p - buffer) == data_size()));
...
template <class T>
size_t FieldFace::load_(T* array_face, const T* field_face, int m3[3],
                        int n3[3], int i3[3], bool accumulate) throw() {
...
template <class T>
size_t FieldFace::store_(T* ghost, const T* array, int m3[3], int n3[3],
                         int i3[3], bool accumulate) throw() {
#ifdef FORTRAN_STORE
  ...
  if (use_fortran_store && (sizeof(T) != sizeof(long double))) {
    if (sizeof(T) == sizeof(float)) {
      FORTRAN_NAME(field_face_store_4)
      (ghost_4 + im, array_4, m3, n3, &iaccumulate);
    } else if (sizeof(T) == sizeof(double)) {
      FORTRAN_NAME(field_face_store_8)
      (ghost_8 + im, array_8, m3, n3, &iaccumulate);
    } else if (sizeof(T) == sizeof(long double)) {
      FORTRAN_NAME(field_face_store_16)
      (ghost_16 + im, array_16, m3, n3, &iaccumulate);
    } else {
      ERROR1("FieldFace::store_()", "unknown float precision sizeof(T) = %lu\n",
             sizeof(T));
    }
  } else {
    if (accumulate) {
...
template <class T>
void FieldFace::copy_(T* vd, int md3[3], int nd3[3], int id3[3], const T* vs,
                      int ms3[3], int ns3[3], int is3[3],
...
void FieldFace::mul_by_density_(Field field, int index_field, const int i3[3],
                                const int n3[3], const int m3[3]) {
...
void FieldFace::div_by_density_(Field field, int index_field, const int i3[3],
                                const int n3[3], const int m3[3]) {
  if (field.is_temporary(index_field)) return;
