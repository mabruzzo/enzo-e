...
const std::map<std::string, double (*)(double)> Param::function_map = {
    ...
    {"atan", atan},   {"atanh", atanh}, {"cbrt", cbrt},     {"ceil", ceil},
    {"cos", cos},     {"cosh", cosh},   {"erfc", erfc},     {"erf", erf},
    {"exp", exp},     {"expm1", expm1}, {"fabs", fabs},     {"floor", floor},
    {"j0", j0},       {"j1", j1},       {"lgamma", lgamma}, {"log10", log10},
    {"log1p", log1p}, {"logb", logb},   {"log", log},       {"sin", sin},
    {"sinh", sinh},   {"sqrt", sqrt},   {"tan", tan},       {"tanh", tanh},
    {"y0", y0},       {"y1", y1},       {"rint", rint},
};
  ...
  if (l_expr) {
    ...
    switch ((*node)->type) {
      ...
      case enum_node_function:
        ...
        break;
      default:
        WARNING("Param::pup_expr_()", "Unknown node type");
        break;
    }
...
void Param::write(FILE* file_pointer, std::string full_parameter,
                  int write_type)
...
/// @param full_parameter  Name of this parameter including groups
{
  ...
  std::string parameter =
      (i_group == std::string::npos)
          ? full_parameter
          : full_parameter.substr(i_group + 1, std::string::npos);
...
void Param::evaluate_float(int n, double* result, double* x, double* y,
                           double* z, double t, struct node_expr* node)
...
/// @param t time value
{
  ...
  switch (node->type) {
    case enum_node_operation:
      ASSERT("Param::evaluate_float()", "node is NULL", (node != NULL));
      ...
      switch (node->op_value) {
        ...
        case enum_op_div:
          ...
          break;
        case enum_op_pow:
          for (i = 0; i < n; i++) result[i] = pow(left[i], right[i]);
          break;
        default:
        case enum_op_le:
        case enum_op_lt:
    ...
    case enum_node_variable:
      switch (node->var_value) {
        case 'x':
          if (x)
            for (i = 0; i < n; i++) result[i] = x[i];
          break;
        case 'y':
          if (y)
            for (i = 0; i < n; i++) result[i] = y[i];
          break;
        case 'z':
          if (z)
            for (i = 0; i < n; i++) result[i] = z[i];
          break;
        case 't':
          for (i = 0; i < n; i++) result[i] = t;
          break;
        default:
          ERROR1("Param::evaluate_float",
      ...
      break;
    case enum_node_function:
      ASSERT2("Param::evaluate_float()", "Error in function %p: left %p",
              node ? node->fun_value : NULL, left, left != NULL);
      for (i = 0; i < n; i++) result[i] = (*(node->fun_value))(left[i]);
      break;
    case enum_node_unknown:
    default:
      ERROR1("Param::evaluate_float", "unknown expression type %d", node->type);
      break;
  }
...
void Param::evaluate_logical(int n, bool* result, double* x, double* y,
                             double* z, double t, struct node_expr* node)
/// @param node Head node of the tree defining the floating-point expression
/// @param n Length of the result buffer
/// @param result Array in which to store the expression evaluations
...
/// @param t Array of time values
{
  ...
  if (node->left && (node->left->type == enum_node_operation)) {
    ...
    if ((node->op_value == enum_op_and) || (node->op_value == enum_op_or)) {
  ...
  if (node->type == enum_node_operation) {
    ...
    ASSERT3("Param::evaluate_logical()",
            "Error in operation %d: left_float %p right_float %p", op,
            left_float, right_float,
            (op == enum_op_and || op == enum_op_or) ||
                (left_float != NULL && right_float != NULL));
    ASSERT3("Param::evaluate_logical()",
            "Error in operation %d: left_logical %p right_logical %p", op,
    ...
    switch (op) {
      ...
      case enum_op_or:
        ...
        break;
      default:
        ERROR1("Param::evaluate_logical", "unknown expression type %d",
               node->type);
        break;
    }
  }
