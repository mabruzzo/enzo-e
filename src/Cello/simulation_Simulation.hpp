...
public:  // interface
  ...
  Simulation(const char* parameter_file, int n);
  ...
  int rank() const throw() { return rank_; }

  /// Return the Problem container object
  Problem* problem() const throw() { return problem_; }

  /// Return the Hierarchy
  Hierarchy* hierarchy() const throw() { return hierarchy_; }

  /// Return the Parameters
  Parameters* parameters() const throw() { return parameters_; }

  /// Return the Config
  const Config* config() const throw() { return config_; }

  /// Return the scalar descriptors
  ScalarDescr* scalar_descr_long_double() throw() {
    return scalar_descr_long_double_;
  }
  ScalarDescr* scalar_descr_double() throw() { return scalar_descr_double_; }
  ScalarDescr* scalar_descr_int() throw() { return scalar_descr_int_; }
  ScalarDescr* scalar_descr_long_long() throw() {
    return scalar_descr_long_long_;
  }
  ScalarDescr* scalar_descr_sync() throw() { return scalar_descr_sync_; }
  ScalarDescr* scalar_descr_void() throw() { return scalar_descr_void_; }
  ScalarDescr* scalar_descr_index() throw() { return scalar_descr_index_; }

  /// Return the field descriptor
  FieldDescr* field_descr() const throw() { return field_descr_; }

  /// Return the particle descriptor
  ParticleDescr* particle_descr() const throw() { return particle_descr_; }

  /// Return the performance object associated with each cycle
  Performance* performance() throw() { return performance_; }

  /// Return the monitor object
  Monitor* monitor() const throw() { return monitor_; }

  void set_cycle(int cycle) throw() { cycle_ = cycle; }
  void set_initial_cycle(int cycle) throw() { cycle_initial_ = cycle; }
  void set_time(double time) throw() { time_ = time; }
  void set_dt(double dt) throw() { dt_ = dt; }
  void set_stop(bool stop) throw() { stop_ = stop; }
  ...
  int initial_cycle() const throw() { return cycle_initial_; };
  ...
  int phase() const throw() { return phase_; };

  /// Return the current phase of the simulation
  void set_phase(int phase) const throw() { phase_ = phase; };

  /// Return the load balancing schedule
  Schedule* schedule_balance() const throw() { return schedule_balance_; };
  ...
  void set_projections_tracing(bool value) { projections_tracing_ = value; }

  bool projections_tracing() const { return projections_tracing_; }

  Schedule* projections_schedule_on() const { return projections_schedule_on_; }
...
public:  // virtual functions
  ...
  void output_enter();
  void p_output_start(int index_output) { output_start(index_output); }
  ...
  void p_monitor_performance() { monitor_performance(); };
  ...
  int new_register_refresh(const Refresh& refresh) {
    ...
    ASSERT("Simulation::new_register_refresh()", "id_refresh must be >= 0",
           (id_refresh >= 0));
  ...
  void refresh_set_name(int id, std::string name) {
    if (id >= int(refresh_name_.size())) refresh_name_.resize(id + 1);
    refresh_name_[id] = name;
  }

  std::string refresh_name(int id) const {
    return (0 <= id && id < int(refresh_name_.size())) ? refresh_name_[id]
                                                       : "UNKNOWN";
  }

  /// Return the given refresh object
  Refresh& refresh_list(int id_refresh) { return refresh_list_[id_refresh]; }

  /// Return the number of refresh objects registered
  int refresh_count() const { return refresh_list_.size(); }
...
protected:  // functions
  ...
  Schedule* create_schedule_(std::string var, std::string type, double start,
                             double stop, double step);

  void create_checkpoint_link() {
    if (CkMyPe() == 0) {
      CkPrintf("creating symlink %s -> %s\n", dir_checkpoint_, "Checkpoint");
