...
#define N 8
  ...
  int trace[][3] = {{0, 0, 0}, {1, 0, 0}, {0, 1, 0}, {0, 0, 1},
  ...
  for (int i = 1; i < N; i++) {
    ...
    unit_func("child");
    unit_assert(c3[0] == trace[i - 1][0] && c3[1] == trace[i - 1][1] &&
                c3[2] == trace[i - 1][2]);
    unit_func("index_child");
    unit_assert(i8[i].index_child(trace[i][0], trace[i][1], trace[i][2]) ==
                i8[i + 1]);
  }
  ...
  for (int iz = 0; iz < nz; iz += 2) {
    for (int iy = 0; iy < ny; iy += 2) {
      for (int ix = 0; ix < nx; ix += 2) {
        ...
        l_equal =
            l_equal && (p == index_root[i + dx + dy + dz].index_parent(-2));
        ...
        l_equal =
            l_equal && (p != index_root[i + 2 * dx + dy].index_parent(-2));
        l_equal =
            l_equal && (p != index_root[i + 2 * dx + dz].index_parent(-2));
        l_equal =
            l_equal && (p != index_root[i + 2 * dz + dy].index_parent(-2));
      }
    }
  }
  unit_assert(l_equal);
  ...
  for (int iz = 0; iz < nz; iz += 4) {
    for (int iy = 0; iy < ny; iy += 4) {
      for (int ix = 0; ix < nx; ix += 4) {
        ...
        l_equal = l_equal &&
                  (p == index_root[i + dx].index_parent(-2).index_parent(-2));
        l_equal =
            l_equal &&
            (p == index_root[i + 2 * dx].index_parent(-2).index_parent(-2));
        l_equal =
            l_equal &&
            (p == index_root[i + 3 * dx].index_parent(-2).index_parent(-2));
        l_equal =
            l_equal &&
            (p == index_root[i + 2 * dy].index_parent(-2).index_parent(-2));
        l_equal =
            l_equal &&
            (p == index_root[i + 3 * dy].index_parent(-2).index_parent(-2));
        l_equal =
            l_equal &&
            (p == index_root[i + 2 * dz].index_parent(-2).index_parent(-2));
        l_equal =
            l_equal &&
            (p == index_root[i + 3 * dz].index_parent(-2).index_parent(-2));
        l_equal =
            l_equal &&
            (p != index_root[i + 4 * dx].index_parent(-2).index_parent(-2));
        l_equal =
            l_equal &&
            (p != index_root[i + 4 * dy].index_parent(-2).index_parent(-2));
        l_equal =
            l_equal &&
            (p != index_root[i + 4 * dz].index_parent(-2).index_parent(-2));
      }
    }
  }
  unit_assert(l_equal);
      ...
      for (f1[1] = -1; f1[1] <= 1; f1[1]++) {
        for (f1[2] = -1; f1[2] <= 1; f1[2]++) {
          ...
          if (level > 0) {
            ...
            fp[0] = ((f1[0] == 1 && c1[0] == 0) || (f1[0] == -1 && c1[0] == 1))
                        ? 0
                        : f1[0];
            fp[1] = ((f1[1] == 1 && c1[1] == 0) || (f1[1] == -1 && c1[1] == 1))
                        ? 0
                        : f1[1];
            fp[2] = ((f1[2] == 1 && c1[2] == 0) || (f1[2] == -1 && c1[2] == 1))
                        ? 0
                        : f1[2];
