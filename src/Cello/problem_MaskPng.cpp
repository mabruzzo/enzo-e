...
MaskPng::MaskPng(std::string file_name, double xm, double xp, double ym,
                 double yp) throw()
    : Mask(), mask_(NULL), nx_(0), ny_(0), xm_(xm), xp_(xp), ym_(ym), yp_(yp) {
...
void MaskPng::evaluate(bool* mask, double t, int ndx, int nx, double* xv,
  ...
  for (int ix = 0; ix < nx; ix++) {
    ...
    for (int iy = 0; iy < ny; iy++) {
      ...
      for (int iz = 0; iz < nz; iz++) {
        ...
        mask[i_mask] = (xm_ <= x && x <= xp_) && (ym_ <= y && y <= yp_)
                           ? mask_[i_png]
                           : false;
      }
    }
