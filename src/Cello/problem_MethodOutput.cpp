...
static std::vector<std::string> read_str_vec_(ParameterGroup p,
  ...
  for (int i = 0; i < length; i++) {
    out[i] = p.list_value_string(i, name);
  }
  return out;
}
...
static std::vector<std::string> read_path_param_(ParameterGroup p,
  ...
  if (p.type(name) == parameter_string) {
    ...
    if (out.size() == 0) {
      ERROR2("read_path_param_", "%s:%s is empty", root_path.c_str(),
             name.c_str());
    }
    return out;
  }
...
MethodOutput::MethodOutput(const Factory* factory, ParameterGroup p) noexcept
    : MethodOutput(
          factory, read_path_param_(p, "file_name"),
          read_path_param_(p, "path_name"), read_str_vec_(p, "field_list"),
          read_str_vec_(p, "particle_list"), p.value_integer("ghost_depth", 0),
...
MethodOutput::MethodOutput(const Factory* factory,
                           std::vector<std::string> file_name,
                           std::vector<std::string> path_name,
                           std::vector<std::string> field_list,
                           std::vector<std::string> particle_list,
                           int ghost_depth, int min_face_rank, bool all_fields,
                           bool all_particles, bool all_blocks, int blocking_x,
                           int blocking_y, int blocking_z) noexcept
    : Method(),
  ...
  ASSERT("MethodOutput()",
         ...
         "to be non-empty",
         (all_fields || all_particles || field_list.size() > 0 ||
...
void MethodOutput::compute_continue(Block* block) {
  ...
  std::string file_name = cello::expand_name(&file_name_, count, cycle, time);
...
FileHdf5* MethodOutput::file_open_(Block* block, int a3[3]) {
  ...
  std::string path_name = cello::expand_name(&path_name_, (*counter),
                                             block->cycle(), block->time());

  // Generate file name
  int count = file_count_(block);
  std::string file_name =
      cello::expand_name(&file_name_, count, block->cycle(), block->time());

  if (block->index().is_root()) {
    Monitor::instance()->print("Output", "MethodOutput writing data file %s",
...
void MethodOutput::file_write_block_(FileHdf5* file, Block* block,
                                     MsgOutput* msg_output) {
  const bool is_local = (msg_output == nullptr);

  std::string block_name =
      (is_local) ? block->name() : msg_output->block_name();

  IoBlock* io_block =
      (is_local) ? factory_->create_io_block() : msg_output->io_block();
  ...
  double* lower = (is_local) ? block_lower : msg_output->block_lower();
  double* upper = (is_local) ? block_upper : msg_output->block_upper();
  ...
  if (is_local) {
    ...
    data_allocated = true;
    data = new Data(nx, ny, nz, num_field_data, lower[0], lower[1], lower[2],
                    upper[0], upper[1], upper[2], cello::field_descr(),
  ...
  for (std::size_t i_f = 0; i_f < field_list_.size(); i_f++) {
    ...
    io_field_data->field_array(&buffer, &name, &type, &mx, &my, &mz, &nx, &ny,
                               &nz);
...
DataMsg* MethodOutput::create_data_msg_(Block* block) {
  ...
  FieldFace* field_face =
      block->create_face(if3, ic3, g3, refresh_same, refresh);
