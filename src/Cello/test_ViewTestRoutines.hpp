  ...
  for (int iz = 0; iz < mz; iz++) {
    for (int iy = 0; iy < my; iy++) {
      for (int ix = 0; ix < mx; ix++) {
        arr(iz, iy, ix) =
            val_range.start + static_cast<T>(count) * val_range.step;
        count++;
      }
    }
  }
...
void assert_allequal3D_(const CelloView<T, 3>& a, const CelloView<T, 3>& b,
  ...
  ASSERT6("assert_allequal3D",
          ...
          "(%d,%d,%d) and (%d,%d,%d)",
          a.shape(0), a.shape(1), a.shape(2), b.shape(0), b.shape(1),
          b.shape(2),
    ...
    for (int iy = 0; iy < my; iy++) {
      for (int ix = 0; ix < mx; ix++) {
        ...
        if (!match) {
          ...
          CkPrintf(
              "One array has a value of %e at (%d,%d,%d), the other has "
              "a value of %e",
...
template <typename T>
void assert_allequal3D_(const CelloView<T, 3>& a, const T& b, const char* file,
                        int line) {
  const int mz = a.shape(0);
  const int my = a.shape(1);
  const int mx = a.shape(2);
...
void assert_shape3D(const CelloView<T, 3>& a,
  ...
  ASSERT("assert_shape3D", "The array doesn't have the expected shape",
         (a.shape(0) == expected[0]) && (a.shape(1) == expected[1]) &&
