...
void Config::read_adapt_(Parameters* p) throw() {
  ...
  for (int ia = 0; ia < num_adapt; ia++) {
    ...
    if (p->type(prefix + "min_refine") == parameter_list) {
      ...
      adapt_min_refine2[ia] =
          p->list_value_float(0, prefix + "min_refine", 0.3);
      ...
      adapt_max_coarsen[ia] =
          p->value(prefix + "max_coarsen", 0.5 * adapt_min_refine[ia]);
    }

    adapt_max_level[ia] =
        p->value(prefix + "max_level", std::numeric_limits<int>::max());

    adapt_level_exponent[ia] = p->value(prefix + "level_exponent", 0.0);

    adapt_output[ia] = p->value_string(prefix + "output", "");

    adapt_include_ghosts[ia] =
        p->value_logical(prefix + "include_ghosts", false);
...
void Config::read_balance_(Parameters* p) throw() {
  ...
  ASSERT1("Config::read_balance_",
          ...
          balance_type.c_str(),
          ((balance_type == "charm") || (balance_type == "cello")));
...
void Config::read_boundary_(Parameters* p) throw() {
  ...
  const bool multi_boundary = (p->type("Boundary:list") == parameter_list);

  num_boundary = multi_boundary ? p->list_length("Boundary:list") : 1;
  ...
  for (int ib = 0; ib < num_boundary; ib++) {
    ...
    if (multi_boundary) prefix = prefix + boundary_list[ib] + ":";

    boundary_type[ib] = p->value_string(prefix + "type", "unknown");

    std::string axis_str = p->value_string(prefix + "axis", "all");
    if (axis_str == "all") {
      boundary_axis[ib] = axis_all;
    } else if (axis_str == "x") {
      boundary_axis[ib] = axis_x;
    } else if (axis_str == "y") {
      boundary_axis[ib] = axis_y;
    } else if (axis_str == "z") {
      boundary_axis[ib] = axis_z;
    } else {
      ERROR2("Config::read()", "Unknown %s %s", (prefix + "axis").c_str(),
             axis_str.c_str());
    }

    std::string face_str = p->value_string(prefix + "face", "all");
    if (face_str == "all") {
      boundary_face[ib] = face_all;
    } else if (face_str == "lower") {
      boundary_face[ib] = face_lower;
    } else if (face_str == "upper") {
      boundary_face[ib] = face_upper;
    } else {
      ERROR2("Config::read()", "Unknown %s %s", (prefix + "face").c_str(),
             face_str.c_str());
    }

    boundary_mask[ib] = (p->type(prefix + "mask") == parameter_logical_expr);

    boundary_field_list[ib] =
        p->value_full_strlist(prefix + "field_list", true);
  }
}
...
void Config::read_field_(Parameters* p) throw() {
  ...
  for (int index_field = 0; index_field < num_fields; index_field++) {
    ...
    field_centering[0][index_field] =
        p->list_value_logical(0, param, true) ? 0 : 1;
    field_centering[1][index_field] =
        p->list_value_logical(1, param, true) ? 0 : 1;
    field_centering[2][index_field] =
        p->list_value_logical(2, param, true) ? 0 : 1;
  }
  ...
  if (precision_str == "default")
    field_precision = precision_default;
    ...
    field_precision = precision_quadruple;
  else {
    ERROR1("Config::read()", "Unknown precision %s", precision_str.c_str());
  }
...
void Config::read_initial_(Parameters* p) throw() {
  ...
  for (int i = 0; i < num_initial; i++) {
    std::string name = p->list_value_string(i, "Initial:list");
...
void Config::read_mesh_(Parameters* p) throw() {
  ...
  int mx = mesh_root_blocks[0] =
      p->list_value_integer(0, "Mesh:root_blocks", 1);
  int my = mesh_root_blocks[1] =
      p->list_value_integer(1, "Mesh:root_blocks", 1);
  int mz = mesh_root_blocks[2] =
      p->list_value_integer(2, "Mesh:root_blocks", 1);
  ...
  mesh_max_initial_level =
      p->value_integer("Adapt:max_initial_level", mesh_max_level);
  ...
  if (mesh_max_level > 0) {
    if ((az_shape[0] < 2 * field_ghost_depth[0]) ||
      ...
      std::string gd_str = format_mesh_dims_(mesh_root_rank, field_ghost_depth);
      ERROR2("Config::read",
  ...
  if (refined_regions_lower.size() > 0 &&
      refined_regions_lower.size() != mesh_max_initial_level) {
    ERROR2("Config::read_mesh_()",
           "The number of lower coordinates defining regions to refine (%d) "
           "should equal Adapt:max_initial_level (%d)",
           refined_regions_lower.size(), mesh_max_initial_level);
  }
  if (refined_regions_upper.size() > 0 &&
      refined_regions_upper.size() != mesh_max_initial_level) {
    ERROR2("Config::read_mesh_()",
           "The number of upper coordinates defining regions to refine (%d) "
           "should equal Adapt:max_initial_level (%d)",
           refined_regions_upper.size(), mesh_max_initial_level);
  }
}
...
void Config::read_method_(Parameters* p) throw() {
  ...
  for (int index_method = 0; index_method < num_method; index_method++) {
    std::string name = p->list_value_string(index_method, "Method:list");
    ...
    method_type[index_method] = p->value_string(full_name + ":type", name);
  }
}
...
void Config::read_output_(Parameters* p) throw() {
  ...
  for (int index_output = 0; index_output < num_output; index_output++) {
    ...
    if (output_type[index_output] == "unknown") {
      ERROR1("Config::read", "Output:%s:type parameter is undefined",
    ...
    if (p->type("field_list") == parameter_list) {
      ...
      for (int i = 0; i < length; i++) {
        output_field_list[index_output][i] =
            p->list_value_string(i, "field_list", "");
      }
    }
    ...
    if (p->type("particle_list") == parameter_list) {
      ...
      for (int i = 0; i < length; i++) {
        output_particle_list[index_output][i] =
            p->list_value_string(i, "particle_list", "");
      }
    }
    ...
    if (output_type[index_output] == "image") {
      if (p->type("axis") != parameter_unknown) {
        ...
        WARNING1("Config::read()", "output_axis[%d] set to z", index_output);
      ...
      output_image_ghost[index_output] = p->value_logical("image_ghost", false);
        ...
        for (int i = 0; i < size; i++) {
          if (p->list_type(i, "colormap") == parameter_float) {
            ...
            if (name[0] == '#') {
              ...
              ASSERT1("Config::read_output()", "Unrecognized color \"%s\"",
            ...
            ERROR1("Config::read_output()", "Unknown colormap list type %d\n",
      ...
      for (int axis = 0; axis < 3; axis++) {
        output_image_lower[index_output][axis] = p->list_value_float(
            axis, "image_lower", -std::numeric_limits<double>::max());
        output_image_upper[index_output][axis] = p->list_value_float(
            axis, "image_upper", std::numeric_limits<double>::max());
      }
    }
  }
}
...
void Config::read_particle_(Parameters* p) throw() {
  ...
  for (int it = 0; it < num_particles; it++) {
    ...
    for (int ia = 0; ia < nc; ia++) {
      ...
      ASSERT3("read_particle_",
              "Particle type %d constant %d has unknown constant name %s", it,
              ia, name.c_str(), name != "unknown");
    ...
    ASSERT1("read_particle_", "Particle type %d has no attributes", it,
            na2 > 0);

    ASSERT2("read_particle_",
            "Particle type %d attributes list length %d must be even", it, na2,
            na2 % 2 == 0);
    ...
    for (int ia = 0; ia < na; ia++) {
      ...
      std::string type =
          p->list_value_string(2 * ia + 1, attrib_str, "unknown");

      ASSERT3("read_particle_",
              "Particle type %d attribute %d has unknown attribute name %s", it,
              ia, name.c_str(), name != "unknown");
  ...
  for (int index_particle = 0; index_particle < num_particles;
       index_particle++) {
...
void Config::read_performance_(Parameters* p) throw() {
  ...
  if (p->type("Performance:papi:counters") == parameter_list) {
    ...
    for (int i = 0; i < length; i++) {
      performance_papi_counters[i] =
          p->list_value_string(i, "Performance:papi:counters", "");
      TRACE2("performance_papi_counters[%d] = %s", i,
             performance_papi_counters[i].c_str());
    }
  }
#endif
  ...
  if (p->type("Performance:projections:schedule_off:var") !=
      parameter_unknown) {
    p->group_set(0, "Performance");
    p->group_push("projections");
  ...
  performance_projections_on_at_start =
      p->value_logical("Performance:projections:on_at_start", true);

  // Check that both projections_on and off schedules are defined or undefined
  // together
  if ((i_on == -1 && i_off == -1) || (i_on != -1 && i_off != -1)) {
    ...
    ERROR2("Config::read_performance_()",
           "Performance:projections:schedule_on [%d] and "
           "Performance:projections:schedule_off [%d]\n"
...
void Config::read_solver_(Parameters* p) throw() {
  ...
  for (int index_solver = 0; index_solver < num_solvers; index_solver++) {
    std::string name = p->list_value_string(index_solver, "Solver:list");
    ...
    solver_solve_type[index_solver] =
        p->value_string(full_name + ":solve_type", "leaf");

    solver_iter_max[index_solver] =
        p->value_integer(full_name + ":iter_max", 1000);

    solver_res_tol[index_solver] = p->value_float(full_name + ":res_tol", 1e-6);

    solver_diag_precon[index_solver] =
        p->value_logical(full_name + ":diag_precon", false);

    solver_monitor_iter[index_solver] =
        p->value_integer(full_name + ":monitor_iter", 0);

    solver_restrict[index_solver] =
        p->value_string(full_name + ":restrict", "linear");

    solver_prolong[index_solver] =
        p->value_string(full_name + ":prolong", "linear");

    solver_min_level[index_solver] =
        p->value_integer(full_name + ":min_level", 0);

    solver_max_level[index_solver] =
        p->value_integer(full_name + ":max_level", mesh_max_level);

    solver_field_x[index_solver] =
        p->value_string(full_name + ":field_x", "unknown");

    solver_field_b[index_solver] =
        p->value_string(full_name + ":field_b", "unknown");
  }
}
...
void Config::read_stopping_(Parameters* p) throw() {
  stopping_cycle =
      p->value_integer("Stopping:cycle", std::numeric_limits<int>::max());
  stopping_time =
      p->value_float("Stopping:time", std::numeric_limits<double>::max());

  stopping_seconds = std::numeric_limits<double>::max();

  if (p->type("Stopping:seconds") != parameter_unknown) {
    stopping_seconds =
        p->value_float("Stopping:seconds", std::numeric_limits<double>::max());
  } else if (p->type("Stopping:minutes") != parameter_unknown) {
    stopping_seconds =
        p->value_float("Stopping:minutes", std::numeric_limits<double>::max());
    stopping_seconds *= 60;
  } else if (p->type("Stopping:hours") != parameter_unknown) {
    stopping_seconds =
        p->value_float("Stopping:hours", std::numeric_limits<double>::max());
...
int Config::read_schedule_(Parameters* p, const std::string group) {
  ...
  if (schedule_var[index] == "cycle")
    var_is_int = true;
  else if (schedule_var[index] == "time")
    var_is_int = false;
  else if (schedule_var[index] == "seconds")
    var_is_int = false;
  else if (schedule_var[index] == "minutes")
    var_is_int = false;
  else if (schedule_var[index] == "hours")
    var_is_int = false;
  else {
    ERROR2("Config::read",
  ...
  if (type_is_interval) {
    ...
    if (n == 0) {
      ERROR1("Config::read", "Schedule variable %s has length 0",
             (group + ":list").c_str());
    }
