...
void MsgOutput::set_data_msg(DataMsg* data_msg) {
  if (data_msg_) {
    WARNING("MsgOutput::set_data_msg()", "overwriting existing data_msg_");
    delete data_msg_;
  }
  data_msg_ = data_msg;
}
...
void* MsgOutput::pack(MsgOutput* msg) {
  ...
  ASSERT2("MsgOutput::pack()", "buffer size mismatch %ld allocated %d packed",
...
Index MsgOutput::index_send() { return index_send_; }

//----------------------------------------------------------------------

void MsgOutput::set_index_send(Index index) { index_send_ = index; }
