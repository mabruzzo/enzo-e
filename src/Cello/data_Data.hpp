...
public:  // interface
  /// Constructor
  Data(int nx, int ny, int nz, int num_field_data, double xm, double xp,
       double ym, double yp, double zm, double zp,
  ...
  int num_field_data() const throw() { return num_field_data_; }
  ...
  void field_cells(double* x, double* y, double* z, int gx = 0, int gy = 0,
                   int gz = 0) const {
  ...
  void field_cell_faces(double* x, double* y, double* z, int gx = 0, int gy = 0,
                        int gz = 0, int cx = 1, int cy = 1, int cz = 1) const;

  /// Return the cell widths of Fields
  void field_cell_width(double* hx, double* hy = 0, double* hz = 0) const;
  ...
  const ParticleData* particle_data() const throw() { return particle_data_; }

  /// Return the Particle data
  ParticleData* particle_data() throw() { return particle_data_; }
  ...
  Particle particle() throw() {
    return Particle(cello::particle_descr(), particle_data_);
  }
  ...
  const FluxData* flux_data() const throw() { return flux_data_; }

  /// Return the Flux data
  FluxData* flux_data() throw() { return flux_data_; }
  ...
  ScalarData<double>* scalar_data_double() { return &scalar_data_double_; }
  ScalarData<long double>* scalar_data_long_double() {
    return &scalar_data_long_double_;
  }
  ScalarData<int>* scalar_data_int() { return &scalar_data_int_; }
  ScalarData<long long>* scalar_data_long_long() {
    return &scalar_data_long_long_;
  }
  ScalarData<Sync>* scalar_data_sync() { return &scalar_data_sync_; }
  ScalarData<void*>* scalar_data_void() { return &scalar_data_void_; }
  ScalarData<Index>* scalar_data_index() { return &scalar_data_index_; }

  /// Return the Scalar objects
  Scalar<long double> scalar_long_double() {
    return Scalar<long double>(cello::scalar_descr_long_double(),
                               &scalar_data_long_double_);
  }
  Scalar<double> scalar_double() {
    return Scalar<double>(cello::scalar_descr_double(), &scalar_data_double_);
  }
  Scalar<int> scalar_int() {
    return Scalar<int>(cello::scalar_descr_int(), &scalar_data_int_);
  }
  Scalar<long long> scalar_long_long() {
    return Scalar<long long>(cello::scalar_descr_long_long(),
                             &scalar_data_long_long_);
  }
  Scalar<Sync> scalar_sync() {
    return Scalar<Sync>(cello::scalar_descr_sync(), &scalar_data_sync_);
  }
  Scalar<void*> scalar_void() {
    return Scalar<void*>(cello::scalar_descr_void(), &scalar_data_void_);
  }
  Scalar<Index> scalar_index() {
    return Scalar<Index>(cello::scalar_descr_index(), &scalar_data_index_);
  }
