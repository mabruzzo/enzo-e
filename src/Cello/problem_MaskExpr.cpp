...
MaskExpr::MaskExpr(Param* param) throw() : Mask(), param_(param) {}

//----------------------------------------------------------------------

void MaskExpr::copy_(const MaskExpr& mask) throw() { param_ = mask.param_; }
...
void MaskExpr::evaluate(bool* mask, double t, int ndx, int nx, double* xv,
                        int ndy, int ny, double* yv, int ndz, int nz,
                        double* zv) const {
  ASSERT6("MaskExpr::evaluate",
          "mask size (%d %d %d) needs to be at least (%d %d %d)", ndx, ndy, ndz,
          nx, ny, nz, (ndx >= nx) && (ndy >= ny) && (ndz >= nz));
