...
PARALLEL_MAIN_BEGIN {
  ...
  struct face_test_type {
    ...
    double time_step_1;
    double time_step_2;
  } test = {{0, 0, 0}, {10, 6, 8}, {0, 0, 0}, {0.25, 0.5, 1.0}, 0.125, 0.125};
    ...
    for (int iaxis = 0; iaxis < rank; iaxis++) {
      for (int iface = 0; iface < 2; ++iface) {
        for (int ilevel = 0; ilevel < num_level; ++ilevel) {
          ...
          int index_field = 3;
          auto face_fluxes_1 = new FaceFluxes(face_1, index_field, n3[0], n3[1],
                                              n3[2], c3[0], c3[1], c3[2]);
          auto face_fluxes_2 = new FaceFluxes(face_2, index_field, n3[0], n3[1],
                                              n3[2], c3[0], c3[1], c3[2]);
          ...
          unit_assert(my == ((fy != 0)     ? 1
                             : (rank >= 2) ? (n3[1] + c3[1])
                                           : 1));
          unit_assert(mz == ((fz != 0)     ? 1
                             : (rank >= 3) ? (n3[2] + c3[2])
                                           : 1));
          ...
          //--------------------------------------------------
          {
            ...
            {
              double sum_abs = 0.0;
              cello_float* fluxes_1 =
                  face_fluxes_1->flux_array(&dx1, &dy1, &dz1);
              for (int iz = 0; iz < mz1; iz++) {
                for (int iy = 0; iy < my1; iy++) {
                  for (int ix = 0; ix < mx1; ix++) {
                    int i = ix * dx1 + iy * dy1 + iz * dz1;
            ...
            {
              double sum_abs = 0;
              cello_float* fluxes_2 =
                  face_fluxes_2->flux_array(&dx2, &dy2, &dz2);
              for (int iz = 0; iz < mz2; iz++) {
                for (int iy = 0; iy < my2; iy++) {
                  for (int ix = 0; ix < mx2; ix++) {
                    int i = ix * dx2 + iy * dy2 + iz * dz2;
