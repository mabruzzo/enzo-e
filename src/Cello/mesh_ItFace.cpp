...
ItFace::ItFace(int rank, int rank_limit, int periodic[3], int n3[3],
               Index index, const int* ic3, const int* ipf3) throw()
    : if3_(),
      ic3_(),
      ipf3_(),
      rank_(rank),
...
Index ItFace::index() const { return index_.index_neighbor(if3_, n3_); }
...
bool ItFace::is_reset() const { return if3_[0] == -2; }
...
bool ItFace::valid_() const {
  ...
  int rank_face = rank_ - (abs(if3_[0]) + abs(if3_[1]) + abs(if3_[2]));
  ...
  if (ic3_.size() > 0) {
    if (ipf3_.size() == 0) {
      ...
      for (int axis = 0; axis < rank_; axis++) {
        ...
        if (ipf3_[axis] == 0 && ((ic3_[axis] == 0 && if3_[axis] == -1) ||
                                 (ic3_[axis] == 1 && if3_[axis] == 1)))
          l_parent = false;
      }
    }
  }
