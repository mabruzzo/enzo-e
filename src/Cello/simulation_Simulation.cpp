...
Simulation::Simulation(const char* parameter_file, int n)
...
Simulation::~Simulation() { deallocate_(); }
...
void Simulation::pup(PUP::er& p) {
  ...
  ASSERT1("Simulation::pup()",
          ...
          msg_refine_map_.size(), (msg_refine_map_.size() == 0));
...
void Simulation::refine_create_block(MsgRefine* msg) {
  ...
  if (msg_refine_map_[index] != NULL) {
    ...
    ASSERT3("Simulation::p_refine_create_block",
            "index %08x %08x %08x is already in the msg_refine mapping", v3[0],
            v3[1], v3[2], (msg == NULL));
  }

  msg_refine_map_[index] = msg;

  cello::block_array()[index].insert(process_type(CkMyPe()),
                                     MsgType::msg_refine);
}
...
void Simulation::initialize_simulation_() throw() {
  ...
  ASSERT("Simulation::initialize_simulation_()",
         "Parameter 'Mesh:root_rank' must be specified", rank_ != 0);
...
void Simulation::initialize_performance_() throw() {
  ...
  for (size_t i = 0; i < config_->performance_papi_counters.size(); i++) {
    p->new_counter(counter_type_papi, config_->performance_papi_counters[i]);
  }
#endif
  ...
  if (index_on >= 0) {
    projections_schedule_on_ = Schedule::create(
        config_->schedule_var[index_on], config_->schedule_type[index_on],
        config_->schedule_start[index_on], config_->schedule_stop[index_on],
        config_->schedule_step[index_on], config_->schedule_list[index_on]);
  }
  if (index_off >= 0) {
    projections_schedule_off_ = Schedule::create(
        config_->schedule_var[index_off], config_->schedule_type[index_off],
        config_->schedule_start[index_off], config_->schedule_stop[index_off],
        config_->schedule_step[index_off], config_->schedule_list[index_off]);
  }
#endif
...
void Simulation::initialize_data_descr_() throw() {
  ...
  ASSERT1("Simulation::initialize_data_descr_",
          "Illegal Field:alignment parameter value %d", alignment,
          1 <= alignment);
  ...
  for (size_t it = 0; it < config_->particle_list.size(); it++) {
    ...
    for (int ic = 0; ic < nc; ic++) {
      ...
      ASSERT3("Simulation::initialize_data_descr_()",
              ...
              "has unknown type \"%s\"",
              config_->particle_list[it].c_str(), name.c_str(),
      ...
      switch (type) {
        ...
        case type_double:
          *f8 = config_->particle_constant_value[it][ic];
          break;
        case type_quadruple:
          *f16 = config_->particle_constant_value[it][ic];
          break;
        case type_int8:
          *i8 = config_->particle_constant_value[it][ic];
          break;
        case type_int16:
          *i16 = config_->particle_constant_value[it][ic];
          break;
        case type_int32:
          *i32 = config_->particle_constant_value[it][ic];
          break;
        case type_int64:
          *i64 = config_->particle_constant_value[it][ic];
          break;
        default:
    ...
    for (int ia = 0; ia < na; ia++) {
      ...
      ASSERT3("Simulation::initialize_data_descr_()",
              "Unknown Particle type \"%s\" attribute \"%s\" has unknown type "
              "\"%s\"",
              config_->particle_list[it].c_str(), name.c_str(),
  ...
  for (int index_particle = 0; index_particle < num_particles;
    ...
    for (int index_group = 0; index_group < num_groups; index_group++) {
      std::string group =
          config_->particle_group_list[index_particle][index_group];
...
void Simulation::initialize_hierarchy_() throw() {
  ...
  ASSERT("Simulation::initialize_hierarchy_",
         "data must be initialized before hierarchy", field_descr_ != NULL);
  ...
  hierarchy_ = factory()->create_hierarchy(refinement, config_->mesh_min_level,
                                           config_->mesh_max_level);

  hierarchy_->set_lower(config_->domain_lower[0], config_->domain_lower[1],
                        config_->domain_lower[2]);
  hierarchy_->set_upper(config_->domain_upper[0], config_->domain_upper[1],
  ...
  int p3[3] = {lp3[0] ? root_blocks[0] : 0, lp3[1] ? root_blocks[1] : 0,
...
void Simulation::initialize_balance_() throw() {
  ...
  schedule_balance_ =
      (index == -1)
          ? NULL
          : Schedule::create(
                config_->schedule_var[index], config_->schedule_type[index],
                config_->schedule_start[index], config_->schedule_stop[index],
                config_->schedule_step[index], config_->schedule_list[index]);
}
...
void Simulation::data_insert_block(Block* block) {
  ...
  PARALLEL_PRINTF("%d: ++sync_output_begin_ %d %lu\n", CkMyPe(),
                  sync_output_begin_.stop(), hierarchy_->num_blocks());
#endif
  if (hierarchy_) {
...
void Simulation::monitor_performance() {
  ...
  int n = 14 + 2 * num_solver +
          (hierarchy_->max_level() - hierarchy_->min_level() + 1) + nr * nc;
  ...
  counters_reduce[m++] =
      Hierarchy::num_particles_node;  // 14 max_node_particles
  for (int i = 0; i < num_solver; i++) {
    counters_reduce[m++] =
        cello::simulation()->get_solver_max_iter(i);  // 15 max_node_particles
  }

  ASSERT2("Simulation::monitor_performance()",
          "Actual array length %d != expected array length %d", m, n, (m == n));
  ...
  if (CkMyPe() == 0) {
    ...
    for (int i = 0; i < num_solver; i++) {
      ...
      monitor()->print("Performance", "solver num-%s-iter %lld",
                       problem()->solver(i)->name().c_str(), num_solver_iter);
    }

    monitor()->print("Performance", "counter num-msg-coarsen %lld",
                     msg_coarsen);
    monitor()->print("Performance", "counter num-msg-refine %lld", msg_refine);
    monitor()->print("Performance", "counter num-msg-refresh %lld",
                     msg_refresh);
    ...
    monitor()->print("Performance", "counter num-particle-data %lld",
                     particle_data);
    ...
    for (int i = hierarchy_->min_level(); i <= hierarchy_->max_level(); i++) {
      ...
      monitor()->print("performance", "simulation num-blocks-level %d %lld", i,
                       num_blocks_level);
    ...
    monitor()->print("Performance", "simulation num-leaf-blocks %lld",
                     num_leaf_blocks);
    monitor()->print("Performance", "simulation num-total-blocks %lld",
                     num_total_blocks);
    ...
    for (int i = 0; i < num_solver; i++) {
      ...
      monitor()->print("Performance", "solver max-%s-iter %lld",
                       problem()->solver(i)->name().c_str(), max_solver_iters);
    }
    cello::simulation()->clear_solver_iter();  // clear it for the next solve

    monitor()->print("Performance", "simulation max-proc-blocks %lld",
                     max_proc_blocks);
    monitor()->print("Performance", "simulation max-node-blocks %lld",
                     max_node_blocks);
    monitor()->print("Performance", "simulation max-proc-particles %lld",
                     max_proc_particles);
    monitor()->print("Performance", "simulation max-node-particles %lld",
                     max_node_particles);
    ...
    monitor()->print(
        ...
        avg_proc_blocks / max_proc_blocks, avg_proc_blocks, max_proc_blocks);
    monitor()->print(
        "Performance", "simulation balance-eff-blocks-node %f (%.0f/%lld)",
        avg_node_blocks / max_node_blocks, avg_node_blocks, max_node_blocks);

    if (num_particles > 0) {
      const double avg_proc_particles = 1.0 * num_particles / CkNumPes();
      const double avg_node_particles = 1.0 * num_particles / CkNumNodes();
      monitor()->print("Performance",
                       "simulation balance-eff-particles-core %f (%.0f/%lld)",
                       avg_proc_particles / max_proc_particles,
                       avg_proc_particles, max_proc_particles);
      monitor()->print("Performance",
                       "simulation balance-eff-particles-node %f (%.0f/%lld)",
                       avg_node_particles / max_node_particles,
                       avg_node_particles, max_node_particles);
    }

    ASSERT3("Simulation::monitor_performance()",
            "Actual array length %d != expected array length 2 + %d + %d", m,
            num_sum, num_max, (m == 2 + num_sum + num_max));
  }
#ifdef TRACE_PROCESS_MEMORY
  Memory* memory = Memory::instance();
  CkPrintf(
      "TRACE_PERF proc %d cycle %d bytes curr %lld high %lld highest %lld\n",
      CkMyPe(), cycle_, memory->bytes(), memory->bytes_high(),
      memory->bytes_highest());
#endif
