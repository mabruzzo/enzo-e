...
MethodOrderHilbert::MethodOrderHilbert(int min_level) throw()
  ...
  is_weight_child_ =
      cello::scalar_descr_long_long()->new_value(name() + ":weight_child", n);
  is_sync_index_ =
      cello::scalar_descr_sync()->new_value(name() + ":sync_index");
  is_sync_weight_ =
      cello::scalar_descr_sync()->new_value(name() + ":sync_weight");
}
...
void Block::r_method_order_hilbert_continue(CkReductionMsg* msg) {
  delete msg;
  static_cast<MethodOrderHilbert*>(this->method())->compute_continue(this);
}
...
//======================================================================
void MethodOrderHilbert::send_weight(Block* block, int weight_child,
                                     bool self) {
  // update own weight
  ...
  if ((!self || block->is_leaf()) && level > min_level_) {
    ...
    Index index_next =
        hilbert_next(block->index(), rank, block->is_leaf(), min_level_);
...
void Block::p_method_order_hilbert_weight(int ic3[3], int weight,
                                          Index index_child) {
  static_cast<MethodOrderHilbert*>(this->method())
      ->recv_weight(this, ic3, weight, false);
}

//----------------------------------------------------------------------

void MethodOrderHilbert::recv_weight(Block* block, int ic3[3], int weight,
                                     bool self) {
...
void MethodOrderHilbert::send_index(Block* block, int index_parent, int count,
                                    bool self) {
  *pcount_(block) = count;
  if (!block->is_leaf()) {
    ...
    for (int i = 0; i < cello::num_children(); i++) {
      ...
      cello::block_array()[index_child].p_method_order_hilbert_index(index,
                                                                     count);
...
void Block::p_method_order_hilbert_index(int index, int count) {
  static_cast<MethodOrderHilbert*>(this->method())
      ->recv_index(this, index, count, false);
}

void MethodOrderHilbert::recv_index(Block* block, int index, int count,
                                    bool self) {
  {
    char buffer[80];
...
void Block::r_method_order_hilbert_complete(CkReductionMsg* msg) {
  delete msg;
  static_cast<MethodOrderHilbert*>(this->method())->compute_complete(this);
}
...
Index MethodOrderHilbert::hilbert_next(Index index, int rank, bool is_leaf,
  ...
  // If the block has children then the first child (according to the hilbert
  // order) is the next index.
  if (!is_leaf) {
    int T = states[m];
    int ic = hilbert_ind_to_coord(T, 0);
    index_next.push_child(ic & 1, (ic >> 1) & 1, (ic >> 2) & 1, min_level);

    // Otherwise the next index is the next child of the first ancestor block
    // whose children haven't been completely processed yet
  } else {
    ...
    // NOTE: this loop walks up the tree until it finds an index where last
    // isn't true.
...
int MethodOrderHilbert::HPM[12][8] = {
    {0, 1, 3, 2, 6, 7, 5, 4}, {6, 7, 5, 4, 0, 1, 3, 2},
    {5, 7, 6, 4, 0, 2, 3, 1}, {3, 1, 0, 2, 6, 4, 5, 7},
    {0, 1, 5, 4, 6, 7, 3, 2}, {6, 2, 3, 7, 5, 1, 0, 4},
    {5, 1, 0, 4, 6, 2, 3, 7}, {3, 2, 6, 7, 5, 4, 0, 1},
    {0, 4, 6, 2, 3, 7, 5, 1}, {6, 4, 0, 2, 3, 1, 5, 7},
    {5, 7, 3, 1, 0, 2, 6, 4}, {3, 7, 5, 1, 0, 4, 6, 2}};

// Hilbert ind to Next state Map
int MethodOrderHilbert::HNM[12][8] = {
    {8, 4, 4, 3, 3, 5, 5, 10},  {9, 5, 5, 2, 2, 4, 4, 11},
    {6, 10, 10, 1, 1, 8, 8, 7}, {7, 11, 11, 0, 0, 9, 9, 6},
    {8, 0, 0, 6, 6, 1, 1, 11},  {1, 9, 9, 7, 7, 10, 10, 0},
    {2, 10, 10, 4, 4, 9, 9, 3}, {11, 3, 3, 5, 5, 2, 2, 8},
    {0, 4, 4, 9, 9, 7, 7, 2},   {5, 1, 1, 8, 8, 3, 3, 6},
    {6, 2, 2, 11, 11, 0, 0, 5}, {3, 7, 7, 10, 10, 4, 4, 1}};

// Point to Hilbert ind Map
int MethodOrderHilbert::PHM[12][8] = {
    {0, 1, 3, 2, 7, 6, 4, 5}, {4, 5, 7, 6, 3, 2, 0, 1},
    {4, 7, 5, 6, 3, 0, 2, 1}, {2, 1, 3, 0, 5, 6, 4, 7},
    {0, 1, 7, 6, 3, 2, 4, 5}, {6, 5, 1, 2, 7, 4, 0, 3},
    {2, 1, 5, 6, 3, 0, 4, 7}, {6, 7, 1, 0, 5, 4, 2, 3},
    {0, 7, 3, 4, 1, 6, 2, 5}, {2, 5, 3, 4, 1, 6, 0, 7},
    {4, 3, 5, 2, 7, 0, 6, 1}, {4, 3, 7, 0, 5, 2, 6, 1}};

// Point to Next state Map
int MethodOrderHilbert::PNM[12][8] = {
    {8, 4, 3, 4, 10, 5, 3, 5},  {2, 4, 11, 4, 2, 5, 9, 5},
    {1, 7, 8, 8, 1, 6, 10, 10}, {11, 11, 0, 7, 9, 9, 0, 6},
    {8, 0, 11, 1, 6, 0, 6, 1},  {10, 10, 9, 9, 0, 7, 1, 7},
    {10, 10, 9, 9, 4, 2, 4, 3}, {2, 8, 3, 11, 2, 5, 3, 5},
    {0, 2, 9, 9, 4, 7, 4, 7},   {1, 3, 8, 8, 1, 3, 5, 6},
    {11, 11, 0, 2, 5, 6, 0, 2}, {10, 10, 1, 3, 4, 7, 4, 7}};
...
int MethodOrderHilbert::CSM[4][4] = {
    {1, 0, 3, 0}, {0, 2, 1, 1}, {2, 1, 2, 3}, {3, 3, 0, 2}};