...
MethodFluxCorrect::MethodFluxCorrect(
  ...
  ASSERT1("MethodFluxCorrect::MethodFluxCorrect",
          "Must add field(s) to the field group \"%s\"", group_.c_str(),
          (nf > 0));
  for (int i_f = 0; i_f < nf; i_f++) {
...
void MethodFluxCorrect::compute(Block* block) throw() {
  ...
  block->refresh_start(ir_pre_, CkIndex_Block::p_method_flux_correct_refresh());
}

//----------------------------------------------------------------------

void Block::p_method_flux_correct_refresh() {
  static_cast<MethodFluxCorrect*>(this->method())
      ->compute_continue_refresh(this);
}
...
void MethodFluxCorrect::compute_continue_refresh(Block* block) throw() {
  ...
  if (block->is_leaf()) {
    ...
    for (int i_f = 0; i_f < nf; i_f++) {
      ...
      const bool scale_by_density = groups->is_in(field.field_name(index_field),
  ...
  block->contribute((nf + 1) * sizeof(long double), reduce,
...
void Block::r_method_flux_correct_sum_fields(CkReductionMsg* msg) {
  static_cast<MethodFluxCorrect*>(this->method())
      ->compute_continue_sum_fields(this, msg);
}
  ...
  if (block->index().is_root()) {
    ...
    for (int i_f = 0; i_f < nf; i_f++) {
      ...
      cello::monitor()->print(
          ...
          field_name.c_str(), field_sum_[i_f], digits,
...
static void flux_correct_helper_(cello_float* const field_array, int mx, int my,
                                 int mz, int nx, int ny, int nz, int i_f,
                                 FluxData* flux_data, const int rank,
...
static void divide_flux_corrected_cells_by_density_(
    const cello_float* const input_array,
    const cello_float* const density_field, cello_float* const output_array,
    const bool (&correction_performed)[3][2], const int rank, int mx, int my,
    int mz, int nx, int ny, int nz) {
  {
    const int axis = 0;
...
void MethodFluxCorrect::flux_correct_(Block* block) {
  ...
  if (enable_ && block->is_leaf()) {
    ...
    if (scratch_.size() == 0) {
      scratch_.resize(2 * num_field_elements);
    }
    ...
    if (i_f_density > -1) {
      flux_correct_helper_(density_array, mx, my, mz, nx, ny, nz, i_f_density,
                           flux_data, rank, perform_correction);
    }
      ...
      if (groups->is_in(field_name, "make_field_conservative")) {
        ...
        ASSERT1("MethodFluxCorrect::flux_correct_",
                ("The \"density\" field must exist to perform flux "
                 "corrections on \"%s\"."),
                field_name.c_str(), density_array != nullptr);
        ...
        flux_correct_helper_(temp_cons_array, mx, my, mz, nx, ny, nz, i_f,
                             flux_data, rank, perform_correction);
        ...
        flux_correct_helper_(field_array, mx, my, mz, nx, ny, nz, i_f,
                             flux_data, rank, perform_correction);
      }
    }
  }
