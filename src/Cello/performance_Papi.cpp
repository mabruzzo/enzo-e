...
int Papi::num_events() const throw() { return num_events_; }
...
int Papi::add_event(std::string event) throw() {
  if (!is_initialized_) {
    ...
    if (retval != PAPI_OK) {
      if (warnings_)
        WARNING2("PAPI::add_event", "PAPI_add_event for %d returned %d", id,
                 retval);
      return 0;
    } else {
...
void Papi::start_events() throw() {
  ...
  if (!is_started_ && num_events_ > 0) {
    ...
    if (warnings_) WARNING("Papi::start_events", "Events already started");
  }
  is_started_ = true;
}
...
void Papi::stop_events() throw() {
  if (is_started_ && num_events_ > 0) {
    ...
    if (warnings_) WARNING("Papi::stop_events", "Events already stopped");
  }
  is_started_ = false;
}
