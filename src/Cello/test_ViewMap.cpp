...
public:
  ...
  template <typename T>
  void require_all_zero_(ViewMap<T>& view_map, const std::string& expected_name,
    ...
    for (auto name : names) {
      ...
      if (contiguous) {
        if (name == "") {
          unit_func_quiet(
              "ViewMap",
              "ViewMap(std::vector<std::string>, std::array<int,3>)");

          view_map = new ViewMap<int>(keys, shape);
          this->require_all_zero_(*view_map, "", keys, shape);
        } else {
          unit_func_quiet("ViewMap",
                          "ViewMap(std::string, std::vector<std::string>, "
                          "std::array<int,3>)");
        ...
        if (name == "") {
          unit_func_quiet(
              "ViewMap",
              "ViewMap(std::vector<std::string>, std::vector<CelloView<T,3>>)");
...
public:
  ...
  void test_view_accessor(bool bracket_index) {
    ...
    const std::vector<std::string> keys = {"density", "velocity_x",
                                           "total_energy"};
    std::array<int, 3> shape = {4, 6, 8};
    ...
    // ViewMap::at are seen when the CelloView is accessed by target accessor
    {
      ...
      assign_range_3Darray(view_map->at("velocity_x"), ValRange<int>(10000, 1));
