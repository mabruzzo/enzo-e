...
void DataMsg::set_coarse_array(Field field, int iam3[3], int iap3[3],
  ...
  const int nf3[3] = {(ifps3[0] - ifms3[0]), (ifps3[1] - ifms3[1]),
                      (ifps3[2] - ifms3[2])};
  const int na3[3] = {(iap3[0] - iam3[0]), (iap3[1] - iam3[1]),
                      (iap3[2] - iam3[2])};
  ...
  for (int i_f = 0; i_f < nf; i_f++) {
    ...
    ASSERT1("DataMsg::set_coarse_array",
            "Field-to-coarse array axis ratio r=%g is not 1.0 or 2.0", r,
            (r == 1.0 || r == 2.0));
#endif
    int rd = r;
...
char* DataMsg::save_data(char* buffer) const {
  ...
  ASSERT2("DataMsg::save_data()", "Expecting buffer size %d actual size %d",
          data_size(), (pc - buffer), (data_size() == (pc - buffer)));
...
void DataMsg::update(Data* data, bool is_local, bool is_kept) {
  ...
  if (pd != nullptr) {
    ...
    if (is_kept) cello::simulation()->data_insert_particles(count);
  }
  ...
  if (face_fluxes_list_.size() > 0) {
    for (size_t i = 0; i < face_fluxes_list_.size(); i++) {
      ...
      flux_data->sum_neighbor_fluxes(face_fluxes, face.axis(), 1 - face.face(),
                                     i);
  ...
  const int na3[3] = {(iap3_cf_[0] - iam3_cf_[0]), (iap3_cf_[1] - iam3_cf_[1]),
                      (iap3_cf_[2] - iam3_cf_[2])};

  const int na = na3[0] * na3[1] * na3[2];

  if (na > 0) {
    const int na3[3] = {(iap3_cf_[0] - iam3_cf_[0]),
    ...
    for (int i_f = 0; i_f < nf; i_f++) {
      ...
      const int ic0 =
          iam3_cf_[0] + m3_c[0] * (iam3_cf_[1] + m3_c[1] * iam3_cf_[2]);
