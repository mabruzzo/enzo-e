...
Schedule::Schedule() throw()
    : active_(false), type_(schedule_type_unknown), last_(-1), timer_() {
  timer_.start();
}
...
// static
Schedule* Schedule::create(std::string var, std::string type, double start,
  ...
  ASSERT("Schedule::create", "Bad shedule parameters", schedule != NULL);
