...
public:  // interface
  ...
  const char* attribute_array(ParticleDescr* pd, int it, int ia, int ib) const {
    return (const char*)((ParticleData*)this)->attribute_array(pd, it, ia, ib);
  }
  ...
  void scatter(ParticleDescr*, int it, int ib, int np, const bool* mask,
               const int* index, int n, ParticleData* particle_array[],
  ...
  bool position(ParticleDescr* particle_descr, int it, int ib, double* x,
  ...
  void position_update(ParticleDescr* particle_descr, int it, int ib,
                       long double dx, long double dy, long double dz);

  /// Fill a vector of velocity coordinates for the given type and batch
  bool velocity(ParticleDescr* particle_descr, int it, int ib, double* vx,
...
private:  /// functions
  ...
  void check_arrays_(ParticleDescr* particle_descr, std::string file,
                     int line) const;

  /// Copy the given floating point attribute of given type (float,
  /// double, quad, etc.) to the given coordinate double position
  /// array.
  void copy_attribute_float_(ParticleDescr* particle_descr, int type, int it,
                             int ib, int ia, double* coord);
  ...
  void update_attribute_float_(ParticleDescr* particle_descr, int type, int it,
                               int ib, int ia, long double da);
  ...
  /// e.g. int64_t
  void copy_position_int_(ParticleDescr* particle_descr, int type, int it,
                          int ib, int ia, double* coord);

  /// Update version of copy_position_int_()
  void update_position_int_(ParticleDescr* particle_descr, int type, int it,
                            int ib, int ia, int64_t da);

  void write_ifrite(ParticleDescr* particle_descr, int it,
                    std::string file_name, double xm, double ym, double zm,
