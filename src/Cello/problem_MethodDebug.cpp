...
MethodDebug::MethodDebug(int num_fields, int num_particles, bool l_print,
                         bool l_coarse, bool l_ghost) throw()
    : Method(),
...
void MethodDebug::compute(Block* block) throw() {
  ...
  if (block->is_leaf()) {
      ...
      for (int iz = gz; iz < mz - gz; iz++) {
        for (int iy = gy; iy < my - gy; iy++) {
          for (int ix = gx; ix < mx - gx; ix++) {
            ...
            reduce[k + kmax] =
                std::max(reduce[k + 1], (long double)(values[i]));
    ...
    position[0].resize(mb);
    position[0].clear();
    position[1].resize(mb);
    position[1].clear();
    position[2].resize(mb);
    position[2].clear();
      ...
      for (int ib = 0; ib < nb; ib++) {
        ...
        for (int i = 0; i < cello::rank(); i++) {
          for (int ip = 0; ip < np; ip++) {
            ...
            reduce[k + 4 * i + kmin] =
                std::min(reduce[k + 4 * i + 0], (long double)(value));
            reduce[k + 4 * i + kmax] =
                std::max(reduce[k + 4 * i + 1], (long double)(value));
    ...
    ASSERT2("MethodDebug::compute()", "reduce array mismatch %d != %d", k,
            num_reduce + 1, (k == num_reduce + 1));
  }
...
#ifdef DEBUG_DEBUG
  {
    ...
    for (int i_f = 0; i_f < num_fields_; i_f++) {
      ...
      cello::monitor()->print("Method",
                              "Field %s %s min %Lg max %Lg sum %Lg cnt %Lg",
                              name.c_str(), block->name().c_str(), reduce[id],
                              reduce[id + 1], reduce[id + 2], reduce[id + 3]);
      id += 4;
    }
    Particle particle = block->data()->particle();
    for (int it = 0; it < num_particles_; it++) {
      const std::string name = particle.type_name(it).c_str();
      cello::monitor()->print("Method", "Particle %s %s num_particles %d",
      ...
      for (int i = 0; i < 3; i++) {
        ...
        cello::monitor()->print(
            ...
            name.c_str(), axis[i], block->name().c_str(), reduce[id],
            reduce[id + 1], reduce[id + 2], reduce[id + 3]);
        id += 4;
      }
    }
  ...
  block->contribute((1 + num_reduce) * sizeof(long double), reduce,
...
void Block::r_method_debug_sum_fields(CkReductionMsg* msg) {
  static_cast<MethodDebug*>(this->method())->compute_continue(this, msg);
}

//----------------------------------------------------------------------

void MethodDebug::compute_continue(Block* block, CkReductionMsg* msg) throw() {
  ...
  const int num_reduce = 4 * (num_fields_ + 3 * num_particles_);
  ASSERT2("MethodDebug::compute_continue()", "reduce array mismatch %d != %d",
  ...
  if (block->index().is_root()) {
    ...
    for (int i_f = 0; i_f < num_fields_; i_f++) {
      std::string name = field.field_name(i_f).c_str();
      cello::monitor()->print(
          "Method", "Field %s min %20.16Lg avg %20.16Lg max %20.16Lg",
          name.c_str(), field_min_[i_f], field_sum_[i_f] / field_count_[i_f],
          field_max_[i_f]);
    }
    for (int it = 0; it < num_particles_; it++) {
      for (int i = 0; i < 3; i++) {
        ...
        cello::monitor()->print(
            ...
            name.c_str(), axis[i], particle_min_[i][it],
    ...
    for (int i_f = 0; i_f < num_fields_; i_f++) {
      ...
      if (l_print_) {
        ...
        snprintf(buffer, 255, "field-%s-%s-%03d.data",
                 field.field_name(i_f).c_str(), block->name().c_str(),
                 block->cycle());
