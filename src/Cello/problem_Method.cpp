...
Method::Method(double courant) throw()
    : schedule_(NULL), courant_(courant), neighbor_type_(neighbor_leaf) {
...
Method::~Method() throw() { delete schedule_; }
...
int Method::add_refresh_(int neighbor_type) {
  ...
  Refresh refresh_default(ghost_depth, min_face_rank, neighbor_type,
                          sync_neighbor, 0);
...
int Method::refresh_id_post() const { return ir_post_; }
