...
extern void png_array(const char* filename, float* array, int gx, int gy,
                      int gz, int mx, int my, int mz, const char* file,
                      int line, int axis = 2, int px = 0, int py = 0,
                      double scale = 1.0);
//----------------------------------------------------------------------
// Defines
//----------------------------------------------------------------------
...
#define TRACE_FIELD_GM(NAME, FIELD, SCALE, gx, gy, gz, mx, my, mz, PLOT)       \
  {                                                                            \
    ...
    CkPrintf("DEBUG_FIELD (%g) [%g] | (%g : %g %g %g) %s:%d %s\n", sum_real,   \
             sum_all, SCALE*(FIELD[gx + mx * (gy + my * gz)]),                 \
             SCALE*(FIELD[(gx + 1) + mx * (gy + my * gz)]),                    \
             SCALE*(FIELD[gx + mx * ((gy + 1) + my * gz)]),                    \
             SCALE*(FIELD[gx + mx * (gy + my * (gz + 1))]), __FILE__,          \
             __LINE__, NAME);                                                  \
    ...
    png_array(filename, (float*)(FIELD), gx, gy, gz, mx, my, mz, __FILE__,     \
              __LINE__, 2, 16, 16, SCALE);                                     \
    snprintf(filename, sizeof(filename), "enzo-e-%s.png", NAME);               \
    png_array(filename, (float*)(FIELD), 0, 0, 0, mx, my, mz, __FILE__,        \
              __LINE__, 2, 16, 16, SCALE);                                     \
  }
#define TRACE_FIELD_(NAME, FIELD, SCALE)                                       \
  TRACE_FIELD_GM(NAME, FIELD, SCALE, gx_, gy_, gz_, mx_, my_, mz_, false)
#define TRACE_FIELD(NAME, FIELD, SCALE)                                        \
  TRACE_FIELD_GM(NAME, FIELD, SCALE, gx, gy, gz, mx, my, mz, false)
#define PRINT_FIELD_(NAME, FIELD, SCALE)                                       \
  TRACE_FIELD_GM(NAME, FIELD, SCALE, gx_, gy_, gz_, mx_, my_, mz_, true)
#define PRINT_FIELD(NAME, FIELD, SCALE)                                        \
  TRACE_FIELD_GM(NAME, FIELD, SCALE, gx, gy, gz, mx, my, mz, true)
