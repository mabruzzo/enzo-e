...
void print_levels(int n, Adapt** adapt, int min_level, int max_level,
  ...
  for (int i0 = 0; i0 < n; i0++)
    CkPrintf(i0 % 2 ? "%1X " : "%1X", adapt[i0]->is_converged() ? 1 : 0);
  CkPrintf("\n");
}
...
PARALLEL_MAIN_BEGIN {
  ...
  for (int run = 0; run < num_runs; run++) {
    ...
    while (!all_converged(n, adapt) && iteration < 10) {
      for (int i0 = 0; i0 < n; i0++) {
        ...
        if (im >= 0) {
          ...
          adapt[i0]->update_neighbor(index[im], level_min, level_max,
                                     can_coarsen);
        }
        const int ip = i0 + 1;
        if (ip <= n - 1) {
          adapt[ip]->get_level_bounds(&level_min, &level_max, &can_coarsen);
          adapt[i0]->update_neighbor(index[ip], level_min, level_max,
                                     can_coarsen);
        }
    ...
    for (int i0 = 0; i0 < n; i0++) {
      ...
      if (im != i0) {
        if (std::abs(adapt[im]->level_min() - adapt[i0]->level_min()) > 1)
          jumps = true;
      }
      // check for converged
