...
#define TRACE_MSG_INITIAL(MSG)                                                 \
  CkPrintf("TRACE_MSG_INITIAL %d %s %d\n", CkMyPe(), MSG, __LINE__);           \
  fflush(stdout);
#else
#define TRACE_MSG_INITIAL(MSG) /* ... */
#endif
...
void* MsgInitial::pack(MsgInitial* msg) {
  ...
  ASSERT2("MsgInitial::pack()", "buffer size mismatch %ld allocated %d packed",
...
void MsgInitial::set_field_data(std::string field_name, char* data,
...
void MsgInitial::get_field_data(std::string* field_name, char** data,
                                int* data_precision) {
...
void MsgInitial::set_dataset(int n4[4], double h4[4], int nx, int ny, int nz,
...
void MsgInitial::get_dataset(int n4[4], double h4[4], int* nx, int* ny, int* nz,
                             int* IX, int* IY, int* IZ) {
  for (int i = 0; i < 4; i++) {
    n4[i] = n4_[i];
    h4[i] = h4_[i];
...
void MsgInitial::set_particle_data(std::string particle_name,
...
void MsgInitial::get_particle_data(std::string* particle_name,
                                   std::string* particle_attribute, char** data,
                                   int* data_size, int* data_precision) {
