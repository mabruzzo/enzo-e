...
void Box::compute_block_start(BoxType bt) {
  ...
  if (level_[bt] == -1) {
    for (int i = 0; i < rank_; i++) {
      block_start_[bt][i] =
          block_size_[i] * (2.0 * face_[bt][i] - child_[bt][i]);
    }
  } else if (level_[bt] == 0) {
    for (int i = 0; i < rank_; i++) {
    ...
    for (int i = 0; i < rank_; i++) {
      block_start_[bt][i] =
          block_size_[i] * (face_[bt][i] + 0.5 * child_[bt][i]);
    }
  }
...
void Box::compute_region() {
  ...
  for (int i = 0; i < rank_; i++) {
    ...
    const int recv_stop = int(ceil(
        block_start_[0][i] + ri * (block_size_[i] + ghost_depth_recv_[i])));
    region_start_[i] = std::max(-ghost_depth_send_[i], recv_start);
    region_stop_[i] =
        std::min(block_size_[i] + ghost_depth_send_[i], recv_stop);
  }
}

//----------------------------------------------------------------------

bool Box::get_start_size(int index_start[3], int index_size[3],
                         BlockType block_intersect, BlockType block_coords,
                         bool lpad) {
  bool intersect = get_start_stop(index_start, index_size, block_intersect,
                                  block_coords, lpad);
  for (int i = 0; i < rank_; i++) {
...
bool Box::get_start_stop(int index_min[3], int index_max[3],
                         BlockType block_intersect, BlockType block_coords,
                         bool lpad) {
  if (pad_ != 0) {
    ...
    ASSERT("Box::get_start_stop",
           ...
           !((lpad == false) && ((block_coords == BlockType::receive_coarse) ||
  ...
  const bool l_intersect =
      intersect_regions_(index_min, index_max, block_min, block_max);
...
bool Box::intersect_regions_(int index_min[3], int index_max[3],
